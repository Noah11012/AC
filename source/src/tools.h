...
#define DEBUG(v)                                                                                \
    if (DEBUGCOND)                                                                              \
    {                                                                                           \
        std::cout << behindpath (__FILE__) << ":" << __LINE__ << " " << __FUNCTION__ << "(..) " \
                  << v << std::endl;                                                            \
    }
#define DEBUGVAR(v)                                                                             \
    if (DEBUGCOND)                                                                              \
...
#define NEARZERO                                                                \
    (0.000245f) /* epsilon for 1<<LARGEST_FACTOR, use to check if "zero enough" \
                 */

#ifdef WIN32
#ifndef __GNUC__
#pragma warning(3 : 4189)       // local variable is initialized but not referenced
#pragma warning(disable : 4244) // conversion from 'int' to 'float', possible
                                // loss of data
...
//#pragma GCC diagnostic ignored "-Wformat-zero-length" // apparently doesn't
// work from precompiled header -> moved to makefile
#else
...
struct packetbuf : ucharbuf
{
    ...
    : ucharbuf (packet->data, packet->dataLength), packet (packet), growth (0)
    {
    }
...
template <class T> struct bitbuf
{
    T &q;
    int blen;
...
template <class T> struct vector
{
    ...
    vector (const vector &v) : buf (NULL), alen (0), ulen (0) { *this = v; }
    ...
    template <class ST> void sort (int(__cdecl *cf) (ST *, ST *), int i = 0, int n = -1)
    {
        qsort (&buf[i], n < 0 ? ulen : n, sizeof (T), (int(__cdecl *) (const void *, const void *))cf);
    }

    template <class ST> T *search (T *key, int(__cdecl *cf) (ST *, ST *), int i = 0, int n = -1)
    {
        return (T *)bsearch (key, &buf[i], n < 0 ? ulen : n, sizeof (T),
    ...
    void advance (int sz) { ulen += sz; }

    void addbuf (const databuf<T> &p) { advance (p.length ()); }
...
static inline bool htcmp (const char *x, const char *y) { return !strcmp (x, y); }

static inline uint hthash (const uchar *key) { return *((uint *)key); }

static inline bool htcmp (const uchar *x,
                          const uchar *y) // assume "uchar *" points to 32byte public keys
{
    return !memcmp (x, y, 32);
}

static inline uint hthash (int key) { return key; }

static inline bool htcmp (int x, int y) { return x == y; }

static inline uint hthash (uint key) { return key; }

static inline bool htcmp (uint x, uint y) { return x == y; }
...
template <class K, class T> struct hashtable
{
    ...
    hashtable (int size = 1 << 10) : size (size)
    {
        numelems = 0;
        chunks = NULL;
        unused = NULL;
...
#define HTFIND(success, fail)                      \
    ...
    T *access (const K &key) { HTFIND (&c->data, NULL); }

    T &access (const K &key, const T &data) { HTFIND (c->data, insert (key, h)->data = data); }

    T &operator[] (const K &key) { HTFIND (c->data, insert (key, h)->data); }
...
template <class T, int SIZE> struct ringbuf
{
    ...
    T *peek (int *n) // look at n without removing (reduces n, if the data wraps
                     // around or isn't available)
    {
        if (*n > length ()) *n = length ();
        if (out + *n >= SIZE) *n = SIZE - out;
        return data + out;
    ...
    void add (const T &e) // add one (crashes, if full) -- between threads, better
                          // use stage & commit
    {
        ASSERT (!full ());
        volatile int i;
    ...
    T &operator[] (int i) // [0] is the one, that remove() will get next (in other
                          // words, [0] is the oldest entry, [length()-1] was the
                          // last added one, loopv goes from old to new)
    {
        ASSERT (i >= 0 && i < length ());
...
#define enumeratekt(ht, k, e, t, f, b)                                            \
    loopi ((ht).size) for (hashtable<k, t>::chain *enumc = (ht).table[i]; enumc;) \
    {                                                                             \
        hashtable<k, t>::const_key &e = enumc->key;                               \
        t &f = enumc->data;                                                       \
        enumc = enumc->next;                                                      \
        b;                                                                        \
    }
#define enumeratek(ht, k, e, b)                                     \
...
struct stopwatch
{
    ...
    void start () { millis = SDL_GetTicks (); }
...
#else
#if defined(__GNUC__) && /*!defined(__clang__) && */ !defined(__ICL) && __GNUC__ >= 4 \
...
{
    return ((uint64_t)endianswap32 ((uint)n) << 32) | ((uint64_t)endianswap32 ((uint) (n >> 32)));
}
#endif
#endif
template <class T> inline T endianswap (T n)
{
    ...
    s.t = n;
    loopi (sizeof (T)) d.u[i] = s.u[sizeof (T) - 1 - i];
    return d.t;
}
...
{
    return *(const uchar *)&islittleendian ? n : endianswap (n);
}
template <class T> inline void lilswap (T *buf, int len)
{
    if (!*(const uchar *)&islittleendian) endianswap (buf, len);
}
template <class T> inline T bigswap (T n)
{
    return *(const uchar *)&islittleendian ? endianswap (n) : n;
}
...
/* workaround for some C platforms that have these two functions as macros - not
 * used anywhere */
#ifdef getchar
#undef getchar
#endif
...
inline const char *timestring (bool local = false, const char *fmt = NULL, char *buf = _timestringbuffer)
{
    return timestring (time (NULL), local, fmt, buf);
}
...
extern void
listsubdirs (const char *dir, vector<char *> &subdirs, int(__cdecl *sf) (const char **, const char **));
extern bool listdir (const char *dir, const char *ext, vector<char *> &files);
extern void listfiles (const char *dir,
                       const char *ext,
                       vector<char *> &files,
                       int(__cdecl *sf) (const char **, const char **) = NULL);
...
extern void
parseupdatelist (hashtable<const char *, int> &ht, char *buf, const char *prefix = NULL, const char *suffix = NULL);
...
extern int cmpintasc (const int *a,
                      const int *b); // leads to ascending sort order
extern int cmpintdesc (const int *a,
                       const int *b); // leads to descending sort order
...
struct sl_semaphore
{
    ...
    sl_semaphore (int init,
                  int *errorcount); // init: initial semaphore value; errorcount:
                                    // pointer to error counter for
                                    // semaphore-related errors or NULL
    ~sl_semaphore ();
    void wait ();                      // blocks, until semaphore gets available
    int trywait ();                    // returns 0, if semaphore was locked (like wait(), but
                                       // returns !=0 instead of blocking)
    int timedwait (int howlongmillis); // like trywait(), but waits for a litte
