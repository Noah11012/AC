...
template <class T> static inline void putaint_ (T &p, int n)
{
    DEBUGVAR (n);
    if (n < 253 && n > -2)
        p.put (n - 125);
...
template <class T> static inline void sendstring_ (const char *text, T &p)
{
    const char *t = text;
    if (t)
    {
...
{
    char *res = dst;
    bool nowhite = (flags & FTXT_NOWHITE) != 0, // removes all whitespace; adding ALLOWBLANKS,
                                                // ALLOWNL or ALLOWTAB enables exceptions
    ...
    safecs = (flags & FTXT_SAFECS) != 0, // removes all special chars that may execute commands when
    ...
    filename = (flags & FTXT_FILENAME) != 0, // strict a-z, 0-9 and "-_.()" (also translates "[]" and
                                             // "{}" to "()"), removes everything between '<' and '>'
    allowslash = (flags & FTXT_ALLOWSLASH) != 0, // only in combination with FTXT_FILENAME
    mapname = (flags & FTXT_MAPNAME) != 0, // only allows lowercase chars, digits, '_', '-' and '.';
    ...
    for (int c = *src; c; c = *++src)
    {
        ...
        if (trans)
        {
            if (leet)
            {
                const char *org = "1374@5$2!*#%80|+", *asc = "letaassziohzhoit", *a = strchr (org, c);
                if (a) c = asc[a - org];
            }
            if (filename)
            {
...
{
    return (n >= -1 && size_t (n + 1) < sizeof (modefullnames) / sizeof (modefullnames[0])) ?
           modefullnames[n + 1] :
           "unknown";
}
const char *acronymmodestr (int n)
{
    return (n >= -1 && size_t (n + 1) < sizeof (modeacronymnames) / sizeof (modeacronymnames[0])) ?
...
{
    return (n >= 0 && (size_t)n < sizeof (voteerrors) / sizeof (voteerrors[0])) ? voteerrors[n] : "unknown";
}
