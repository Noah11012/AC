...
// special encoding for asymmetric small integers: -1..252 go into one byte,
// -256..65279 go into three bytes and any other int goes into five bytes (the
// network encoding is compatible with basic getint)
template <class T>
static inline void putaint_(T &p, int n) {
  DEBUGVAR(n);
  if (n < 253 && n > -2)
    p.put(n - 125);
  else if (n < 0xFF00 && n >= -256) {
    n += 256;
    p.put(0x80);
    p.put(n);
    p.put(n >> 8);
...
// much smaller encoding for unsigned integers up to 28 bits, but can handle
// signed
template <class T>
...
ucharbuf *getgzbuf(
    ucharbuf &p)  // fetch a gzipped buffer; needs to be freed properly later
{
  int size = getuint(p);
  ...
  if (sizegz > 0 && p.remaining() >= sizegz && sizegz <= MAXGZMSGSIZE &&
    ...
    if (data) {
      ...
      } else if (uncompress(data, &rawsize, &p.buf[p.len], sizegz) != Z_OK ||
                 rawsize - size != 0) {
        p.forceoverread();
        delete[] data;
        return nullptr;
...
// dst can be identical to src; dst needs to be of size "min(len, strlen(s)) +
// 1" returns dst
...
char *filtertext(char *dst, const char *src, int flags, int len) {
  char *res = dst;
  bool nowhite = (flags & FTXT_NOWHITE) !=
                 0,  // removes all whitespace; adding ALLOWBLANKS, ALLOWNL or
                     // ALLOWTAB enables exceptions
      allowblanks = (flags & FTXT_ALLOWBLANKS) !=
                    0,  // only in combination with FTXT_NOWHITE
      allownl =
          (flags & FTXT_ALLOWNL) != 0,  // only in combination with FTXT_NOWHITE
      allowtab = (flags & FTXT_ALLOWTAB) !=
                 0,  // only in combination with FTXT_NOWHITE
      nocolor =
          (flags & FTXT_NOCOLOR) != 0,  // removes all '\f' + following char
      tabtoblank =
          (flags & FTXT_TABTOBLANK) != 0,  // replaces \t with single blanks
      fillblanks = (flags & FTXT_FILLBLANKS) != 0,  // convert ' ' to '_'
      safecs = (flags & FTXT_SAFECS) !=
               0,  // removes all special chars that may execute commands when
                   // evaluate arguments; the argument still requires
                   // encapsulation by ""
      ...
      filename = (flags & FTXT_FILENAME) !=
                 0,  // strict a-z, 0-9 and "-_.()" (also translates "[]" and
                     // "{}" to "()"), removes everything between '<' and '>'
      allowslash = (flags & FTXT_ALLOWSLASH) !=
                   0,  // only in combination with FTXT_FILENAME
      mapname = (flags & FTXT_MAPNAME) !=
                0,  // only allows lowercase chars, digits, '_', '-' and '.';
                    // probably should be used in combination with TOLOWER
      cropwhite = (flags & FTXT_CROPWHITE) !=
                  0,  // removes leading and trailing whitespace
      pass = false;
  ...
  for (int c = *src; c; c = *++src) {
    ...
    if (trans) {
      if (leet) {
        const char *org = "1374@5$2!*#%80|+", *asc = "letaassziohzhoit",
                   *a = strchr(org, c);
        if (a) c = asc[a - org];
      }
      if (filename) {
        if (c == '>')
          insidepointybrackets = false;
        else if (c == '<')
          insidepointybrackets = true;
        if (insidepointybrackets || c == '>')
          continue;  // filter anything between '<' and '>' as this may contain
                     // commands for texture loading
        const char *org = "[]{}\\", *asc = "()()/", *a = strchr(org, c);
        if (a) c = asc[a - org];
      }
      if (tolow)
        c = tolower(c);
    ...
    if (filename &&
        !(islower(c)
          ...
          || isupper(c)
#endif
          || isdigit(c) || strchr("._-()", c) || (allowslash && c == '/')))
      continue;
    ...
    if (c == '\1') {  // always filter encoded igraphs
      if (src[1]) ++src;
      continue;
    }
...
void trimtrailingwhitespace(char *s) {
  for (int n = (int)strlen(s) - 1; n >= 0 && isspace(s[n]); n--) s[n] = '\0';
}

const char *modefullnames[] = {"demo playback",
                               "team deathmatch",
                               "coopedit",
                               "deathmatch",
                               "survivor",
...
const char *modeacronymnames[] = {
    "DEMO", "TDM", "coop", "DM",   "SURV",  "TSURV",  "CTF",   "PF",
    "BTDM", "BDM", "LSS",  "OSOK", "TOSOK", "BOSOK",  "HTF",   "TKTF",
    "KTF",  "TPF", "TLSS", "BPF",  "BLSS",  "BTSURV", "BTOSOK"};

const char *voteerrors[] = {"voting is currently disabled",
                            "there is already a vote pending",
                            "already voted",
                            "can't vote that often",
                            "this vote is not allowed in the current "
                            "environment (singleplayer/multiplayer)",
                            "no permission",
                            "invalid vote",
                            "server denied your call",
                            "the next map/mode is already set"};
const char *mmfullnames[] = {"open", "private", "match"};

const char *fullmodestr(int n) {
  return (n >= -1 &&
          size_t(n + 1) < sizeof(modefullnames) / sizeof(modefullnames[0]))
             ? modefullnames[n + 1]
             : "unknown";
}
const char *acronymmodestr(int n) {
  return (n >= -1 && size_t(n + 1) <
                         sizeof(modeacronymnames) / sizeof(modeacronymnames[0]))
             ? modeacronymnames[n + 1]
             : "UNK";
}  // 'n/a' bad on *nix filesystem (demonameformat)
const char *modestr(int n, bool acronyms) {
  return acronyms ? acronymmodestr(n) : fullmodestr(n);
}
const char *voteerrorstr(int n) {
  return (n >= 0 && (size_t)n < sizeof(voteerrors) / sizeof(voteerrors[0]))
             ? voteerrors[n]
             : "unknown";
}
const char *mmfullname(int n) {
  return (n >= 0 && n < MM_NUM) ? mmfullnames[n] : "unknown";
}

int defaultgamelimit(int gamemode) { return m_teammode ? 15 : 10; }

static const int msgsizes[] =  // size inclusive message token, 0 for variable
                               // or not-checked sizes
    {SV_SERVINFO,
     5,
     SV_WELCOME,
     2,
