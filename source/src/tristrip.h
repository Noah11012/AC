...
struct tristrip
{
    struct drawcall
    {
        ...
        drawcall () {}
        drawcall (GLenum type, GLuint start, GLsizei count = 0)
        : type (type), start (start), minvert (~0U), maxvert (0), count (count)
        {
        }
    };
    ...
    struct triangle
    {
        ...
        {
            int num = 0;
            loopi (3) if (n[i] < UNUSED) num++;
            return num;
        }

        bool hasvert (ushort idx) const
        {
            loopi (3) if (v[i] == idx) return true;
            return false;
        }
        ushort diffvert (ushort v1, ushort v2)
        {
            loopi (3) if (v[i] != v1 && v[i] != v2) return v[i];
            return UNUSED;
        }

        bool haslink (ushort neighbor) const
        {
            loopi (3) if (n[i] == neighbor) return true;
            return false;
        }
    };
    ...
    template <class T> void addtriangles (const T *tris, int numtris)
    {
    ...
    void removeconnectivity (ushort i)
    {
        ...
        loopj (3) if (tri.n[j] < UNUSED)
        {
            ...
            bool linked = false;
            loopk (3) if (neighbor.n[k] == i)
            {
                linked = true;
                neighbor.n[k] = REMOVED;
                break;
            }
            if (linked)
            {
    ...
    void removenodes (int i) { loopj (3) nodes[triangles[i].v[j]]--; }
    ...
    ushort nexttriangle (triangle &tri, bool &nextswap, ushort v1 = UNUSED, ushort v2 = UNUSED)
    {
        ...
        loopi (3) if (tri.n[i] < UNUSED)
        {
            ...
            if (v1 != UNUSED)
            {
                ...
                else if (nexttri.hasvert (v2))
                    continue;
                else
                    score += nodes[v1] > nodes[v2] ? 1 : -1;
    ...
    void buildstrip (vector<ushort> &strip, bool reverse = false, bool prims = false)
    {
        ...
        int from = findedge (first, triangles[cur]), to = findedge (first, triangles[cur], first.v[from]);
    ...
    void buildstrips (vector<ushort> &strips, vector<drawcall> &draws, bool prims = true, bool degen = false)
    {
        ...
        while (remaining ())
        {
            ...
            if (!strips.empty () && degen)
            {
                strips.dup ();
                strips.add (strip[0]);
            }
            else
                draws.add (drawcall (GL_TRIANGLE_STRIP, strips.length ()));
        ...
        if (dbgts)
            conoutf ("strips = %d, tris = %d, inds = %d, merges = %d", numstrips, numtris,
                     numtris + numstrips * 2, (degen ? 2 : 1) * (numstrips - 1));
    }
};

static inline uint hthash (const tristrip::edge &x) { return x.from ^ x.to; }
static inline bool htcmp (const tristrip::edge &x, const tristrip::edge &y)
{
