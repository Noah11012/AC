...
struct winservice : servercontroller
{
    ...
    void start () // starts the server again on a new thread and returns once the
                  // windows service has stopped
    {
        SERVICE_TABLE_ENTRY dispatchtable[]
        = { { (LPSTR)name, (LPSERVICE_MAIN_FUNCTION)callbacks::main }, { NULL, NULL } };
        if (StartServiceCtrlDispatcher (dispatchtable))
            exit (EXIT_SUCCESS);
        else
            fatal ("an error occurred running the AC server as windows service. make "
                   "sure you start the server from the service control manager and not "
    ...
    void report (DWORD state, DWORD wait)
    {
        ...
        = SERVICE_START_PENDING == state || SERVICE_STOP_PENDING == state ? 0 : SERVICE_ACCEPT_STOP;
    ...
    /*void log(const char *msg, bool error)
    {
        ...
        if(eventsrc)
        {
            int eventid = ((error ? 0x11 : 0x1) << 10) & (0x1 << 9) &
    (FACILITY_NULL << 6) & 0x1; // TODO: create event definitions LPCTSTR msgs[1]
    = { msg }; int r = ReportEvent(eventsrc, error ? EVENTLOG_ERROR_TYPE :
    EVENTLOG_INFORMATION_TYPE, 0, 4, NULL, 1, 0, msgs, NULL);
