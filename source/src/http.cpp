    ...
    { // (in a loop because of possible redirects)
        ...
        while (buf.dataLength > 0)
        {
            ...
            if (enet_socket_wait (tcp, &events, 250) >= 0 && events)
            {
                ...
                {
                    if (!reconnected)
                    { // line went dead, maybe the server closed it -
                ...
                if (enet_socket_wait (tcp, &events, 250) >= 0 && events)
                {
                    ...
                    { // still getting header
                        ...
                        if (!endhead && --headendlen)
                            endhead = strstr ((const char *)rawrec.getbuf (),
                        ...
                        if (endhead)
                        {
                            ...
                            if ((f = strstr (u, "CONTENT-ENCODING: ")) && (n = strchr (f, LF))
                                && (f = strstr (f, "GZIP")) && f < n)
                                gzipped = 1;
                    ...
                    { // check if data is complete
                        ...
                        { // n * "chunksize\r\nchunkdata\r\n" + "0\r\n\r\n"
                            while (strchr (d, LF))
                            {
                                ...
                                if (*e == LF && rawrec.length () >= chunklen + (e - d) + 2
                                    && (e[chunklen + 1] == LF || (e[chunklen + 1] == CR && e[chunklen + 2] == LF)))
                        ...
                        else
                        {
                            if (contentlength > 0)
                            {
                                ...
                                progress = float(rawrec.length () - 1) / (maxtransfer | 1); // not accurate, but moving
                        }
                    }
                    rawrec.drop ();
                }
        ...
        { // TCP connection showed some error before the response
...
char *urlencode (const char *s, bool strict)
{
    ...
    while (*s)
    {
        if (isalnum (*s) || strchr ("-_.~", *s) || (!strict && strchr (":/?#[]@!$&'()*+,;=", *s)))
