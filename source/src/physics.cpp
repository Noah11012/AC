...
float raycube (const vec &o, const vec &ray, vec &surface)
{
    ...
    for (nr = 0; nr < 512; nr++) // sam's suggestion :: I found no map which got nr > 350
    {
        ...
        {
            if ((!dx && !dy) || s->wtex == DEFAULT_SKY || (!SOLID (s) && v.z > ceil && s->ctex == DEFAULT_SKY))
                return -1;
                ...
                float &lo) // collide with physent
{
    ...
    if ((d->type == ENT_PLAYER && o->type == ENT_PLAYER ? dr.sqrxy () < r * r :
    ...
    for (int i = clentstats.firstclip; i < ents.length (); i++)
    {
        ...
        if (e.type == CLIP
            || (e.type == PLCLIP
                && (d->type == ENT_BOT || d->type == ENT_PLAYER
                    || (d->type == ENT_BOUNCE && ((bounceent *)d)->plclipped)))) // don't allow bots to hack themselves into
                                                                                 // plclips - Bukz 2011/04/14
        {
            ...
            default: // classic unrotated clip, possibly tilted
                hitarea = fabs (e.x - d->o.x) < float(e.attr2) / ENTSCALE5 + d->radius
                          && fabs (e.y - d->o.y) < float(e.attr3) / ENTSCALE5 + d->radius;
                break;
            case 3: // clip rotated 45°
            {
                float rx = (e.x - d->o.x) * SQRT2HALF, ry = (e.y - d->o.y) * SQRT2HALF,
            ...
            if (hitarea)
            {
                ...
                if (e.attr6) switch (e.attr7 & 3)
                    { // incredibly ugly solution - but it only
                      // applies on the one tilted clip we stand on
                    case 1:
                        cz += (floor (0.5f
                                      + clamp (d->o.x - e.x + d->radius * (e.attr6 > 0 ? 1 : -1),
                                               -float(e.attr2) / ENTSCALE5, float(e.attr2) / ENTSCALE5)))
                              * float(e.attr6) / (4 * ENTSCALE10);
                        break; // tilt x
                    case 2:
                        cz += (floor (0.5f
                                      + clamp (d->o.y - e.y + d->radius * (e.attr6 > 0 ? 1 : -1),
                                               -float(e.attr3) / ENTSCALE5, float(e.attr3) / ENTSCALE5)))
                              * float(e.attr6) / (4 * ENTSCALE10);
                        break; // tilt y
                    }
                ...
                if (dz + (d->type != ENT_BOUNCE ? 1.26 : 0) > cz + ch || dz + playerheight < cz)
                    cornersurface = 0;
            }
        }
        else if (e.type == MAPMODEL)
        {
            ...
            {
                const auto mmz = float(S (e.x, e.y)->floor + mmi->zoff + float(e.attr3) / ENTSCALE5);
...
bool collide (physent *d, bool spawn, float drop, float rise)
{
    ...
    const int applyclip = d->type == ENT_BOT || d->type == ENT_PLAYER
        ...
        for (int x = x1; x <= x2; x++) // collide with map
        {
            ...
            case CORNER:
            {
                ...
                if (!match && d->type == ENT_BOUNCE)
                    match = q == (d->vel.x < 0) * 2 + (d->vel.y * d->vel.x < 0); // when coming towards corner
                ...
                if (h)
                    tagclipped = false; // for corners between non-solids, tagclips extend the
...
void resizephysent (physent *pl, int moveres, int curtime, float min, float max)
{
    ...
    loopi (moveres)
    {
        ...
        pl->o.z += h;
        if (collide (pl)
            && !(pl != player1 && player1->isspectating () && player1->spectatemode == SM_FOLLOW1ST)) // don't check collision during
            ...
            if (pl->type != ENT_CAMERA)
            {
                ...
                else
                {
                    if (pl->onfloor || water)
                    {
                        ...
                        {
                            pl->jumpnext = false;
                            bool doublejump = pl->lastjump && lastmillis - pl->lastjump < 250
                                              && pl->strafe != 0
    ...
    if (!editfly) loopi (moveres) // discrete steps collision detection & sliding
        {
            ...
            pl->o.y += f * d.y;
            pl->o.z += f * d.z;
            volatile vec gcco3 (pl->o.x, pl->o.y,
            ...
            if ((pl->type == ENT_CAMERA && pl == camera1 && spect3rd) || !collide (pl, false, drop, rise))
                continue;
            int cornersurface1 = cornersurface;
            vec o_trying = pl->o; // o_trying = o_null + f * d  (= one microstep in
                                  // desired direction)
            if (!cornersurface1)
            { // brute-force check, if it is a corner hit after
              // all
                if (pl->type != ENT_BOUNCE)
                {
                    int collx = 0, colly = 0;
                ...
                { // try really, really hard to detect corners for bounces (to
                  // compensate for the not-so-micro bounceent-microsteps)
                    vec vd (d.x, d.y, 0);
                    static const float a = 3.0f * PI2 / 10.0f, ca = cosf (a),
            ...
            if (pl->type == ENT_CAMERA
                || (pl->type == ENT_PLAYER && pl->state == CS_DEAD && ((playerent *)pl)->spectatemode != SM_FLY))
            {
                pl->o = o_null;
                break;
            }
            if (pl->type != ENT_BOUNCE && hitplayer)
            {
                ...
                if (i == 0 && pl->type == ENT_BOT)
                    pl->yaw += (dr.cxy (d) > 0 ? 2 : -2); // force the bots to change direction
...
void movebounceent (bounceent *p, int moveres, bool local) { moveplayer (p, moveres, local); }
...
dir (left, strafe, 1, k_left, k_right) dir (right, strafe, -1, k_right, k_left)
...
FVARP (scopesensscale,
       1e-3f,
       0.5f,
       1000.0f);                       // if scoped, sens = sensitivity * scopesensscale (roughly)
float autoscopesensscale = 0.4663077f; // roughly scopefov/fov, recalculated if fov or scopefov
...
void mousemove (int idx, int idy)
{
    ...
    bool zooming = player1->weaponsel->type == GUN_SNIPER
                   && ((sniperrifle *)player1->weaponsel)->scoped; // check if player uses scope
    float dx = idx, dy = idy;
    if (mfilter > 0.0001f)
    ...
    if (mouseaccel > 0.0001f && curtime && (idx || idy))
        cursens += 0.02f * mouseaccel * sqrtf (dx * dx + dy * dy) / curtime; // optionally accelerated
    if (zooming)
    { //      when scoped:
        if (scopesens > 0.0001f)
            ...
            cursens *= autoscopesens ? autoscopesensscale :
