...
struct console : consolebuffer<cline>
{
    ...
    void setconskip (int n)
    {
        int visible_lines
        = (int)(min (fullconsole ?
                     ((VIRTH * 2 - 2 * CONSPAD - 2 * FONTH / 3) * (fullconsole == 1 ? altconsize : fullconsize)) / 100 :
                     FONTH * consize,
    ...
    void render () override
    {
        ...
        int conheight
        = min (fullconsole ? (h * (fullconsole == 1 ? altconsize : fullconsize)) / 100 : FONTH * consize, h);
        ...
        if (!fullconsole && confade)
        {
            ...
            {
                numl = 0;
                loopvrev (conlines) if (totalmillis - conlines[i].millis < confade * 1000)
                {
                    numl = i + 1;
                    break;
                }
...
int rendercommand (int x, int y, int w)
{
    ...
    y -= height - FONTH;
    if (x >= 0)
        draw_text (s, x, y, 0xFF, 0xFF, 0xFF, 0xFF,
                   cmdline.pos >= 0 ? cmdline.pos + strlen (useprompt) + 1 : (int)strlen (s), w);
    return height;
}
...
COMMANDF (_resetallbinds, "", () {
    if (_defaultbinds) loopv (keyms) loopj (keym::NUMACTIONS) bindkey (&keyms[i], "", j);
});
...
COMMANDF (searchspecbinds, "s", (const char *action) {
...
void savehistory ()
{
    ...
    {
        hline *h = history[i];
        if (!h->action && !h->prompt && strncmp (h->buf, "/ ", 2)) f->printf ("%s\n", h->buf);
    }
    delete f;
}
