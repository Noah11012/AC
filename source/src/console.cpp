...
struct console : consolebuffer<cline> {
  ...
  void setconskip(int n) {
    int visible_lines =
        (int)(min(fullconsole
                      ? ((VIRTH * 2 - 2 * CONSPAD - 2 * FONTH / 3) *
                         (fullconsole == 1 ? altconsize : fullconsize)) /
                            100
                      : FONTH * consize,
                  (VIRTH * 2 - 2 * CONSPAD - 2 * FONTH / 3)) /
              (CONSPAD + 2 * FONTH / 3)) -
    ...
    conskip =
        clamp(conskip + n, 0,
              clamp(conlines.length() - visible_lines, 0, conlines.length()));
  }
  ...
  void toggleconsole() {
    if (!fullconsole)
      ...
      fullconsole = (fullconsole + 1) % 3;
    conopen = fullconsole;
  }

  void addline(const char *sf) {
    consolebuffer<cline>::addline(sf, totalmillis);
    numconlines++;
  }
  ...
  void render() override {
    ...
    int conheight = min(
        fullconsole ? (h * (fullconsole == 1 ? altconsize : fullconsize)) / 100
                    : FONTH * consize,
        h);

    if (fullconsole)
      blendbox(CONSPAD, CONSPAD, conwidth + CONSPAD + 2 * FONTH / 3,
               conheight + CONSPAD + 2 * FONTH / 3, true);
    ...
    if (!fullconsole && confade) {
      if (!conskip) {
        numl = 0;
        loopvrev(conlines) if (totalmillis - conlines[i].millis <
                               confade * 1000) {
          numl = i + 1;
          break;
        }
    ...
    loopi(numl) {
      ...
      char *line = conlines[idx].line;
      draw_text(line, CONSPAD + FONTH / 3, y, 0xFF, 0xFF, 0xFF,
                fullconsole ? 0xFF : conalpha, -1, conwidth);
      int width, height;
      text_bounds(line, width, height, conwidth);
      y += height;
    }
...
void clientlogf(const char *s, ...) {
  ...
  char *p, *l = sp;
  do {  // break into single lines first
    if ((p = strchr(l, '\n'))) *p = '\0';
    printf("%s%s\n", ts, l);
    if (clientlogfile) {
      clientlogfile->printf("%s%s\n", ts, l);
      if (--clientloglinesremaining <= 0) DELETEP(clientlogfile);
    } else if (bootclientlog)
      cvecprintf(*bootclientlog, "%s%s\n", ts, l);
    l = p + 1;
  } while (p);
}
SVAR(conline, "n/a");
...
int rendercommand(int x, int y, int w) {
  ...
  if (x >= 0)
    draw_text(
        s, x, y, 0xFF, 0xFF, 0xFF, 0xFF,
        cmdline.pos >= 0 ? cmdline.pos + strlen(useprompt) + 1 : (int)strlen(s),
        w);
  return height;
}
...
inline const char *keycmd(int type) {
  return type >= 0 && type < keym::NUMACTIONS ? keycmds[type] : "";
}
...
keym **findbinda(const char *action, int type) {
  ...
  res.setsize(0);
  loopv(keyms) if (!strcasecmp(keyms[i].actions[type], action))
      res.add(&keyms[i]);
  res.add(NULL);
  return res.getbuf();
}
...
COMMANDF(_resetallbinds, "", () {
  if (_defaultbinds)
    loopv(keyms) loopj(keym::NUMACTIONS) bindkey(&keyms[i], "", j);
});

bool bindkey(keym *km, const char *action, int type) {
  if (!km || type < keym::ACTION_DEFAULT || type >= keym::NUMACTIONS)
    return false;
  if (strcmp(action, km->actions[type])) {
    if (!keypressed || keyaction != km->actions[type])
      delete[] km->actions[type];
...
void keybind(const char *key, int type) {
  ...
  if (type < keym::ACTION_DEFAULT || type >= keym::NUMACTIONS) {
    conoutf("invalid bind type \"%d\"", type);
    return;
  }
...
COMMANDF(keybind, "s",
         (const char *key) { keybind(key, keym::ACTION_DEFAULT); });
COMMANDF(keyspecbind, "s",
         (const char *key) { keybind(key, keym::ACTION_SPECTATOR); });
COMMANDF(keyeditbind, "s",
         (const char *key) { keybind(key, keym::ACTION_EDITING); });

COMMANDF(bind, "ss", (const char *key, const char *action) {
  bindk(key, action, keym::ACTION_DEFAULT);
});
COMMANDF(specbind, "ss", (const char *key, const char *action) {
  bindk(key, action, keym::ACTION_SPECTATOR);
});
COMMANDF(editbind, "ss", (const char *key, const char *action) {
  bindk(key, action, keym::ACTION_EDITING);
});

COMMANDF(searchbinds, "s",
         (const char *action) { searchbinds(action, keym::ACTION_DEFAULT); });
COMMANDF(searchspecbinds, "s",
         (const char *action) { searchbinds(action, keym::ACTION_SPECTATOR); });
COMMANDF(searcheditbinds, "s",
         (const char *action) { searchbinds(action, keym::ACTION_EDITING); });
...
void onrelease(char *s) { addreleaseaction(s); }
...
void pasteconsole(char *dst) {
  ...
  for (char *cbline = cb, *cbend;
       commandlen + 1 < MAXSTRLEN && cbline < &cb[cbsize]; cbline = cbend + 1) {
    cbend = (char *)memchr(cbline, '\0', &cb[cbsize] - cbline);
    if (!cbend) cbend = &cb[cbsize];
    if (commandlen + cbend - cbline + 1 > MAXSTRLEN)
      cbend = cbline + MAXSTRLEN - commandlen - 1;
...
void execbind(keym &k, bool isdown) {
  ...
  if (isdown) {
    ...
    char *&action = k.actions[state][0] ? k.actions[state]
                                        : k.actions[keym::ACTION_DEFAULT];
    keyaction = action;
    keypressed = &k;
    execute(keyaction);
    keypressed = nullptr;
...
void consolekey(int code, bool isdown, int cooked, SDLMod mod) {
  ...
  if (isdown) {
    switch (code) {
      ...
      case SDLK_ESCAPE:
        if (mod &
            KMOD_LSHIFT)  // LSHIFT+ESC restores buffer from before complete()
        {
...
void keypress(int code, bool isdown, int cooked, SDLMod mod) {
  ...
  else if (saycommandon)
    consolekey(code, isdown, cooked, mod);       // keystrokes go to commandline
  else if (!menukey(code, isdown, cooked, mod))  // keystrokes go to menu
  {
    if (haskey) execbind(*haskey, isdown);
  }
  if (isdown)
    exechook(HOOK_SP, "KEYPRESS", "KEYPRESS %d", code);
  else
    exechook(HOOK_SP, "KEYRELEASE", "%d", code);
}
...
void writebinds(stream *f) {
  loopv(keyms) {
    ...
    loopj(3) if (*km->actions[j] &&
                 (!km->unchangeddefault[j] || omitunchangeddefaultbinds < 2))
        f->printf(
            "%s%s \"%s\" %s\n",
