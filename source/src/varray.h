...
template <class T> static inline void attrib (T x)
{
    T *buf = (T *)data.pad (sizeof (T));
    buf[0] = x;
}

template <class T> static inline void attrib (T x, T y)
{
...
template <class T> static inline void attrib (T x, T y, T z)
{
...
template <class T> static inline void attrib (T x, T y, T z, T w)
{
...
template <size_t N, class T> static inline void attribv (const T *v)
{
...
static int enabled = 0, numattribs = 0, attribmask = 0, numlastattribs = 0, lastattribmask = 0,
           vertexsize = 0, lastvertexsize = 0;
...
void begin (GLenum mode) { primtype = mode; }
