...
void computeraytable(float vx, float vy, float fov) {
  ...
  loopj(NUMRAYS4) {
    int i = ((j << 2) & (NUMRAYS4 - 1)) | (j >> NUMRAYBITS),
        k = i / 4;  // interleaved ray calculation
    ...
    if ((apitch > 45  // must be bigger if fov>120
         ...
         (angle < byaw + PI2 && angle > syaw + PI2)) &&
        vinside)  // try to avoid tracing ray if outside of frustrum
    {
      ...
      if (!firstquarter) {
        ...
        if (m-- > 3) {
          if (m > 100) m = 100;
          sx += dx * m;  // skip to the nearest already found wall, so the last
                         // three quarters only clean out the edges
          sy += dy * m;
        }
      }
      for (;;) {
...
static inline float ma(float x, float y) {
  return x == 0 ? (y > 0 ? 2 : -2) : y / x;
}
#endif

int isoccluded(float vx, float vy, float cx, float cy,
               float csize)  // v = viewer, c = cube to test
{
  ...
  // find highest and lowest angle in the occlusion map that this cube spans,
  // based on its most left and right points on the border from the viewer
  // pov... I see no easier way to do this than this silly code below
  ...
  if (cx <= vx)  // ABDFG
  {
    if (cx + csize < vx)  // ADF
    {
      ...
      {
        if (cy + csize < vy) {
          if ((ydist = vy - (cy + csize)) > odist) return 2;
          h = ca(-(cx - vx), ydist) + 4;
          l = ca(xdist, -(cy - vy)) + 4;
        }  // A
        else {
          h = ma(xdist, -(cy + csize - vy)) + 4;
          l = ma(xdist, -(cy - vy)) + 4;
        }  // D
      } else {
        if ((ydist = cy - vy) > odist) return 2;
        h = ca(cy + csize - vy, xdist) + 2;
        l = ca(ydist, -(cx - vx)) + 2;
      }     // F
    } else  // BG
    {
      if (cy <= vy) {
        if (cy + csize < vy) {
          if ((ydist = vy - (cy + csize)) > odist) return 2;
          h = ma(ydist, cx - vx) + 6;
          l = ma(ydist, cx + csize - vx) + 6;
        }  // B
        else
          return 0;
      } else {
        if ((ydist = cy - vy) > odist) return 2;
        h = ma(ydist, -(cx + csize - vx)) + 2;
        l = ma(ydist, -(cx - vx)) + 2;
      }  // G
    }
  } else  // CEH
  {
    ...
    if (cy <= vy)  // CE
    {
      if (cy + csize < vy) {
        if ((ydist = vy - (cy + csize)) > odist) return 2;
        h = ca(-(cy - vy), xdist) + 6;
        l = ca(ydist, cx + csize - vx) + 6;
      }  // C
      else {
        h = ma(xdist, cy - vy);
        l = ma(xdist, cy + csize - vy);
      }  // E
    } else {
      if ((ydist = cy - vy) > odist) return 2;
      h = ca(cx + csize - vx, ydist);
      l = ca(xdist, cy + csize - vy);
    }  // H
  }

  float dist = xdist + ydist - 1;  // 1 needed?
  int si = int(h * (NUMRAYS / 8)) +
  ...
  for (int i = si; i <= ei; i++) {
    if (dist < rdist[i & (NUMRAYS - 1)])
      return 0;  // if any value in this segment of the occlusion map is further
                 // away then cube is not occluded
  }
