...
int resolverloop(void *data) {
  ...
  if (!thread || SDL_GetThreadID(thread) != SDL_ThreadID()) return 0;
...
bool resolverwait(const char *name, ENetAddress *address) {
  ...
  for (;;) {
    ...
    show_out_of_renderloop_progress(min(float(timeout) / RESOLVERLIMIT, 1.0f),
                                    text);
    SDL_Event event;
...
// could set timeouts on sockets, but this is more reliable and gives more
// control
...
int connectwithtimeout(ENetSocket sock, const char *hostname,
                       ENetAddress &address) {
  if (isdedicated) {
  ...
  defformatstring(text)("connecting to %s:%d... (esc to abort)", hostname,
                        address.port);
  show_out_of_renderloop_progress(0, text);
  ...
  for (;;) {
    ...
    timeout = SDL_GetTicks() - starttime;
    show_out_of_renderloop_progress(min(float(timeout) / CONNLIMIT, 1.0f),
                                    text);
    SDL_Event event;
    while (SDL_PollEvent(&event)) {
      if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_ESCAPE)
...
void addserver(const char *servername, int serverport, int weight) {
  ...
  loopv(servers) if (strcmp(servers[i]->name, servername) == 0 &&
                     servers[i]->port == serverport) return;
...
int chooseextping(serverinfo *si) {
  ...
  if (!si->uplinkqual_age || totalmillis - si->uplinkqual_age > 60 * 1000)
    return EXTPING_UPLINKSTATS;
  return EXTPING_NOP;
}
...
void pingservers(bool issearch, serverinfo *onlyconnected) {
  ...
  if (onlyconnected) {
    ...
    loopi(maxservpings ? min(servers.length(), maxservpings)
                       : servers.length()) {
...
void checkresolver() {
  ...
  loopv(servers) {
    ...
    if (si.address.host == ENET_HOST_ANY) {
      if (si.resolved == serverinfo::UNRESOLVED) {
        si.resolved = serverinfo::RESOLVING;
...
void checkpings() {
  ...
  while (enet_socket_wait(pingsock, &events, 0) >= 0 && events) {
    ...
    int query = getint(p);
    switch (query) {  // cleanup additional query info
...
#define ADDINFOLINE(msg)                                \
  ...
  sendstring(msg, q)
          ...
          case EXTPING_SERVERINFO: {
            ...
            if (strlen(text) != 2) {
              ADDINFOLINE(
                  "this server does not provide additional information");
              break;
            }
            strcpy(si->lang, text);
            while (p.remaining()) {
          ...
          case EXTPING_UPLINKSTATS: {
            ...
            if (si->maxclients > 3) {
              ...
              loopi(si->maxclients - 3) {
                ...
                if (si->maxclients < 8 || i > 2) {
                  ...
                  if (ts > maxs) maxs = ts;
                  if (ts > 0x40 && tc > maxc)
                    maxc = tc;  // spent time = 2 ^ ((ts >> 4) - 1) * 30 sec, so
                                // 0x50 is 8..15 minutes
                }
              }
              ...
              if (maxs < 0x50)
                ...
                si->uplinkqual = 5;  // choke_percentage = 1 / 2 ^ (15 - tc), so
                                     // 0x03 is 0.02%
              else if (maxc < 6)
                si->uplinkqual = 4;  // 0.2%
              else if (maxc == 6)
                ...
                si->uplinkqual = 2;  // 1.6% (yep, that IS bad)
              else
                si->uplinkqual = 1;
            }
            if (si->getinfo == EXTPING_UPLINKSTATS) {
              ...
              if (si->maxclients < 4) {
                ADDINFOLINE(
                    "server is too small for uplink quality statistics");
              } else {
                ...
                for (int i = 4; i <= si->maxclients; i++) {
                  ...
                  loopj(15) concatformatstring(
                      msg, "\a%c ",
                      '0' + ((si->uplinkstats[i] & 0x0F) > j) +
                          2 * ((si->uplinkstats[i] & 0xF0) > (j << 4)));
...
void serversortprepare() {
  loopv(servers) {
    ...
    if (si.protocol != PROTOCOL_VERSION)
      si.weight += 200;
    else if (!si.numplayers)
      si.weight += 500;
    else {
      si.weight += 700;
      if (serversortpreferofficial && securemapcheck(si.map, false))
        si.weight += 100;
    }
...
int sicompare(serverinfo **ap, serverinfo **bp) {
  ...
  if (a->weight < b->weight) return dir;
  enet_uint32 ai = ENET_NET_TO_HOST_32(a->address.host),
              bi = ENET_NET_TO_HOST_32(b->address.host);
  ...
  switch (serversort) {
    ...
    case SBS_MODE:  // mode
    {
      const char *am = modestr(a->mode, modeacronyms > 0),
                 *bm = modestr(b->mode, modeacronyms > 0);
...
vector<char *> favcats;
const char *fc_als[] = {"weight", "tag",   "desc", "red",    "green",
                        "blue",   "alpha", "keys", "ignore", "image"};
enum {
  FC_WEIGHT = 0,
  FC_TAG,
  FC_DESC,
...
VARF(showonlyfavourites, 0, 0, 100, {
  if (showonlyfavourites > favcats.length()) {
    conoutf("showonlyfavourites: %d out of range (0..%d)", showonlyfavourites,
            favcats.length());
...
void addfavcategory(const char *refdes) {
  ...
  loopv(favcats) if (!strcmp(favcats[i], text)) {
    intret(i + 1);
    return;
  }
  ...
  loopi(sizeof(defk) / sizeof(defk[0])) {
    if (!alx[defk[i]]) alias(favcatargname(text, defk[i]), defv[i]);
  }
...
void listfavcats() {
  const char *str =
      conc((const char **)favcats.getbuf(), favcats.length(), true);
  result(str);
  delete[] str;
}

bool favcatcheckkey(serverinfo &si,
                    const char *key) {  // IP:port, #gamemode ,%mapname, desc
  string text, keyuc;
  if (isdigit(*key))  // IP
  {
    formatstring(text)("%s:%d", si.name, si.port);
    return !strncmp(text, key, strlen(key));
  } else if (si.address.host != ENET_HOST_ANY && si.ping != 9999)
    switch (*key) {
      case '#':
      ...
      case '$':
        if (key[1]) {
          ...
          filtertext(text, text,
                     FTXT_NOCOLOR | FTXT_NOWHITE | FTXT_ALLOWBLANKS);
          int cnt = 0;
      ...
      default:
        filtertext(text, si.sdesc,
                   FTXT_NOCOLOR | FTXT_NOWHITE | FTXT_ALLOWBLANKS |
                       (showonlyfavourites > 0 ? FTXT_TOUPPER : 0));
...
const char *favcatcheck(serverinfo &si, const char *ckeys,
  ...
  while (k) {
    if (favcatcheckkey(si, k)) {
      res = true;
      if (autokeys && !(isdigit(*k) && strchr(k, ':')))
        concatformatstring(autokeys, *autokeys ? " %s" : "%s", k);
    } else {
...
void serverbrowseralternativeviews(int shiftdirection) {
  ...
  if (cats.length()) {
    loopv(cats) {
      if (cats[i] == showonlyfavourites) {
        showonlyfavourites =
            cats[(i + shiftdirection + cats.length()) % cats.length()];
        return;
      }
    }
  }
...
bool assignserverfavourites() {
  ...
  loopvj(favcats) {
    loopi(FC_NUM) {
      alx[i] = getalias(favcatargname(favcats[j], i));
      alxn[i] = alx[i] ? atoi(alx[i]) : 0;
    }
        ...
        if ((!alxn[FC_IGNORE] || showonlythiscat) && favcatcheckkey(si, k)) {
          ...
          if (si.favcat == -1 || showonlythiscat) {
            ...
            if (alxn[FC_ALPHA]) {
              ...
              new (si.bgcolor) color(
                  ((float)alxn[FC_RED]) / 100, ((float)alxn[FC_GREEN]) / 100,
                  ((float)alxn[FC_BLUE]) / 100, ((float)alxn[FC_ALPHA]) / 100);
            } else
              DELETEP(si.bgcolor)
          }
        }
  ...
  loopv(servers) if (servers[i]->favcat == -1) { DELETEA(servers[i]->bgcolor); }
  return res;
}
...
void refreshservers(void *menu, bool init) {
  ...
  if (isinfo) {
    if (lastselectedserver) {
      bool found = false;
      loopv(servers) if (lastselectedserver == servers[i]) {
        found = true;
        break;
      }
    ...
    if (lastselectedserver) {
      ...
      if (si.infotexts.length() && !pinglastselected) {
        ...
        if (init || pinglastselected || totalmillis - lastinfo >= servpingrate)
          pingservers(false, lastselectedserver);
  ...
  if ((init && issearch) ||
      totalmillis - lastinfo >=
          (servpingrate * (issearch ? 2 : 1)) /
  ...
  bool showfavtag = (assignserverfavourites() || !serverbrowserhidefavtag) &&
                    serverbrowserhidefavtag != 2;
  serversortprepare();
  ...
  if (menu) {
    ...
    loopv(servers) {
      ...
      if ((!showallservers && si.lastpingmillis <= servermenumillis) ||
          (si.maxclients > MAXCL && searchlan < 2))
        continue;  // no pong yet or forbidden
      int banned = ((si.pongflags >> PONGFLAG_BANNED) & 1) |
                   ((si.pongflags >> (PONGFLAG_BLACKLIST - 1)) & 2);
      bool showthisone =
          !(banned && showonlygoodservers) &&
          !(showonlyfavourites > 0 && si.favcat != showonlyfavourites - 1);
      ...
      if (si.address.host != ENET_HOST_ANY && si.ping != 9999) {
        if (si.protocol != PROTOCOL_VERSION) {
          if (!showonlygoodservers)
            formatstring(si.full)(
                "%s:%d [%s]", si.name, si.port,
                si.protocol < 0
                    ? "modded version"
                    : (si.protocol < PROTOCOL_VERSION ? "older protocol"
                                                      : "newer protocol"));
          else
            showthisone = false;
        } else {
          if (!hidefavicons && !showweights && showfavtag && si.favcat > -1)
            favimage = getalias(favcatargname(favcats[si.favcat], FC_IMAGE));
          copystring(text,
                     si.favcat > -1 && !favimage ? favcattags[si.favcat] : "");
          if (showweights) concatformatstring(text, "(%d)", si.weight);
          if (showfavtag && !favimage)
            formatstring(si.full)("\fs%s\fr\t", text);
          else
            copystring(si.full, showfavtag ? "\t" : "");
          concatformatstring(si.full, "\fs\f%c%s\t\fs\f%c%d/%d\fr\t\a%c  ",
                             basecolor, colorping(si.ping), plnumcolor,
                             si.numplayers, si.maxclients, '0' + si.uplinkqual);
          if (si.map[0]) {
            concatformatstring(si.full, "%s, %s", si.map,
                               modestr(si.mode, modeacronyms > 0));
            if (showmr) concatformatstring(si.full, ", (%d)", si.minremain);
          } else
            concatformatstring(si.full, "empty");
          concatstring(si.full, ": ");
          if (serverbrowserhideip < 2)
            concatformatstring(si.full, "\fs%s%s:%d\fr",
                               serverbrowserhideip == 1 ? "\f4" : "", si.name,
                               si.port);
          concatformatstring(si.full, "\fr %s", si.sdesc);
        }
      } else {
        if (!showonlygoodservers)
          formatstring(si.full)(si.address.host != ENET_HOST_ANY
                                    ? "%s:%d [waiting for server response]"
                                    : "%s:%d [unknown host]",
                                si.name, si.port);
        else
          showthisone = false;
      ...
      if (issearch && showthisone) {
        bool found = false;
        loopvj(si.playernames) if (matchplayername(si.playernames[j])) {
          found = true;
          break;
        };
      ...
      if (showthisone) {
        ...
        if (sbconnectexists) {
          ...
          for (char *p = text; (p = strchr(p, '\"')); *p++ = ' ')
            ;
          formatstring(si.cmd)("sbconnect %s %d %d %d %d %d", si.name, si.port,
                               serverfull ? 1 : 0, needspasswd ? 1 : 0, mmode,
                               banned);
          text[max(int(MAXSTRLEN - strlen(si.cmd) - 10), 0)] = '\0';
          concatformatstring(si.cmd, " \"%s\"", text);
        } else
          formatstring(si.cmd)("connect %s %d", si.name, si.port);
        menuimagemanual(menu, favimage, "serverquality", si.full, si.cmd,
                        si.bgcolor, si.description);
        ...
        if ((shownamesinbrowser && servers[i] == oldsel &&
          ...
          loopvj(si.playernames) {
            ...
            concatformatstring(
                t, " \t\fs%s%s\fr",
                !issearch || matchplayername(si.playernames[j]) ? "" : "\f4",
                si.playernames[j]);
            cur++;
            if (cur == 4) {
      ...
      if (!(showonlyfavourites > 0 &&
            (servers[i]->favcat != showonlyfavourites - 1)))
        allplayers += servers[i]->numplayers;
    }
        ...
        *titles[NUMSERVSORT] =
            {
                ...
                "%sping\t%s plr\tserver (\fs\f0max players\fr)%s%s",  // 2:
                                                                      // maxplayers
                "%sping\t%s plr\fs\f0\fr\tserver (\fs\f0minutes "
                "remaining\fr)%s%s",  // 3: minutes remaining
                ...
                "%sping\t%s plr\tserver (\fs\f0description\fr)%s%s"  // 7:
                                                                     // description
            };
    defformatstring(allplrs)("%d", allplayers);
    formatstring(title)(
        titles[serversort], showfavtag ? "fav\t" : "",
        !issearch && showallplayersnumber ? allplrs : "",
        issearch ? "      search results for \f3" : "     (F1: Help/Settings)",
        issearch ? cursearch : "");
    ...
    if (issearch) {
      if (curnl == 0)
        formatstring(notfoundmsg)("\t\tpattern \fs\f3%s\fr not found.",
                                  cursearch);
    }
...
bool serverskey(void *menu, int code, bool isdown, int unicode) {
  int fk[] = {SDLK_1, SDLK_2, SDLK_3, SDLK_4, SDLK_5,
  ...
  loopi(sizeof(fk) / sizeof(fk[0])) if (code == fk[i] && favcats.inrange(i)) {
    ...
    loopvj(servers) if (menu && (servers[j]->menuline_from <= sel &&
                                 servers[j]->menuline_to > sel)) {
      string ak;
      ak[0] = '\0';
      const char *keyalias = favcatargname(favcats[i], FC_KEYS),
                 *key = getalias(keyalias),
                 *rest = favcatcheck(*servers[j], key, ak),
                 *desc = getalias(favcatargname(favcats[i], FC_DESC));
      if (!desc) desc = "";
      if (*ak) {  // server was automatically added to this favourite group,
                  // don't remove
        conoutf(
            "server \"\fs%s\fr\" is in category '\fs%s\fr' because of key "
            "'%s', please remove manually",
            servers[j]->sdesc, desc, ak);
      } else if (rest) {  // remove from favourite group
        conoutf(
            "removing server \"\fs%s\fr\" from favourites category '\fs%s\fr' "
            "(rest '%s')",
            servers[j]->sdesc, desc, rest);
        alias(keyalias, rest);
      } else {  // add IP:port to group
        ...
        if (key && *key) {
          ...
          alias(keyalias, text);
        conoutf(
            "adding server \"\fs%s\fr\" to favourites category '\fs%s\fr' (new "
            "'%s')",
            servers[j]->sdesc, desc, getalias(keyalias));
      }
      return true;
    }
  }
...
int progress_callback_retrieveservers(void *data, float progress) {
  if (progress < 0)
    show_out_of_renderloop_progress(progress + 1.0f,
...
void retrieveservers(vector<char> &data) {
  if (mastertype) {
    ...
    if (h.set_host(mastername)) {
      formatstring(progresstext)(
          "retrieving servers from %s:%d... (esc to abort)", mastername,
          masterport);
      defformatstring(url)(
          "/retrieve.do?action=list&name=%s&version=%d&build=%d",
          urlencode(global_name, true), AC_VERSION, getbuildtype() | (1 << 16));
    ...
    clfail = false;
    defformatstring(text)("retrieving servers from %s:%d... (esc to abort)",
    ...
    while (reqlen > 0) {
      ...
      timeout = SDL_GetTicks() - starttime;
      show_out_of_renderloop_progress(min(float(timeout) / RETRIEVELIMIT, 1.0f),
                                      text);
    ...
    if (reqlen <= 0)
      for (;;) {
        ...
        show_out_of_renderloop_progress(
            min(float(timeout) / RETRIEVELIMIT, 1.0f), text);
        if (interceptkey(SDLK_ESCAPE)) timeout = RETRIEVELIMIT + 1;
        if (timeout > RETRIEVELIMIT) break;
      }
    if (data.length()) data.add('\0');
...
void updatefrommaster(int *force) {
  ...
  if (!*force && lastupdate &&
      totalmillis - lastupdate < masterupdatefrequency * 1000)
    return;
  ...
  if (data.empty()) {
    if (!clfail)
      conoutf(
          "Master server is not replying. \f1Get more information at "
          "http://masterserver.cubers.net/");
    cllock = !clfail;
  } else {
    ...
    clearservers();
    if (!strncmp(data.getbuf(), "addserver", 9))
      cllock = false;  // the ms could reply other thing... but currently, this
                       // is useless
    if (!cllock) {
      execute(data.getbuf());
