...
static serverinfo *
newserver (const char *name, uint ip = ENET_HOST_ANY, int port = CUBE_DEFAULT_SERVER_PORT, int weight = 0)
{
...
#undef ADDINFOLINE
        ...
        if (si->pongflags > 0)
        {
            ...
            if (si->pongflags & (1 << PONGFLAG_BANNED)) sp = "you are banned from this server";
...
vector<char *> favcats;
const char *fc_als[]
= { "weight", "tag", "desc", "red", "green", "blue", "alpha", "keys", "ignore", "image" };
enum
{
    FC_WEIGHT = 0,
    FC_TAG,
...
{ // IP:port, #gamemode ,%mapname, desc
    ...
    if (isdigit (*key)) // IP
    {
        ...
        case '$':
            if (key[1])
            {
                formatstring (text) (R"(%s "%s" %d %d %d %d %d "%s" %d %d)", key + 1, si.map,
                                     si.mode, si.ping, si.minremain, si.numplayers, si.maxclients,
                                     si.name, si.port, si.pongflags);
...
void serverbrowseralternativeviews (int shiftdirection)
{
    ...
    if (cats.length ())
    {
        loopv (cats)
        {
            ...
            {
                showonlyfavourites = cats[(i + shiftdirection + cats.length ()) % cats.length ()];
                return;
            }
        }
    }
...
bool assignserverfavourites ()
{
    ...
    loopvj (favcats)
    {
        ...
        while (k)
        {
            ...
            {
                serverinfo &si = *servers[i];
                if (si.address.host == ENET_HOST_ANY || si.ping == 9999 || si.protocol != PROTOCOL_VERSION)
                    continue;
                if ((!alxn[FC_IGNORE] || showonlythiscat) && favcatcheckkey (si, k))
                {
                    ...
                    if (si.favcat == -1 || showonlythiscat)
                    {
                        ...
                        if (alxn[FC_ALPHA])
                        {
                            ...
                            new (si.bgcolor)
                            color (((float)alxn[FC_RED]) / 100, ((float)alxn[FC_GREEN]) / 100,
...
void refreshservers (void *menu, bool init)
{
    ...
    if ((init && issearch)
        || totalmillis - lastinfo
           >= (servpingrate * (issearch ? 2 : 1))
              / (maxservpings ? max (1, (servers.length () + maxservpings - 1) / maxservpings) : 1))
    ...
    if (menu)
    {
        ...
        loopv (servers)
        {
            ...
            if (si.address.host != ENET_HOST_ANY && si.ping != 9999)
            {
                if (si.protocol != PROTOCOL_VERSION)
                {
                    if (!showonlygoodservers)
                        formatstring (si.full) ("%s:%d [%s]", si.name, si.port,
                                                si.protocol < 0 ?
                                                "modded version" :
                                                (si.protocol < PROTOCOL_VERSION ? "older protocol" : "newer protocol"));
                    else
                        showthisone = false;
                }
                else
                {
                    ...
                    concatformatstring (si.full, "\fs\f%c%s\t\fs\f%c%d/%d\fr\t\a%c  ", basecolor,
                                        colorping (si.ping), plnumcolor, si.numplayers,
                                        si.maxclients, '0' + si.uplinkqual);
                    if (si.map[0])
                    {
                    ...
                    if (serverbrowserhideip < 2)
                        concatformatstring (si.full, "\fs%s%s:%d\fr",
                                            serverbrowserhideip == 1 ? "\f4" : "", si.name, si.port);
            ...
            if (showthisone)
            {
                ...
                if (sbconnectexists)
                {
                    ...
                    formatstring (si.cmd) ("sbconnect %s %d %d %d %d %d", si.name, si.port,
                                           serverfull ? 1 : 0, needspasswd ? 1 : 0, mmode, banned);
                ...
                if ((shownamesinbrowser && servers[i] == oldsel && si.playernames.length ()) || issearch)
                {
                    ...
                    loopvj (si.playernames)
                    {
                        ...
                        concatformatstring (t, " \t\fs%s%s\fr",
            ...
            if (!(showonlyfavourites > 0 && (servers[i]->favcat != showonlyfavourites - 1)))
        ...
        formatstring (title) (titles[serversort], showfavtag ? "fav\t" : "",
...
bool serverskey (void *menu, int code, bool isdown, int unicode)
{
    ...
    loopi (sizeof (fk) / sizeof (fk[0])) if (code == fk[i] && favcats.inrange (i))
    {
        ...
        loopvj (servers) if (menu && (servers[j]->menuline_from <= sel && servers[j]->menuline_to > sel))
        {
            string ak;
            ak[0] = '\0';
            const char *keyalias = favcatargname (favcats[i], FC_KEYS), *key = getalias (keyalias),
                       *rest = favcatcheck (*servers[j], key, ak),
                       *desc = getalias (favcatargname (favcats[i], FC_DESC));
            if (!desc) desc = "";
            if (*ak)
            { // server was automatically added to this favourite group,
              // don't remove
                conoutf ("server \"\fs%s\fr\" is in category '\fs%s\fr' because of key "
                         "'%s', please remove manually",
                         servers[j]->sdesc, desc, ak);
            }
            else if (rest)
            { // remove from favourite group
                conoutf ("removing server \"\fs%s\fr\" from favourites category '\fs%s\fr' "
                         "(rest '%s')",
            ...
            { // add IP:port to group
                ...
                if (key && *key)
                {
                    ...
                    alias (keyalias, text);
                conoutf ("adding server \"\fs%s\fr\" to favourites category '\fs%s\fr' (new "
                         "'%s')",
...
{
    if (progress < 0)
        show_out_of_renderloop_progress (progress + 1.0f, "waiting for response (esc to abort)");
    else
...
void retrieveservers (vector<char> &data)
{
    if (mastertype)
    {
        ...
        if (h.set_host (mastername))
        {
            ...
            defformatstring (url) ("/retrieve.do?action=list&name=%s&version=%d&build=%d",
...
void updatefrommaster (int *force)
{
    ...
    {
        if (!clfail)
            conoutf ("Master server is not replying. \f1Get more information at "
