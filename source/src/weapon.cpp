...
static inline bool intersectbox (const vec &o,
                                 const vec &rad,
                                 const vec &from,
                                 const vec &to,
                                 vec *end) // if lineseg hits entity bounding box
{
    ...
    if (p->x <= o.x + rad.x && p->x >= o.x - rad.x && p->y <= o.y + rad.y && p->y >= o.y - rad.y
        && p->z <= o.z + rad.z && p->z >= o.z - rad.z)
    {
        if (end) *end = *p;
        return true;
    }
...
static inline bool
intersectcylinder (const vec &from, const vec &to, const vec &start, const vec &end, float radius, float &dist)
{
...
playerent *
intersectclosest (const vec &from, const vec &to, const playerent *at, float &bestdistsquared, int &hitzone, bool aiming = true)
{
    ...
    loopv (players)
    {
        ...
        if (!o || o == at
            || (o->state != CS_ALIVE && (aiming || (o->state != CS_EDITING && o->state != CS_LAGGED))))
            continue;
...
inline bool intersecttriangle (const vec &from,
                               const vec &dir,
                               const vec &v0,
                               const vec &v1,
                               const vec &v2,
...
inline bool
intersecttriangle2 (const vec &from, const vec &to, const vec &v0, const vec &v1, const vec &v2, vec *end, float *_t)
{
    vec dir = to;
...
inline bool
intersectcorner (const vec &from, const vec &dir, int x, int y, int size, bool cdir, vec *end, float *_t)
{
...
void intersectgeometry (const vec &from,
            ...
            if (t > 0) loopk (2)
                {
                    ...
                    { // cube s is a probable hit, examine further
                        ...
                        if (s->type == CORNER)
                        {
                            ...
                            vec newto;
                            float newdist;
                            if (intersectcorner (from, d, bx, by, bs, !(q & 1), &newto, &newdist)
                                && newdist < distmin && (!h || newto.z < ns->floor || newto.z > ns->ceil))
                                to = newto, distmin = newdist;
                            if (d.z)
                            { // intersect with z == const plane where the corner
                              // ends
                                ...
                                vec pz = d;
                                pz.mul (tz).add (from);
                                if (pz.x >= bx && pz.x <= bx + bs && pz.y >= by && pz.y <= by + bs && tz < distmin)
                                    to = pz, distmin = tz;
                                if (h)
                                { // intersect with the triangles where socket corners
                                  // end
                                    ...
                                    if (hit && cx >= 0 && cy >= 0 && cx <= bs && cy <= bs && tz < distmin)
                                        to = pz, distmin = tz;
                                }
                            }
                        }
                        else
                        {
                            ...
                            if (SOLID (s) || s->type == SPACE || (s->type == CHF && s->floor > p.z)
                            ...
                            float h = downwards ?
                                      s->floor - (isflat && s->type == FHF ? vdelta[0] / 4.0f : 0) :
                                      s->ceil + (isflat && s->type == CHF ? vdelta[0] / 4.0f : 0);
                            if (isflat || (downwards && s->type == CHF) || (!downwards && s->type == FHF))
                            { // intersect with z == const plane on either
                            ...
                            if (!isflat)
                            { // sloped CHF or FHF: check two triangles,
                              ...
                              // can be seen looking upwards)
                                ...
                                v1 (x + 1, y, hh + vdelta[1] / 4.0f), v2 (x, y + 1, hh + vdelta[2] / 4.0f),
                                v3 (x + 1, y + 1, hh + vdelta[3] / 4.0f);
                                float newdist;
                                vec newto;
                                if (s->type == FHF)
                                {
                                    if (intersecttriangle (from, d, v0, v1, v2, &newto, &newdist) && newdist < distmin)
                                        to = newto, distmin = newdist;
                                    if (intersecttriangle (from, d, v3, v1, v2, &newto, &newdist) && newdist < distmin)
                                        to = newto, distmin = newdist;
                                }
                                else
                                {
                                    if (intersecttriangle (from, d, v0, v1, v3, &newto, &newdist) && newdist < distmin)
                                        to = newto, distmin = newdist;
                                    if (intersecttriangle (from, d, v2, v0, v3, &newto, &newdist) && newdist < distmin)
                                        to = newto, distmin = newdist;
                                }
                            }
                        }
...
void movebounceents ()
{
    loopv (bounceents) if (bounceents[i])
    {
        ...
        if ((p->bouncetype == BT_NADE || p->bouncetype == BT_GIB) && p->applyphysics ())
...
void shorten (const vec &from, vec &target, float distsquared)
{
    ...
    if (m < 0.07f)
        target.add (vec (0.1f, 0.1f, 0.1f)); // if "from == target" just fake a target to
    ...
    if (d->weaponsel->type == GUN_SHOTGUN)
    {
        ...
        loopk (3) loopi (SGRAYS)
        {
            ...
            if (hits[h])
            {
                ...
                int info = (withBONUS ? SGDMGBONUS : 0) | (numhits_c << 8) | (numhits_m << 16)
                           | (numhits_o << 24);
                if (numhits)
                    hitpush (dmgreal, o, d, from, to, d->weaponsel->type, numhits == SGRAYS * 3, info);
...
void r_accuracy (int h)
{
    ...
    if (accuracy && valid_weapon (i) && i != GUN_CPISTOL)
    {
        ...
        {
            formatstring (line) ("\f5%5.1f%% (%d/%d): \f0%s", acc, (int)accuracym[i].hits,
                                 accuracym[i].shots, weapstr (i));
        }
        blendbox (x_offset, y_offset + FONTH, x_offset + text_width (line) + 2 * FONTH,
                  y_offset - FONTH, false, -1);
...
#undef RNDD
    ...
    if (recoiltest)
    {
        ...
        owner->pitchvel = min (powf (shots / (float)(recoilincrease), 2.0f) + (float)(recoilbase) / 10.0f,
                               (float)(maxrecoil) / 10.0f);
    }
    else
    {
        owner->vel.add (vec (unitv).mul (recoil / dist).mul (owner->crouching ? 0.75 : 1.0f));
        owner->pitchvel = min (powf (shots / (float)(g.recoilincrease), 2.0f) + (float)(g.recoilbase) / 10.0f,
...
void weapon::renderhudmodel (int lastaction, int index)
{
    ...
    rendermodel (path, wm.anim | ANIM_DYNALLOC | (righthanded == index ? ANIM_MIRROR : 0) | (emit ? ANIM_PARTICLE : 0),
                 0, -1, wm.pos, 0, p->yaw + 90, p->pitch + wm.k_rot, 40.0f, wm.basetime, nullptr,
                 nullptr, 1.28f);
}
...
{
    if (!editmode)
        drawcrosshair (owner, teamwarning ? CROSSHAIR_TEAMMATE : owner->weaponsel->type);
    else
...
{
    return this != owner->weaponsel && owner->state == CS_ALIVE && !owner->weaponchanging;
}
...
void grenadeent::_throw (const vec &from, const vec &vel)
{
    ...
    if (local)
    {
        addmsg (SV_THROWNADE, "ri7", int(o.x * DMF), int(o.y * DMF), int(o.z * DMF),
                int(vel.x * DMF), int(vel.y * DMF), int(vel.z * DMF), lastmillis - millis);
...
void grenadeent::onmoved (const vec &dist) { distsincebounce += dist.magnitude (); }
            ...
            if (this == owner->weaponsel)
            {
                if (quicknade || !mag)
                {
                    ...
                    if (quicknade)
                        ...
                        = owner->prevweaponsel; // switch to previous weapon immediately
                    else
                        owner->nextweaponsel = owner->weaponsel = owner->primweap; // switch to primary immediately
                }
            }
            reset ();
            return false;
...
void grenades::renderstats ()
{
    ...
    draw_text (gunstats, oldfashionedgunstats ? HUDPOS_GRENADE + HUDPOS_NUMBERSPACING + 25 : HUDPOS_GRENADE + HUDPOS_NUMBERSPACING,
               823);
}
...
bool gun::attack (vec &targ)
{
    ...
    if (burstshotssettings[this->type] > 0 && shots >= burstshotssettings[this->type])
...
{
    return shots > 2 ? 70 : (info.spread + (shots > 0 ? (shots == 1 ? 5 : 10) : 0));
...
{
    return scoped && lastmillis - scoped_since > SCOPESETTLETIME ? info.recoil / 3 : info.recoil;
}
...
void sniperrifle::renderaimhelp (bool teamwarning)
{
    ...
    if (!editmode)
    {
        if (scoped || teamwarning)
            drawcrosshair (owner, teamwarning ? CROSSHAIR_TEAMMATE : CROSSHAIR_SCOPE, nullptr, 24.0f);
    }
    else
        drawcrosshair (owner, CROSSHAIR_EDIT);
}
    ...
    if (player1->akimbo)
    {
        ...
        if (a.timerout () || player1->state == CS_DEAD || player1->state == CS_SPECTATE)
        {
            ...
            if (player1->weaponsel->type == GUN_AKIMBO
                || (player1->weaponchanging && player1->nextweaponsel->type == GUN_AKIMBO))
            {
