...
const char *mdlattrnames[] = { "keywords", "desc",         "defaults", "usage",    "author",
                               "license",  "distribution", "version",  "requires", "" };
...
mapmodelinfo *getmminfo (int i) { return mapmodels.inrange (i) ? &mapmodels[i] : nullptr; }
                         ...
                         char *opt) // delete mapmodel slot - only if unused or "purge" is specified
{
    ...
    if (!purgeall) loopv (ents) if (ents[i].type == MAPMODEL && ents[i].attr2 == *n)
        {
                       ...
                       char *name) // edit slot parameters != ""
{
    ...
    if (mapmodels.inrange (*n))
    {
        ...
        if ((*rad || *h || *zoff || *scale || *name) && !noteditmode ("editmapmodelslot")
            && !multiplayer ("editmapmodelslot"))
        { // change attributes
        ...
        formatstring (res) ("%d %d %d %s \"%s\"", mmi.rad, mmi.h, mmi.zoff, floatstr (mmi.scale, true),
...
int tempmmunsort (tempmmslot *a, tempmmslot *b) { return a->oldslot[0] - b->oldslot[0]; }
...
void sortmapmodelslots (char **args, int numargs)
{
    ...
    conoutf ("%d mapmodel slots%s, %d %sslots merged", tempslots.length (), nosort ? "" : " sorted",
             mapmodels.length () - tempslots.length (), mergeused ? "" : "unused ");
...
void setmodelattributes (const char *name, mapmodelattributes &ma)
{
    ...
    mapmodelattributes **mrp = mdlregistry.access (name), *mr = mrp ? *mrp : nullptr, *r = mr;
    if (!r)
    {
...
void mapmodelregister_ (char **args,
...
{
    enumerate (mdlregistry, mapmodelattributes *, m, loopi (MMA_NUM) DELSTRING (m->n[i]); DELETEP (m););
    mdlregistry.clear ();
    mapmodelchanged = 1;
}
                  ...
                  bool trydl) // load model by name (optional) or from mapmodels[i]
{
    ...
    else
    {
        ...
        if (loadingmodel && m)
        {
            ...
            {
                conoutf ("\f3mapmodel config error: disabling face culling in combination "
...
void writemapmodelattributes ()
{
    ...
    if (f)
    {
        ...
        mmas.sort (mmasort);
        f->printf ("// automatically written on exit. this is cached information "
...
void listallmapmodelattributes (char **args,
...
void renderbatchedmodel (model *m, batchedmodel &b)
{
    ...
    if (stenciling)
    {
        m->render (b.anim | ANIM_NOSKIN, b.varseed, b.speed, b.basetime, b.o, b.roll, b.yaw,
                   b.pitch, b.d, a, b.scale);
        return;
    }
                  ...
                  float scale)
{
    if (popdeadplayers && d && a)
    {
        ...
        if (acv == ANIM_DECAY || acv == ANIM_LYING_DEAD || acv == ANIM_CROUCH_DEATH || acv == ANIM_DEATH)
            return;
    }
    ...
    if (rad >= 0)
    {
        if (!rad)
        {
            rad = m->radius;
            if (roll != 0 || pitch != 0)
                rad = max (m->radius, m->zradius); // FIXME: this assumes worst-case even
...
{
    const char *names[]
    = { "idle",        "run",           "attack",      "pain",         "jump",      "land",
        "flipoff",     "salute",        "taunt",       "wave",         "point",     "crouch idle",
        "crouch walk", "crouch attack", "crouch pain", "crouch death", "death",     "lying dead",
        "flag",        "gun idle",      "gun shoot",   "gun reload",   "gun throw", "mapmodel",
        "trigger",     "decay",         "all" };
...
void renderclient (playerent *d, const char *mdlname, const char *vwepname, int tex)
{
    ...
    else if (d->weaponsel == d->lastattackweapon && lastmillis - d->lastaction < 300
             && d->lastpain < d->lastaction && !d->weaponsel->reloading
             && (d->weaponsel->type != GUN_GRENADE || ((grenades *)d->weaponsel)->cookingmillis > 0))
    {
    ...
    rendermodel (mdlname, anim | ANIM_DYNALLOC, tex, 1.5f, o, 0, d->yaw + 90, d->pitch / 4, speed,
                 basetime, d, a);
    if (!stenciling && !reflecting && !refracting)
    {
...
void renderclientp (playerent *d)
{
    ...
    else
    {
        if (!m_teammode || !teamdisplaymode)
            ...
            {
            case 1:
                formatstring (skin) ("%s/%s/%02d_%svest.jpg", skinbase, teamname, skinid, team ? "blue" : "red");
                break;
            case 2:
            default:
                formatstring (skin) ("%s/%s/%s.jpg", skinbase, teamname, team ? "blue" : "red");
                break;
            }
    }
    string vwep;
...
{
    playerent *d;
    loopv (players) if ((d = players[i]) && d->state != CS_SPAWNING && d->state != CS_SPECTATE
