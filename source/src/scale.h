...
#define OP(c, n) \
    dst[n]       \
    = (uint (src[n]) + uint (src[n + BPP]) + uint (src[stride + n]) + uint (src[stride + n + BPP])) >> 2
            PIXELOP
#undef OP
        }
        src += stride;
...
#undef OP
            for (uchar *ycur = src, *xend = &ycur[wfrac * BPP], *yend = &src[hfrac * stride];
                 ycur < yend; ycur += stride, xend += stride)
            {
...
static void FUNCNAME (scaletexture) (uchar *src, uint sw, uint sh, uchar *dst, uint dw, uint dh)
{
    ...
    {
        uint y2 = y + hfrac, yi2 = y2 >> 12, h = y2 - y, ih = yi2 - yi, ylow, yhigh;
        if (yi < yi2)
        {
            ylow = 0x1000U - (y & 0xFFFU);
            yhigh = y2 & 0xFFFU;
        }
        else
        {
        ...
        {
            uint x2 = x + wfrac, xi2 = x2 >> 12, w = x2 - x, iw = xi2 - xi, iy = 0, xlow, xhigh;
            if (xi < xi2)
            {
                xlow = 0x1000U - (x & 0xFFFU);
                xhigh = x2 & 0xFFFU;
            }
            else
            {
...
#define OP(c, n) c##t = 0
            DEFPIXEL
#undef OP
            for (uchar *ycur = &src[xi * BPP], *xend = &ycur[max (iw, 1U) * BPP],
                       *yend = &ycur[stride * (max (ih, 1U) + (yhigh ? 1 : 0))];
                 ycur < yend; ycur += stride, xend += stride, iy++)
            {
