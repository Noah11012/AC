...
static void extrudeshadowtiles(int x1, int y1, int x2, int y2, int x3, int y3) {
  ...
  int lx = x1, rx = x1, fracl = 0, fracr = 0, dlx, dly, drx, dry;
  if (x2 <= x3) {
    dlx = x2;
    dly = y2;
    drx = x3;
  ...
  for (; cy < y2; cy++) {
    ...
    if (cx1 < SHADOWCOLUMNS && cx2 >= 0)
      shadowtiles[cy] |= (SHADOWCOLUMNMASK >>
                          (SHADOWCOLUMNS - (min(cx2, SHADOWCOLUMNS - 1) + 1))) &
                         (SHADOWCOLUMNMASK << max(cx1, 0));
  }
...
static void addshadowtiles(float x1, float y1, float x2, float y2) {
  ...
  int tx1 =
          clamp(int(floor((y1 + 1) / 2 * SHADOWCOLUMNS)), 0, SHADOWCOLUMNS - 1),
      ty1 = clamp(int(floor((x1 + 1) / 2 * SHADOWROWS)), 0, SHADOWROWS - 1),
      tx2 =
          clamp(int(floor((y2 + 1) / 2 * SHADOWCOLUMNS)), 0, SHADOWCOLUMNS - 1),
      ty2 = clamp(int(floor((x2 + 1) / 2 * SHADOWROWS)), 0, SHADOWROWS - 1);

  uint mask = (SHADOWCOLUMNMASK >> (SHADOWCOLUMNS - (tx2 + 1))) &
              (SHADOWCOLUMNMASK << tx1);
  for (int y = ty1; y <= ty2; y++) shadowtiles[y] |= mask;
}

bool addshadowbox(const vec &bbmin, const vec &bbmax, const vec &extrude,
                  const glmatrixf &mat) {
  vec4 v[8];
  ...
  loopi(8) {
    vec4 &p = v[i];
    mat.transform(vec(i & 1 ? bbmax.x : bbmin.x, i & 2 ? bbmax.y : bbmin.y,
                      i & 4 ? bbmax.z : bbmin.z),
                  p);
    if (p.z >= -p.w) {
  ...
  if (front < 8 || ev.z < -ev.w) loopi(8) {
      ...
      if (p.z >= -p.w) {
        ...
        float t = ev.z / (ev.z - p.z), w = ev.w + t * (p.w - ev.w),
              x = (ev.x + t * (p.x - ev.x)) / w,
              y = (ev.y + t * (p.y - ev.y)) / w;
        sx1 = min(sx1, x);
        sy1 = min(sy1, y);
      ...
      loopj(3) {
        ...
        if (o.z < -o.w) continue;
        float t = p.z / (p.z - o.z), w = p.w + t * (o.w - p.w),
              x = (p.x + t * (o.x - p.x)) / w, y = (p.y + t * (o.y - p.y)) / w;
        sx1 = min(sx1, x);
        sy1 = min(sy1, y);
        sx2 = max(sx2, x);
        sy2 = max(sy2, y);
      ...
      if (ev.z < -ev.w) continue;
      float t = p.z / (p.z - ev.z), w = p.w + t * (ev.w - p.w),
            x = (p.x + t * (ev.x - p.x)) / w, y = (p.y + t * (ev.y - p.y)) / w;
      sx1 = min(sx1, x);
      sy1 = min(sy1, y);
      sx2 = max(sx2, x);
      sy2 = max(sy2, y);
  ...
  if (ev.z >= -ev.w) {
    ...
    auto tx = int(floor(SHADOWCOLUMNS * (y + 1) / 2)),
        ...
        tx2 = int(floor(SHADOWCOLUMNS * (sy2 + 1) / 2)),
        ty2 = int(floor(SHADOWROWS * (sx2 + 1) / 2));
    if (tx < tx1) {
      if (ty < ty1) {
        swap(ty1, ty2);
        tx1--;
  ...
  noextrusion:;
  } else if (sx1 >= 1 || sy1 >= 1 || sx2 <= -1 || sy2 <= -1)
    return false;
  shadowcasters++;
...
static void rendershadowtiles() {
  ...
  float clipx1 = (shadowy1 + 1) / 2, clipy1 = (shadowx1 + 1) / 2,
        clipx2 = (shadowy2 + 1) / 2, clipy2 = (shadowx2 + 1) / 2;
  if (!shadowclip) {
    clipx1 = clipy1 = 0;
    clipx2 = clipy2 = 1;
  }
  ...
  loop(y, SHADOWROWS + 1) {
    ...
    while (mask) {
      ...
      uint strip = (SHADOWCOLUMNMASK >> (SHADOWCOLUMNS - x)) &
                   (SHADOWCOLUMNMASK << xstart);
      int yend = y;
      do {
        shadowtiles[yend] &= ~strip;
        yend++;
      } while ((shadowtiles[yend] & strip) == strip);
      float vx = xstart * tw, vy = y * th, vw = (x - xstart) * tw,
            vh = (yend - y) * th, vx1 = max(vx, clipx1), vy1 = max(vy, clipy1),
            vx2 = min(vx + vw, clipx2), vy2 = min(vy + vh, clipy2);
      glVertex2f(vy1, vx1);
      glVertex2f(vy1, vx2);
      glVertex2f(vy2, vx2);
      glVertex2f(vy2, vx1);
...
void drawstencilshadows() {
  ...
  if (hasST2 || hasSTS) {
    ...
    if (hasST2) {
      ...
      glStencilOpSeparate_(GL_BACK, GL_KEEP, GL_KEEP,
                           hasSTW ? GL_INCR_WRAP_EXT : GL_INCR);
      glStencilOpSeparate_(GL_FRONT, GL_KEEP, GL_KEEP,
                           hasSTW ? GL_DECR_WRAP_EXT : GL_DECR);
    }
  ...
  // punts to software mode if separate stencil op is set, even while stencil
  // disabled, when drawing lines!
  glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);
