...
static void extrudeshadowtiles (int x1, int y1, int x2, int y2, int x3, int y3)
{
    ...
    for (; cy < y2; cy++)
    {
        ...
        if (cx1 < SHADOWCOLUMNS && cx2 >= 0)
            shadowtiles[cy] |= (SHADOWCOLUMNMASK >> (SHADOWCOLUMNS - (min (cx2, SHADOWCOLUMNS - 1) + 1)))
...
bool addshadowbox (const vec &bbmin, const vec &bbmax, const vec &extrude, const glmatrixf &mat)
{
    ...
    if (front < 8 || ev.z < -ev.w) loopi (8)
        {
            ...
            if (p.z >= -p.w)
            {
                ...
                float t = ev.z / (ev.z - p.z), w = ev.w + t * (p.w - ev.w),
                      x = (ev.x + t * (p.x - ev.x)) / w, y = (ev.y + t * (p.y - ev.y)) / w;
                sx1 = min (sx1, x);
                sy1 = min (sy1, y);
                sx2 = max (sx2, x);
                sy2 = max (sy2, y);
    ...
    if (ev.z >= -ev.w)
    {
        ...
        if ((sx1 >= 1 && x >= 1) || (sy1 >= 1 && y >= 1) || (sx2 <= -1 && x <= -1) || (sy2 <= -1 && y <= -1))
            return false;
        auto tx = int(floor (SHADOWCOLUMNS * (y + 1) / 2)), ty = int(floor (SHADOWROWS * (x + 1) / 2)),
...
static void rendershadowtiles ()
{
    ...
    float clipx1 = (shadowy1 + 1) / 2, clipy1 = (shadowx1 + 1) / 2, clipx2 = (shadowy2 + 1) / 2,
          clipy2 = (shadowx2 + 1) / 2;
    if (!shadowclip)
    {
        clipx1 = clipy1 = 0;
    ...
    loop (y, SHADOWROWS + 1)
    {
        ...
        while (mask)
        {
            ...
            float vx = xstart * tw, vy = y * th, vw = (x - xstart) * tw, vh = (yend - y) * th,
                  vx1 = max (vx, clipx1), vy1 = max (vy, clipy1), vx2 = min (vx + vw, clipx2),
