...
color *menuselbgcolor, *menuseldescbgcolor = nullptr;
static int menurighttabwidth = 888; // width of sliders and text input fields (is adapted to font
                                    // and screen size later)
...
{
    menurighttabwidth = max (VIRTW / 4,
                             15 * text_width ("w")); // adapt to screen and font size every time a menu opens
    if (!name)
    {
        setcurmenu (nullptr);
        return;
...
{
    if (isselection ())
        blendbox (x - FONTH / 4, y - FONTH / 6, x + w + FONTH / 4, y + FONTH + FONTH / 6, false, -1, c);
    else
...
{
    return parent->allowinput && !parent->hotkeys && parent->items.inrange (parent->menusel)
...
struct mitemmanual : mitem
{
    ...
    mitemmanual (gmenu *parent, char *text, char *action, char *hoveraction, color *bgcolor, const char *desc)
    : mitem (parent, bgcolor, mitem::TYPE_MANUAL), text (text), action (action), hoveraction (hoveraction), desc (desc)
    {
    }
    ...
    void render (int x, int y, int w) override
    {
    ...
    const char *getdesc () override { return desc; }
    const char *gettext () override { return text; }
    const char *getaction () override { return action; }
};

struct mitemtext : mitemmanual
{
    mitemtext (gmenu *parent, char *text, char *action, char *hoveraction, color *bgcolor, const char *desc = nullptr)
...
struct mitemimagemanual : mitemmanual
{
    ...
    mitemimagemanual (gmenu *parent,
                      const char *filename,
                      const char *altfontname,
                      char *text,
                      char *action,
                      char *hoveraction,
                      color *bgcolor,
    ...
    ~mitemimagemanual () override = default;
    int width () override
    {
        if (image && *text != '\t')
...
struct mitemimage : mitemimagemanual
{
    mitemimage (gmenu *parent,
                ...
                color *bgcolor,
                const char *desc = nullptr,
                const char *altfont = nullptr)
    : mitemimagemanual (parent, filename, altfont, text, action, hoveraction, bgcolor, desc)
    {
    }
    ~mitemimage () override
    {
...
struct mitemmapload : mitemmanual
{
    ...
    mitemmapload (gmenu *parent, const char *filename, char *text, char *action, char *hoveraction, const char *desc)
    : mitemmanual (parent, text, action, hoveraction, nullptr, desc),
      filename (filename),
      mapmessage (nullptr),
      image (nullptr)
    {
    ...
    void key (int code, bool isdown, int unicode) override
    {
    ...
    void render (int x, int y, int w) override
    {
        ...
        if (isselection ())
        {
            ...
            { // load map description and preview picture
                ...
                if (browsefiledesc)
                { // checking map messages is slow and may be
                  // disabled
                    mapmessage = getfiledesc (cgzpath, behindpath (filename),
                                              "cgz"); // check for regular map
                    if (!mapmessage)
                        mapmessage
                        = getfiledesc ((cgzpath = "packages" PATHDIVS "maps" PATHDIVS "official"),
...
struct mitemtextinput : mitemtext
{
    ...
    mitemtextinput (gmenu *parent, char *text, char *value, char *action, char *hoveraction, color *bgcolor, int maxchars, int maskinput)
    ...
    int width () override { return text_width (text) + menurighttabwidth; }
    ...
    void render (int x, int y, int w) override
    {
        ...
        draw_text (text, x, y, c, c, c);
        auto cibl = (int)strlen (input.buf); // current input-buffer length
        int iboff = input.pos > 14 ?
                    (input.pos < cibl ? input.pos - 14 : cibl - 14) :
                    (input.pos == -1 ? (cibl > 14 ? cibl - 14 : 0) : 0); // input-buffer offset
        string showinput, tempinputbuf;
        int sc = 14;
        copystring (tempinputbuf, input.buf);
        if (hideinput && !(SDL_GetModState () & MOD_KEYS_CTRL))
        { // "mask" user input with asterisks, use for
        ...
        draw_text (showinput, x + w - menurighttabwidth, y, c, c, c, 255,
                   sel && !greyedout ? (input.pos >= 0 ? (input.pos > sc ? sc : input.pos) : cibl) : -1);
    }
...
struct mitemslider : mitem
{
    ...
    mitemslider (gmenu *parent, char *text, int _min, int _max, char *value, char *display, char *action, color *bgcolor, bool wrap, bool isradio)
      ...
      isradio (isradio)
    {
        ...
        if (!*display || *enddigits)
        {
            ...
            {
                if (max_ != -1)
                    clientlogf ("menuitemslider: display string length (%d) doesn't match "
                                "max-min (%d) \"%s\" [%s]",
    ...
    int width () override
    {
        return text_width (text) + (isradio ? 0 : menurighttabwidth) + maxvaluewidth + 2 * FONTH;
    }
    ...
    void render (int x, int y, int w) override
    {
        ...
        int tw = text_width (text), ow = isradio ? text_width (curval) : menurighttabwidth,
        ...
        if (sel)
        {
            ...
            if (pos - ow > FONTH / 2)
                renderbg (x + w - pos + ow + FONTH / 2, y, pos - ow - FONTH / 2, menuseldescbgcolor);
        }
        int c = greyedout ? 128 : 255;
        draw_text (text, x, y, c, c, c);
        ...
        if (!isradio)
        {
            ...
            auto offset = (int)(cval / ((float)range) * menurighttabwidth);
            blendbox (x + w - menurighttabwidth + offset - FONTH / 6, y,
                      x + w - menurighttabwidth + offset + FONTH / 6, y + FONTH, false, -1,
                      greyedout ? &gray : (sel ? &whitepulse : &white));
        }
    }
    ...
    void displaycurvalue ()
    {
        ...
        {
            *curval = '\0';
            loopv (opts) concatformatstring (curval, "%s\1%c %s", i ? "  " : "",
...
struct mitemkeyinput : mitem
{
    ...
    mitemkeyinput (gmenu *parent, char *text, char *bindcmd, color *bgcolor, int bindtype)
    ...
    ~mitemkeyinput () override
    {
        DELETEA (text);
        DELETEA (bindcmd);
    }

    int width () override { return text_width (text) + text_width (keyname ? keyname : " "); }
    ...
    void render (int x, int y, int w) override
    {
        ...
        if (isselection ())
        {
            blendbox (x + w - tw - FONTH, y - FONTH / 6, x + w + FONTH, y + FONTH + FONTH / 6,
                      false, -1, capture ? &capturec : nullptr);
    ...
    void init () override
    {
        ...
        capture = false;
        menuheader (parent, nullptr, nullptr);
    }
    ...
    void key (int code, bool isdown, int unicode) override
    {
        ...
        if (isdown)
        {
            if (!known)
            {
                ...
                {
                    static defformatstring (
                    keybound) ("\f3Warning: \"%s\" key is already in use. You can press ESC to "
                               "cancel.",
                               km->name);
                    menuheader (parent, nullptr, keybound);
                }
                newkeys.add (code);
...
struct mitemcheckbox : mitem
{
    ...
    mitemcheckbox (gmenu *parent, char *text, char *value, char *action, int pos, color *bgcolor)
    ...
    void render (int x, int y, int w) override
    {
        ...
        int offs
        = ((menurighttabwidth - FONTH) * ((msctrl % 41) ? pos : (50 + 50 * sinf (totalmillis / 300.0f + y)))) / 100;
        int c = greyedout ? 128 : 255;
        ...
        if (sel)
        {
            ...
            if (offs > FONTH / 2)
                renderbg (x + w - offs + FONTH / 2, y, offs - FONTH / 2, menuseldescbgcolor);
        }
        draw_text (text, x, y, c, c, c);
        x -= offs;
        ...
        if (checked)
        {
            int x1 = x + w - boxsize - FONTH / 6, x2 = x + w + FONTH / 6, y1 = y - FONTH / 6,
                y2 = y + boxsize + FONTH / 6;
               ...
               bool forwardkeys)
{
    ...
    if (!m)
    {
        ...
        m = &menus[name];
        m->name = name;
        m->initaction = nullptr;
        m->header = m->footer = nullptr;
        m->menusel = 0;
    }
    m->title = title;
    DELSTRING (m->mdl);
...
void menuheader (void *menu, char *header, char *footer, bool heap)
{
    ...
    m.header = (header && *header) || heap ? header : nullptr;
    m.footer = (footer && *footer) || heap ? footer : nullptr;
    m.headfootheap = heap; // false: "manual"
}

COMMANDF (menuheader, "ss", (char *header, char *footer) {
    if (lastmenu)
        menuheader (lastmenu, *header ? newstring (header) : NULL, *footer ? newstring (footer) : NULL, true);
});
...
void menufont (char *menu, const char *usefont)
{
    ...
    {
        DELSTRING (m->usefont);
        m->usefont = usefont && *usefont ? newstring (usefont) : nullptr;
    }
}
COMMAND (menufont, "ss");
...
{
    if (!lastmenu) return;
    lastmenu->items.add (new mitemtext (lastmenu, newstring (text), newstring (action[0] ? action : text),
                                        hoveraction[0] ? newstring (hoveraction) : nullptr, nullptr,
                                        *desc ? newstring (desc) : nullptr));
}
...
void menuitemimage (char *name, char *text, char *action, char *hoveraction)
{
    ...
    if (fileexists (name, "r") || findfile (name, "r") != name)
        lastmenu->items.add (
        new mitemimage (lastmenu, name, newstring (text), action[0] ? newstring (action) : nullptr,
                        hoveraction[0] ? newstring (hoveraction) : nullptr, nullptr));
    else
        lastmenu->items.add (new mitemtext (lastmenu, newstring (text), newstring (action[0] ? action : text),
                                            hoveraction[0] ? newstring (hoveraction) : nullptr, nullptr));
...
{
    if (!lastmenu) return;
    lastmenu->items.add (new mitemimage (lastmenu, nullptr, newstring (text),
                                         action[0] ? newstring (action) : nullptr,
                                         hoveraction[0] ? newstring (hoveraction) : nullptr,
                                         nullptr, desc[0] ? newstring (desc) : nullptr, altfont));
}
...
void menuitemmapload (char *name, char *action, char *hoveraction, char *desc)
{
    if (!lastmenu) return;
    lastmenu->items.add (
    new mitemmapload (lastmenu, newstring (name), newstring (name), *action ? newstring (action) : nullptr,
                      *hoveraction ? newstring (hoveraction) : nullptr, *desc ? newstring (desc) : nullptr));
}
...
void menuitemtextinput (char *text, char *value, char *action, char *hoveraction, int *maxchars, int *maskinput)
{
    ...
    lastmenu->items.add (new mitemtextinput (lastmenu, newstring (text), newstring (value),
                                             action[0] ? newstring (action) : nullptr,
                                             hoveraction[0] ? newstring (hoveraction) : nullptr,
                                             nullptr, *maxchars, *maskinput));
}
...
{
    if (!lastmenu) return;
    lastmenu->items.add (new mitemslider (lastmenu, newstring (text), *min_, *max_, newstring (value),
                                          display, action[0] ? newstring (action) : nullptr,
                                          nullptr, *wrap != 0, false));
}
...
{
    if (!lastmenu) return;
    lastmenu->items.add (new mitemslider (lastmenu, newstring (text), *min_, *max_, newstring (value), display,
                                          action[0] ? newstring (action) : nullptr, nullptr, true, true));
}
...
{
    if (!lastmenu) return;
    lastmenu->items.add (new mitemcheckbox (lastmenu, newstring (text), newstring (value),
                                            action[0] ? newstring (action) : nullptr,
                                            clamp (*pos, 0, 100), nullptr));
}
...
{
    if (!lastmenu) return;
    lastmenu->items.add (new mitemkeyinput (lastmenu, newstring (text), newstring (bindcmd),
                                            nullptr, keym::ACTION_DEFAULT));
}
...
void menudirlist (char *dir, char *ext, char *action, int *image, char *searchfile)
{
    ...
    d->image = *image != 0;
    d->searchfile = searchfile[0] ? newstring (searchfile) : nullptr;
    d->subdiraction = nullptr;
    d->subdirdots = false;
}
...
void menudirlistsub (char *action, int *dots)
{
    ...
    d->subdiraction = *action ? newstring (action) : nullptr;
}
COMMAND (menudirlistsub, "si");

void chmenutexture (char *menu, char *texname, char *title)
{
    gmenu *m = *menu ? menus.access (menu) : nullptr;
    if (!m) return;
...
bool menukey (int code, bool isdown, int unicode, SDLMod mod)
{
    ...
    if (isdown)
    {
        ...
        {
            if (!curmenu->allowinput) return false;
            if (curmenu->keyfunc && (*curmenu->keyfunc) (curmenu, code, isdown, unicode))
                return true;
...
bool rendermenumdl ()
{
    ...
    rendermodel (isplayermodel ? "playermodels" : m.mdl, m.anim | ANIM_DYNALLOC, tex, -1, pos, 0,
                 yaw, pitch, 0, 0, nullptr, a, m.scale ? m.scale / 25.0f : 1.0f);
...
void gmenu::conprintmenu ()
{
    ...
    loopv (items)
    {
        conoutf ("%03d: %s%s%s", 1 + i, items[i]->gettext (), items[i]->getaction () ? "|\t|" : "",
    ...
    if (dirlist && dirlist->dir && dirlist->ext)
    {
        ...
        if (dirlist->subdiraction)
        {
            ...
            loopv (subdirs)
            {
                ...
                items.add (new mitemtext (this, newstring (cdir), newstring (act), nullptr, nullptr, nullptr));
            }
        }
        ...
        if (customsortorder)
            sortorderindex = getlistindex (customsortorder, menufilesortorders, true, 0);
        ...
        loopv (files)
        {
            char *f = files[i], *d = (strcmp (dirlist->ext, "cgz") || dirlist->searchfile) && browsefiledesc ?
            ...
            if (dirlist->image)
            {
                ...
                items.add (new mitemimage (this, newstring (fullname), f,
                                           newstring (dirlist->action), nullptr, nullptr, d));
            }
            else if (!strcmp (dirlist->ext, "cgz"))
            {
                if (!dirlist->searchfile || filefound)
                {
                    items.add (new mitemmapload (this, newstring (f), newstring (f),
                                                 newstring (dirlist->action), nullptr, nullptr));
                }
                DELSTRING (d);
            }
            ...
            {
                if (!dirlist->searchfile || filefound)
                    items.add (new mitemtext (this, f, newstring (dirlist->action), nullptr, nullptr, d));
            }
            else
                items.add (new mitemtext (this, f, newstring (dirlist->action), nullptr, nullptr, d));
        }
...
void gmenu::render ()
{
    ...
    bool synctabs = title != nullptr || synctabstops;
    const char *t = title;
    if (!t)
    {
        static string buf;
    ...
    int w = 0, footermaxw = 2 * VIRTW - 6 * FONTH, hitems = headeritems (), footwidth, footheight,
        maxfootheight = 0;
    ...
    int pagesize = MAXMENU - hitems, offset = menusel - (menusel % pagesize),
        mdisp = min (items.length (), pagesize), cdisp = min (items.length () - offset, pagesize),
    ...
    {
        if (offset > 0)
            drawarrow (1, x + w + FONTH * 3 / 2 - FONTH * 5 / 6, y - FONTH * 5 / 6, FONTH * 2 / 3);
        if (offset + pagesize < items.length ())
            drawarrow (0, x + w + FONTH * 3 / 2 - FONTH * 5 / 6, y + h + FONTH / 6, FONTH * 2 / 3);
...
void refreshapplymenu (void *menu, bool init)
{
    ...
    m->items.add (new mitemtext (m, newstring ("No"), newstring ("echo [..restart AssaultCube to apply the new settings]"),
                                 nullptr, nullptr));
