...
int wizardmain(int argc, char **argv) {
  ...
  printf(
      "Before setting up a new server, please ensure you've read the rules "
      "at:\n"
  ...
  printf("Done\n\n");
  printf(
      "Note: You can start %s directly the next time you want to use the same "
  ...
  if (wsname[0]) {
    ...
    if ((r = installer.IsInstalled()) != 0) {
      ...
      if (r == -1)
        ...
        printf(
            "A windows service with this name (%s) is already installed: %u\n",
    ...
    printf("Done\n\n");
    printf(
        "Note: You can now manage your AC server using services.msc and "
        "sc.exe\n\n");
  }
