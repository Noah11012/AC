...
int wizardmain (int argc, char **argv)
{
    ...
    printf ("Before setting up a new server, please ensure you've read the rules "
            "at:\n"
            "\thttp://masterserver.cubers.net/rules.html\n\n");
    printf ("You will also need to ensure that the UDP port you choose is open.\n"
            "Whatever port you choose, you will need to forward that port, plus one "
            "port after that.\n"
            "If you're having issues, use and forward the default ports 28763 and "
            "28764\n"
    ...
    printf ("Now to specify some optional settings for your server.\n"
            ...
            "If you're unsure about what to specify for the settings, leave the "
            "field blank.\n\n"
            "Read http://assault.cubers.net/docs/commandline.html for a description "
            "of these settings.\n\n");
    ...
    if (wsname[0]) readarg ("win service display", wsdisplayname, sizeof (wsdisplayname));

#endif

    printf ("\nWriting your configuration to %s ... ", outfile);
    fflush (stdout);
    ...
    printf ("Done\n\n");
    printf ("Note: You can start %s directly the next time you want to use the same "
    ...
    if (wsname[0])
    {
        ...
        printf ("Installing the AC Server as windows service ... ");
        fflush (stdout);
        ...
        if ((r = installer.IsInstalled ()) != 0)
        {
            printf ("Failed!\n");
            if (r == -1)
                printf ("Error accessing the Service Control Manager\n");
            else if (r == 1)
                printf ("A windows service with this name (%s) is already installed: %u\n", wsname,
                        (uint)GetLastError ());
            return EXIT_FAILURE;
        }
        ...
        if ((r = installer.Install ()) != 1)
        {
            ...
            if (r == -1)
                ...
                printf ("Could not create the new windows service: %u\n", (uint)GetLastError ());
            return EXIT_FAILURE;
        }

        printf ("Done\n\n");
        printf ("Note: You can now manage your AC server using services.msc and "
                "sc.exe\n\n");
    }
