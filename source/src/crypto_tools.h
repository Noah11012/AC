...
// The Ed25519 sources were taken from the crypto_sign/ed25519 subdirectory of
// the SUPERCOP benchmarking tool. The Ed25519 software is in the public domain.

// this file (crypto_tools.h) contains fe25519.c, fe25519.h, ge25519.c,
// ge25519.h, sc25519.c and sc25519.h. ge25519_base.data is kept as a separate
// file and keypair.c, open.c and sign.c got merged into crypto.cpp.
...
void fe25519_freeze (fe25519 *r)
{
    ...
    for (i = 30; i > 0; i--) m &= equal (r->v[i], 255);
    m &= ge (r->v[0], 237);

    m = (uint32_t) - int(m);

    r->v[31] -= m & 127;
    for (i = 30; i > 0; i--) r->v[i] -= m & 255;
    r->v[0] -= m & 237;
}
...
int fe25519_iszero (const fe25519 *x)
{
    ...
    fe25519_freeze (&t);
    r = equal (t.v[0], 0);
    for (i = 1; i < 32; i++) r &= equal (t.v[i], 0);
    return r;
}
...
void fe25519_mul (fe25519 *r, const fe25519 *x, const fe25519 *y)
{
    ...
    for (i = 0; i < 32; i++)
        for (j = 0; j < 32; j++) t[i + j] += x->v[i] * y->v[j];

    for (i = 32; i < 63; i++) r->v[i - 32] = t[i - 32] + times38 (t[i]);
...
static const uint32_t mu[33] = { 0x1B, 0x13, 0x2C, 0x0A, 0xA3, 0xE5, 0x9C, 0xED, 0xA7, 0x29, 0x63,
...
static void reduce_add_sub (sc25519 *r)
{
    ...
    mask = b - 1;
    for (i = 0; i < 32; i++) r->v[i] ^= mask & (r->v[i] ^ t[i]);
}
...
{
    int i;
    for (i = 0; i < 16; i++) r->v[i] = x->v[i];
    for (i = 0; i < 16; i++) r->v[16 + i] = 0;
}
...
const ge25519 ge25519_base = { { { 0x1A, 0xD5, 0x25, 0x8F, 0x60, 0x2D, 0x56, 0xC9, 0xB2, 0xA7, 0x25,
                               ...
                               { { 0xA3, 0xDD, 0xB7, 0xA5, 0xB3, 0x8A, 0xDE, 0x6D, 0xF5, 0x52, 0x51,
...
static void p1p1_to_p2_ (ge25519_p3 *r,
                         const ge25519_p1p1 *p) // to avoid errors because of aliasing
{
...
int ge25519_unpackneg_vartime (ge25519_p3 *r, const unsigned char p[32])
{
    ...
    /* 3. Check whether sqrt computation gave correct result, multiply by sqrt(-1)
     * if not: */
    fe25519_square (&chk, &r->x);
    fe25519_mul (&chk, &chk, &den);
    if (!fe25519_iseq_vartime (&chk, &num)) fe25519_mul (&r->x, &r->x, &ge25519_sqrtm1);

    /* 4. Now we have one of the two square roots, except if input was not a
     * square */
    fe25519_square (&chk, &r->x);
    fe25519_mul (&chk, &chk, &den);
    if (!fe25519_iseq_vartime (&chk, &num)) return -1;

    /* 5. Choose the desired square root according to parity: */
    if (fe25519_getparity (&r->x) != (1 - par)) fe25519_neg (&r->x, &r->x);
