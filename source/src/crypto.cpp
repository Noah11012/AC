...
#define tround(a, b, c, x)                                           \
  ...
  b *= mul;
    ...
    tround(a, b, c, x0) tround(b, c, a, x1) tround(c, a, b, x2)
        tround(a, b, c, x3) tround(b, c, a, x4) tround(c, a, b, x5)
            tround(a, b, c, x6) tround(b, c, a, x7)
...
void gensboxes() {
  ...
  loop(pass, 5) loopi(256) for (int sb = 0; sb < 1024; sb += 256) {
    ...
    if (abc >= 3) {
      abc = 0;
      compress((const chunk *)str, state);
    }
    loop(col, 8) {
      uchar val = ((uchar *)&sboxes[sb + i])[col];
      ((uchar *)&sboxes[sb + i])[col] =
          ((uchar *)&sboxes[sb + ((uchar *)&state[abc])[col]])[col];
      ((uchar *)&sboxes[sb + ((uchar *)&state[abc])[col]])[col] = val;
    }
  }
}

struct incremental_buffer {
  int len, total;
  union {
    uchar u[64];
    chunk c[8];
  };
  incremental_buffer() { len = total = 0; }
};
...
void hash_incremental(const uchar *msg, int len, hashval &val,
                      incremental_buffer *b) {
  ASSERT(b && b->len >= 0 && b->len < 64);
  b->total += len;
  while (b->len + len >= 64) {
    if (b->len > 0) {  // fill up buffer and compress
      int fill = 64 - b->len;
      ...
      len -= fill;
      msg += fill;
    } else {  // compress directly from msg
      compress((chunk *)msg, val.chunks);
      len -= 64;
      msg += 64;
    }
  }
  if (len > 0) {  // pur rest in buffer
...
void tigerhash_add(uchar *hash, const void *msg, int len, void *state) {
  tiger::hash_incremental((const uchar *)msg, len, *((tiger::hashval *)hash),
                          (tiger::incremental_buffer *)state);
}

void tigerhash_finish(uchar *hash, void *state) {
  if (hash)
    tiger::hash_finish(*((tiger::hashval *)hash),
                       (tiger::incremental_buffer *)state);
  else
    delete (tiger::incremental_buffer *)state;
}
...
///////////////////////////////////////////  SHA512
///////////////////////////////////////////////////////////////////////
// adapted from pseudocode and various public domain sources (probably all
// originating in LibTomCrypt) (consider this code public domain, too)

#define RR64(x, n) (((x) >> (n)) | ((x) << (64 - (n))))
#define SHA512ROUND(a, b, c, d, e, f, g, h, i)             \
  d += h = h + (RR64(e, 14) ^ RR64(e, 18) ^ RR64(e, 41)) + \
           (g ^ (e & (f ^ g))) + K[i] + W[i];              \
  h += (RR64(a, 28) ^ RR64(a, 34) ^ RR64(a, 39)) + (((a | b) & c) | (a & b));
#define SHA512SIZE (512 / 8)

void sha512_compress(uint64_t *state,
                     const uchar *inbuf)  // sha512 main function: compress one
                                          // chunk of 128 bytes
{
  ...
  loopi(64) W[i + 16] =
      W[i] + W[i + 9] +
      (RR64(W[i + 14], 19) ^ RR64(W[i + 14], 61) ^ (W[i + 14] >> 6)) +
      (RR64(W[i + 1], 1) ^ RR64(W[i + 1], 8) ^ (W[i + 1] >> 7));
...
void sha512(uchar *hash, const uchar *msg, int msglen) {
  uint64_t state[8] = {0x6a09e667f3bcc908ULL, 0xbb67ae8584caa73bULL,
...
#ifdef _DEBUG
int __hashtest()  // crash, if hash functions don't work properly - no one ever
                  // tested the big endian code
{
  const char *msg = "The quick brown fox jumps over the lazy dog";
  const uchar ht[] = {0x6d, 0x12, 0xa4, 0x1e, 0x72, 0xe6, 0x44, 0xf0,
                      0x17, 0xb6, 0xf0, 0xe2, 0xf7, 0xb4, 0x4c, 0x62,
                      0x85, 0xf0, 0x6d, 0xd5, 0xd2, 0xc5, 0xb0, 0x75};
  const uchar hs[] = {
      0x07, 0xe5, 0x47, 0xd9, 0x58, 0x6f, 0x6a, 0x73, 0xf7, 0x3f, 0xba,
      0xc0, 0x43, 0x5e, 0xd7, 0x69, 0x51, 0x21, 0x8f, 0xb7, 0xd0, 0xc8,
      0xd7, 0x88, 0xa3, 0x09, 0xd7, 0x85, 0x43, 0x6b, 0xbb, 0x64, 0x2e,
      0x93, 0xa2, 0x52, 0xa9, 0x54, 0xf2, 0x39, 0x12, 0x54, 0x7d, 0x1e,
      0x8a, 0x3b, 0x5e, 0xd6, 0xe1, 0xbf, 0xd7, 0x09, 0x78, 0x21, 0x23,
      0x3f, 0xa0, 0x53, 0x8f, 0x3d, 0xb8, 0x54, 0xfe, 0xe6};
  tiger::hashval hash;
  tiger::hash((uchar *)msg, (int)strlen(msg), hash);
  uchar shash[SHA512SIZE];
  sha512(shash, (uchar *)msg, strlen(msg));
  int res =
      memcmp(hash.bytes, ht, TIGERHASHSIZE) | memcmp(shash, hs, SHA512SIZE);
  ASSERT(!res);
  return res;
}
...
// Landon Curt Noll, and Phong Vo. It is public domain. We use the
// 32-bit-version here.

inline void fnv1a_init(uint32_t &hash) { hash = 2166136261UL; }
...
uint randomMT() {
  ...
  mt_state[cur] = y =
      mt_state[cur < MT_N - MT_M ? cur + MT_M : cur + MT_M - MT_N] ^ (y >> 1) ^
      (-int(y & 1U) & MT_K);
  y ^= (y >> 11);
  y ^= (y << 7) & 0x9D2C5680U;
  y ^= (y << 15) & 0xEFC60000U;
...
///////////////////////////////////////////////  Entropy collector
///////////////////////////////////////////////////////////////////////////////////
// * collects entropy (true random data) through mouse moves and keyboard action
...
void entropy_add_block(const uchar *s, int len) {
  for (; len > 0; s += 128, len -= 128) {
    int bl = min(len, 128);
    xor_block(entpool + rnd(ENTPOOLSIZE / 8 - 16) * 8, s,
              bl);  // use pseudorandom to mix entropy into the pool in a hard
                    // to predict fashion
...
void entropy_get(uchar *buf, int len) {
  ...
  loopi(len) sha512_compress((uint64_t *)(tempbuf + (i & ~7)),
                             entpool + rnd(ENTPOOLSIZE / 8) * 8);
...
///////////////////////////////////////////////////////////////////////////
// * basically a hash function that delivers keylen bytes calculated from pass
// and salt
// * but uses a lot of memory and a quite complex algorithm, to make brute-force
// offline-attacks on short passwords as hard as possible (even if someone uses
// fpgas)
// * last line of defense against "evil little brothers"

void passphrase2key(const char *pass, const uchar *salt, int saltlen,
                    uchar *key, int keylen, int *iterations, int maxtime,
                    int memusage) {
  memset(key, 0, keylen);
  if (!*pass) return;                   // password "" ->
  memusage = clamp(memusage, 2, 1024);  // keep memory usage between 2MB and 1GB
  int memsize = (1 << 20) * memusage, passlen = strlen(pass),
  ...
  if ((memusage & 1) && passlen + saltlen >= 8)
  ...
  loopi(tmpbuflen) seedMT_do(
      *((uint *)(tmpbuf + i)), (uint *)(hugebuf + memchunk * i),
      memchunk /
          sizeof(uint));  // spread the passphrase all over the huge buffer

// start mixing
#define mixit(x, y)                                                       \
  (((((uint *)tmpbuf)[(roundsdone ^ (x)) % (tmpbuflen / sizeof(uint))]) % \
    ((y) / 8 - (x) % 11)) *                                               \
   8)
  stopwatch watch;
  watch.start();
  ...
  uint shortseed = 0;
  do {
    loopirev(memsize / 127) shortseed =
        ((uint *)hugebuf)[shortseed % (memsize / sizeof(uint))];
    seedMT_do(shortseed, (uint *)tmpbuf, tmpbuflen / sizeof(uint));
    loopi(2999) sha512_compress(
        (uint64_t *)(hugebuf + mixit(i * 5, memsize - SHA512SIZE)),
        hugebuf + mixit(i * 17, memsize - 128));
    roundsdone++;
  } while ((!*iterations && watch.elapsed() < maxtime) ||
           (*iterations &&
            *iterations > roundsdone));  // *iterations == 0: time-limited,
                                         // otherwise rounds-limited

  // extract the key
  if (keylen * 8 > 192)
    sha512(tmpbuf, hugebuf, memsize);
  else
    tiger::hash(hugebuf, memsize, *((tiger::hashval *)tmpbuf));
  loopi(SHA512SIZE)
      sha512_compress(((uint64_t *)tmpbuf) + (i / 8),
                      hugebuf + (((tmpbuf[i] * 68111) % (memsize - 128)) & ~7));
  memcpy(key, tmpbuf, min(keylen, tmpbuflen));  // max keylen is 2 * SHA512SIZE
  DEBUG("passphrase2key: " << watch.elapsed() << " ms, " << roundsdone
                           << " rounds, " << memsize << " bytes used");
...
const char *bin2hex(char *d, const uchar *s,
                    int len) {  // d needs to be at least of size 2 * len + 1
  len *= 2;
        ...
        nullptr)  // derive private key from even more secret bulk of entropy
{
  ...
  loopi(33) {  // find privte key with the public key with the least getint()
               // escape codes (yes, that decreases the keyrange. sue me.)
  ...
  ed25519_pubkey_from_private(
      privkey,
      temp + minescpos);  // (the resulting private key is actually itself a
                          // public key of a private derived from prepriv)
  if (privpriv) memcpy(privpriv, temp + minescpos, 32);
}

void ed25519_sign(
    uchar *sm, int *smlen, const uchar *m, int mlen,
    const uchar *sk) {  // sk: 32-byte private key + 32-byte public key
...
#ifdef _DEBUG
void ed25519test(
    char *vectorfilename)  // check ed25519 functions with test vectors from
                           // http://ed25519.cr.yp.to/python/sign.input
{
  ...
  if (!vectorfile) {
    conoutf("could not read test vector file %s", vectorfilename);
    return;
  }
  conoutf("loaded %d bytes from %s", vectorfilesize, vectorfilename);
  watch.start();
  int lines = 0, inputerrors = 0, pubfail = 0, signfail = 0, verifyfail = 0,
      verifyfail2 = 0;
  for (char *l = strtok(vectorfile, "\n"); l;
       l = strtok(NULL,
                  "\n")) {  // a line consists of 32 bytes private key, 32 byte
                            // public key, ":", 32 byte public key (again), ":",
                            // message, ":", signed message, ":"
    char *vprivpub = l, *vpub = strchr(l, ':'), *vmsg, *vsmsg, hextemp[65];
    int linelen = strlen(l), msglen = 0;
    uchar privpub[64], pub[32], *msg = new uchar[linelen],
                                *smsg = new uchar[linelen], temp[64];
    lines++;
    if (!vpub || !(vmsg = strchr(vpub + 1, ':')) ||
        !(vsmsg = strchr(vmsg + 1, ':')) ||
        hex2bin(privpub, vprivpub, 64) != 64 ||
        hex2bin(pub, ++vpub, 32) != 32 ||
        (msglen = hex2bin(msg, ++vmsg, linelen)) < 0 ||
        hex2bin(smsg, ++vsmsg, linelen) != (msglen + 64) ||
        memcmp(msg, smsg + 64, msglen) || memcmp(privpub + 32, pub, 32)) {
      inputerrors++;
      continue;
    }  // line does not match template
    ed25519_pubkey_from_private(temp, privpub);
    if (strncmp(bin2hex(hextemp, temp, 32), vpub, 64)) {
      pubfail++;
      continue;
    }  // private and public key don't match
    ed25519_sign(msg, &linelen, msg, msglen, privpub);
    if (linelen != msglen + 64 || memcmp(msg, smsg, linelen)) {
      signfail++;
      continue;
    }  // newly signed message doesn't match vector data
    if (!ed25519_sign_check(smsg, msglen + 64, pub)) {
      verifyfail++;
      continue;
    }  // verification of signed message failed
    smsg[msglen % 64]++;
    if (ed25519_sign_check(smsg, msglen + 64, pub)) {
      verifyfail2++;
      continue;
    }  // verification of altered signed message didn't fail (...but should've)
    smsg[msglen % 64]--;
    if (msglen) {
      smsg[42 % msglen]++;
      if (ed25519_sign_check(smsg, msglen + 64, pub)) {
        verifyfail2++;
        continue;
      }  // verification of altered signed message didn't fail (...but
         // should've)
      smsg[42 % msglen]--;
    }
    pub[msglen % 32]++;
  ...
  conoutf(
      "%d vector file errors, %d private-pubkey mismatches, %d sign fails, %d "
      "verify fails, %d missed verify fails",
      inputerrors, pubfail, signfail, verifyfail, verifyfail2);
}
...
void ed25519speedtest()  // check speed of ed25519 functions
{
  ...
  conoutf("create public key from private key: %.2f ms",
          watch.elapsed() / 200.0f);
  memcpy(sk, priv, 32);
  memcpy(sk + 32, pub, 32);
  watch.start();
  ...
  conoutf("verify signature of 64-byte message: %.2f ms",
          watch.elapsed() / 200.0f);
}
...
//////////////////////////////////// game key management
///////////////////////////////////////////////////////////////////////////
// * the game key is the primary player key that is associated with his player
// account
...
VARP(authmemusage, 2, 24,
     (1 << 10) - 1);  // megabytes of RAM to use for password hash (when using a
                      // new password)
VARP(authrounds, 0, 0, INT_MAX - 1);  // create new password hashes with a fixed
                                      // number of rounds (if authrounds > 0)
VARP(
...
static int passdeferred(
    void *pass)  // decrypt the private key in the background
{
  uchar keyhash[32];
  int iterations = passdargs.pwdcfg >> 10;
  passphrase2key((char *)pass, passdargs.salt, 16, keyhash, 32, &iterations, 0,
                 passdargs.pwdcfg & 0x3ff);
...
void authsetup(char **args, int numargs)  // set up private and public keys
{
  ...
  uchar *priv = buf + 24 + (offs & 0xfc),
        *prepriv = priv + 112 + ((offs >> 9) & 0xfc), res = 0;
  ...
  if (passdrunning) {
    if (numargs < 0) {  // finish password decryption (background mode)
      ...
      return;  // ignore further "authsetup" commands while a password
  ...
  if (numargs > 0) {
    if (!strcasecmp(args[0], "PRE")) {
      ...
      preprivlen = numargs > 1 ? clamp(int(strlen(args[1])) / 2, preprivminlen,
                                       preprivmaxlen)
                               : 32;
      ...
      if (iterations)
        passphrase2key(numargs > 1 ? args[1] : "", psalt, 16, keyhash,
                       preprivlen, &iterations, 0, preprivpwdcfg & 0x3fe);
      ...
      if (iterations)
        passphrase2key(numargs > 1 ? args[1] : "", salt, 16, keyhash, 32,
                       &iterations, 0, privpwdcfg & 0x3ff);
      ...
      preprivlen = clamp((numargs > 1) ? atoi(args[1]) : 42, preprivminlen,
                         preprivmaxlen);
      ...
      if (preprivlen && !preprivpwdcfg) {
        if (numargs > 1 && args[1][0])  // new password
        {
          ...
          passphrase2key(args[1], psalt, 16, keyhash, preprivlen, &iterations,
                         authmaxtime, authmemusage & 0x3fe);
        ...
        if (f) {
          f->printf(
              ...
              "// either print it out and delete it or move it to a "
              "thumbdrive.\n"
          ...
          if (preprivpwdcfg)
            f->printf(" %s %u", bin2hex(hextemp, psalt, 16),
                      preprivpwdcfg & ~1);
          f->printf("\n\n");
          delete f;
        }
      ...
      if (!privpwdcfg) {
        ...
        if (numargs > 1 && args[1][0])  // new password
        {
          entropy_get(salt, 16);
          int iterations = authrounds;
          passphrase2key(args[1], salt, 16, keyhash, 32, &iterations,
                         authmaxtime, authmemusage);
        ...
        if (f) {
          ...
          if (privpwdcfg)
            f->printf(" %s %u", bin2hex(hextemp, salt, 16), privpwdcfg);
          f->printf("\nauthsetup pub %s\n\n",
        ...
        DELETEA(oldfile);
                   ...
                   args[0],
                   "UNARMED"))  // FIXME: this is for testing purposes only - to
                                // test simultaneous logins from the same
                                // account - delete before release!
    {
      // authsetup unarmed
      // pub aa55aab6d746c2000f3f6dc133c49c5d0485e0e44cbf036757e7945bca20106f
      // priv aa55aa55aa55aa55aa55aa55aa55aae5a973aa55aa55aa55aa55aa55aa55aa55
      hex2bin(
          priv,
          "aa55aa55aa55aa55aa55aa55aa55aae5a973aa55aa55aa55aa55aa55aa55aa55",
          32);
      ed25519_pubkey_from_private(pub, priv);
      privpwdcfg = 0;
    }
...
bool delauthkey(const char *name) {
  ...
  loopvrev(
      authkeys) if (!strcmp(authkeys[i]->name, name)) delete authkeys.remove(i);
...
void authkey_(char **args, int numargs)  // set up misc keys
{
  if (numargs > 0) {
    ...
    if (!strcasecmp(args[0], "CLEAR")) {  // authkey clear
    ...
    } else if (!strcasecmp(args[0], "LIST")) {  // authkey list
      if (authkeys.length()) {
        conoutf("name\tpubkey");
        loopv(authkeys) conoutf("%s\t%s", authkeys[i]->name,
                                bin2hex(buf, authkeys[i]->sk + 32, 32));
      } else
        conoutf("no authkeys");
    } else if (!strcasecmp(args[0], "DELETE")) {  // authkey delete name
      if (numargs > 1) delauthkey(args[1]);
    } else if (!strcasecmp(args[0], "NEW")) {  // authkey new name
    ...
    } else if (!strcasecmp(args[0], "SELFCERT")) {  // authkey selfcert name
      const uchar *sk;
      if (numargs > 1 && (sk = getauthkey(args[1]))) {
        defformatstring(cmd)(
            ...
            "\n",
            bin2hex(buf, sk + 32, 32), args[1], numargs > 2 ? args[2] : "",
            args[1]);
        execute(cmd);
      }
    }
...
// pub aa558eb043105536d72a433d8fa121797f134805c526727c43f57cefdaec289b  priv
// aa55aa55aa55aa55aa55aa55aa55aac97034aa55aa55aa55aa55aa55aa55aa55
uchar rootcert[] = {0xaa, 0x55, 0x8e, 0xb0, 0x43, 0x10, 0x55, 0x36,
                    0xd7, 0x2a, 0x43, 0x3d, 0x8f, 0xa1, 0x21, 0x79,
                    0x7f, 0x13, 0x48, 0x05, 0xc5, 0x26, 0x72, 0x7c,
                    0x43, 0xf5, 0x7c, 0xef, 0xda, 0xec, 0x28, 0x9b};

static const char *certheader = "AC-CERT v1 ";
static const int certheaderlen = strlen(certheader),
                 certheaderlenfull = certheaderlen + 128 + 1;
static const char *certtypekeywords[] = {"misc",   "dev",    "master",
                                         "server", "player", "clanboss",
                                         "clan"};  // needs to match the enums!
static int certtypemaxrank[] = {
    0, 1, 2, 3, 3, 3, 0};  // max allowed rank of some cert types
static int certtypeexpiration[] = {
    0,
    0,
    0,
    2 * CERTTIMEMONTH,
    CERTTIMEMONTH,
    3 * CERTTIMEMONTH,
    CERTTIMEMONTH};  // time after which certs are usually re-signed, after 3x
                     // the time they expire
...
bool cert::parse()  // parse orgmsg[] and check the signature
{
  ...
  DELETEA(workmsg);
  lines.setsize(0);
  if (orglen > certheaderlenfull && orglen < (1 << 20) &&
      !strncmp(orgmsg, certheader, certheaderlen) &&
      orgmsg[certheaderlen + 128] == '\n') {  // frame ok
    workmsg = new char[orglen + 1];
    memcpy(workmsg, orgmsg, orglen + 1);
    uchar *signature = (uchar *)orgmsg + certheaderlenfull - 64;
    if (hex2bin(signature, workmsg + certheaderlen, 64) ==
        64) {  // parse all lines
      certline line;
        ...
        if (line.val && line.val > l) {
          ...
          *line.val++ = '\0';
          ...
          if (!strcasecmp(line.key,
            ...
            lines.add(line);
            if (!strcasecmp(line.key, "type"))
              type = getlistindex(line.val, certtypekeywords, false, 0);
          }
        }
      }
      if (signedby) {  // check signature (if signature contained a "signed-by"
                       // line)
        if (ed25519_sign_check(signature, orglen - certheaderlenfull + 64,
                               signedby)) {
          ischecked = true;
    ...
    if (ischecked && pubkey && !name)
      bin2hex((name = workmsg), pubkey,
              32);  // if cert has no name, use pubkey as name
  }
  return ischecked;
}
...
char *cert::getnewcertfilename(const char *subpath) {
  ...
  defformatstring(s)(CERTFILEDIR "%s%s%s_%s_%d." CERTFILEEXT,
                     subpath ? subpath : "", subpath ? PATHDIVS : "",
                     certtypekeywords[type], fname, signeddate);
  return newstring(s);
}
...
void makecert::addline(const char *key, const char *val, const char *com) {
  ...
  if (*key && *val && endbuf - curbuf > keylen + vallen + comlen + 3) {
    ...
    if (!strcasecmp(l.key, "name"))
      c->name = l.val;
    else if (!strcasecmp(l.key, "type"))
      c->type = getlistindex(l.val, certtypekeywords, false, 0);
  }
}
...
char *makecert::sign(const uchar *keypair, const char *com) {
  char hextemp[129];
  vector<char> msg;
  cvecprintf(msg, "signed-by: %s%s%s\n", bin2hex(hextemp, keypair + 32, 32),
             com && *com ? "  // " : "", com ? com : "");
  loopv(c->lines)
      cvecprintf(msg, "%s: %s%s%s\n", c->lines[i].key, c->lines[i].val,
                 c->lines[i].comment[0] ? " // " : "", c->lines[i].comment);
  cvecprintf(msg, "signed-date: %d // %s\n", (c->signeddate = aktcerttime()),
             timestring("%c"));
  int len = msg.length();
...
COMMANDF(newcert, "v",
         (char **args, int numargs)  // create and sign certs from cubescript
         {
           ...
           if (numargs > 0) {
             if (!strcasecmp(args[0], "CLEAR")) {
               ...
               if (numargs > 2 && args[1][0] && args[2][0])
                 mc->addline(args[1], args[2], numargs > 3 ? args[3] : "");
             } else if (!strcasecmp(args[0], "SIGN")) {
               // newcert sign privkey|authkeyname [comment]
               uchar keypair[64];  // 32byte private + 32byte public
               if (numargs > 1 && ((getauthkey(args[1]) &&
                                    memcpy(keypair, getauthkey(args[1]), 32)) ||
                                   (strlen(args[1]) == 64 &&
                                    hex2bin(keypair, args[1], 32) ==
                                        32))) {  // sign cert and write to file
...
void marksuperseded(cert *chain)  // checks all
{
  for (cert *c = chain; c; c = c->next) {
    if (c->isvalid)
      for (cert *r = c->next; r; r = r->next) {
        if (r->isvalid && c->isvalid) {
          if ((c->type == r->type && c->name && r->name &&
               !strcmp(c->name, r->name)) ||
              (c->pubkey && r->pubkey &&
               !memcmp(c->pubkey, r->pubkey,
                       32))) {  // same name and type or same pubkey
            cert *x =
                c->signeddate < r->signeddate ? c : r;  // older one has to go
            x->superseded = true;
            x->isvalid = false;
          }
        }
  ...
  for (cert *c = chain; c; c = c->next) {  // check, if certs lost their parent
                                           // during the cleanup above
    if (c->isvalid && c->parent && (!c->parent->isvalid || !c->parent->rank))
      c->rank = 0;
  }
}

void rebuildcerttree()  // determines the rank of all certs; ages them; removes
                        // all kinds of illegal certs
{
  int act = aktcerttime();
  cert *treechain =
    ...
    loopv(certs)  // build tree, rank by rank
    {
      ...
      if (c->isvalid && !c->rank) {
        for (cert *r = treechain; r; r = r->next) {
          if (r->rank && r->isvalid && r->pubkey &&
              !memcmp(r->pubkey, c->signedby, 32)) {
            c->rank = r->rank + 1;
            if (c->rank > certtypemaxrank[c->type]) {  // maxrank exceeded (no,
                                                       // you can not sign your
                                                       // own master cert...)
              c->isvalid = false;
              c->illegal = true;
            } else {  // include cert into tree
              c->parent = r;
              c->next = treechain;
              treechain = c;
              if (c->pubkey)
                onemoreround = true;  // more public keys in the tree -> need to
                                      // check the remaining certs again
            }
            break;
          }
        }
  ...
  marksuperseded(
      treechain);  // of the valid certs that are not in the tree: remove
                   // outdated ones, where newer versions exist
  loopv(certs)     // move files of invalid certs to subdirectories
  {
    cert *c = certs[i];
    if (!c->isvalid) {
      if (c->superseded)
        c->movecertfile("superseded");
      else if (c->blacklisted)
        c->movecertfile("blacklisted");
      else if (c->illegal)
        c->movecertfile("illegal");
      else if (c->expired) {
      }  // c->movecertfile("expired");
      else
        c->movecertfile("invalid2");
    }
  }
}
...
void listcerts() {
  loopv(certs) {
    cert *c = certs[i];
    conoutf(
        "certs[%d]: %16s, t:%s, r:%d, l:%d, sd:%d, d2e:%d, d2r:%d, "
        "parent:%d%s%s%s%s%s%s%s",
        i, c->name, certtypekeywords[c->type], c->rank, c->lines.length(),
        c->signeddate, c->days2expire, c->days2renew, certs.find(c->parent),
        c->ischecked ? ", ischecked" : "", c->isvalid ? ", isvalid" : "",
        c->needsrenewal ? ", needsrenewal" : "", c->expired ? ", expired" : "",
        c->superseded ? ", superseded" : "",
        c->blacklisted ? ", blacklisted" : "", c->illegal ? ", illegal" : "");
  }
}
COMMAND(listcerts, "");
#endif
...
static const char *hashchunktoa(
    tiger::chunk h)  // portable solution instead of printf("%llx")
...
const char *genpwdhash(const char *name, const char *pwd, int salt) {
  static string temp;
  formatstring(temp)("%s %d %s %s %d", pwd, salt, name, pwd,
                     iabs(PROTOCOL_VERSION));
  tiger::hashval hash;
  tiger::hash((uchar *)temp, (int)strlen(temp), hash);
  formatstring(temp)("%s %s %s", hashchunktoa(hash.chunks[0]),
                     hashchunktoa(hash.chunks[1]),
                     hashchunktoa(hash.chunks[2]));
  return temp;
}
