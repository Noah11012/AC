...
void *tigerhash_init (uchar *hash) { return (void *)tiger::hash_init (*((tiger::hashval *)hash)); }
...
{
    if (hash)
        tiger::hash_finish (*((tiger::hashval *)hash), (tiger::incremental_buffer *)state);
    else
                  ...
                  // tested the big endian code
{
    ...
    const uchar ht[] = { 0x6d, 0x12, 0xa4, 0x1e, 0x72, 0xe6, 0x44, 0xf0, 0x17, 0xb6, 0xf0, 0xe2,
                         0xf7, 0xb4, 0x4c, 0x62, 0x85, 0xf0, 0x6d, 0xd5, 0xd2, 0xc5, 0xb0, 0x75 };
    const uchar hs[]
...
uint randomMT ()
{
    ...
    mt_state[cur] = y
    = mt_state[cur < MT_N - MT_M ? cur + MT_M : cur + MT_M - MT_N] ^ (y >> 1) ^ (-int(y & 1U) & MT_K);
    y ^= (y >> 11);
    y ^= (y << 7) & 0x9D2C5680U;
    y ^= (y << 15) & 0xEFC60000U;
    y ^= (y >> 18);
...
void passphrase2key (const char *pass, const uchar *salt, int saltlen, uchar *key, int keylen, int *iterations, int maxtime, int memusage)
{
    ...
    int memsize = (1 << 20) * memusage, passlen = strlen (pass), tmpbuflen = 2 * SHA512SIZE,
        pplen = 2 * (saltlen + passlen);
    ...
    loopi (tmpbuflen) seedMT_do (*((uint *)(tmpbuf + i)), (uint *)(hugebuf + memchunk * i),
                                 memchunk / sizeof (uint)); // spread the passphrase all over the huge buffer

// start mixing
#define mixit(x, y) \
    (((((uint *)tmpbuf)[(roundsdone ^ (x)) % (tmpbuflen / sizeof (uint))]) % ((y) / 8 - (x) % 11)) * 8)
    stopwatch watch;
    watch.start ();
    int roundsdone = 0;
    uint shortseed = 0;
    do
    {
        ...
        loopi (2999) sha512_compress ((uint64_t *)(hugebuf + mixit (i * 5, memsize - SHA512SIZE)),
                                      hugebuf + mixit (i * 17, memsize - 128));
        roundsdone++;
    } while ((!*iterations && watch.elapsed () < maxtime)
             || (*iterations && *iterations > roundsdone)); // *iterations == 0: time-limited,
    ...
    loopi (SHA512SIZE) sha512_compress (((uint64_t *)tmpbuf) + (i / 8),
...
void privkey_from_prepriv (unsigned char *privkey,
                           const unsigned char *prepriv,
                           int preprivlen,
                           unsigned char *privpriv = nullptr) // derive private key from even more secret bulk of entropy
{
    uchar temp[SHA512SIZE], pub[32], testpriv[32];
    sha512 (temp, prepriv, preprivlen);
    int esc, minesc = 32, minescpos = 0;
    loopi (33)
    { // find privte key with the public key with the least getint()
    ...
    ed25519_pubkey_from_private (privkey,
...
#ifndef STANDALONE
#ifdef _DEBUG
void ed25519test (char *vectorfilename) // check ed25519 functions with test vectors from
                                        // http://ed25519.cr.yp.to/python/sign.input
{
    ...
    { // a line consists of 32 bytes private key, 32 byte
      ...
      // message, ":", signed message, ":"
        ...
        if (!vpub || !(vmsg = strchr (vpub + 1, ':')) || !(vsmsg = strchr (vmsg + 1, ':'))
            || hex2bin (privpub, vprivpub, 64) != 64 || hex2bin (pub, ++vpub, 32) != 32
            || (msglen = hex2bin (msg, ++vmsg, linelen)) < 0 || hex2bin (smsg, ++vsmsg, linelen) != (msglen + 64)
    ...
    conoutf ("%d vector file errors, %d private-pubkey mismatches, %d sign fails, %d "
...
void authsetup (char **args, int numargs) // set up private and public keys
{
    ...
    static uchar *buf = nullptr, pub[32], keyhash[128], psalt[16], salt[16], preprivlen = 0;
    ...
    if (numargs > 0)
    {
        ...
        else if (!strcasecmp (args[0], "PPASS"))
        {
            ...
            if (iterations)
                passphrase2key (numargs > 1 ? args[1] : "", psalt, 16, keyhash, preprivlen,
                                &iterations, 0, preprivpwdcfg & 0x3fe);
        ...
        else if (!strcasecmp (args[0], "PASS"))
        {
            ...
            if (iterations)
                passphrase2key (numargs > 1 ? args[1] : "", salt, 16, keyhash, 32, &iterations, 0,
                                privpwdcfg & 0x3ff);
        ...
        else if (!strcasecmp (args[0], "NEWPPASS"))
        {
            ...
            if (preprivlen && !preprivpwdcfg)
            {
                ...
                if (f)
                {
                    ...
                    if (preprivpwdcfg)
                        f->printf (" %s %u", bin2hex (hextemp, psalt, 16), preprivpwdcfg & ~1);
                    f->printf ("\n\n");
                    delete f;
                }
                DELETEA (oldfile);
    ...
    if (numargs > 0)
    {
        ...
        { // authkey list
            if (authkeys.length ())
            {
                conoutf ("name\tpubkey");
                loopv (authkeys)
                conoutf ("%s\t%s", authkeys[i]->name, bin2hex (buf, authkeys[i]->sk + 32, 32));
            }
            else
                conoutf ("no authkeys");
        }
        ...
        { // authkey selfcert name
            ...
            if (numargs > 1 && (sk = getauthkey (args[1])))
            {
                defformatstring (cmd) ("newcert start\n"
                                       ...
                                       "\n",
                                       bin2hex (buf, sk + 32, 32), args[1],
                                       numargs > 2 ? args[2] : "", args[1]);
                execute (cmd);
            }
        }
    }
...
uchar rootcert[]
= { 0xaa, 0x55, 0x8e, 0xb0, 0x43, 0x10, 0x55, 0x36, 0xd7, 0x2a, 0x43, 0x3d, 0x8f, 0xa1, 0x21, 0x79,
...
static const char *certtypekeywords[]
= { "misc", "dev", "master", "server", "player", "clanboss", "clan" }; // needs to match the enums!
static int certtypemaxrank[] = { 0, 1, 2, 3, 3, 3, 0 }; // max allowed rank of some cert types
static int certtypeexpiration[]
= { 0, 0, 0, 2 * CERTTIMEMONTH, CERTTIMEMONTH, 3 * CERTTIMEMONTH, CERTTIMEMONTH }; // time after which certs are usually re-signed, after 3x
...
bool cert::parse () // parse orgmsg[] and check the signature
{
    ...
    if (orglen > certheaderlenfull && orglen < (1 << 20)
        && !strncmp (orgmsg, certheader, certheaderlen) && orgmsg[certheaderlen + 128] == '\n')
    { // frame ok
        ...
        { // parse all lines
            ...
            if (signedby)
            { // check signature (if signature contained a "signed-by"
              // line)
...
char *cert::getnewcertfilename (const char *subpath)
{
    ...
    defformatstring (s) (CERTFILEDIR "%s%s%s_%s_%d." CERTFILEEXT, subpath ? subpath : "",
                         subpath ? PATHDIVS : "", certtypekeywords[type], fname, signeddate);
    return newstring (s);
}
          ...
          (char **args, int numargs) // create and sign certs from cubescript
          {
              ...
              if (numargs > 0)
              {
                  ...
                  else if (!strcasecmp (args[0], "SIGN"))
                  {
                      ...
                      if (numargs > 1
                          && ((getauthkey (args[1]) && memcpy (keypair, getauthkey (args[1]), 32))
                              || (strlen (args[1]) == 64 && hex2bin (keypair, args[1], 32) == 32)))
...
void marksuperseded (cert *chain) // checks all
{
    for (cert *c = chain; c; c = c->next)
    {
        if (c->isvalid)
            for (cert *r = c->next; r; r = r->next)
            {
                ...
                {
                    if ((c->type == r->type && c->name && r->name && !strcmp (c->name, r->name))
                        || (c->pubkey && r->pubkey && !memcmp (c->pubkey, r->pubkey, 32)))
    ...
    for (cert *c = chain; c; c = c->next)
    { // check, if certs lost their parent
...
{
    int act = aktcerttime ();
    cert *treechain = nullptr; // all nested list searches are done on a chain of relevant certs,
    ...
    marksuperseded (treechain); // of the valid certs that are not in the tree: remove
...
void listcerts ()
{
    ...
    {
        cert *c = certs[i];
        conoutf ("certs[%d]: %16s, t:%s, r:%d, l:%d, sd:%d, d2e:%d, d2r:%d, "
                 "parent:%d%s%s%s%s%s%s%s",
                 i, c->name, certtypekeywords[c->type], c->rank, c->lines.length (), c->signeddate,
                 c->days2expire, c->days2renew, certs.find (c->parent), c->ischecked ? ", ischecked" : "",
                 c->isvalid ? ", isvalid" : "", c->needsrenewal ? ", needsrenewal" : "",
                 c->expired ? ", expired" : "", c->superseded ? ", superseded" : "",
...
const char *genpwdhash (const char *name, const char *pwd, int salt)
{
    ...
    formatstring (temp) ("%s %s %s", hashchunktoa (hash.chunks[0]), hashchunktoa (hash.chunks[1]),
