...
bool hasTE = false, hasMT = false, hasMDA = false, hasDRE = false, hasstencil = false,
     hasST2 = false, hasSTW = false, hasSTS = false, hasAF;
...
void gl_checkextensions ()
{
    ...
    if (hasext (exts, "GL_ARB_multitexture"))
    {
        ...
        = (PFNGLCLIENTACTIVETEXTUREARBPROC)getprocaddress ("glClientActiveTextureARB");
    ...
    if (hasext (exts, "GL_ATI_separate_stencil"))
    {
        ...
        = (PFNGLSTENCILFUNCSEPARATEATIPROC)getprocaddress ("glStencilFuncSeparateATI");
        hasSTS = true;
    }
...
#ifdef WIN32
    char broken_res[]
    = { 0x44, 0x69, 0x66, 0x62, 0x75, 0x21, 0x46, 0x6f, 0x68, 0x6a, 0x6f, 0x66, 0x01 };
...
void drawreflection (float hf, int w, int h, float changelod, bool refract)
{
    ...
    render_world (camera1->o.x, camera1->o.y, refract ? camera1->o.z : hf, changelod, (int)camera1->yaw,
                  (refract ? 1 : -1) * (int)camera1->pitch, dynfov (), fovy, size, size);
...
void renderzones (float z)
{
    ...
    loopv (zones)
    {
        glColor4f (((zones[i].color >> 16) & 0xFF) / 255.0f,
                   ((zones[i].color >> 8) & 0xFF) / 255.0f, (zones[i].color & 0xFF) / 255.0f, 0.3f);
        glBegin (GL_QUADS);
...
void drawminimap (int w, int h)
{
    ...
    transplayer ();
    resetcubes ();
    render_world (camera1->o.x, camera1->o.y, camera1->o.z, 1.0f, (int)camera1->yaw,
                  (int)camera1->pitch, 90.0f, 90.0f, size, size);
    setupstrips ();
    setuptmu (0, "T * P x 2");
    ...
    glVertex3f (clmapdims.x1 + clmapdims.xspan - 0.02f, clmapdims.y1 + clmapdims.yspan - 0.02f,
                clmapdims.maxceil);
...
bool outsidemap (physent *pl)
{
    ...
    return SOLID (s) || pl->o.z < s->floor - (s->type == FHF ? s->vdelta / 4 : 0)
...
void gl_drawframe (int w, int h, float changelod, float curfps, int elapsed)
{
    ...
    = waterreflect && !editmode && (!mapoverride_nowaterreflect || ignoreoverride_nowaterreflect);
    ...
    float fogc[4]
    = { (fogcolour >> 16) / 256.0f, ((fogcolour >> 8) & 255) / 256.0f, (fogcolour & 255) / 256.0f, 1.0f },
    ...
    glClear ((outsidemap (camera1) ? GL_COLOR_BUFFER_BIT : 0) | GL_DEPTH_BUFFER_BIT
             | (effective_stencilshadow && hasstencil && stencilbits >= 8 ? GL_STENCIL_BUFFER_BIT : 0));

    glEnable (GL_TEXTURE_2D);

    resetcubes ();

    render_world (camera1->o.x, camera1->o.y, camera1->o.z, changelod, (int)camera1->yaw,
                  (int)camera1->pitch, dynfov (), fovy, w, h);
    ...
    int nquads = renderwater (hf, !effective_waterreflect || underwater ? 0 : reflecttex,
