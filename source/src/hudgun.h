...
struct weaponmove
{
    ...
    void calcmove (vec aimdir, int lastaction, playerent *p)
    {
        ...
        if (p->weaponchanging)
        {
            ...
            float progress
            = clamp ((lastmillis - p->weaponchanging) / (float)weapon::weaponchangetime, 0.0f, 1.0f);
        ...
        else if (p->weaponsel->reloading)
        {
            ...
            float reloadtime = (float)p->weaponsel->info.reloadtime,
                  progress = clamp ((lastmillis - p->weaponsel->reloading) / reloadtime, 0.0f,
                                    clamp (1.0f - (p->lastaction + p->weaponsel->gunwait - lastmillis) / reloadtime,
                                           0.5f, 1.0f));
            k_rot = -90 * sinf (progress * PI);
        }
        else
