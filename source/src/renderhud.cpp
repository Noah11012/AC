...
void drawicon(Texture *tex, float x, float y, float s, int col, int row,
              float ts) {
...
void drawvoteicon(float x, float y, int col, int row, bool noblend) {
  ...
  if (tex) {
    if (noblend)
      ...
      turn_on_transparency(votealpha);  // if(transparency && !noblend)
...
void drawscope(bool preload) {
  ...
  float sz = min(VIRTW, VIRTH), x1 = VIRTW / 2 - sz / 2 + scopecenterfix,
        x2 = VIRTW / 2 + sz / 2, y1 = VIRTH / 2 - sz / 2,
        y2 = VIRTH / 2 + sz / 2, border = (512 - 64 * 2) / 512.0f;
  ...
  loopi(8 + 1) {
    float c = 0.5f * (1 + border * cosf(i * PI2 / 8.0f)),
          s = 0.5f * (1 + border * sinf(i * PI2 / 8.0f));
    glTexCoord2f(c, s);
...
Texture *loadcrosshairtexture(const char *c) {
  ...
  if (crosshair == notexture)
    crosshair = textureload("packages/crosshairs/default.png", 3);
  return crosshair;
}

void loadcrosshair(const char *type, const char *filename) {
  int index = CROSSHAIR_DEFAULT;
  if (!*filename) {  // special form: loadcrosshair filename   // short for
                     // "loadcrosshair default filename"
    if (strcasecmp(type, "reset"))
      filename = type;
    else {  // "loadcrosshair reset" does exactly that
  ...
  } else if (strchr(type, '.')) {  // old syntax "loadcrosshair filename type",
                                   // remove this in 2020
    const char *oldcrosshairnames[CROSSHAIR_NUM + 1] = {
        "default", "teammate", "scope",  "knife",  "pistol",
        "carbine", "shotgun",  "smg",    "sniper", "ar",
        "cpistol", "grenades", "akimbo", ""};
    ...
    if (index > 2)
      ...
      index += NUMGUNS;
    filename = type;
  } else {  // new syntax with proper gun names
    index = getlistindex(type, crosshairnames, false, CROSSHAIR_DEFAULT);
  }
  if (index == CROSSHAIR_DEFAULT) {
    loopi(CROSSHAIR_NUM) if (i != CROSSHAIR_TEAMMATE && i != CROSSHAIR_SCOPE)
        crosshairs[i] = loadcrosshairtexture(filename);
  } else {
...
void drawcrosshair(playerent *p, int n, color *c, float size) {
  ...
  if (!crosshair) {
    ...
    if (!crosshair)
      crosshair = crosshairs[CROSSHAIR_DEFAULT] =
          loadcrosshairtexture("default.png");
  }
  ...
  float chsize = s *
                 ((p == player1 && p->weaponsel->type == GUN_ASSAULT &&
                   p->weaponsel->shots > 3) &&
                          (crosshairfx == 1 || crosshairfx == 3)
                      ? 1.4f
                      : 1.0f) *
                 (n == CROSSHAIR_TEAMMATE ? 2.0f : 1.0f);
  glBegin(GL_TRIANGLE_STRIP);
  glTexCoord2f(0, 0);
...
void updatedmgindicator(playerent *p, vec &attack) {
  ...
  vec base_d = p->o;
  base_d.sub(attack);
  damagedirections[(int(742.5f - p->yaw - base_d.anglexy()) / 45) & 0x7] =
      lastmillis + damageindicatortime;
}
...
void drawdmgindicator() {
  ...
  auto size = (float)damageindicatorsize;
  loopi(8) {
...
void drawktfindicator(playerent *p) {
  ...
  loopi(2) {
    ...
    if (f.state == CTFF_INBASE)
      flagpos = f.pos;
    else if (f.state == CTFF_STOLEN && f.actor && f.actor != p)
      flagpos = f.actor->o;
  }
...
int getprevweaponsel(playerent *p)  // get previous weapon or, if we don't know
                                    // it, get the primary
{
  return p->prevweaponsel->type == GUN_ASSAULT &&
                 p->primweap->type != GUN_ASSAULT
             ? p->primweap->type
             : p->prevweaponsel->type;
}
...
void drawequipicons(playerent *p) {
  ...
  if (p->mag[GUN_GRENADE])
    drawequipicon(
        oldfashionedgunstats ? (HUDPOS_GRENADE + 25) * 2 : HUDPOS_GRENADE * 2,
        1650, 3, 1);

  // weapons
  int c = p->weaponsel->type != GUN_GRENADE ? p->weaponsel->type
                                            : getprevweaponsel(p),
      r = 0;
  if (c == GUN_AKIMBO || c == GUN_CPISTOL)
    c = GUN_PISTOL;  // same icon for akimb & pistol
  if (c > 3) {
    c -= 4;
    r = 1;
  }
...
void drawradarent(float x, float y, float yaw, int col, int row, float iconsize,
                  bool pulse, const char *label = nullptr, ...) PRINTFARGS(8, 9);

void drawradarent(float x, float y, float yaw, int col, int row, float iconsize,
                  bool pulse, const char *label, ...) {
  glPushMatrix();
  if (pulse)
    glColor4f(1.0f, 1.0f, 1.0f,
              0.2f + (sinf(lastmillis / 30.0f) + 1.0f) / 2.0f);
  else
    glColor4f(1, 1, 1, 1);
  glTranslatef(x, y, 0);
  glRotatef(yaw, 0, 0, 1);
...
struct hudmessages : consolebuffer<hudline> {
  ...
  void editline(int type, const char *sf) {
    if (conlines.length() &&
        ((conlines[0].type & HUDMSG_TYPE) == (type & HUDMSG_TYPE) ||
         conlines[0].type & HUDMSG_OVERWRITE)) {
      conlines[0].millis = totalmillis;
      conlines[0].type = type;
      copystring(conlines[0].line, sf);
    } else
      consolebuffer<hudline>::addline(sf, totalmillis).type = type;
  }
  void render() override {
    ...
    loopi(min(conlines.length(), 3)) if (totalmillis - conlines[i].millis <
                                         dispmillis) {
      cline &c = conlines[i];
      int tw = text_width(c.line);
      draw_text(c.line, int(tw > VIRTW * 0.9f ? 0 : (VIRTW * 0.9f - tw) / 2),
                int(((VIRTH * 0.9f) / 4 * 3) + FONTH * i +
                    pow((totalmillis - c.millis) / (float)dispmillis, 4) *
                        VIRTH * 0.9f / 4.0f));
    }
    glPopMatrix();
  }
...
vec getradarpos() {
  ...
  return {VIRTW - 10 - VIRTH / 28 - overlaysize, 10 + VIRTH / 52, 0};
}
...
void drawradar_showmap(playerent *p, int w, int h) {
  ...
  glPushMatrix();
  bool spect3rd = p->spectatemode > SM_FOLLOW1ST &&
                  p->spectatemode <= SM_FOLLOW3RD_TRANSPARENT;
  playerent *d = spect3rd ? players[p->followplayercn] : p;
  int p_baseteam = p->team == TEAM_SPECT && spect3rd
                       ? team_base(players[p->followplayercn]->team)
                       : team_base(p->team);
  extern GLuint minimaptex;
  vec centerpos(VIRTW / 2, VIRTH / 2, 0.0f);
  if (showmapbackdrop) {
    glDisable(GL_TEXTURE_2D);
    if (showmapbackdrop == 2)
      glBlendFunc(GL_ONE_MINUS_DST_ALPHA, GL_ONE_MINUS_SRC_COLOR);
    loopi(2) {
      int cg =
          i ? (showmapbackdrop == 2
                   ? ((int)(255 * (100 - showmapbackdroptransparency) / 100.0f))
                   : 0)
            : (showmapbackdrop == 2
                   ? ((int)(255 * (100 - showmapbackdroptransparency) / 100.0f))
                   : 64);
      ...
      glColor3ub(cg, cg, cg);
      glBegin(GL_QUADS);
      glVertex2f(centerpos.x - halfviewsize - co,
                 centerpos.y + halfviewsize + co);
      glVertex2f(centerpos.x + halfviewsize + co,
                 centerpos.y + halfviewsize + co);
      glVertex2f(centerpos.x + halfviewsize + co,
                 centerpos.y - halfviewsize - co);
  ...
  if (!(p->isspectating() && spect3rd))
    drawradarent(ppv.x, ppv.y, p->yaw,
                 (p->state == CS_ALIVE || p->state == CS_EDITING)
                     ? (isattacking(p) ? 2 : 0)
                     : 1,
                 2, iconsize, isattacking(p), "%s",
                 colorname(p));  // local player
  loopv(players)                 // other players
  {
    ...
    if (p->team < TEAM_SPECT &&
        ((m_teammode && !isteam(p_baseteam, pl_baseteam)) ||
         (!m_teammode && !(spect3rd && d == pl))))
      continue;
    if (p->team == TEAM_SPECT &&
        !(spect3rd && (isteam(p_baseteam, pl_baseteam) || d == pl)))
      continue;
    vec rtmp = vec(pl->o).sub(mdd).mul(coordtrans);
    drawradarent(rtmp.x, rtmp.y, pl->yaw,
                 pl->state == CS_ALIVE ? (isattacking(pl) ? 2 : 0) : 1,
                 spect3rd && d == pl ? 2 : pl_baseteam, iconsize,
                 isattacking(pl), "%s", colorname(pl));
  }
  if (m_flags) {
    ...
    loopi(2)  // flag items
    {
      ...
      bool hasflagent =
          e && e->x != -1 &&
          e->y !=
              -1;  // no base for flagentitydummies (HTF on maps without bases)
      if (hasflagent) {
        vec pos = vec(e->x, e->y, 0).sub(mdd).mul(coordtrans);
        drawradarent(pos.x, pos.y, 0, m_ktf ? 2 : f.team, 3, iconsize,
                     false);  // draw bases
      }
      if ((f.state == CTFF_INBASE && hasflagent) || f.state == CTFF_DROPPED) {
        vec fltxoff = vec(8, -8, 0);
        vec cpos = vec(f.pos.x, f.pos.y, f.pos.z)
                       .sub(mdd)
                       .mul(coordtrans)
                       .add(fltxoff);
        float flgoff = fabs((radarentsize * 2.1f) - 8);
        drawradarent(cpos.x + flgoff, cpos.y - flgoff, 0, 3, m_ktf ? 2 : f.team,
                     iconsize,
                     false);  // draw on entity pos or whereever dropped
      }
      if (f.state == CTFF_STOLEN) {
        if (m_teammode && !m_ktf && player1->team == TEAM_SPECT &&
            p->spectatemode > SM_FOLLOW3RD_TRANSPARENT)
          continue;
        ...
        if (f.actor) {
          ...
          if (tm) {
            ...
            drawradarent(apos.x, apos.y, 0, 3, m_ktf ? 2 : f.team, iconsize,
                         true);  // draw near flag thief
          }
        }
      }
    }
...
void drawradar_vicinity(playerent *p, int w, int h) {
  ...
  glDisable(GL_BLEND);
  circle(minimaptex, halfviewsize, halfviewsize, halfviewsize,
         usecenter.x / (float)gdim, usecenter.y / (float)gdim, scaleh,
         31);  // Draw mimimaptext as radar background
  glTranslatef(halfviewsize, halfviewsize, 0);

  if (!(p->isspectating() && spect3rd))
    drawradarent(0, 0, p->yaw,
                 (p->state == CS_ALIVE || p->state == CS_EDITING)
                     ? (isattacking(p) ? 2 : 0)
                     : 1,
                 2, iconsize, isattacking(p), "%s",
    ...
    loopi(2)  // flag items
    {
      ...
      if (hasflagent) {
        ...
        if (pos.sqrxy() < d2s) {
          pos.mul(scaled);
          drawradarent(pos.x, pos.y, 0, m_ktf ? 2 : f.team, 3, iconsize,
                       false);  // draw bases [circle doesn't need rotating]
        }
      }
      if ((f.state == CTFF_INBASE && hasflagent) || f.state == CTFF_DROPPED) {
        ...
        if (cpos.sqrxy() < d2s) {
          ...
          drawradarent(cpos.x + offx, cpos.y - offy, camera1->yaw, 3,
                       m_ktf ? 2 : f.team, iconsize,
                       false);  // draw flag on entity pos or whereever dropped
        }
      }
      if (f.state == CTFF_STOLEN) {
        if (m_teammode && !m_ktf && player1->team == TEAM_SPECT &&
        ...
        if (f.actor) {
          ...
          if (tm) {
            ...
            if (apos.sqrxy() < d2s) {
              apos.mul(scaled);
              drawradarent(apos.x, apos.y, camera1->yaw, 3, m_ktf ? 2 : f.team,
                           iconsize, true);  // draw near flag thief
            }
          }
        }
      }
  ...
  quad(bordertex->id, centerpos.x - halfviewsize - 16,
       centerpos.y - halfviewsize - 16, radarviewsize + 32, 0, 0, 1, 1);
  if (!hidecompass) {
    static Texture *compasstex = nullptr;
    if (!compasstex)
      compasstex = textureload("packages/misc/compass-rose.png", 3);
    glPushMatrix();
    glTranslatef(centerpos.x, centerpos.y, 0);
    glRotatef(-camera1->yaw, 0, 0, 1);
    quad(compasstex->id, -halfviewsize - 8, -halfviewsize - 8,
         radarviewsize + 16, 0, 0, 1, 1);
    glPopMatrix();
  }
}
...
void drawradar(playerent *p, int w, int h) {
  if (showmap)
    drawradar_showmap(p, w, h);
  else
    drawradar_vicinity(p, w, h);
}
...
void drawteamicons(int w, int h, bool spect) {
  ...
  if (player1->team < TEAM_SPECT || spect)
    quad(icons->id, VIRTW - VIRTH / 12 - 10, 10, VIRTH / 12,
         team_base(spect ? players[player1->followplayercn]->team
                         : player1->team)
             ? 0.5f
             : 0,
         0, 0.49f, 1.0f);
...
void *damageblendplayer = nullptr;
...
void damageblend(int n, void *p) {
  ...
  damageblendmillis =
      min(damageblendmillis + n * damagescreenfactor, lastmillis + 3999);
  damageblendplayer = p;
}

inline char rangecolor(int val, const char *colors, int thres1, int thres2,
                       int thres3) {
  if (val < thres1)
    return colors[0];
  else if (val < thres2)
...
const char *medal_str[] = {
    "Best Fragger",
    "Dude that dies a lot"};  // just some medals string tests, nothing serious
...
void drawscores() {
  ...
  top += 10;
  left += 10;
  const float txtdx = 160, txtdy = 30, medalsdy = 130;
  glColor4f(1, 1, 1, 1);
  float desttime = 0;
  loopi(END_MDS) {
    if (a_medals[i].assigned) {
      ...
      draw_textf("%s %s: %d", left + txtdx, top + txtdy, medal_str[i],
                 mpl->name, a_medals[i].item);
      top += medalsdy;
    }
  }
...
void gl_drawhud(int w, int h, int curfps, int nquads, int curvert,
  ...
  bool command = getcurcommand(nullptr) ? true : false;
  bool reloading =
      lastmillis < p->weaponsel->reloading + p->weaponsel->info.reloadtime;
  if (p->state == CS_ALIVE || p->state == CS_EDITING) {
    bool drawteamwarning = crosshairteamsign && targetplayer &&
                           isteam(targetplayer->team, p->team) &&
                           targetplayer->state == CS_ALIVE;
  ...
  bool is_spect = ((player1->spectatemode == SM_FOLLOW1ST ||
                    player1->spectatemode == SM_FOLLOW3RD ||
                    player1->spectatemode == SM_FOLLOW3RD_TRANSPARENT) &&
                   players.inrange(player1->followplayercn) &&
  ...
  if (command)
    commandh -= rendercommand(-1, HUDPOS_Y_BOTTOMLEFT,
                              VIRTW - FONTH);  // dryrun to get height
  else if (infostr)
    draw_text(infostr, HUDPOS_X_BOTTOMLEFT, HUDPOS_Y_BOTTOMLEFT);
  else if (targetplayer && showtargetname)
    draw_text(colorname(targetplayer), HUDPOS_X_BOTTOMLEFT,
              HUDPOS_Y_BOTTOMLEFT);
  glLoadIdentity();
  ...
  if (!is_spect && !editmode && !watchingdemo && !command &&
      curmenu == scoremenu)
    r_accuracy(commandh);
  if (!hideconsole) renderconsole();
  formatstring(enginestateinfo)("%d %d %d %d %d", curfps, lod_factor(), nquads,
                                curvert, xtraverts);
  ...
  if (wallclock)
    filtertext(
        ltime,
        timestring(*ltimeformat != 'U', ltimeformat + int(*ltimeformat == 'U')),
        FTXT_TOLOWER);

  if (showstats) {
    if (showstats == 2 && !dbgpos) {
      int left = (VIRTW - 225 - 10) * 2, top = (VIRTH * 7 / 8) * 2,
          ttll = VIRTW * 2 - 3 * FONTH / 2, lf = lod_factor();
      ...
      formatstring(text)("\f%c%d", rangecolor(curfps, "xwvu", 30, 100, 150),
                         curfps);
      draw_text(text, ttll - text_width(text), top);
      formatstring(text)("\f%c%d", rangecolor(lf, "uvwx", 199, 299, 399), lf);
      draw_text(text, ttll - text_width(text), top + 80);
      formatstring(text)("\f%c%d", rangecolor(nquads, "uvwx", 3999, 5999, 7999),
                         nquads);
      draw_text(text, ttll - text_width(text), top + 160);
      formatstring(text)(
          "\f%c%d", rangecolor(curvert, "uvwx", 3999, 5999, 7999), curvert);
      draw_text(text, ttll - text_width(text), top + 240);
      formatstring(text)(
          "\f%c%d", rangecolor(xtraverts, "uvwx", 3999, 5999, 7999), xtraverts);
      draw_text(text, ttll - text_width(text), top + 320);

      if (wallclock) draw_text(ltime, ttll - text_width(ltime), top - 90);
      if (unsavededits)
        draw_text("U", ttll - text_width("U"),
                  top - 90 - (wallclock ? 2 * FONTH / 2 : 0));
    } else {
      if (dbgpos) {
        pushfont("mono");
        formatstring(text)("%05.2f YAW", p->yaw);
        draw_text(text, VIRTW * 2 - (text_width(text) + FONTH),
                  VIRTH * 2 - 17 * FONTH / 2);
        formatstring(text)("%05.2f PIT", p->pitch);
        draw_text(text, VIRTW * 2 - (text_width(text) + FONTH),
                  VIRTH * 2 - 15 * FONTH / 2);
        formatstring(text)("%05.2f X  ", p->o.x);
        draw_text(text, VIRTW * 2 - (text_width(text) + FONTH),
                  VIRTH * 2 - 13 * FONTH / 2);
        formatstring(text)("%05.2f Y  ", p->o.y);
        draw_text(text, VIRTW * 2 - (text_width(text) + FONTH),
                  VIRTH * 2 - 11 * FONTH / 2);
        formatstring(text)("%05.2f Z  ", p->o.z);
        draw_text(text, VIRTW * 2 - (text_width(text) + FONTH),
                  VIRTH * 2 - 9 * FONTH / 2);
        popfont();
      }
      defformatstring(c_val)("fps %d", curfps);
      draw_text(c_val, VIRTW * 2 - (text_width(c_val) + FONTH),
                VIRTH * 2 - 3 * FONTH / 2);

      if (wallclock)
        draw_text(ltime, VIRTW * 2 - text_width(ltime) - FONTH,
                  VIRTH * 2 - 5 * FONTH / 2);
      if (unsavededits)
        draw_text("U", VIRTW * 2 - text_width("U") - FONTH,
                  VIRTH * 2 - (wallclock ? 7 : 5) * FONTH / 2);
    }
  } else if (wallclock)
    draw_text(ltime, VIRTW * 2 - text_width(ltime) - FONTH,
              VIRTH * 2 - 3 * FONTH / 2);
  ...
  if (editmode && !hideeditinfopanel) {
    ...
    static char *editip = nullptr;
    if (!lasteditip || totalmillis - lasteditip >
                           editinfopanelmillis) {  // update edit info panel
    ...
    if (editip && dimeditinfopanel) {
      ...
      draw_text(editip, VIRTW * 2 - w - FONTH, VIRTH * 13 / 8 - h, 255, 255,
                255, dimeditinfopanel);
    }
  ...
  if (!intermission && lastgametimeupdate != 0) {
    ...
    if (gametimedisplay)
      draw_text(gtime,
                (VIRTW - 225 - 10) * 2 - (text_width(gtime) / 2 + FONTH / 2),
                20);
  }

  if (hidevote < 2 && multiplayer(nullptr)) {
    extern votedisplayinfo *curvote;

    if (curvote && curvote->millis >= totalmillis &&
        !(hidevote == 1 && curvote->localplayervoted &&
          curvote->result == VOTE_NEUTRAL)) {
      const int left = 2 * HUDPOS_X_BOTTOMLEFT, top = VIRTH;
      defformatstring(str)("%s called a vote:",
                           curvote->owner ? colorname(curvote->owner) : "");
      ...
      formatstring(str)("%d yes vs. %d no", curvote->stats[VOTE_YES],
      ...
      switch (curvote->result) {
        case VOTE_NEUTRAL:
          ...
          if (!curvote->localplayervoted)
            draw_text("\f3press F1/F2 to vote yes or no", left, top + 560, 255,
                      255, 255, votealpha);
          break;
        default:
          drawvoteicon(left, top, (curvote->result - 1) & 1, 1, false);
          formatstring(str)("\f3vote %s",
                            curvote->result == VOTE_YES ? "PASSED" : "FAILED");
          draw_text(str, left, top + 560, 255, 255, 255, votealpha);
          break;
      }
    }
  }
  // else draw_textf("%c%d here F1/F2 will be praised during a vote", 20*2,
  // VIRTH+560, '\f', 0); // see position (left/top) setting in block above

  if (menu)
    rendermenu();
  else if (command)
    renderdoc(40, VIRTH, max(commandh * 2 - VIRTH, 0));

  if (!hidespecthud && !menu && p->state == CS_DEAD &&
      p->spectatemode <= SM_DEATHCAM) {
    glLoadIdentity();
    glOrtho(0, VIRTW * 3 / 2, VIRTH * 3 / 2, 0, -1, 1);
    const int left = (VIRTW)*3 / 2, top = (VIRTH * 3 / 2) * 3 / 4;
    draw_textf("SPACE to change view",
               left - (text_width("SCROLL to change player") + FONTH / 2), top);
    if (multiplayer(nullptr) || watchingdemo)
      draw_textf("SCROLL to change player",
                 left - (text_width("SCROLL to change player") + FONTH / 2),
                 top + 80);
  }
  ...
  if (!hidespecthud && spectating && player1->spectatemode != SM_DEATHCAM) {
    ...
    if (player1->team == TEAM_SPECT)
      specttext = "GHOST";
    else if (player1->team == TEAM_CLA_SPECT)
      specttext = "[CLA]";
    else if (player1->team == TEAM_RVSF_SPECT)
      specttext = "[RVSF]";
    draw_text(specttext, VIRTW / 40, VIRTH / 10 * 7);
    if (is_spect) {
      defformatstring(name)("Player %s",
                            colorname(players[player1->followplayercn]));
      draw_text(name, VIRTW / 40, VIRTH / 10 * 8);
    }
  }

  if (p->state == CS_ALIVE ||
      (p->state == CS_DEAD && p->spectatemode == SM_DEATHCAM)) {
    glLoadIdentity();
    glOrtho(0, VIRTW / 2, VIRTH / 2, 0, -1, 1);
    ...
    if (p->state == CS_ALIVE && !hidehudequipment) {
      ...
      if (p->armour)
        draw_textf("%d", HUDPOS_ARMOUR + HUDPOS_NUMBERSPACING, 823, p->armour);
      if (p->weaponsel && valid_weapon(p->weaponsel->type)) {
        glMatrixMode(GL_MODELVIEW);
        if (p->weaponsel->type != GUN_GRENADE)
          p->weaponsel->renderstats();
        else if (p->prevweaponsel->type == GUN_AKIMBO ||
                 p->prevweaponsel->type == GUN_PISTOL)
          p->weapons[p->akimbo ? GUN_AKIMBO : GUN_PISTOL]->renderstats();
        else
          p->weapons[getprevweaponsel(p)]->renderstats();
    ...
    if ((m_flags || m_teammode) && !hideteamscorehud) {
      ...
      if (!offs || scores[2] || scores[3])
        loopi(2)  // flag state or frag-counter
        {
          bool inbase = flaginfos[i].state == CTFF_INBASE || offs;
          drawctficon(i * 120 + VIRTW / 4.0f * 3.0f, 1650, 120,
                      i + (inbase || flagscorehudtransparency == 2 ? 0 : 2),
                      offs / 2, 1 / 4.0f,
                      inbase || !flagscorehudtransparency ? 255 : 100);
          if (m_teammode) {
            defformatstring(count)("\f%c%d", cc[i], scores[i + offs]);
            draw_text(
                count,
                i * 120 + VIRTW / 4.0f * 3.0f + 60 - text_width(count) / 2,
                1590);
          }
        }
      if (!offs) {
        ...
        if ((flaginfos[0].state == CTFF_STOLEN && flaginfos[0].actor == p &&
             flaginfos[0].ack) ||
            (flaginfos[1].state == CTFF_STOLEN && flaginfos[1].actor == p &&
             flaginfos[1].ack && ++ft)) {
          drawctficon(VIRTW - 225 - 10, VIRTH * 5 / 8, 225, ft, 1, 1 / 2.0f,
                      (sinf(lastmillis / 100.0f) + 1.0f) * 128);
        }
      }
    }
  }
...
Texture *startscreen = nullptr;

void loadingscreen(const char *fmt, ...) {
  if (!startscreen)
    startscreen = textureload("packages/misc/startscreen.png", 3);
...
static void bar(float bar, int o, float r, float g, float b) {
  ...
  glBegin(GL_TRIANGLE_STRIP);
  loopk(10) {
    float c = 1.2f * cosf(PI / 2 + k / 9.0f * PI),
          s = 1 + 1.2f * sinf(PI / 2 + k / 9.0f * PI);
  ...
  glBegin(GL_TRIANGLE_STRIP);
  loopk(10) {
    float c = cosf(PI / 2 + k / 9.0f * PI),
          s = 1 + sinf(PI / 2 + k / 9.0f * PI);
    glVertex2f(x2 - c * FONTH, y1 + s * FONTH);
    glVertex2f(x1 + c * FONTH, y1 + s * FONTH);
  }
  glEnd();
...
void show_out_of_renderloop_progress(
    float bar1, const char *text1, float bar2,
    const char *text2)  // also used during loading
{
  c2skeepalive();
