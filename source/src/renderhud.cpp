...
void drawscope (bool preload)
{
    ...
    float sz = min (VIRTW, VIRTH), x1 = VIRTW / 2 - sz / 2 + scopecenterfix, x2 = VIRTW / 2 + sz / 2,
          y1 = VIRTH / 2 - sz / 2, y2 = VIRTH / 2 + sz / 2, border = (512 - 64 * 2) / 512.0f;
...
void loadcrosshair (const char *type, const char *filename)
{
    ...
    if (!*filename)
    { // special form: loadcrosshair filename   // short for
    ...
    { // old syntax "loadcrosshair filename type",
      // remove this in 2020
        const char *oldcrosshairnames[CROSSHAIR_NUM + 1]
        = { "default", "teammate", "scope", "knife",   "pistol",   "carbine", "shotgun",
            "smg",     "sniper",   "ar",    "cpistol", "grenades", "akimbo",  "" };
    ...
    {
        loopi (CROSSHAIR_NUM) if (i != CROSSHAIR_TEAMMATE && i != CROSSHAIR_SCOPE) crosshairs[i]
        = loadcrosshairtexture (filename);
    }
    else
    {
...
void drawcrosshair (playerent *p, int n, color *c, float size)
{
    ...
    if (!crosshair)
    {
        ...
        if (!crosshair)
            crosshair = crosshairs[CROSSHAIR_DEFAULT] = loadcrosshairtexture ("default.png");
    }
    ...
    float chsize = s
                   * ((p == player1 && p->weaponsel->type == GUN_ASSAULT && p->weaponsel->shots > 3)
...
int getprevweaponsel (playerent *p) // get previous weapon or, if we don't know
                                    // it, get the primary
{
    return p->prevweaponsel->type == GUN_ASSAULT && p->primweap->type != GUN_ASSAULT ?
           p->primweap->type :
...
void drawequipicons (playerent *p)
{
    ...
    if (p->mag[GUN_GRENADE])
        drawequipicon (oldfashionedgunstats ? (HUDPOS_GRENADE + 25) * 2 : HUDPOS_GRENADE * 2, 1650, 3, 1);
...
void drawradarent (float x, float y, float yaw, int col, int row, float iconsize, bool pulse, const char *label, ...)
{
    glPushMatrix ();
    if (pulse)
        glColor4f (1.0f, 1.0f, 1.0f, 0.2f + (sinf (lastmillis / 30.0f) + 1.0f) / 2.0f);
    else
        glColor4f (1, 1, 1, 1);
    glTranslatef (x, y, 0);
    glRotatef (yaw, 0, 0, 1);
...
struct hudmessages : consolebuffer<hudline>
{
    ...
    {
        if (conlines.length ()
            && ((conlines[0].type & HUDMSG_TYPE) == (type & HUDMSG_TYPE) || conlines[0].type & HUDMSG_OVERWRITE))
        {
            conlines[0].millis = totalmillis;
        ...
        loopi (min (conlines.length (), 3)) if (totalmillis - conlines[i].millis < dispmillis)
        {
            ...
            draw_text (c.line, int(tw > VIRTW * 0.9f ? 0 : (VIRTW * 0.9f - tw) / 2),
                       int(((VIRTH * 0.9f) / 4 * 3) + FONTH * i
                           + pow ((totalmillis - c.millis) / (float)dispmillis, 4) * VIRTH * 0.9f / 4.0f));
        }
        glPopMatrix ();
    }
};
...
void drawradar_showmap (playerent *p, int w, int h)
{
    ...
    int p_baseteam = p->team == TEAM_SPECT && spect3rd ? team_base (players[p->followplayercn]->team) :
                                                         team_base (p->team);
    ...
    if (showmapbackdrop)
    {
        ...
        loopi (2)
        {
            int cg
            = i ?
              (showmapbackdrop == 2 ? ((int)(255 * (100 - showmapbackdroptransparency) / 100.0f)) : 0) :
              (showmapbackdrop == 2 ? ((int)(255 * (100 - showmapbackdroptransparency) / 100.0f)) : 64);
            int co = i ? 0 : 4;
            glColor3ub (cg, cg, cg);
            glBegin (GL_QUADS);
    ...
    if (!(p->isspectating () && spect3rd))
        drawradarent (ppv.x, ppv.y, p->yaw,
                      (p->state == CS_ALIVE || p->state == CS_EDITING) ? (isattacking (p) ? 2 : 0) : 1,
    ...
    loopv (players)                   // other players
    {
        ...
        if (p->team < TEAM_SPECT
            && ((m_teammode && !isteam (p_baseteam, pl_baseteam)) || (!m_teammode && !(spect3rd && d == pl))))
            continue;
        if (p->team == TEAM_SPECT && !(spect3rd && (isteam (p_baseteam, pl_baseteam) || d == pl)))
            continue;
        vec rtmp = vec (pl->o).sub (mdd).mul (coordtrans);
        drawradarent (rtmp.x, rtmp.y, pl->yaw, pl->state == CS_ALIVE ? (isattacking (pl) ? 2 : 0) : 1,
                      spect3rd && d == pl ? 2 : pl_baseteam, iconsize, isattacking (pl), "%s",
                      colorname (pl));
    }
    if (m_flags)
    {
        ...
        loopi (2) // flag items
        {
            ...
            if ((f.state == CTFF_INBASE && hasflagent) || f.state == CTFF_DROPPED)
            {
                ...
                drawradarent (cpos.x + flgoff, cpos.y - flgoff, 0, 3, m_ktf ? 2 : f.team, iconsize,
                              false); // draw on entity pos or whereever dropped
            }
            if (f.state == CTFF_STOLEN)
            {
                if (m_teammode && !m_ktf && player1->team == TEAM_SPECT && p->spectatemode > SM_FOLLOW3RD_TRANSPARENT)
                    continue;
...
void drawradar_vicinity (playerent *p, int w, int h)
{
    ...
    glDisable (GL_BLEND);
    circle (minimaptex, halfviewsize, halfviewsize, halfviewsize, usecenter.x / (float)gdim,
            usecenter.y / (float)gdim, scaleh,
        ...
        loopi (2) // flag items
        {
            ...
            if ((f.state == CTFF_INBASE && hasflagent) || f.state == CTFF_DROPPED)
            {
                ...
                if (cpos.sqrxy () < d2s)
                {
                    ...
                    drawradarent (cpos.x + offx, cpos.y - offy, camera1->yaw, 3, m_ktf ? 2 : f.team, iconsize,
    ...
    quad (bordertex->id, centerpos.x - halfviewsize - 16, centerpos.y - halfviewsize - 16,
          radarviewsize + 32, 0, 0, 1, 1);
    if (!hidecompass)
    {
...
void drawteamicons (int w, int h, bool spect)
{
    ...
    if (player1->team < TEAM_SPECT || spect)
        quad (icons->id, VIRTW - VIRTH / 12 - 10, 10, VIRTH / 12,
              team_base (spect ? players[player1->followplayercn]->team : player1->team) ? 0.5f : 0,
              0, 0.49f, 1.0f);
}
...
= { "Best Fragger", "Dude that dies a lot" }; // just some medals string tests, nothing serious
...
void drawscores ()
{
    ...
    loopi (END_MDS)
    {
        if (a_medals[i].assigned)
        {
            ...
            draw_textf ("%s %s: %d", left + txtdx, top + txtdy, medal_str[i], mpl->name, a_medals[i].item);
            top += medalsdy;
        }
    }
...
void gl_drawhud (int w, int h, int curfps, int nquads, int curvert, bool underwater, int elapsed)
{
    ...
    {
        bool drawteamwarning = crosshairteamsign && targetplayer && isteam (targetplayer->team, p->team)
    ...
    bool is_spect = ((player1->spectatemode == SM_FOLLOW1ST || player1->spectatemode == SM_FOLLOW3RD
    ...
    if (command)
        ...
        draw_text (colorname (targetplayer), HUDPOS_X_BOTTOMLEFT, HUDPOS_Y_BOTTOMLEFT);
    glLoadIdentity ();
    glOrtho (0, VIRTW * 2, VIRTH * 2, 0, -1, 1);
    extern void r_accuracy (int h);
    extern void *scoremenu;
    extern gmenu *curmenu;
    if (!is_spect && !editmode && !watchingdemo && !command && curmenu == scoremenu)
        r_accuracy (commandh);
    ...
    if (wallclock)
        filtertext (ltime, timestring (*ltimeformat != 'U', ltimeformat + int(*ltimeformat == 'U')), FTXT_TOLOWER);
    ...
    if (showstats)
    {
        if (showstats == 2 && !dbgpos)
        {
            ...
            formatstring (text) ("\f%c%d", rangecolor (curfps, "xwvu", 30, 100, 150), curfps);
            ...
            formatstring (text) ("\f%c%d", rangecolor (nquads, "uvwx", 3999, 5999, 7999), nquads);
            draw_text (text, ttll - text_width (text), top + 160);
            formatstring (text) ("\f%c%d", rangecolor (curvert, "uvwx", 3999, 5999, 7999), curvert);
            draw_text (text, ttll - text_width (text), top + 240);
            formatstring (text) ("\f%c%d", rangecolor (xtraverts, "uvwx", 3999, 5999, 7999), xtraverts);
            draw_text (text, ttll - text_width (text), top + 320);

            if (wallclock) draw_text (ltime, ttll - text_width (ltime), top - 90);
            if (unsavededits)
                draw_text ("U", ttll - text_width ("U"), top - 90 - (wallclock ? 2 * FONTH / 2 : 0));
        }
        else
        {
            if (dbgpos)
            {
                ...
                draw_text (text, VIRTW * 2 - (text_width (text) + FONTH), VIRTH * 2 - 17 * FONTH / 2);
                formatstring (text) ("%05.2f PIT", p->pitch);
                draw_text (text, VIRTW * 2 - (text_width (text) + FONTH), VIRTH * 2 - 15 * FONTH / 2);
                formatstring (text) ("%05.2f X  ", p->o.x);
                draw_text (text, VIRTW * 2 - (text_width (text) + FONTH), VIRTH * 2 - 13 * FONTH / 2);
                formatstring (text) ("%05.2f Y  ", p->o.y);
                draw_text (text, VIRTW * 2 - (text_width (text) + FONTH), VIRTH * 2 - 11 * FONTH / 2);
                formatstring (text) ("%05.2f Z  ", p->o.z);
                draw_text (text, VIRTW * 2 - (text_width (text) + FONTH), VIRTH * 2 - 9 * FONTH / 2);
                popfont ();
            }
            ...
            if (wallclock)
                draw_text (ltime, VIRTW * 2 - text_width (ltime) - FONTH, VIRTH * 2 - 5 * FONTH / 2);
            if (unsavededits)
        ...
        draw_text (ltime, VIRTW * 2 - text_width (ltime) - FONTH, VIRTH * 2 - 3 * FONTH / 2);
    ...
    if (hidevote < 2 && multiplayer (nullptr))
    {
        ...
        if (curvote && curvote->millis >= totalmillis
            && !(hidevote == 1 && curvote->localplayervoted && curvote->result == VOTE_NEUTRAL))
        {
    ...
    if (!hidespecthud && !menu && p->state == CS_DEAD && p->spectatemode <= SM_DEATHCAM)
    {
        ...
        if (multiplayer (nullptr) || watchingdemo)
            draw_textf ("SCROLL to change player",
                        left - (text_width ("SCROLL to change player") + FONTH / 2), top + 80);
    }
    ...
    if (p->state == CS_ALIVE || (p->state == CS_DEAD && p->spectatemode == SM_DEATHCAM))
    {
        ...
        if (p->state == CS_ALIVE && !hidehudequipment)
        {
            ...
            if (p->weaponsel && valid_weapon (p->weaponsel->type))
            {
                ...
                else if (p->prevweaponsel->type == GUN_AKIMBO || p->prevweaponsel->type == GUN_PISTOL)
            ...
            if (!offs || scores[2] || scores[3]) loopi (2) // flag state or frag-counter
                {
                    ...
                    drawctficon (i * 120 + VIRTW / 4.0f * 3.0f, 1650, 120,
                                 i + (inbase || flagscorehudtransparency == 2 ? 0 : 2), offs / 2,
                                 1 / 4.0f, inbase || !flagscorehudtransparency ? 255 : 100);
                    if (m_teammode)
                    {
