...
void resetpckservers ()
{
    ...
    conoutf ("\f3Restart AssaultCube to take effect after resetting the list of "
...
void getpckserver () // return a table of all package servers with four columns
{
    ...
    {
        loopv (pckservers) cvecprintf (res, "\"%s\" %d %d %d\n", pckservers[i]->host, pckservers[i]->priority,
                                       pckservers[i]->ping, pckservers[i]->resolved);
        sem_pckservers.post ();
    }
                ...
                else
                {
                    ...
                    if (DEBUGCOND)
                    {
                        ...
                        cvecprintf (pckping_log, "lheader: %s",
                                    escapestring (h.header ? h.header : h.rawrec.getbuf (), true, true));
                        pckping_log.add ('\n');
                    }
#endif
                    SDL_mutexV (pckpinglog_lock);
...
int pingallpckservers (void *data)
{
    ...
    if (autodownload)
    {
        ...
        loopv (pckservers) if (pckservers[i]->priority > -1000)
        pckthreads.add (SDL_CreateThread (pingpckserver,
                                          pckservers[i])); // start pinging all servers at once
        loopv (pckthreads) if (pckthreads[i])
        SDL_WaitThread (pckthreads[i], &nop); // wait for all ping threads to finish
        ...
        cvecprintf (pckping_log, "c\f4(automatic media file download deactivated)\n");
...
bool requirepackage (int type, const char *name, const char *host)
{
    ...
    formatstring (pck->requestname) ("/%s%s%s", pck->fullpath, type == PCK_MAP ? ".cgz" : "",
    ...
    DEBUG ("name: " << pck->name << ", fullpath: " << pck->fullpath << ", requestname: " << pck->requestname
                    << ", host: " << pck->host << ", type: " << pck->type
                    << ", iszip: " << pck->iszip << ", strictnames: " << pck->strictnames
                    << ", server: " << pck->server << ", exts: " << (pck->exts ? *pck->exts : ""));
    pendingpackages.add (pck);
    return true;
}

VAR (rereadtexturelists,
     0,
     1,
    ...
    { // unpack zip file
        ...
        if (mz)
        {
            ...
            loopv (files)
            {
                ...
                {
                    if (pck->strictnames)
                    { // only base names and listed extensions
                      // allowed
                    ...
                    { // any name with listed extension, written to subdirectory
                        ...
                        if ((isok = (zlen > elen && !strcmp (zname + zlen - elen, pck->exts[n]))))
                ...
                if (isok && !strcmp (cleanzname, zname))
                {
                    ...
                    if (zf)
                    {
                        ...
                        DEBUGCODE (clientlogf ("extracted %d bytes from %s %s, written to %s", got,
                                               pck->name, files[i], filename));
                        delete zf;
                    }
                }
                else
                    conoutf ("\f3illegal filename \"%s\" in %s", files[i], pck->requestname);
            }
            zipmanualclose (mz);
        }
        else
                ...
                pckserver *s) // download one package from one server
{
    ...
    if (*u.domain && h.set_host (u.domain))
    {
        ...
        else
        {
            if (h.response == 200)
            {
                DEBUGCODE (clientlogf ("downloaded %s:%d%s, %d bytes (%d raw), %d msec", u.domain,
                                       h.ip.port, url, got, h.contentlength, h.elapsedtime));
                processdownload (pck, h.outstream);
                h.outstream = nullptr; // deleted by processdownload()
                return true;
            }
            else
                clientlogf ("download %s:%d%s failed, server response %d%s%s", u.domain, h.ip.port,
                            url, h.response, h.err ? ", err: " : "", h.err ? h.err : "");
        }
        DELETEP (h.outstream);
    }
    else
...
int pckserversort (pckserver **a, pckserver **b)
{
    if ((*a)->resolved != (*b)->resolved)
        return (*b)->resolved - (*a)->resolved; // group resolvable hosts at the top
...
int downloadpackages (bool loadscr) // get all pending packages
{
    ...
    if (sem_pckservers.timedwait (1000))
    { // can't lock the server list -> this means, the server ping
    ...
    loopvrev (pendingpackages)
    {
        ...
        if (!*pck->host && pckservers.inrange (pck->server) && pckservers[pck->server]->resolved)
        {
            if (dlpackage (h, pck, pckservers[pck->server]))
                ...
                pck->server = -1; // auto-assigned server didn't work - try any other now
        }
    }
        ...
        loopv (pckservers)
        {
            ...
            if (s->priority > -10000)
                f->printf ("addpckserver %s %d // ping: %d, resolved: %d, updates: %d\n", s->host,
                           s->priority, s->ping, s->resolved, s->updates.numelems);
        }
        delete f;
    }
}
