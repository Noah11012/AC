...
void resetpckservers() {
  ...
  conoutf(
      "\f3Restart AssaultCube to take effect after resetting the list of "
      "packages source servers");
}
...
void getpckserver()  // return a table of all package servers with four columns
{
  ...
  if (sem_pckservers.timedwait(1000))
    cvecprintf(
        res,
        "\"serverlist is busy, try again later\" ");  // single-word return
                                                      // value signals an error
  else {
    loopv(pckservers) cvecprintf(res, "\"%s\" %d %d %d\n", pckservers[i]->host,
                                 pckservers[i]->priority, pckservers[i]->ping,
                                 pckservers[i]->resolved);
    sem_pckservers.post();
  }
...
vector<char>
    pckping_log;  // access controlled by sem_pckservers (and pckpinglog_lock)
const char *updatestxt = "/updates.txt";

int pingpckserver(
    void *data)  // fetch updates.txt from a media server, measure required time
{
  httpget h;
  urlparse u;
  ...
  cvecprintf(pckping_log, "lresolving hostname %s %s (%s)\n", u.domain,
             s->resolved ? "succeeded" : "failed", iptoa(ntohl(h.ip.host)));
  SDL_mutexV(pckpinglog_lock);
  if (s->resolved) {
    if (*u.port) h.set_port(atoi(u.port));
    defformatstring(url)("%s%s", u.path, updatestxt);
    int res = h.get(url, 5000, 10000, 0,
                    true);  // http HEAD request only - to get a nice ping value
    if (res >= 0 && h.response > 0) {
      s->ping = h.elapsedtime;
      SDL_mutexP(pckpinglog_lock);
      cvecprintf(pckping_log,
                 "lhttp head %s:%d%s response time: %d, status: %d, "
                 "contentlength: %d\n",
                 u.domain, h.ip.port, url, s->ping, h.response,
                 h.contentlength);
#ifdef _DEBUG
      if (DEBUGCOND) {
        cvecprintf(pckping_log, "l%s", escapestring(h.header, true, true));
        pckping_log.add('\n');
      }
#endif
      SDL_mutexV(pckpinglog_lock);
      if (h.response == 200) {
        ...
        if (res >= 0 && h.response == 200) {  // parse updates.txt
          h.outvec->add('\0');
          s->updates.clear();
          parseupdatelist(s->updates,
                          (char *)h.outvec->getbuf());  // don't filter entries
          SDL_mutexP(pckpinglog_lock);
          cvecprintf(pckping_log,
                     "lgot %s:%d%s: %d bytes, %d valid update lines\n",
                     u.domain, h.ip.port, url, res, s->updates.numelems);
          SDL_mutexV(pckpinglog_lock);
        } else {
          SDL_mutexP(pckpinglog_lock);
          cvecprintf(
              pckping_log, "lretrieving %s:%d%s failed: response %d, err: %s\n",
              u.domain, h.ip.port, url, h.response, h.err ? h.err : "(null)");
#ifdef _DEBUG
          if (DEBUGCOND) {
            if (!h.header) {
              h.rawrec.setsize(min(h.rawrec.length(), MAXSTRLEN));
              h.rawrec.add('\0');
            }
            cvecprintf(pckping_log, "lheader: %s",
                       escapestring(h.header ? h.header : h.rawrec.getbuf(),
                                    true, true));
            pckping_log.add('\n');
          }
#endif
...
int pingallpckservers(void *data) {
  ...
  if (autodownload) {
    ...
    loopv(pckservers) if (pckservers[i]->priority > -1000) pckthreads.add(
        SDL_CreateThread(pingpckserver,
                         pckservers[i]));  // start pinging all servers at once
    loopv(pckthreads) if (pckthreads[i]) SDL_WaitThread(
        pckthreads[i], &nop);  // wait for all ping threads to finish
    SDL_DestroyMutex(pckpinglog_lock);
    loopv(pckservers) {
      pckserver *s = pckservers[i];
      if (s->priority <= -1000)
        disabled++;
      else if (s->resolved && s->ping)
        good++;
    }
    int bad = pckservers.length() - good - disabled;
    cvecprintf(
        pckping_log,
        "csuccessfully pinged %d media server%s, %d failure%s, %d disabled\n",
        good, good == 1 ? "" : "s", bad, bad == 1 ? "" : "s", disabled);
  } else
    cvecprintf(pckping_log,
               "c\f4(automatic media file download deactivated)\n");
...
void pollautodownloadresponse()  // thread-safe feedback from the
                                 // autodownload-updater thread
{
  ...
  if (!sem_pckservers.trywait()) {
    if (pckping_log.length()) {
      ...
      do {  // break into single lines
        if ((p = strchr(l, '\n'))) *p = '\0';
        if (*l == 'c')
          conoutf("%s", l + 1);  // line starts with "c" -> console
        else if (*l == 'e')
          donepolling = 1;  // line starts with "e" -> end polling
        else
          clientlogf("%s", l + 1);  // line starts with "l" -> log
        l = p + 1;
      } while (p);
...
const char *mmexts[] = {".md2", ".md3", ".cfg", ".txt", ".jpg", ".png", ""},
           *mapexts[] = {".cgz", ".cfg", ""},
           *sbexts[] = {"_lf.jpg", "_rt.jpg", "_ft.jpg",      "_bk.jpg",
...
bool requirepackage(int type, const char *name, const char *host) {
  ...
  switch (type) {
    ...
    default:
      conoutf("\f3requirepackage: illegal package type %d (\"%s\")", type,
              name);  // should probably be fatal()
      delete pck;
      return false;
  }
  string upath;
  ...
  unixpath(upath);
  filtertext(pck->name, name,
             FTXT__MEDIAFILEPATH);  // also changes backslashes to slashes
  if (strlen(upath) > 100 || strcmp(upath, pck->name) ||
      (checkprefix && strncmp(pck->name, checkprefix, strlen(checkprefix))) ||
      (type == PCK_MOD && !validzipmodname(upath))) {
    conoutf("\f3requirepackage: illegal media path \"%s\" (type: %d)", name,
            type);
    delete pck;
    return false;
  }
  ...
  formatstring(pck->requestname)(
      "/%s%s%s", pck->fullpath, type == PCK_MAP ? ".cgz" : "",
      pck->iszip ? ".zip" : "");  // special case map zips: file ending .cgz.zip

  loopv(pendingpackages) if (!strcmp(
                                 pendingpackages[i]->requestname,
                                 pck->requestname)) {  // request already queued
    delete pck;
    return false;
  }
  pck->type = type;
  if (host) copystring(pck->host, host);  // assign fixed server
  DEBUG("name: " << pck->name << ", fullpath: " << pck->fullpath
                 << ", requestname: " << pck->requestname
                 << ", host: " << pck->host << ", type: " << pck->type
                 << ", iszip: " << pck->iszip << ", strictnames: "
                 << pck->strictnames << ", server: " << pck->server
                 << ", exts: " << (pck->exts ? *pck->exts : ""));
  pendingpackages.add(pck);
  return true;
}

VAR(rereadtexturelists, 0, 1,
    1);  // flag to indicate additional texture files to trigger a menu rebuild
         // (defaults to "1" to build the menus after startup)
VAR(rereadsoundlists, 0, 1, 1);  // same for map sounds

void processdownload(package *pck,
                     stream *f)  // write downloaded data to file(s)
{
  f->seek(0);
  if (pck->iszip) {  // unpack zip file
    ...
    if (mz) {
      ...
      loopv(files) {
        ...
        const char *zname =
            behindpath(files[i]);  // ignore _all_ paths inside the zip
        filtertext(cleanzname, zname, FTXT__MEDIAFILENAME);
        bool isok = false;
        for (int n = 0; *pck->exts[n]; n++) {
          if (pck->strictnames) {  // only base names and listed extensions
                                   // allowed
        ...
        if (isok && !strcmp(cleanzname, zname)) {
          ...
          if (zf) {
            ...
            zipmanualread(mz, i, zf,
                          MAXMEDIADOWNLOADFILESIZE);  // extract file from zip
                                                      // and write to @filename
            DEBUGCODE(clientlogf("extracted %d bytes from %s %s, written to %s",
                                 got, pck->name, files[i], filename));
            delete zf;
          }
        } else
          conoutf("\f3illegal filename \"%s\" in %s", files[i],
                  pck->requestname);
      }
      zipmanualclose(mz);
    } else
      clientlogf("failed to open zip file %s", pck->requestname);
  } else {  // write single file
    stream *d = openfile(path(pck->fullpath, true), "wb");
    if (d) {
      streamcopy(
          d, f,
          pck->type == PCK_MOD ? MAXMODDOWNLOADSIZE : MAXMEDIADOWNLOADFILESIZE);
      delete d;
    }
    delete f;
  }
...
int progress_callback_dlpackage(void *data, float progress) {
  if (progress_of < 0) {
    defformatstring(txt)("downloading %s... (esc to abort)",
                         (const char *)data);
    if (progress < 0)
    ...
    loadingscreen(
        "downloading package %d of %d...\n%s  \fs%s%d%%\fr\n(ESC to cancel)",
        progress_n, progress_of, (const char *)data,
        progress < 0 ? "\f3waiting " : "", int(100.0 * fabs(progress)));
...
bool dlpackage(httpget &h, package *pck,
               pckserver *s)  // download one package from one server
{
  urlparse u;
  ...
  if (*u.domain && h.set_host(u.domain)) {
    ...
    if (got < 0 || !h.response) {
      if (!canceldownloads && s)
        s->ping = 0;  // received a hard error from the server connection,
                      // better not try this one again
      clientlogf("download %s:%d%s failed, err: %s", u.domain, h.ip.port, url,
                 h.err ? h.err : "(null)");
      h.disconnect();
    } else {
      if (h.response == 200) {
        DEBUGCODE(clientlogf("downloaded %s:%d%s, %d bytes (%d raw), %d msec",
        ...
        clientlogf("download %s:%d%s failed, server response %d%s%s", u.domain,
                   h.ip.port, url, h.response, h.err ? ", err: " : "",
                   h.err ? h.err : "");
    }
    DELETEP(h.outstream);
  } else {
...
int packagesort_server(package **a, package **b) {
  return (*b)->server - (*a)->server;
}
int packagesort_name(package **a, package **b) {
  return strcmp((*b)->name, (*a)->name);
}
int packagesort_host(package **a, package **b) {
...
int pckserversort(pckserver **a, pckserver **b) {
  if ((*a)->resolved != (*b)->resolved)
    return (*b)->resolved -
           (*a)->resolved;  // group resolvable hosts at the top
  if ((*a)->priority != (*b)->priority)
    return (*b)->priority - (*a)->priority;  // manual priority wins over ping
                                             // (high priority to the top)
  if ((*a)->ping && (*b)->ping)
    return (*a)->ping - (*b)->ping;  // smaller ping wins
...
int downloadpackages(bool loadscr)  // get all pending packages
{
  ...
  if (sem_pckservers.timedwait(
          1000)) {  // can't lock the server list -> this means, the server ping
                    // threads have not finished yet
    if (pendingpackages.length())
      conoutf("\f3failed to download packages: still pinging the servers...");
    return 0;
  }
  ...
  loopv(pendingpackages) {
    ...
    loopvj(pckservers) {
      ...
      if (s->priority > -1000 && s->resolved && s->ping &&
          (rev = s->updates.access(pck->requestname)) && *rev > maxrev) {
        maxrev = *rev;
        pck->server = j;
      }
    }
  ...
  loopvrev(pendingpackages) {
    ...
    package *pck = pendingpackages[i];
    if (!*pck->host && pckservers.inrange(pck->server) &&
        pckservers[pck->server]->resolved) {
      if (dlpackage(h, pck, pckservers[pck->server]))
        delete pendingpackages.remove(i);
      else
        pck->server =
            -1;  // auto-assigned server didn't work - try any other now
    }
  }
  ...
  // get all packages with manually assigned hosts - those will be deleted from
  // the list regardless of success!
...
void writepcksourcecfg() {
  if (pckservers.length()) {
    ...
    f->printf("autodownloaddebug %d\n",
              autodownloaddebug);  // saved.cfg is loaded too late for this to
                                   // be in it and be efficient...
    loopv(pckservers) {
      pckserver *s = pckservers[i];
      if (s->priority > -10000)
        f->printf("addpckserver %s %d // ping: %d, resolved: %d, updates: %d\n",
                  s->host, s->priority, s->ping, s->resolved,
                  s->updates.numelems);
    }
    delete f;
  }
...
void getmod(char *name) {
  if (requirepackage(PCK_MOD, name) && downloadpackages(false))
    conoutf("mod package %s successfully downloaded", name);
  else
    conoutf("failed to download mod package %s", name);
}
COMMAND(getmod, "s");
