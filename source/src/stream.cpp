...
char *makerelpath(const char *dir, const char *file, const char *prefix,
  ...
  if (file[0] == '<') {
    ...
    if (end) {
      ...
      copystring(&tmp[len], file,
                 min(sizeof(tmp) - len, size_t(end + 2 - file)));
      file = end + 1;
    }
  }
...
char *path(char *s) {
  ...
  for (char *t = c; (t = strpbrk(t, "/\\"));) {
    *t++ = PATHDIV;
    size_t d = strspn(t, "/\\");
    if (d)
      memmove(t, t + d, strlen(t + d) + 1);  // remove multiple path delimiters
  }
  // collapse ".."-parts
  for (char *prevdir = nullptr, *curdir = s;;) {
    prevdir = curdir[0] == PATHDIV ? curdir + 1 : curdir;
    curdir = strchr(prevdir, PATHDIV);
    if (!curdir) break;
    if (prevdir + 1 == curdir && prevdir[0] == '.') {  // simply remove "./"
      memmove(prevdir, curdir + 1, strlen(curdir));
      curdir = prevdir;
    } else if (curdir[1] == '.' && curdir[2] == '.' &&
               curdir[3] == PATHDIV) {  // collapse "/foo/../" to "/"
      if (prevdir + 2 == curdir && prevdir[0] == '.' && prevdir[1] == '.')
        continue;  // foo is also ".." -> skip
...
#else
  if (access(path, R_OK | (mode[0] == 'w' || mode[0] == 'a' ? W_OK : 0)) == -1)
    exists = false;
#endif
  return exists;
}
...
char *getregszvalue(HKEY root, const char *keystr, const char *query) {
  HKEY key;
  if (RegOpenKeyEx(HKEY_CURRENT_USER, keystr, 0, KEY_READ, &key) ==
      ERROR_SUCCESS) {
    DWORD type = 0, len = 0;
    if (RegQueryValueEx(key, query, 0, &type, 0, &len) == ERROR_SUCCESS &&
        type == REG_SZ) {
...
bool havehomedir() { return homedir[0] != '\0'; }
...
const char *findfile(const char *filename, const char *mode) {
  ...
  while (!strncmp(".." PATHDIVS, filename, 3))
    filename += 3;  // skip leading "../" (don't allow access to files below "AC
                    // root dir")
  static string s;
  ...
  if (mode[0] == 'w' ||
      mode[0] == 'a') {  // create missing directories, if necessary
    string dirs;
    copystring(dirs, s);
...
bool listsubdir(const char *dir, vector<char *> &subdirs) {
  ...
  if (Find != INVALID_HANDLE_VALUE) {
    do {
      if ((FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
        ...
        subdirs.add(newstring(FindFileData.cFileName));
  ...
  if (d) {
    struct dirent *de, b;
    while (!readdir_r(d, &b, &de) && de != nullptr) {
#ifdef _DIRENT_HAVE_D_TYPE
      if (de->d_type == DT_DIR && de->d_name[0] != '.')
        subdirs.add(newstring(de->d_name));
      else if (de->d_type == DT_UNKNOWN && de->d_name[0] != '.')
#endif
      {
        struct stat s;
        auto dl = (int)strlen(pathname);
        concatformatstring(pathname, "/%s", de->d_name);
        if (!lstat(pathname, &s) && S_ISDIR(s.st_mode) && de->d_name[0] != '.')
          subdirs.add(newstring(de->d_name));
        pathname[dl] = '\0';
      }
    }
...
void listsubdirs(const char *dir, vector<char *> &subdirs,
                 int(__cdecl *sf)(const char **, const char **)) {
...
#endif
  subdirs.sort(sf);
  for (int i = subdirs.length() - 1; i > 0; i--) {  // remove doubles
    ...
    do {
      if (!(FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
        files.add(newstring(FindFileData.cFileName,
                            (int)strlen(FindFileData.cFileName) - extsize));
    } while (FindNextFile(Find, &FindFileData));
    FindClose(Find);
    return true;
  }
    ...
    while (!readdir_r(d, &b, &de) && de != nullptr) {
      ...
      if (isreg) {
        if (!ext)
          ...
          if (namelength > 0 && de->d_name[namelength] == '.' &&
              strncmp(de->d_name + namelength + 1, ext, extsize - 1) == 0)
...
void listfilesrecursive(const char *dir, vector<char *> &files, int level) {
  ...
  loopv(dirs) {
    if (dirs[i][0] !=
        '.')  // ignore "." and ".." (and also other directories starting with
              // '.', like it is unix-convention - and doesn't hurt on windows)
    {
...
static int rwopsseek(SDL_RWops *rw, int offset, int whence) {
  auto *f = (stream *)rw->hidden.unknown.data1;
  if ((!offset && whence == SEEK_CUR) || f->seek(offset, whence))
    return f->tell();
  return -1;
}
...
static int rwopsclose(SDL_RWops *rw) { return 0; }
...
struct filestream : stream {
  ...
  bool seek(long offset, int whence) override {
    return fseek(file, offset, whence) >= 0;
  }
  void fflush() override {
    if (file) ::fflush(file);
  }
  int read(void *buf, int len) override { return (int)fread(buf, 1, len, file); }
  int write(const void *buf, int len) override { return (int)fwrite(buf, 1, len, file); }
  int getchar() override { return fgetc(file); }
  bool putchar(int c) override { return fputc(c, file) != EOF; }
  bool getline(char *str, int len) override { return fgets(str, len, file) != nullptr; }
...
struct gzstream : stream {
  ...
  z_stream zfile;
  uchar *buf{NULL};
  bool reading{false}, writing{false}, autoclose{false};
  uint crc{0};
  int headersize{0};
  ...
  bool checkheader() {
    readbuf(10);
    if (readbyte() != MAGIC1 || readbyte() != MAGIC2 ||
        readbyte() != Z_DEFLATED)
      return false;
    int flags = readbyte();
  ...
  bool open(stream *f, const char *mode, bool needclose, int level) {
    ...
    if (reading) {
      if (inflateInit2(&zfile, -MAX_WBITS) != Z_OK) reading = false;
               ...
               deflateInit2(&zfile, level, Z_DEFLATED, -MAX_WBITS,
                            min(MAX_MEM_LEVEL, 8), Z_DEFAULT_STRATEGY) != Z_OK)
      writing = false;
  ...
  void finishreading() {
    ...
    if (dbggz) {
      ...
      if (checksize != zfile.total_out)
        conoutf("gzip size check failed: read %u, calculated %u", checksize,
                (uint)zfile.total_out);
    }
#endif
  }
  ...
  void finishwriting() {
    ...
    uchar trailer[8] = {uchar(crc & 0xFF),
                        uchar((crc >> 8) & 0xFF),
                        uchar((crc >> 16) & 0xFF),
                        uchar((crc >> 24) & 0xFF),
                        uchar(zfile.total_in & 0xFF),
                        uchar((zfile.total_in >> 8) & 0xFF),
                        uchar((zfile.total_in >> 16) & 0xFF),
                        uchar((zfile.total_in >> 24) & 0xFF)};
  ...
  long tell() override {
    return reading ? zfile.total_out : (writing ? zfile.total_in : -1);
  }
  ...
  bool seek(long offset, int whence) override {
    ...
    if (whence == SEEK_END) {
      ...
      offset += zfile.total_out;

    if (offset >= (int)zfile.total_out)
      offset -= zfile.total_out;
    else if (offset < 0 || !file->seek(headersize, SEEK_SET))
      return false;
    else {
  ...
  bool flush() {
    if (zfile.next_out && zfile.avail_out < BUFSIZE) {
      if (file->write(buf, BUFSIZE - zfile.avail_out) !=
          int(BUFSIZE - zfile.avail_out))
        return false;
    }
    zfile.next_out = buf;
...
struct vecstream : stream {
  ...
  vecstream(vector<uchar> *s, bool autodelete)
      : data(s), pointer(0), autodelete(autodelete) {}
  ~vecstream() override {
    if (autodelete) {
      DELETEP(data);
    }
  ...
  void close() override { DELETEP(data); }
  bool end() override { return data ? pointer >= data->length() : true; }
  long tell() override { return data ? pointer : -1; }
  long size() override { return data ? data->length() : -1; }
  ...
  int printf(const char *fmt, ...) override  // limited to MAXSTRLEN
  {
    int len = 0;
    if (data) {
...
struct memstream : stream {
  ...
  memstream(const uchar *s, int size, int *refcnt)
      : data(s), memsize(size), pointer(0), refcount(refcnt) {
    if (refcnt) (*refcnt)++;
  }
  ~memstream() override { close(); }
  ...
  bool end() override { return data ? pointer >= memsize : true; }
  long tell() override { return data ? pointer : -1; }
  long size() override { return data ? memsize : -1; }
...
#endif
  if (!strncmp(filename, "zip://", 6)) return nullptr;
...
int streamcopy(stream *dest, stream *source, int maxlen) {
  int got = 0, len;
  uchar copybuf[1024];
  while (got < maxlen && (len = source->read(copybuf, 1024)))
    got += dest->write(copybuf, len);
  return got;
}

#ifndef STANDALONE
void filerotate(const char *basename, const char *ext, int keepold,
                const char *oldformat)  // rotate old logfiles
{
