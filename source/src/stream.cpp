...
void listfilesrecursive (const char *dir, vector<char *> &files, int level)
{
    ...
    loopv (dirs)
    {
        if (dirs[i][0] != '.') // ignore "." and ".." (and also other directories starting with
...
bool delfile (const char *path) { return strncmp (path, "zip://", 6) ? !remove (path) : false; }
...
struct filestream : stream
{
    ...
    bool seek (long offset, int whence) override { return fseek (file, offset, whence) >= 0; }
...
struct gzstream : stream
{
    ...
    long tell () override { return reading ? zfile.total_out : (writing ? zfile.total_in : -1); }
...
#ifndef STANDALONE
void filerotate (const char *basename,
                 const char *ext,
                 int keepold,
