enum // static entity types
{
    ...
    LIGHT,       // lightsource, attr1 = radius, attr2 = intensity (or attr2..4 =
                 // r-g-b)
    PLAYERSTART, // attr1 = angle, attr2 = team
    I_CLIPS,
    I_AMMO,
    I_GRENADE, // attr1 = elevation
    I_HEALTH,
    I_HELMET,
    I_ARMOUR,
    I_AKIMBO,
    MAPMODEL, // attr1 = angle, attr2 = idx, attr3 = elevation, attr4 = texture,
              // attr5 = pitch, attr6 = roll
    ...
    CLIP,     // attr1 = elevation, attr2 = xradius, attr3 = yradius, attr4 = height,
              // attr6 = slope, attr7 = shape
    PLCLIP,   // attr1 = elevation, attr2 = xradius, attr3 = yradius, attr4 =
              // height, attr6 = slope, attr7 = shape
    DUMMYENT, // temporary entity without any function - will not be saved to map
              // files, used to mark positions and for scripting
    MAXENTTYPES
};
...
struct persistent_entity // map entity
{
    ...
    char attr6;
    unsigned char attr7;
    persistent_entity (short x, short y, short z, uchar type, short attr1, uchar attr2, uchar attr3, uchar attr4)
    : x (x),
      y (y),
      z (z),
      attr1 (attr1),
...
struct entity : persistent_entity
{
    ...
    int lastmillis;
    entity (short x, short y, short z, uchar type, short attr1, uchar attr2, uchar attr3, uchar attr4)
    : persistent_entity (x, y, z, type, attr1, attr2, attr3, attr4), spawned (false)
    {
    }
    entity () {}
    bool fitsmode (int gamemode)
    {
        return !m_noitems && isitem (type) && !(m_noitemsnade && type != I_GRENADE)
               && !(m_pistol && type == I_AMMO);
    }
    void transformtype (int gamemode)
    {
        if (m_noitemsnade && type == I_CLIPS)
            type = I_GRENADE;
        else if (m_pistol && (type == I_AMMO || type == I_GRENADE))
            type = I_CLIPS;
    }
};
...
struct guninfo
{
    char modelname[23], title[42];
    short sound, reload, reloadtime, attackdelay, damage, piercing, projspeed, part, spread, recoil,
    magsize, mdl_kick_rot, mdl_kick_back, recoilincrease, recoilbase, maxrecoil, recoilbackfade, pushfactor;
    bool isauto;
};
...
// note: team_isactive and team_base can/should be used to check the limits for
// arrays of size '2'
static inline const char *team_string (int t, bool abbr = false)
{
    const char **n = abbr ? teamnames_s : teamnames;
    return team_isvalid (t) ? n[t] : n[TEAM_NUM + 1];
}
...
class dynent : public physent // animated ent
{
    ...
    animstate prev[2], current[2]; // md2's need only [0], md3's need both for
                                   // the lower&upper model
...
class playerstate
{
    ...
    itemstat *itemstats (int type)
    {
        ...
        {
        case I_CLIPS:
            return &ammostats[GUN_PISTOL];
        case I_AMMO:
            return &ammostats[primary];
        case I_GRENADE:
            return &ammostats[GUN_GRENADE];
        case I_AKIMBO:
            return &ammostats[GUN_AKIMBO];
        case I_HEALTH:
        case I_HELMET:
        case I_ARMOUR:
    ...
    bool canpickup (int type)
    {
        ...
        {
        case I_CLIPS:
            return ammo[akimbo ? GUN_AKIMBO : GUN_PISTOL]
                   < ammostats[akimbo ? GUN_AKIMBO : GUN_PISTOL].max;
        case I_AMMO:
            return ammo[primary] < ammostats[primary].max;
        case I_GRENADE:
            return mag[GUN_GRENADE] < ammostats[GUN_GRENADE].max;
        case I_HEALTH:
            return health < powerupstats[type - I_HEALTH].max;
        case I_HELMET:
        case I_ARMOUR:
    ...
    void pickup (int type)
    {
        ...
        case I_AMMO:
            ...
            break;
        case I_GRENADE:
            additem (ammostats[GUN_GRENADE], mag[GUN_GRENADE]);
            break;
        case I_HEALTH:
        ...
        case I_HELMET:
        case I_ARMOUR:
            additem (powerupstats[type - I_HEALTH], armour);
            break;
        case I_AKIMBO:
            akimbo = true;
    ...
    int dodamage (int damage, int gun)
    {
        ...
        {
        case 0:
            ad = (int)(16.0f / 25.0f * armour);
            break; // 16
        case 1:
            ad = (int)(17.0f / 25.0f * armour) - 1;
            break; // 33
        case 2:
            ad = (int)(4.0f / 25.0f * armour) + 25;
            break; // 37
        case 3:
            ad = (int)(4.0f / 25.0f * armour) + 25;
            break; // 41
...
class playerent : public dynent, public playerstate
{
    ...
    int eardamagemillis;
    float maxroll, maxrolleffect, movroll,
    effroll; // roll added by movement and damage
    ...
    : curskin (0),
      ...
      lastvoicecom (0),
      lastdeath (0),
      clientrole (CR_DEFAULT),
      team (TEAM_SPECT),
      spectatemode (SM_NONE),
      eardamagemillis (0),
      maxroll (ROLLMOVDEF),
      maxrolleffect (ROLLEFFDEF),
      movroll (0),
      effroll (0),
      prevweaponsel (NULL),
      weaponsel (NULL),
    ...
    bool isspectating ()
    {
        return state == CS_SPECTATE || (state == CS_DEAD && spectatemode > SM_NONE);
    }
...
class botent : public playerent
{
    public:
    // Added by Rick
    CBot *pBot; // Only used if this is a bot, points to the bot class if we are
                // the host, for other clients its NULL
    // End add by Rick
