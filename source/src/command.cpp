...
vector<const char *> executionstack; // keep history of recursive command execution (to write to
...
void push (const char *name, const char *action) { pusha (name, newstring (action)); }
COMMAND (push, "ss");
...
{
    if (levels > 1)
        n = exchangestr (n, lookup (newstring (n), min (levels - 1,
                                                        CSLIMIT_LOOKUPNESTING - 1))); // nested ("$$var"), limit to three levels
    ...
    for (bool cont = true; cont;) // for each ; seperated statement
    {
        ...
        else
        {
            ...
            case ID_COMMAND: // game defined commands
            {
                ...
                {
                    char *r = conc ((const char **)w + 1, numargs - 1, strstr (id->sig, "c") != nullptr);
...
#define ARG(i)                                                             \
    (id->sig[i - 1] == 'i' ? ((void *)&(ib##i = strtol (w[i], NULL, 0))) : \
                             (id->sig[i - 1] == 'f' ? ((void *)&(fb##i = atof (w[i]))) : (void *)w[i]))
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        ((void(__cdecl *) (void *, void *, void *))id->fun) (ARG (1), ARG (2), ARG (3));
                        break;
                    case 4:
                        ((void(__cdecl *) (void *, void *, void *, void *))id->fun) (ARG (1), ARG (2),
                                                                                     ARG (3), ARG (4));
                        break;
                    case 5:
                        ((void(__cdecl *) (void *, void *, void *, void *,
                                           void *))id->fun) (ARG (1), ARG (2), ARG (3), ARG (4), ARG (5));
                        break;
                    case 6:
                        ((void(__cdecl *) (void *, void *, void *, void *, void *,
                                           void *))id->fun) (ARG (1), ARG (2), ARG (3), ARG (4),
                                                             ARG (5), ARG (6));
                        break;
                    case 7:
                        ((void(__cdecl *) (void *, void *, void *, void *, void *, void *,
                                           void *))id->fun) (ARG (1), ARG (2), ARG (3), ARG (4),
                                                             ARG (5), ARG (6), ARG (7));
                        break;
                    case 8:
                        ((void(__cdecl *) (void *, void *, void *, void *, void *, void *, void *,
                                           void *))id->fun) (ARG (1), ARG (2), ARG (3), ARG (4),
                                                             ARG (5), ARG (6), ARG (7), ARG (8));
                        break;
                    default:
                        fatal ("command %s has too many arguments (signature: %s)", id->name, id->sig);
                        break;
                    }
#undef ARG
                }
            ...
            case ID_FVAR: // game defined variables
                ...
                else
                {
                    ...
                    if (f1 < id->minvalf || f1 > id->maxvalf)
                    {
                        ...
                        conoutf ("valid range for %s is %s..%s", id->name, floatstr (id->minvalf),
                                 floatstr (id->maxvalf));
            ...
            case ID_SVAR: // game defined variables
                ...
                {
                    if (id->getfun) ((void(__cdecl *) ())id->getfun) ();
                    conoutf (strchr (*id->storage.s, '"') ? "%s = [%s]" : "%s = \"%s\"", c,
                             *id->storage.s); // var with no value just prints its current value
                }
                else
                {
...
{                   // always use one of HOOK_SP_MP, HOOK_SP or HOOK_MP and then OR them (as
                    ...
                    // HOOK_ARENA
    if (multiplayer (nullptr) && (context & HOOK_FLAGMASK) != HOOK_MP && (context & HOOK_FLAGMASK) != HOOK_SP_MP)
        return false; // hook is singleplayer-only
    if (((context & HOOK_TEAM) && !m_teammode) || ((context & HOOK_NOTEAM) && m_teammode)
        || ((context & HOOK_BOTMODE) && !m_botmode) || ((context & HOOK_FLAGMODE) && m_flags)
...
{
    enumerate (*idents, ident, id, if (id.type == ID_COMMAND || id.type == ID_VAR) {
        *d ^= id.type == ID_VAR ? (size_t) "" - (size_t)id.name : (size_t)identhash - (size_t)id.fun;
        *d *= 16777619;
    });
}
...
bool nickcomplete (char *s, bool reversedirection)
{
    ...
    loopv (players) if (players[i] && !strncasecmp (players[i]->name, cp, nickcompletesize))
    ...
    if (totalmatches)
    {
        ...
        const char *fillin = isig ? matchingigs[nickcompleteidx - matchingnames.length ()] :
...
struct completeval
{
    ...
    : type (type), dir (dir && dir[0] ? newstring (dir) : nullptr), ext (ext && ext[0] ? newstring (ext) : nullptr)
    {
    }
    ~completeval ()
    {
...
{
    return x.type == y.type && (x.dir == y.dir || (x.dir && y.dir && !strcmp (x.dir, y.dir)))
...
void addnickcomplete (char *command) { addcomplete (command, COMPLETE_NICK, nullptr, nullptr); }
...
{ // s is required to be of size "string"!
    ...
    char *cmd = strrchr (s, ';'); // find last ';' (this will not always work properly,
                                  // because it doesn't take quoted texts into account)
    if (!cmd) cmd = s;            // no ';' found: command starts after '/'

    char *openblock = strrchr (cmd + 1, '('),
         *closeblock = strrchr (cmd + 1, ')'); // find last open and closed parenthesis
    ...
    { // commandname completion
        ...
        enumerate (*idents, ident, id,
                   if (!strncasecmp (id.name, cp, completesize) && (id.type != ID_ALIAS || *id.action))
        ...
        if (matchingidents.length ())
        {
            ...
            if (firstrun && !reversedirection && !strcmp (matchingidents[completeidx], cp))
...
void explodelist (const char *s, vector<char *> &elems)
{
    ...
    while (*s)
    {
        ...
        elementskip;
#ifdef STANDALONE
        char *newelem = *elem == '"' ? newstring (elem + 1, s - elem - (s[-1] == '"' ? 2 : 1)) :
                                       newstring (elem, s - elem);
#else
        size_t len = 0;
        char *newelem = *elem == '"' ? newstring (elem + 1, (len = s - elem - (s[-1] == '"' ? 2 : 1))) :
                                       newstring (elem, s - elem);
...
{
    DEBUGCODE (if (pos < 0) conoutf ("\f3warning: negative index for 'at' \"%s\" %d", s,
...
void modifyvar (const char *name, int arg, char op)
{
    ...
    if ((id->type == ID_VAR && id->minval > id->maxval) || (id->type == ID_FVAR && id->minvalf > id->maxvalf))
    {
        conoutf ("variable %s is read-only", id->name);
        return;
    }
...
void enumalias (char *prefix)
{
    ...
    enumerate (*idents, ident, id,
               if (id.type == ID_ALIAS && id.persist && !id.isconst && !id.istemp
                   && !strncmp (id.name, prefix, np)) sids.add (&id););
    sids.sort (sortident);
...
void writecfg ()
{
    ...
    loopi (NUMGUNS)
    f->printf ("  %-7s %s\n", gunnames[i],
               crosshairs[i] && crosshairs[i] != notexture ? behindpath (crosshairs[i]->name) : "\"\"");
    ...
    loopv (sids)
    {
        ...
        f->printf ("%s", !strncmp (rep, id.name, curdef ? 1 : 3) && ++repn < groupvariables && lastdef == curdef ?
                         " ; " :
                         (repn = 0, "\n"));
        rep = id.name;
        lastdef = curdef;
        ...
        {
            if (id.type == ID_VAR)
                f->printf ("  // min: %d, max: %d, def: %d", id.minval, id.maxval, id.defaultval);
            if (id.type == ID_FVAR)
...
void cslimiterr (const char *msg)
{
    conoutf ("\f3[ERROR] cubescript hard limit reached: max %s exceeded (check log "
             "for details)",
             msg);
    scripterr ();
}
...
void listoptions (char *s)
{
    ...
    = { optionnames,   entnames + 1,       entnames + 1,   gunnames,           teamnames,
        teamnames_s,   punctnames,         crosshairnames, menufilesortorders, texturestacktypes,
        cubetypenames, soundprioritynames, soundcategories };
