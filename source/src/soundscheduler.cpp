...
// It keeps a set of reserved sources for each priority level to avoid running
// out of sources by floods of low-priority sounds. Under load it uses priority
// and distance information to reassign its resources.
...
source *sourcescheduler::newsource(int priority, const vec &o) {
  ...
  if (!src) {
    ...
    const float score =
        (priority * soundschedpriorityscore) - (dist * soundscheddistancescore);
    ...
    loopv(sources) {
      source *s = sources[i];
      if (s->priority == SP_HIGHEST)
        continue;  // highest priority sounds can't be replaced
