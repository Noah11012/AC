...
bool initlogging (const char *identity, int facility_, int consolethres, int filethres, int syslogthres, bool logtimestamp)
{
...
#else
        if ((logsock = enet_socket_create (ENET_SOCKET_TYPE_DATAGRAM)) == ENET_SOCKET_NULL
    ...
    defformatstring (msg) ("logging started: console(%s), file(%s", levelname[consolethreshold],
...
bool logline (int level, const char *msg, ...)
{
    ...
    bool logtocon = consolethreshold <= level, logtofile = fp && filethreshold <= level,
...
#else
        {
            defformatstring (text) ("<%d>%s: %s", (16 + facility) * 8 + levels[level], ident,
