...
bool source::position(const vec &pos) { return position(pos.x, pos.y, pos.z); }
...
bool source::position(float x, float y, float z) {
  ...
  //    return !ALERRF("x: %f, y: %f, z: %f", x, y, z);   // when sound bug has
  //    been absent for a while, switch to this version
}
...
bool source::playing() { return (state() == AL_PLAYING); }
...
bool sbuffer::load(bool trydl) {
  ...
  if (!ALERR) {
    ...
    loopi(sizeof(exts) / sizeof(exts[0])) {
      ...
      if (len >= 4 && !strcasecmp(filepath + len - 4, ".ogg")) {
        ...
        if (!ov_open_callbacks(f, &oggfile, nullptr, 0, oggcallbacks)) {
          ...
          do {
            char buffer[BUFSIZE];
            bytes = ov_read(&oggfile, buffer, BUFSIZE, isbigendian(), 2, 1,
                            &bitstream);
            loopi(bytes) buf.add(buffer[i]);
          } while (bytes > 0);

          alBufferData(
              id, info->channels == 2 ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16,
        ...
        switch (wavspec.format)  // map wav header to openal format
        {
          ...
          case AUDIO_S8:
            format =
                wavspec.channels == 2 ? AL_FORMAT_STEREO8 : AL_FORMAT_MONO8;
            break;
          case AUDIO_U16:
          case AUDIO_S16:
            format =
                wavspec.channels == 2 ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16;
            break;
          default:
            SDL_FreeWAV(wavbuf);
            delete f;
    ...
    if (trydl)
      requirepackage(PCK_AUDIO,
                     name);  // only try donwloading after trying all extensions
  }
