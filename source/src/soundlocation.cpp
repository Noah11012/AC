...
location::location(int sound, const worldobjreference &r, int priority)
  ...
  if (!sounds.inrange(sound)) {
    if (lastmillis - warn_about_unregistered_sound >
        30 * 1000)  // delay message to every 30 secs so console is not spammed.
    {
      // occurs when a map contains an ambient sound entity, but sound entity is
      // not found in map cfg file.
      conoutf(
          "\f3ERROR: this map contains at least one unregistered ambient sound "
          "(sound slot# %d)",
          sound);
  ...
  if ((r.type == worldobjreference::WR_ENTITY && cfg->maxuses >= 0 &&
      ...
      cfg->muted ||
      (cfg->audibleradius &&
       dist > cfg->audibleradius))  // check max-use limits and audible radius
  {
    stale = true;
    return;
  }
...
if (l->cfg == &gamesounds[S_MENUENTER] || l->cfg == &gamesounds[S_MENUSELECT] ||
    l->cfg == &gamesounds[S_CALLVOTE] || l->cfg == &gamesounds[S_VOTEPASS] ||
    l->cfg == &gamesounds[S_VOTEFAIL])
  continue;
