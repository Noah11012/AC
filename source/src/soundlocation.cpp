...
: cfg (nullptr), src (nullptr), ref (nullptr), stale (false), playmillis (0)
{
    ...
    if (!sounds.inrange (sound))
    {
        ...
        {
            // occurs when a map contains an ambient sound entity, but sound entity is
            // not found in map cfg file.
            conoutf ("\f3ERROR: this map contains at least one unregistered ambient sound "
                     "(sound slot# %d)",
                     sound);
    ...
    if ((r.type == worldobjreference::WR_ENTITY && cfg->maxuses >= 0 && cfg->uses >= cfg->maxuses)
...
void location::evaluateworldobjref () { src->sourcerelative (ref->nodistance ()); }
...
void location::updatepos ()
{
    ...
    case worldobjreference::WR_ENTITY:
    {
        ...
        if (ref->nodistance ())
        {
            ...
            const auto innerradius = float(eref.ent->attr3); // full gain area / size property
            const auto outerradius = float(eref.ent->attr2); // fading gain area / radius property
        ...
        {
            // use openal distance model to make the sound appear from a certain
            // direction (non-ambient)
            src->position (pos);
            src->gain (vol);
        }
        break;
...
void locvector::updatelocations ()
{
    ...
    loopv (*this)
    {
        ...
        if (l->stale)
            ...
            l->evaluateworldobjref (); // cam changed, evaluate world reference again
    }
}
...
void locvector::deleteworldobjsounds (){ loopv (*this){ location *l = buf[i];
if (!l) continue;
// world-neutral sounds
if (l->cfg == &gamesounds[S_MENUENTER] || l->cfg == &gamesounds[S_MENUSELECT] || l->cfg == &gamesounds[S_CALLVOTE]
    || l->cfg == &gamesounds[S_VOTEPASS] || l->cfg == &gamesounds[S_VOTEFAIL])
    continue;
