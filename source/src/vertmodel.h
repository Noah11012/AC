...
struct vertmodel : model
{
    struct anpos
    {
        ...
        bool operator== (const anpos &a) const
        {
...
#if defined(__GNUC__) && defined(__OPTIMIZE__) && !defined(__clang__) && !defined(__ICL) \
...
#endif
        dyncacheentry *
        gendynverts (animstate &as, anpos &cur, anpos *prev, float ai_t)
        {
            ...
            dyncache.addfirst (d);
            vec *buf = d->verts (), *vert1 = &verts[cur.fr1 * numverts],
                *vert2 = &verts[cur.fr2 * numverts], *pvert1 = NULL, *pvert2 = NULL;
            d->cur = cur;
            d->t = ai_t;
            if (prev)
            {
...
#define ip_v_ai(c) ip (ip_v (pvert, c, prev->t), ip_v (vert, c, cur.t), ai_t)
            if (prev)
                iploop (v = vec (ip_v_ai (x), ip_v_ai (y), ip_v_ai (z))) else iploop (
                v = vec (ip_v (vert, x, cur.t), ip_v (vert, y, cur.t), ip_v (vert, z, cur.t)))
#undef ip
#undef ip_v
#undef ip_v_ai

                if (d->verts () == lastvertexarray) lastvertexarray
                = (void *)-1;
        ...
        void findneighbors ()
        {
            ...
            loopi (numtris)
            {
                ...
                loopj (3)
                {
                    ...
                    if (!edge || int((edge >> shift) & 0xFFFF) != i)
                        t.neighbor[j] = 0xFFFF;
                    else
...
#endif
        shadowcacheentry *
        genshadowvolume (animstate &as, anpos &cur, anpos *prev, float ai_t, vec *buf)
        {
            ...
            d = (owner->numframes > 1 || as.anim & ANIM_DYNALLOC ? dynalloc : statalloc)
                .allocate<shadowcacheentry> (9 * numtris * sizeof (ushort));
            if (!d) return NULL;
            ...
            if (dbgstenc >= (owner->numframes > 1 || as.anim & ANIM_DYNALLOC ? 2 : 1))
                conoutf ("%s: %d tris", owner->filename, int((idx - d->idxs ()) / 3));
...
&& __GNUC__ >= 4 && __GNUC_MINOR__ > 4
...
#endif
        lightcacheentry *
        lightvertexes (animstate &as, anpos &cur, anpos *prev, float ai_t, vec *buf)
        {
            ...
            {
                if (d->lastcalclight != lastcalclight || d->pos != modelpos || d->roll != modelroll
                    || d->yaw != modelyaw || d->pitch != modelpitch || d->cur != cur)
                    continue;
                if (prev)
                {
                    if (d->prev == *prev && d->t == ai_t) return d;
            ...
            d = (owner->numframes > 1 || as.anim & ANIM_DYNALLOC ? dynalloc : statalloc)
                .allocate<lightcacheentry> (numverts * sizeof (lightvert));
            if (!d) return NULL;
        ...
        void render (animstate &as, anpos &cur, anpos *prev, float ai_t)
        {
            ...
            loopi (numdyndraws)
            {
                ...
                if (hasDRE && !builddlist)
                    glDrawRangeElements_ (d.type, d.minvert, d.maxvert, d.count, GL_UNSIGNED_SHORT,
                                          &dynidx[d.start]);
                else
                    glDrawElements (d.type, d.count, GL_UNSIGNED_SHORT, &dynidx[d.start]);
            }
            if (builddlist)
            {
                glEndList ();
    ...
    struct part
    {
        ...
        part ()
        : filename (NULL), anims (NULL), links (NULL), tags (NULL), numtags (0), shadows (NULL), shadowrad (0)
        {
        }
        virtual ~part ()
        {
            ...
            loopi (numframes)
            {
                ...
                if (numverts > 1)
                {
                    for (int j = 1; j < numverts; j++)
                        t->pos.add (m->verts[m->numverts * i + verts[j]]);
                    t->pos.div (numverts);
                }
                t->identity ();
            }
        ...
        void genstrips () { loopv (meshes) meshes[i]->genstrips (); }
...
#define ip_ai_tag(c) ip (ip (tag1p->c, tag2p->c, prev->t), ip (tag1->c, tag2->c, cur.t), ai_t)
            ...
            {
                loopj (3) matrix[j] = ip (tag1->transform[0][j], tag2->transform[0][j],
                                          cur.t); // transform
...
#undef ip
        ...
        bool calcanimstate (int anim, int varseed, float speed, int basetime, dynent *d, animstate &as)
        {
            ...
            else if (anims)
            {
                ...
                if (ais.length ())
                {
                    ...
                    getdefaultanim (as, anim & ANIM_INDEX, varseed);
            }
            else
                getdefaultanim (as, anim & ANIM_INDEX, varseed);
            ...
            if (d && index < 2)
            {
                if (d->lastmodel[index] != this || d->lastanimswitchtime[index] == -1
                    || lastmillis - d->lastrendered > animationinterpolationtime)
                {
                    d->current[index] = as;
                ...
                {
                    if (lastmillis - d->lastanimswitchtime[index] > animationinterpolationtime / 2)
                        d->prev[index] = d->current[index];
                    d->current[index] = as;
        ...
        void render (int anim, int varseed, float speed, int basetime, dynent *d)
        {
            ...
            float ai_t = 0;
            bool doai = !(anim & ANIM_NOINTERP) && d && index < 2
                        && lastmillis - d->lastanimswitchtime[index] < animationinterpolationtime
                        && d->prev[index].range > 0;
            if (doai)
            {
            ...
            loopi (numtags)
            {
                ...
                // render the linked models - interpolate rotation and position of the
                // 'link-tags'
                glmatrixf linkmat;
        ...
        {
            static const uint filter3x3[9] = { 1, 2, 1, 2, 4, 2, 1, 2, 1 };
...
#define FILTER(c0, c1, c2, c3, c4, c5, c6, c7, c8)                                                \
    {                                                                                             \
        uint c = *src, val = (filter3x3[0] * (c0) + filter3x3[1] * (c1) + filter3x3[2] * (c2)     \
        ...
        *dst++ = val / filter3x3sum;                                                              \
        ...
        next++;                                                                                   \
            ...
            for (uint x = 1; x < size - 1; x++)
                FILTER (c, c, c, src[-1], c, src[1], next[-1], next[0], next[1]);
            ...
            for (uint y = 1; y < size - 1; y++)
            {
                ...
                for (uint x = 1; x < size - 1; x++)
                    FILTER (prev[-1], prev[0], prev[1], src[-1], c, src[1], next[-1], next[0], next[1]);
                FILTER (prev[-1], prev[0], c, src[-1], c, c, next[-1], next[0], c);
            }

            FILTER (c, prev[0], prev[1], c, c, src[1], c, c, c);
            for (uint x = 1; x < size - 1; x++)
                FILTER (prev[-1], prev[0], prev[1], src[-1], c, src[1], c, c, c);
...
#undef FILTER
        ...
        bool loadshadows (const char *filename)
        {
            ...
            if (!f) return false;
            shadowheader hdr;
            if (f->read (&hdr, sizeof (shadowheader)) != sizeof (shadowheader))
            {
                delete f;
                return false;
            }
            ...
            if (hdr.size != (1 << dynshadowsize) || hdr.frames != numframes)
            {
                delete f;
                return false;
            }
            ...
            if (f->read (buf, hdr.size * hdr.size * hdr.frames) != hdr.size * hdr.size * hdr.frames)
            {
                delete f;
                delete[] buf;
                return false;
            ...
            loopi (hdr.frames) createtexture (shadows[i], hdr.size, hdr.size,
                                              &buf[i * hdr.size * hdr.size], 3, true, false, GL_ALPHA);
        ...
        void rendershadow (int anim, int varseed, float speed, int basetime, const vec &o, float yaw)
        {
            ...
            if (dynshadowquad)
            {
                glBegin (GL_QUADS);
                glTexCoord2f (0, 1);
                glVertex3f (x1 * c - y1 * s + o.x, y1 * c + x1 * s + o.y, o.z);
                glTexCoord2f (1, 1);
                glVertex3f (x2 * c - y1 * s + o.x, y1 * c + x2 * s + o.y, o.z);
                glTexCoord2f (1, 0);
                glVertex3f (x2 * c - y2 * s + o.x, y2 * c + x2 * s + o.y, o.z);
                glTexCoord2f (0, 0);
                glVertex3f (x1 * c - y2 * s + o.x, y2 * c + x1 * s + o.y, o.z);
                glEnd ();
                xtraverts += 4;
                return;
            }
            ...
            ::rendershadow (int(floor (o.x - shadowrad)), int(floor (o.y - shadowrad)),
                            int(ceil (o.x + shadowrad)), int(ceil (o.y + shadowrad)), texgenS, texgenT);
        }
        ...
        char *shadowfile ()
        {
            ...
            if (!dir)
                ...
                copystring (s, filename, dir - filename + 2);
        ...
        void calcneighbors () { loopv (meshes) meshes[i]->calcneighbors (); }

        void calcbbs () { loopv (meshes) meshes[i]->calcbbs (); }
    };
    ...
    vertmodel (const char *name) : loaded (false) { loadname = newstring (name); }
    ...
    void cleanup () { loopv (parts) parts[i]->cleanup (); }
    ...
    bool hasshadows () { return parts.length () == 1 && parts[0]->shadows; }
    ...
    void calcneighbors () { loopv (parts) parts[i]->calcneighbors (); }

    void calcbbs () { loopv (parts) parts[i]->calcbbs (); }
...
bool vertmodel::enablealphablend = false, vertmodel::enablealphatest = false,
     vertmodel::enabledepthmask = true, vertmodel::enableoffset = false;
