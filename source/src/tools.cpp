...
servsqr *createservworld (const sqr *s,
                                          ...
                                          // same statistics functions on client and server
{
    ...
    loopirev (_cubicsize)
    {
        ...
        = s->type == SOLID ?
          SOLID :
          ((s->type & TAGTRIGGERMASK) | (s->tag & ~TAGTRIGGERMASK)); // guaranteed to not have tagclips
...
int calcmapdims (mapdim_s &md, const servsqr *s, int _ssize)
{
    ...
    if (md.x1 < MINBORD || md.y1 < MINBORD || md.x2 >= _ssize - MINBORD || md.y2 >= _ssize - MINBORD)
...
int calcmapareastats (mapareastats_s &ms, servsqr *_servworld, int _ssize, const mapdim_s &md)
{
    ...
    int xb = min (md.x1 + xc, _ssize - MAS_GRID * xc - md.x1 - 1) / 2,
        yb = min (md.y1 + yc, _ssize - MAS_GRID * yc - md.y1 - 1) / 2; // make sure, all tp points are inside the map area
    int tgx = min (xb, max (3, xc / 3)), tgy = min (yb, max (3, yc / 3)) * _ssize,
        tp[8] = { tgx,
                  ...
                  -2 * tgx - 2 * tgy,
                  -2 * tgx + 2 * tgy,
                  2 * tgx - 2 * tgy }; // 8 positions to try, if the stariing point is solid
    uchar epoch = 1;
    ...
    for (int y = 0; y < MAS_GRID; y++, ss = s += _ssize * yc)
        for (int x = 0; x < MAS_GRID; x++, ss += xc) // measure visible area in MAS_GRID x MAS_GRID probe points
        { // calculate MAS_GRID^2 probe points (with a low-res version of
          // computeraytable()
            servsqr *r = ss, *rr;
            if (SOLID (r)) loopi (8)
                { // if initial position is SOLID, we try 8 points around that
                  // spot
                    if (!SOLID (r + tp[i]))
                    {
                        r += tp[i];
...
void calcentitystats (entitystats_s &es, const persistent_entity *pents, int pentsize)
{
    ...
    { // calculate the distances between all pickups (takes n^2/2
      // calculations - which should be fine for any sane map)
        double d = pow2 (pents[picks[i]].x - pents[picks[j]].x)
...
mapstats *loadmapstats (const char *filename, bool getlayout)
{
    ...
    if (s.hdr.version > MAPVERSION || s.hdr.numents > MAXENTITIES
        ...
        || !f->seek (clamp (s.hdr.headersize - sizeof_header, 0, MAXHEADEREXTRA), SEEK_CUR))
    {
        delete f;
        return nullptr;
    }
...
void stackdumper (unsigned int type, EXCEPTION_POINTERS *ep)
{
    ...
    while (::StackWalk (IMAGE_FILE_MACHINE_I386, GetCurrentProcess (), GetCurrentThread (), &sf,
...
static inline float
det3x3 (float a1, float a2, float a3, float b1, float b2, float b3, float c1, float c2, float c3)
{
...
{
    float a1 = v[0], a2 = v[1], a3 = v[2], a4 = v[3], b1 = v[4], b2 = v[5], b3 = v[6], b4 = v[7],
          c1 = v[8], c2 = v[9], c3 = v[10], c4 = v[11], d1 = v[12], d2 = v[13], d3 = v[14], d4 = v[15];

    return a1 * det3x3 (b2, b3, b4, c2, c3, c4, d2, d3, d4) - b1 * det3x3 (a2, a3, a4, c2, c3, c4, d2, d3, d4)
...
{
    float a1 = m.v[0], a2 = m.v[1], a3 = m.v[2], a4 = m.v[3], b1 = m.v[4], b2 = m.v[5], b3 = m.v[6],
          b4 = m.v[7], c1 = m.v[8], c2 = m.v[9], c3 = m.v[10], c4 = m.v[11], d1 = m.v[12],
          d2 = m.v[13], d3 = m.v[14], d4 = m.v[15];
        ...
        if ((m = strchr (l, ' ')) && (rev = atoi (m + 1))) // string has a space and a number != 0 after it
        {
            *m = '\0';
            if ((!plen
                 || !strncmp (l, prefix,
                              plen))
                && // prefix is either not required or present
                (!slen
                 || (m - l > slen
                     && !strcmp (m - slen,
