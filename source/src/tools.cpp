...
#endif
  strftime(buf, sizeof(string) - 1, fmt && *fmt ? fmt : "%Y%m%d_%H.%M.%S",
           timeinfo);  // sortable time for filenames
  return buf;
}

const char *asctimestr() { return timestring(true, "%c"); }
...
static const uchar transformenttab[] = {
    ...
    /*  8 */ /* I_BOOST        */ I_HEALTH,
    /* I_AKIMBO    */ I_AKIMBO,
    /*  9 */ /* I_GREENARMOUR  */ I_HELMET,
    /* MAPMODEL    */ MAPMODEL,
    /* 10 */ /* I_YELLOWARMOUR */ I_ARMOUR,
    /* CARROT      */ CARROT,
    /* 11 */ /* I_QUAD         */ I_AKIMBO,
    /* LADDER      */ LADDER,
    /* 12 */ /* TELEPORT       */ NOTUSED,
...
int fixmapheadersize(int version,
                     int headersize)  // we can't trust hdr.headersize for file
                                      // versions < 10 (thx flow)
{
  if (version < 4)
    return sizeof(header) - sizeof(int) * 16;
  else if (version == 7 || version == 8)
    return sizeof(header) + sizeof(char) * 128;  // mediareq
  else if (version < 10 || headersize < int(sizeof(header)))
...
servsqr *createservworld(
    const sqr *s,
    int _cubicsize)  // create a server-style floorplan on the client, to use
                     // same statistics functions on client and server
{
  ...
  loopirev(_cubicsize) {
    d->type =
        s->type == SOLID
            ? SOLID
            : ((s->type & TAGTRIGGERMASK) |
               (s->tag & ~TAGTRIGGERMASK));  // guaranteed to not have tagclips
                                             // on SOLID cubes - so we can check
                                             // for SOLID without masks
    d->ceil = s->ceil;
    d->floor = s->floor;
    d->vdelta = s->vdelta;
    d++;
...
int calcmapdims(mapdim_s &md, const servsqr *s, int _ssize) {
  ...
  md.minfloor = 127;
  md.maxceil = -128;
  for (int y = 0; y < _ssize; y++)
    for (int x = 0; x < _ssize; x++) {
  ...
  if (md.x2 < md.x1 ||
      md.y2 <
          md.y1) {  // map is completely solid -> default to empty map values
    md.x1 = md.y1 = 2;
  ...
  if (md.x1 < MINBORD || md.y1 < MINBORD || md.x2 >= _ssize - MINBORD ||
      md.y2 >= _ssize - MINBORD)
    res = -2;  // reject map because of world border violation
...
int calcmapareastats(mapareastats_s &ms, servsqr *_servworld, int _ssize,
                     const mapdim_s &md) {
  ...
  loopirev(MAS_VDELTA_TABSIZE) ms.vdds =
      (ms.vdds << 2) | ((ms.vdd[i] > MAS_VDELTA_THRES) << 1) | (ms.vdd[i] > 0);

  // check occlusion by SOLIDs (destroys vdelta values)
  ss = bb;
  for (int j = md.yspan; j > 0; j--, ss += linegap)
    loopirev(md.xspan)(ss++)->vdelta = 0;  // reset all vdelta values
  int xc = (md.xspan + MAS_GRID / 2) / (MAS_GRID + 1),
      yc = (md.yspan + MAS_GRID / 2) / (MAS_GRID + 1);
  if (md.x1 + MAS_GRID * xc >= _ssize || md.y1 + MAS_GRID * yc >= _ssize)
    return -1;  // malformed map
  int xb = min(md.x1 + xc, _ssize - MAS_GRID * xc - md.x1 - 1) / 2,
      yb = min(md.y1 + yc, _ssize - MAS_GRID * yc - md.y1 - 1) /
           2;  // make sure, all tp points are inside the map area
  int tgx = min(xb, max(3, xc / 3)), tgy = min(yb, max(3, yc / 3)) * _ssize,
      tp[8] = {
          tgx,
          -tgx,
          tgy,
          ...
          -2 * tgx + 2 * tgy,
          2 * tgx -
              2 * tgy};  // 8 positions to try, if the stariing point is solid
  uchar epoch = 1;
  servsqr *s = bb + xc + yc * _ssize;
  ss = s;
  vector<threeint> tab;
  threeint pp;
  for (int y = 0; y < MAS_GRID; y++, ss = s += _ssize * yc)
    for (int x = 0; x < MAS_GRID;
         x++, ss +=
              xc)  // measure visible area in MAS_GRID x MAS_GRID probe points
    {  // calculate MAS_GRID^2 probe points (with a low-res version of
       // computeraytable()
      servsqr *r = ss, *rr;
      if (SOLID(r))
        loopi(8) {  // if initial position is SOLID, we try 8 points around that
                    // spot
          if (!SOLID(r + tp[i])) {
            r += tp[i];
            break;
...
#undef RAYS
  ...
  for (int j = md.yspan; j > 0; j--, ss += linegap) loopirev(md.xspan) {
      if (!SOLID(ss)) {
        ms.total++;
        if (!ss->vdelta)
          ms.rest++;  // count all cubes not in view of one of the probe points
      }
      ss++;
    }
  ASSERT(tab.length() == MAS_GRID2);
  tab.sort(cmpintdesc);
  loopv(tab) {  // sort probe poiints in descending order of area
    ms.ppv[i] = tab[i].val1;
    ms.ppa[i] = tab[i].key;
#ifndef STANDALONE
    ms.ppp[i] = tab[i].val2;
...
void calcentitystats(entitystats_s &es, const persistent_entity *pents,
                     int pentsize) {
#ifndef STANDALONE
  vector<persistent_entity> _pents;
  if (!pents) {  // use regular ents list
  ...
  if (es.flags[0] == 1 && es.flags[1] == 1) {
    vec fd(pents[es.flagents[0]].x - pents[es.flagents[1]].x,
           pents[es.flagents[0]].y - pents[es.flagents[1]].y, 0);
    es.flagentdistance = fd.magnitude();
    es.hasflags = es.flagentdistance >= MINFLAGDISTANCE;
  }
  int r = 0;
  loopvjrev(picks)
      loopirev(j) {  // calculate the distances between all pickups (takes n^2/2
                     // calculations - which should be fine for any sane map)
    double d =
        pow2(pents[picks[i]].x - pents[picks[j]].x) +
        pow2(pents[picks[i]].y - pents[picks[j]].y);  // deltaX^2 + deltaY^2
    frexp(d, &r);
    r /= 2;  // sqrt ;)
...
int cmpintasc(const int *a, const int *b) {
  return *a - *b;
}  // leads to ascending sort order
int cmpintdesc(const int *a, const int *b) {
  return *b - *a;
}  // leads to descending sort order
int stringsort(const char **a, const char **b) { return strcmp(*a, *b); }
int stringsortrev(const char **a, const char **b) { return strcmp(*b, *a); }
int stringsortignorecase(const char **a, const char **b) {
  return strcasecmp(*a, *b);
}
int stringsortignorecaserev(const char **a, const char **b) {
...
mapstats *loadmapstats(const char *filename, bool getlayout) {
  ...
  loopi(s.hdr.numents) {
    ...
    if (e.type == PLAYERSTART &&
        (e.attr2 == 0 || e.attr2 == 1 || e.attr2 == 100))
...
void stackdumper(unsigned int type, EXCEPTION_POINTERS *ep) {
  ...
  formatstring(out)("Win32 Exception: 0x%x [0x%x]\n\n", er->ExceptionCode,
                    er->ExceptionCode == EXCEPTION_ACCESS_VIOLATION
                        ...
                        : -1);
  STACKFRAME sf = {{context->Eip, 0, AddrModeFlat},
                   {},
                   {context->Ebp, 0, AddrModeFlat},
                   {context->Esp, 0, AddrModeFlat},
                   0};
  SymInitialize(GetCurrentProcess(), NULL, TRUE);

  while (::StackWalk(IMAGE_FILE_MACHINE_I386, GetCurrentProcess(),
                     GetCurrentThread(), &sf, context, NULL,
    ...
    DWORD off;
    if (SymGetSymFromAddr(GetCurrentProcess(), (DWORD)sf.AddrPC.Offset, &off,
                          &si.sym) &&
        SymGetLineFromAddr(GetCurrentProcess(), (DWORD)sf.AddrPC.Offset, &off,
                           &li)) {
      char *del = strrchr(li.FileName, '\\');
      formatstring(t)("%s - %s [%d]\n", si.sym.Name,
                      del ? del + 1 : li.FileName, li.LineNumber);
      concatstring(out, t);
    }
  }
...
bool isbigendian() { return !*(const uchar *)&islittleendian; }
...
const char *iptoa(const enet_uint32 ip) {
  ...
  static int buf = 0;
  buf = (buf + 1) % 2;
  formatstring(s[buf])("%d.%d.%d.%d", (ip >> 24) & 255, (ip >> 16) & 255,
                       (ip >> 8) & 255, ip & 255);
  return s[buf];
}
...
int getlistindex(const char *key, const char *list[], bool acceptnumeric,
                 int deflt) {
  int max = 0;
  while (list[max][0])
    if (!strcasecmp(key, list[max]))
      return max;
    else
      max++;
  ...
  if (*key)
    clientlogf("warning: unknown token \"%s\" (not in list [%s])", key, opts);
  delstring(opts);
#endif
  return deflt;
}
...
static inline float det3x3(float a1, float a2, float a3, float b1, float b2,
...
float glmatrixf::determinant() const {
  float a1 = v[0], a2 = v[1], a3 = v[2], a4 = v[3], b1 = v[4], b2 = v[5],
        b3 = v[6], b4 = v[7], c1 = v[8], c2 = v[9], c3 = v[10], c4 = v[11],
...
void glmatrixf::adjoint(const glmatrixf &m) {
  float a1 = m.v[0], a2 = m.v[1], a3 = m.v[2], a4 = m.v[3], b1 = m.v[4],
        b2 = m.v[5], b3 = m.v[6], b4 = m.v[7], c1 = m.v[8], c2 = m.v[9],
        c3 = m.v[10], c4 = m.v[11], d1 = m.v[12], d2 = m.v[13], d3 = m.v[14],
        d4 = m.v[15];
...
bool glmatrixf::invert(const glmatrixf &m, float mindet) {
  ...
  adjoint(m);
  float det = a1 * v[0] + b1 * v[1] + c1 * v[2] +
              d1 * v[3];  // float det = m.determinant();
...
// the standalone linux version uses native linux libraries - and also makes use
// of shared memory
...
int sl_semaphore::trywait() { return SDL_SemTryWait((SDL_sem *)data); }

int sl_semaphore::timedwait(int howlongmillis) {
  return SDL_SemWaitTimeout((SDL_sem *)data, howlongmillis);
}

int sl_semaphore::getvalue() { return SDL_SemValue((SDL_sem *)data); }
...
int sl_semaphore::trywait() { return sem_trywait((sem_t *)data); }
...
// (wrapping threads is slightly ugly, since SDL threads use a different return
// value (int) than pthreads (void *) - and that can't be solved with a
// typecast)
#ifdef AC_USE_SDL_THREADS
struct sl_threadinfo {
  int (*fn)(void *);
  void *data;
  SDL_Thread *handle;
  volatile char done;
...
bool sl_pollthread(void *ti) { return ((sl_threadinfo *)ti)->done != 0; }

// platform dependent stuff not covered by enet (use POSIX or, if possible, SDL)
#ifdef AC_USE_SDL_THREADS
void sl_sleep(int duration) { SDL_Delay(duration); }
#else
...
void parseupdatelist(hashtable<const char *, int> &ht, char *buf,
                     const char *prefix, const char *suffix) {
  for (char *d = buf; *d; d++)
    if (!isalnum(*d) && !strchr("._-() /\n", *d))
      *d = ' ';  // allowed chars in media path strings (except ' ')
  char *p, *l = buf, *m;
  int rev, *revp, plen = prefix ? (int)strlen(prefix) : 0,
                  slen = suffix ? (int)strlen(suffix) : 0;
  do {
    if ((p = strchr(l, '\n'))) *p = '\0';  // break into single lines
    l += strspn(l, " ");                   // skip leading spaces
    if ((m = strchr(l, ' ')) &&
        (rev = atoi(m + 1)))  // string has a space and a number != 0 after it
    {
      *m = '\0';
      if ((!plen ||
           !strncmp(l, prefix,
                    plen)) &&  // prefix is either not required or present
          (!slen ||
           (m - l > slen &&
            !strcmp(m - slen,
                    suffix))))  // suffix is either not required or present
      {
        l += plen;        // skip prefix
        m[-slen] = '\0';  // cut suffix
        revp = ht.access(l);
