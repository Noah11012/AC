...
struct mapaction : serveraction
{
    ...
    void perform ()
    {
        ...
        else if (isdedicated && numclients () > 2 && smode >= 0 && smode != 1
                 && (gamemillis > gamelimit / 4 || scl.demo_interm))
        {
    ...
    bool isvalid ()
    {
        return serveraction::isvalid () && mode != GMODE_DEMO && map[0] && mapok
               && !(isdedicated && !m_mp (mode));
    }
    bool isdisabled () { return maprot.current () && !maprot.current ()->vote; }
    mapaction (char *map, int mode, int time, int caller, bool q)
    : map (map), mode (mode), time (time), queue (q)
    {
        if (isdedicated)
        {
            ...
            mapok = (ms != NULL) && validname
                    && ((mode != GMODE_COOPEDIT && mapisok (ms))
                        || (mode == GMODE_COOPEDIT && !readonlymap (maploc)));
            if (!mapok)
            {
                if (notify)
                {
                    ...
                    {
                        sendservmsg (ms ? (mode == GMODE_COOPEDIT ?
                                           "this map cannot be coopedited in this server" :
                                           "sorry, but this map does not satisfy some quality "
                                           "requisites to be played in MultiPlayer Mode") :
            ...
            { // check, if map supports mode
                ...
                int smode = mode; // 'borrow' the mode macros by replacing a global by
                                  // a local var
                bool spawns = mode == GMODE_COOPEDIT
                              || (m_teammode && !m_ktf ? ms->hasteamspawns : ms->hasffaspawns);
                bool flags = mode != GMODE_COOPEDIT && m_flags && !m_htf ? ms->hasflags : true;
                if (!spawns || !flags || romap)
                { // unsupported mode
                    if (strchr (scl.voteperm, 'P'))
                        role = CR_ADMIN;
                    else if (!strchr (scl.voteperm, 'p'))
                        mapok = false; // default: no one can vote for unsupported mode/map
                                       // combinations
                    defformatstring (msg) ("\f3map \"%s\" does not support \"%s\": ", behindpath (map),
                                           modestr (mode, false));
                    if (romap)
                        concatstring (msg, "map is readonly");
                    else
                    {
        ...
        else
        {
            ...
            mapok = mp == MAP_LOCAL || mp == MAP_OFFICIAL; // clients can only load from packages/maps
                                                           // and packages/maps/official
#ifndef STANDALONE
...
struct playeraction : serveraction
{
    ...
    {
        return valid_client (cn) && clients[cn]->role != CR_ADMIN;
    } // actions can't be done on admins
...
struct forceteamaction : playeraction
{
    ...
    virtual bool isvalid ()
    {
        return valid_client (cn) && team_isvalid (team) && team != clients[cn]->team;
    }
    forceteamaction (int cn, int caller, int team) : playeraction (cn), team (team)
    {
        if (cn != caller) role = roleconf ('f');
        if (isvalid () && !(clients[cn]->state.forced && clients[caller]->role != CR_ADMIN))
            formatstring (desc) ("force player %s to team %s", clients[cn]->name, teamnames[team]);
    }
};
...
struct serverdescaction : serveraction
{
    ...
    bool isvalid ()
    {
        return serveraction::isvalid () && updatedescallowed () && valid_client (cn);
    }
