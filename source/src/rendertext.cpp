...
void newfont(char *name, char *tex, int *defaultw, int *defaulth, int *offsetx,
             int *offsety, int *offsetw, int *offseth) {
...
COMMANDF(curfont, "",
         () { result(curfont && curfont->name ? curfont->name : ""); });

font *getfont(const char *name) { return fonts.access(name); }
...
inline int draw_char_contd(font &f, font::charinfo &info, int charcode, int x,
                           int y) {
  glTexCoord2f(info.left, info.top);
  glVertex2f(x, y);
  glTexCoord2f(info.right, info.top);
  glVertex2f(x + info.w, y);
  glTexCoord2f(info.right, info.bottom);
  glVertex2f(x + info.w, y + info.h);
  glTexCoord2f(info.left, info.bottom);
  glVertex2f(x, y + info.h);
...
void render_igraphs() {
  ...
  loopv(igraphbatch) {
    if (last != igraphbatch[i].key)
      ig = getusedigraph((last = igraphbatch[i].key)),
      ishc = last > 0 && last < 10 ? 1 : 0;
    if (ig && ig->tex) {
      int &x = igraphbatch[i].val1, &y = igraphbatch[i].val2,
          fpos = ((totalmillis + x * x + VIRTW * y) % ig->frames.last() +
                  ig->frames.last()) %
                 ig->frames.last(),
          frame = 0;
      float fw = min(1.0f, float(ig->tex->ys) / ig->tex->xs);
      if (igraphanimate) loopvj(ig->frames) if (ig->frames[j] < fpos) frame++;
      quad(ig->tex->id, x + offs[ishc], y + offs[ishc], w[ishc],
           (frame % max(1, ig->tex->xs / ig->tex->ys)) * fw, 0, fw, 1);
    }
  }
}

// stack[sp] is current color index
static void text_color(char c, char *stack, int size, int &sp, bvec color,
                       int a) {
  if (c == 's')  // save color
  {
    c = stack[sp];
    ...
    if (c == 'r')
      ...
      if (allowblinkingtext && !ignoreblinkingbit) stack[sp] *= -1;
    }  // blinking text - only if allowed
    else
      stack[sp] = c;
    switch (iabs(stack[sp])) {
      case '0':
        color = bvec(2, 255, 128);
        break;  // green: player talk
      case '1':
        color = bvec(96, 160, 255);
        break;  // blue: team chat
      case '2':
        color = bvec(255, 192, 64);
        break;  // yellow: gameplay action messages, only actions done by
                // players - 230 230 20 too bright
      case '3':
        color = bvec(255, 64, 64);
        break;  // red: important errors and notes
      case '4':
      ...
      case '6':
        ...
        break;  // dark brown
      case '7':
        color = bvec(153, 51, 51);
        break;  // dark red: dead admin
      case '8':
        color = bvec(192, 64, 192);
        break;  // magenta
      case '9':
        color = bvec(255, 102, 0);
        break;  // orange

      case 'A':
        color = bvec(0xff, 0xb7, 0xb7);
        break;  // red set
      case 'B':
        color = bvec(0xCC, 0x33, 0x33);
        break;  //
      case 'C':
        color = bvec(0x66, 0x33, 0x33);
        break;  //
      case 'D':
        color = bvec(0xF8, 0x98, 0x4E);
        break;  //

      case 'E':
        color = bvec(0xFF, 0xFF, 0xB7);
        break;  // yellow set
      case 'F':
        color = bvec(0xCC, 0xCC, 0x33);
        break;  //
      case 'G':
        color = bvec(0x66, 0x66, 0x33);
        break;  //
      case 'H':
        color = bvec(0xCC, 0xFC, 0x58);
        break;  //

      case 'I':
        color = bvec(0xB7, 0xFF, 0xB7);
        break;  // green set
      case 'J':
        color = bvec(0x33, 0xCC, 0x33);
        break;  //
      case 'K':
        color = bvec(0x33, 0x66, 0x33);
        break;  //
      case 'L':
        color = bvec(0x3F, 0xFF, 0x98);
        break;  //

      case 'M':
        color = bvec(0xB7, 0xFF, 0xFF);
        break;  // cyan set
      case 'N':
        color = bvec(0x33, 0xCC, 0xCC);
        break;  //
      case 'O':
        color = bvec(0x33, 0x66, 0x66);
        break;  //
      case 'P':
        color = bvec(0x4F, 0xCC, 0xF8);
        break;  //

      case 'Q':
        color = bvec(0xB7, 0xB7, 0xFF);
        break;  // blue set
      case 'R':
        color = bvec(0x33, 0x33, 0xCC);
        break;  //
      case 'S':
        color = bvec(0x33, 0x33, 0x66);
        break;  //
      case 'T':
        color = bvec(0xA0, 0x49, 0xFF);
        break;  //

      case 'U':
        color = bvec(0xFF, 0xB7, 0xFF);
        break;  // magenta set
      case 'V':
        color = bvec(0xCC, 0x33, 0xCC);
        break;  //
      case 'W':
        color = bvec(0x66, 0x33, 0x66);
        break;  //
      case 'X':
        color = bvec(0xFF, 0x01, 0xD5);
        break;  //

      case 'Y':
        color = bvec(0xC7, 0xD1, 0xE2);
        break;  // lt gray
      case 'Z':
        color = bvec(0x32, 0x32, 0x32);
        break;  // dark gray

      case 'u':
        color = bvec(120, 240, 120);
        break;  // stats: green
      case 'v':
        color = bvec(120, 120, 240);
        break;  // stats: blue
      case 'w':
        color = bvec(230, 230, 110);
        break;  // stats: yellow
      case 'x':
...
void text_endcolumns() { DELETEP(columns); }
...
#define TEXTWHITE(idx) \
  if (y + FONTH > hity && x >= hitx) return idx;
#define TEXTLINE(idx) \
  if (y + FONTH > hity) return idx;
...
#define TEXTIGRAPH(idx) \
  queue_igraph((uchar)str[idx], left + x, top + y), x += FONTH + 1;
...
#define TEXTWORD TEXTWORDSKELETON
  ...
  glBlendFunc(GL_SRC_ALPHA,
              curfont->tex->bpp == 32 ? GL_ONE_MINUS_SRC_ALPHA : GL_ONE);
  ...
  if (cursor >= 0) {
    ...
    int cw = curfont->chars.inrange(cc - 33) ? curfont->chars[cc - 33].w + 1
                                             : curfont->defaultw;
...
void reloadfonts() {
  enumerate(fonts, font, f,
            if (!reloadtexture(*f.tex))
                fatal("failed to reload font texture"););
}

void cutcolorstring(char *text,
                    int maxlen) {  // limit string length, ignore color codes
  if (!curfont) return;
  int len = 0;
  ...
  while (*text) {
    if (*text == '\f' && text[1])
      ...
      len += curfont->chars.inrange(*text - curfont->skip)
                 ? curfont->chars[*text - curfont->skip].w
                 : curfont->defaultw;
    if (len > maxlen) {
      *text = '\0';
      break;
...
void addigraph(char *fname) {
  ...
  if (mnem && *mnem && !igraphs.access(mnem)) {  // new mnem
...
#ifdef _DEBUG
      clientlogf(" loaded igraph \"%s\", short \"%s\", %dx%d, %d frame%s",
                 fname, mnem, tex->xs, tex->ys, ig.frames.length(),
                 ig.frames.length() > 1 ? "s" : "");
#endif
    }
  }
  igraphsi[fname] = 0;
...
void updateigraphs()  // read all filenames and parse new ones (also preloads
                      // the textures)
{
  vector<char *> files;
...
void encodeigraphs(char *d, const char *s,
                   int len)  // find known igraphs in a string and substitute
                             // with rendercodes, only used for console messages
{
  if (hideigraphs)
    copystring(d, s, len);
  else {
    ...
    loopi(len) {
      if (*s == ':' && (!i || isspace(s[-1])) && i < len - 1) {
        ...
        if (l && l < MAXSTRLEN) {
          ...
          if ((u = getigraph(mnem))) {  // found one: encode
            *d++ = '\1';
            *d++ = u;
            s += l + 1;
            i++;
...
void enumigraphs(vector<const char *> &igs, const char *s, int len) {
  enumeratek(igraphs, const char *, mnem,
             if (!strncasecmp(mnem, s, len)) igs.add(mnem));
}
