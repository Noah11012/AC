...
void render_igraphs ()
{
    ...
    int last = 0, w[2] = { (FONTH * igraphsize) / 100, (FONTH * igraphsizehardcoded) / 100 },
    ...
    loopv (igraphbatch)
    {
        ...
        if (ig && ig->tex)
        {
            int &x = igraphbatch[i].val1, &y = igraphbatch[i].val2,
                fpos = ((totalmillis + x * x + VIRTW * y) % ig->frames.last () + ig->frames.last ())
                       % ig->frames.last (),
                frame = 0;
...
void reloadfonts ()
{
    enumerate (fonts, font, f, if (!reloadtexture (*f.tex)) fatal ("failed to reload font texture"););
}
...
#ifdef _DEBUG
            clientlogf (" loaded igraph \"%s\", short \"%s\", %dx%d, %d frame%s", fname, mnem,
                        tex->xs, tex->ys, ig.frames.length (), ig.frames.length () > 1 ? "s" : "");
#endif
        }
    }
    igraphsi[fname] = 0;
