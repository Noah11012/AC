...
FVARF (waterlevel, -10000, -10000, 127, {
    ...
    waterlevel = float(hdr.waterlevel) / WATERLEVELSCALING; // waterlevel and hdr.waterlevel need to
                                                            // be kept in sync
});
...
COMMANDF (watercolour, "ssss", (char *r, char *g, char *b, char *a) {
    if (editmode) setwatercolor (r, g, b, a);
});
...
void getwatercolour ()
{
    defformatstring (res) ("%d %d %d %d", hdr.watercolor[0], hdr.watercolor[1], hdr.watercolor[2],
                           hdr.watercolor[3]);
    result (res);
}
...
VERTWC (vertwc,
        ...
        {
            varray::attrib<uchar> (hdr.watercolor[0], hdr.watercolor[1], hdr.watercolor[2],
                                   (uchar) (hdr.watercolor[3] + (max (v, 0.0f) - 0.5f) * 51.0f));
        })
VERTWT (vertwt,
...
void setupmultitexrefract (GLuint reflecttex, GLuint refracttex)
{
    ...
    colortmu (0, hdr.watercolor[0] / 255.0f, hdr.watercolor[1] / 255.0f, hdr.watercolor[2] / 255.0f,
              hdr.watercolor[3] / 255.0f);
...
void setupmultitexreflect (GLuint reflecttex)
{
    ...
    colortmu (0, hdr.watercolor[0] / 255.0f * a, hdr.watercolor[1] / 255.0f * a,
              hdr.watercolor[2] / 255.0f * a, 1.0f - a);
