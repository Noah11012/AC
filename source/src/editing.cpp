...
void toggleedit(bool force) {
  ...
  if (player1->state == CS_SPECTATE) {
    if (watchingdemo) {
      if (player1->spectatemode != SM_FLY)
        spectatemode(SM_FLY);  // release in a place of followed player
    } else
      return;
  }
  ...
  if (editing && (player1->weaponsel->type == GUN_SNIPER &&
  ...
  if (editing && (player1->weaponsel->type == GUN_GRENADE))
    ((grenades *)player1->weaponsel)->onownerdies();
...
bool correctsel(block &s)  // ensures above invariant
{
  ...
  if (s.xs <= 0 || s.ys <= 0)
    ...
    return !OUTBORD(s.x, s.y);
}

bool noteditmode(const char *func) {
  if (!editmode) {
    if (func && func[0] != '\0')
      conoutf("\f4[\f3%s\f4]\f5 is only allowed in edit mode", func);
    else
...
char *editinfo() {
  ...
  if (e >= 0) {
    ...
    formatstring(info)("%s entity: %s (%s)",
                       pinnedclosestent ? "\fs\f3pinned\fr" : "closest",
                       entnames[t], formatentityattributes(c, true));
    ...
    if (t == MAPMODEL) {
      ...
      if (mapconfigdata.mapsoundlines.inrange(c.attr1))
        slotinfo = mapconfigdata.mapsoundlines[c.attr1].name;
    } else
      slotinfo = "";
    if (*slotinfo && (hideeditslotinfo == 0 ||
                      (hideeditslotinfo == 1 && slotinfo == unassigned)))
      concatformatstring(info, " %s", slotinfo);
  }
  if (selset()) {
    concatformatstring(info, "\n\tselection = (%d, %d)", (sels.last()).xs,
                       (sels.last()).ys);
    if (sels.length() > 1)
      concatformatstring(info, " and %d more", sels.length() - 1);
  } else
    concatformatstring(info, "\n\tno selection");
  sqr *s;
  if (!OUTBORD(cx, cy) && (s = S(cx, cy)) && !editfocusdetails(s) &&
      !SOLID(s) && s->tag)
    concatformatstring(info, ", tag 0x%02X", s->tag);
  return info;
}
...
void addselection(int x, int y, int xs, int ys, int h) {
  block &s = sels.add();
  if (h == -999 && !OUTBORD(x + xs / 2, y + ys / 2))
    h = S(x + xs / 2, y + ys / 2)->floor;
  s.x = x;
  s.y = y;
  s.xs = xs;
  s.ys = ys;
...
void resetselections() { sels.shrink(0); }

COMMANDF(select, "iiii", (int *x, int *y, int *xs, int *ys) {
  resetselections();
  addselection(*x, *y, *xs, *ys, -999);
});
COMMANDF(addselection, "iiii", (int *x, int *y, int *xs, int *ys) {
  addselection(*x, *y, *xs, *ys, -999);
});
COMMAND(resetselections, "");

// reset all invalid selections
void checkselections() { loopv(sels) if (!correctsel(sels[i])) sels.remove(i); }

// update current selection, or add a new one
void makesel(bool isnew) {
  if (isnew || sels.length() == 0)
    addselection(min(lastx, cx), min(lasty, cy), iabs(lastx - cx) + 1,
                 iabs(lasty - cy) + 1, max(lasth, ch));
  else {
    block &cursel = sels.last();
    cursel.x = min(lastx, cx);
    cursel.y = min(lasty, cy);
    cursel.xs = iabs(lastx - cx) + 1;
    cursel.ys = iabs(lasty - cy) + 1;
...
#define SEL_ATTR(attr)                                            \
  {                                                               \
    string buf = "";                                              \
    loopv(sels) { concatformatstring(buf, "%d ", sels[i].attr); } \
    result(buf);                                                  \
  }
...
#define sheight(s, t, z)                                                       \
  (!flrceil ? (s->type == FHF ? s->floor - t->vdelta / 4.0f : (float)s->floor) \
...
void cursorupdate()  // called every frame from hud
{
  ...
  editaxis = fabs(camera1->pitch) > 60
                 ? "\161\15\15"[flrceil]
                 : "\160\13\14\157"[(int(camera1->yaw + 45) / 90) & 3];
  ...
  if (sparkletime)
    lastsparkle =
        lastmillis - (lastmillis % 20);  // clip adding sparklies at 50 fps
  ...
  if (showgrid && !blankouthud) {
    for (int ix = cx - GRIDSIZE; ix <= cx + GRIDSIZE; ix++)
      for (int iy = cy - GRIDSIZE; iy <= cy + GRIDSIZE; iy++) {
        ...
        if (sparkletime && showtagclips && showtagclipfocus &&
            s->tag & TAGANYCLIP)
          particle_cube(s->tag & TAGCLIP ? PART_EPICKUP : PART_EMODEL, tagnum,
                        taglife, ix, iy);
        if (s->tag)
    ...
    if (!SOLID(s)) {
      ...
      block b = {cx, cy, 1, 1};
      box(b, ih, sheight(s, SWS(s, 1, 0, sfactor), z),
          sheight(s, SWS(s, 1, 1, sfactor), z),
          sheight(s, SWS(s, 0, 1, sfactor), z));
      linestyle(GRIDS, 0xFF, 0x00, 0x00);
      dot(cx, cy, ih);
      ch = (int)ih;
    }
  } else
    ch = (int)sheight(s, s, z);

  if (selset() && !blankouthud) {
    linestyle(GRIDS, 0xFF, 0x40, 0x40);
    loopv(sels) box(sels[i], (float)sels[i].h, (float)sels[i].h,
                    (float)sels[i].h, (float)sels[i].h);
  }
  ...
  if (!showtagclipfocus && showtagclips && !blankouthud) {
    ...
    loopv(tagclipcubes)  // all non-solid & have clips
    {
      ...
      sqr *s = S(x, y), *o[9];
      if (s->tag & TAGCLIP)
        linestyle(tagcliplinewidth, 0xFF, 0xFF, 0);  // yello
      else
        linestyle(tagcliplinewidth, 0xFF, 0, 0xFF);  // magenta
      ...
      int h = s->floor - (s->type == FHF ? (s->vdelta + 3) / 4 : 0),
          c = s->ceil + (s->type == CHF ? (s->vdelta + 3) / 4 : 0);
      loopk(4) if ((clipped[k] == clipped[k + 1]) &&
                   !(clipped[k] && clipped[k + 5]))
          line(x + xo[k + 1], y + yo[k + 1], h, x + xo[k + 1], y + yo[k + 1],
               c);
      for (; h < c; h++) {
        int k = (h + x + y) & 3;
        if (k < 2 && !clipped[k])
          line(x + xo[k], y + yo[k], h + 1, x + xo[k + 1], y + yo[k + 1], h);
        k = (h - x - y) & 3;
        if (k > 1 && !clipped[k])
          line(x + xo[k], y + yo[k], h + 1, x + xo[k + 1], y + yo[k + 1], h);
      }
      if (sparkletime && tagnumfull)
        particle_cube(s->tag & TAGCLIP ? PART_EPICKUP : PART_EMODEL, tagnumfull,
                      30, x, y);
    }
  }
...
void gotoentity(int *n) {
  ...
  if (ents.inrange(*n) && ents[*n].type != NOTUSED) {
    ...
    player1->yaw =
        ((ents[*n].attr1 / ENTSCALE10) % 360 + 360) % 360;  // works for most ;)
    player1->pitch = 0;
    player1->resetinterp();
  }
}
...
void gotoposition(char *x, char *y, char *z, char *yaw, char *pitch) {
  ...
  player1->resetinterp();
  defformatstring(res)("%s %s %s %d %d", floatstr(player1->o.x),
                       floatstr(player1->o.y), floatstr(player1->o.z),
                       int(player1->yaw), int(player1->pitch));
  result(res);
}
...
void netblockpaste(const block &b, int bx, int by,
                   bool light)  // transmit block to be pasted at bx,by
{
...
void netblockpastexy(ucharbuf *p, int bx, int by, int bxs, int bys, int light) {
  if (bxs < 0 || bys < 0 || bxs > ssize || bys > ssize ||
...
void restoreeditundo(ucharbuf &q) {
  ...
  while (!q.overread() && (type = getuint(q))) {
    ...
    if (worldiodebug) switch (type) {
        ...
        case 20:
          clientlogf(
              "  got %sdo x %d, y %d, xs %d, ys %d, remaining %d, overread %d",
              type == 10 ? "un" : "re", b->x, b->y, b->xs, b->ys, q.remaining(),
              int(q.overread()));
          break;
      }
#endif
  ...
  if (undos.length() || redos.length())
    conoutf("restored editing history: %d undos and %d redos", undos.length(),
            redos.length());
}
...
int rlencodeundo(int type, vector<uchar> &t, block *s) {
  ...
  if (worldiodebug)
    clientlogf(
        "    compressing redo/undo x %d, y %d, xs %d, ys %d, compressed length "
        "%d, cubes %d",
        s->x, s->y, s->xs, s->ys, tmp.length(), s->xs * s->ys);
#endif
  return t.length();
}
...
int backupeditundo(vector<uchar> &buf, int undolimit, int redolimit) {
  ...
  loopvrev(undos) {
    ...
    if (worldiodebug)
      clientlogf(
          "  written undo x %d, y %d, xs %d, ys %d, compressed length %d",
          undos[i]->x, undos[i]->y, undos[i]->xs, undos[i]->ys, tmp.length());
#endif
  }
  loopvrev(redos) {
    tmp.setsize(0);
    ...
    if (worldiodebug)
      clientlogf(
          "  written redo x %d, y %d, xs %d, ys %d, compressed length %d",
          redos[i]->x, redos[i]->y, redos[i]->xs, redos[i]->ys, tmp.length());
#endif
  }
  putuint(buf, 0);
  return numundo;
...
void paste() {
  ...
  if (!mp) {
    ...
    if (sels.length() && copytexconfig)
      texmap = texconfig_paste(copytexconfig, usedslots);
  }
  ...
  loopv(sels) {
    ...
    loopvj(copybuffers) {
      ...
      int dx = copyblock->x - copybuffers[0]->x,
          dy = copyblock->y - copybuffers[0]->y;

      sel.xs = copyblock->xs;
      sel.ys = copyblock->ys;
      sel.x = selx + dx;
      sel.y = sely + dy;
      if (!correctsel(sel) || sel.xs != copyblock->xs ||
          sel.ys != copyblock->ys) {
...
void tofronttex()  // maintain most recently used of the texture lists when
                   // applying texture
{
  loopi(3) {
    int c = curedittex[i];
...
void edittexturestack(char *what,
                      int *slot)  // manually manipulate the "last-used" texture
                                  // lists to put certain textures up front
{
  tofronttex();  // keep laste edited texture
  int n = getlistindex(what, texturestacktypes, true, -1);
  if (n >= 0) {
    loopi(256) if (hdr.texlists[n][i] == *slot) curedittex[n] =
        i;  // find stack index of wanted texture slot
  }
  tofronttex();
}
...
void editdrag(bool isdown) {
  ...
  makesel(isdown);
  if (!isdown)
    for (int i = sels.length() - 2; i >= 0; i--) {
      block &a = sels.last(), &b = sels[i];
      if (a.x == b.x && a.y == b.y && a.xs == b.xs &&
          a.ys == b.ys) {  // making a selection twice will deselect both of it
        sels.drop();
        sels.remove(i);
        break;
      }
...
void editheight(int *flr, int *amount) {
  ...
  loopv(sels) {
    ...
    addmsg(SV_EDITXY, "ri7", EDITXY_HEIGHT, sels[i].x, sels[i].y, sels[i].xs,
           sels[i].ys, isfloor, *amount);
  }
}
...
void renderhudtexturepreview(int slot, int pos, bool highlight) {
  ...
  if (pt) {
    if (pt->xs > pt->ys)
      xs = bs, ys = (xs * pt->ys) / pt->xs;  // keep aspect ratio of texture
    else
      ys = bs, xs = (ys * pt->xs) / pt->ys;
    int xt = x + (bs - xs) / 2, yt = y + (bs - ys) / 2;
    extern int fullbrightlevel;
    framedquadtexture(pt->id, xt, yt, xs, ys, 1, fullbrightlevel);
  } else {  // sky slot: just show blue instead of the texture
    color c(0, 0, 0.6f, 1);
...
void renderhudtexturepreviews() {
  if (lastedittex && hudtexttl && (lastmillis - lastedittex) < hudtexttl) {
    ...
    loopi(5) renderhudtexturepreview(hdr.texlists[atype][startidx + i], i,
                                     startidx + i == idx);
  }
}
...
void edittex(int *type, int *dir) {
  ...
  loopv(sels) {
    ...
    addmsg(SV_EDITXY, "ri7", EDITXY_TEX, sels[i].x, sels[i].y, sels[i].xs,
           sels[i].ys, *type, t, 0);
  }
  unsavededits++;
...
void replace() {
  ...
  loop(x, ssize) loop(y, ssize) {
    ...
    switch (lasttype) {
      case 0:
        if (s->ftex == rtex.ftex) s->ftex = lasttex;
        break;
      case 1:
        if (s->wtex == rtex.wtex) s->wtex = lasttex;
        break;
      case 2:
        if (s->ctex == rtex.ctex) s->ctex = lasttex;
        break;
      case 3:
        if (s->utex == rtex.utex) s->utex = lasttex;
        break;
    }
  }
...
void edittypexy(int type, block &sel) { loopselxy(sel, s->type = type); }

void edittype(int type) {
  EDITSEL("solid|corner|heightfield");
  loopv(sels) {
    block &sel = sels[i];
    if (type == CORNER && (sel.xs != sel.ys ||
                           (sel.xs != 1 && sel.xs != 2 && sel.xs != 4 &&
                            sel.xs != 8 && sel.xs != 16) ||
                           (sel.x | sel.y) & (sel.xs - 1)))
      conoutf("corner selection must be power of 2 aligned");
    else {
      edittypexy(type, sel);
      addmsg(SV_EDITXY, "ri7", EDITXY_TYPE, sel.x, sel.y, sel.xs, sel.ys, type,
             0);
    }
  }
}
...
void equalize(int *flr) {
  ...
  loopv(sels) {
    ...
    addmsg(SV_EDITXY, "ri7", EDITXY_EQUALISE, sel.x, sel.y, sel.xs, sel.ys,
           isfloor, 0);
  }
}
COMMAND(equalize, "i");
...
void setvdelta(int *delta) {
  ...
  loopv(sels) {
    ...
    addmsg(SV_EDITXY, "ri7", EDITXY_VDELTA, sels[i].x, sels[i].y, sels[i].xs,
           sels[i].ys, *delta, 0);
  }
}
...
void arch(int *sidedelta) {
  ...
  loopv(sels) {
    ...
    int *averts =
        sel.xs > sel.ys ? &archverts[sel.xs - 1][0] : &archverts[sel.ys - 1][0];
    addmsg(SV_EDITARCH, "ri5v", sel.x, sel.y, sel.xs, sel.ys, *sidedelta,
           MAXARCHVERT, averts);
    archxy(*sidedelta, averts, sel);
    if (!enlargearchslopeselections) {
      sel.xs--;
      sel.ys--;
    }
  }
...
void slope(int *xd, int *yd) {
  ...
  loopv(sels) {
    block &sel = sels[i];
    addmsg(SV_EDITXY, "ri7", EDITXY_SLOPE, sel.x, sel.y, sel.xs, sel.ys, *xd,
           *yd);
...
void stairsxy(int xd, int yd, block &sel) {
  int off = xd || yd ? 1 : 0, xo = xd < 0 ? 1 - sel.xs : 0,
      yo = yd < 0 ? 1 - sel.ys : 0;
  loopselxy(sel, s->floor +=
                 (xd ? (x + xo) / xd : 0) + (yd ? (y + yo) / yd : 0) + off);
}
...
void stairs(int *xd, int *yd) {
  ...
  loopv(sels) {
    block &sel = sels[i];
    addmsg(SV_EDITXY, "ri7", EDITXY_STAIRS, sel.x, sel.y, sel.xs, sel.ys, *xd,
           *yd);
    stairsxy(*xd, *yd, sel);
  }
}
...
void edittag(int *tag) {
  ...
  loopv(sels) {
    ...
    addmsg(SV_EDITXY, "ri7", EDITXY_TAG, sels[i].x, sels[i].y, sels[i].xs,
           sels[i].ys, *tag, 0);
  }
}
COMMAND(edittag, "i");
...
void edittagclip(char *tag) {
  ...
  if (tolower(*tag) == 'n')
    ...
    nt = TAGPLCLIP;  // "playerclip", "plclip", "pl"
  else if (isalpha(*tag))
    nt = TAGCLIP;  // "clip", "all", "full", "hippo"
  EDITSEL("edittagclip");
  loopv(sels) {
    edittagxy(nt, TAGTRIGGERMASK, sels[i]);
    addmsg(SV_EDITXY, "ri7", EDITXY_TAG, sels[i].x, sels[i].y, sels[i].xs,
           sels[i].ys, nt, TAGTRIGGERMASK);
  }
}
...
void newent(char *what, float *a1, float *a2, float *a3, float *a4) {
  EDITSEL("newent");
  loopv(sels) newentity(-1, sels[i].x, sels[i].y, (int)camera1->o.z, what, *a1,
                        *a2, *a3, *a4);
}
...
void movemap(int *xop, int *yop, int *zop)  // move whole map
{
  ...
  if (!worldbordercheck(MINBORD + max(-xo, 0), MINBORD + max(xo, 0),
                        MINBORD + max(-yo, 0), MINBORD + max(yo, 0), max(zo, 0),
                        max(-zo, 0))) {
    conoutf("not enough space to move the map");
    return;
  }
  if (xo || yo) {
    block b = {max(-xo, 0), max(-yo, 0), ssize - iabs(xo), ssize - iabs(yo)},
          *cp = blockcopy(b);
    cp->x = max(xo, 0);
    cp->y = max(yo, 0);
    blockpaste(*cp);
...
void selfliprotate(block &sel, int dir) {
  ...
  switch (dir) {
    case 1:  // 90 degree clockwise
      for (x = x2; x >= x1; x--)
        for (y = y1; y <= y2; y++) *S(x, y) = *q++;
      break;
    case 2:  // 180 degree
      for (y = y2; y >= y1; y--)
        for (x = x2; x >= x1; x--) *S(x, y) = *q++;
      break;
    case 3:  // 90 degree counterclockwise
      for (x = x1; x <= x2; x++)
        for (y = y2; y >= y1; y--) *S(x, y) = *q++;
      break;
    case 11:  // flip x-axis
      for (y = y1; y <= y2; y++)
        for (x = x2; x >= x1; x--) *S(x, y) = *q++;
  ...
  *dir &= 3;
  ...
  loopv(sels) {
    ...
    if (sel.xs == sel.ys || *dir == 2) {
      ...
      addmsg(SV_EDITXY, "ri7", EDITXY_FLIPROT, sel.x, sel.y, sel.xs, sel.ys,
             *dir, 0);
    }
  }
}
...
void selectionflip(char *axis) {
  ...
  loopv(sels) {
    ...
    addmsg(SV_EDITXY, "ri7", EDITXY_FLIPROT, sels[i].x, sels[i].y, sels[i].xs,
           sels[i].ys, dir, 0);
  }
}
...
void selectionwalk(char *action, char *beginsel, char *endsel) {
  ...
  const char *localvars[] = {
      ...
      "sw_vdelta", "sw_tag",   "sw_r",     "sw_g",     "sw_b",     ""};
  ...
  loopv(sels) {
    ...
    bool haveundo = false, isro = mp && sel.xs * sel.ys > MAXNETBLOCKSQR,
         didwarn = false;
    loop(x, sel.xs) loop(y, sel.ys) {
      sqr *s = S(sel.x + x, sel.y + y), so = *s;
      formatstring(tmp)("%d", sel.x + x);
      alias("sw_abs_x", tmp);
      formatstring(tmp)("%d", sel.y + y);
      alias("sw_abs_y", tmp);
      formatstring(tmp)("%d", x);
      alias("sw_rel_x", tmp);
      formatstring(tmp)("%d", y);
      alias("sw_rel_y", tmp);
#define AATTR(x)                 \
  formatstring(tmp)("%d", s->x); \
  alias("sw_" #x, tmp)
      AATTR(type);
      AATTR(floor);
      AATTR(ceil);
...
#undef AATTR
      ...
      so.type =
          getlistindex(getalias("sw_type"), cubetypenames, true, int(SPACE));
...
#undef GETA
      if (memcmp(s, &so, sizeof(sqr))) {
        if (isro) {
          if (!didwarn)
            conoutf(
                "\f3selectionwalk: selected area %dx%d too big for multiplayer "
                "editing, changes ignored!",
                sel.xs, sel.ys);
          didwarn = true;
        } else {
...
void transformclipentities()  // transforms all clip entities to tag clips, if
                              // they are big enough (so, that no player could
                              // be above or below them)
{  // (hardcoded factor ENTSCALE10 for attr1 and ENTSCALE5 for attr2-4)
  ...
  do {
    ...
    loopv(ents) {
      entity &e = ents[i];
      if ((e.type == CLIP || e.type == PLCLIP) && e.attr2 / ENTSCALE5 &&
          e.attr3 / ENTSCALE5 && e.attr4 / ENTSCALE5) {
        ...
        int r2 = (e.attr2 % ENTSCALE5) > 0,
            r3 = (e.attr3 % ENTSCALE5) > 0;  // fractions
        int x1i = e.x - i2, x2i = e.x + i2 - 1, y1i = e.y - i3,
            y2i = e.y + i3 - 1;  // inner rectangle (fully covered cubes)
        int x1o = x1i - r2, x2o = x2i + r2, y1o = y1i - r3,
            y2o = y2i + r3;  // outer rectangle (partially covered cubes)
        float z1 = S(e.x, e.y)->floor + float(e.attr1) / ENTSCALE10,
              z2 = z1 + float(e.attr4) / ENTSCALE5;
        bool bigenough = true, nodelete = false;
        for (int xx = x1o; xx <= x2o; xx++)
          for (int yy = y1o; yy <= y2o;
               yy++)  // loop over outer rectangle to check, if a clip has the
                      // required height
          {
            if (OUTBORD(xx, yy) || SOLID(S(xx, yy))) continue;
            bool inner = xx >= x1i && xx <= x2i && yy >= y1i &&
                         yy <= y2i;  // flag: xx|yy is inner rectangle
            sqr *s[4] = {S(xx, yy), S(xx + 1, yy), S(xx, yy + 1),
                         S(xx + 1, yy + 1)};
            int vdeltamax = 0;
            loopj(4) if (s[j]->vdelta > vdeltamax) vdeltamax = s[j]->vdelta;
            int floor =
                    s[0]->floor - (s[0]->type == FHF ? (vdeltamax + 3) / 4 : 0),
                ceil =
                    s[0]->ceil - (s[0]->type == CHF ? (vdeltamax + 3) / 4 : 0);
            bool alreadytagged = (s[0]->tag & (TAGCLIP | clipmask)) != 0;
            if ((z1 - floor > allowedspace || ceil - z2 > allowedspace) &&
                !alreadytagged)
              bigenough = false;
            if (!inner && !alreadytagged)
              nodelete = true;  // fractional part of the clip would not be
                                // covered: entity can not be deleted
          }
        if (bigenough) {
          for (int xx = x1i; xx <= x2i; xx++)
            for (int yy = y1i; yy <= y2i;
                 yy++)  // only convert inner rectangle to tag clips
            {
              if (!OUTBORD(xx, yy) && !SOLID(S(xx, yy)))
                S(xx, yy)->tag |= clipmask;
            }
          if (!nodelete) {
            deleted_ents.add(e);
            e.type = NOTUSED;  // only delete entity, if it is now fully covered
                               // in tag clips
            thisrun++;
          }
        }
  ...
  loopi(ssize) loopj(ssize) {
    sqr *s = S(i, j);
    if (s->tag & TAGCLIP) s->tag &= ~TAGPLCLIP;
  }
...
void reseteditor()  // reset only stuff that would cause trouble editing the
                    // next map (don't reset selections, for example)
{
