...
char *editinfo ()
{
    ...
    if (e >= 0)
    {
        ...
        formatstring (info) ("%s entity: %s (%s)", pinnedclosestent ? "\fs\f3pinned\fr" : "closest",
        ...
        else if (t == SOUND)
        {
            ...
            slotinfo = "";
        if (*slotinfo && (hideeditslotinfo == 0 || (hideeditslotinfo == 1 && slotinfo == unassigned)))
    ...
    if (selset ())
    {
        ...
        concatformatstring (info, "\n\tno selection");
    sqr *s;
    if (!OUTBORD (cx, cy) && (s = S (cx, cy)) && !editfocusdetails (s) && !SOLID (s) && s->tag)
...
void cursorupdate () // called every frame from hud
{
    ...
    editaxis = fabs (camera1->pitch) > 60 ? "\161\15\15"[flrceil] :
    ...
    if (sparkletime)
        lastsparkle = lastmillis - (lastmillis % 20); // clip adding sparklies at 50 fps
    ...
    if (showgrid && !blankouthud)
    {
        ...
        if (!SOLID (s))
        {
            ...
            box (b, ih, sheight (s, SWS (s, 1, 0, sfactor), z),
                 sheight (s, SWS (s, 1, 1, sfactor), z), sheight (s, SWS (s, 0, 1, sfactor), z));
    ...
    if (!showtagclipfocus && showtagclips && !blankouthud)
    {
        ...
        loopv (tagclipcubes) // all non-solid & have clips
        {
            ...
            loopk (4) if ((clipped[k] == clipped[k + 1]) && !(clipped[k] && clipped[k + 5]))
            line (x + xo[k + 1], y + yo[k + 1], h, x + xo[k + 1], y + yo[k + 1], c);
            for (; h < c; h++)
            {
                int k = (h + x + y) & 3;
                if (k < 2 && !clipped[k])
            ...
            if (sparkletime && tagnumfull)
                particle_cube (s->tag & TAGCLIP ? PART_EPICKUP : PART_EMODEL, tagnumfull, 30, x, y);
        }
    }
...
void gotoposition (char *x, char *y, char *z, char *yaw, char *pitch)
{
    ...
    player1->resetinterp ();
    defformatstring (res) ("%s %s %s %d %d", floatstr (player1->o.x), floatstr (player1->o.y),
                           floatstr (player1->o.z), int(player1->yaw), int(player1->pitch));
    result (res);
}
...
void netblockpaste (const block &b,
                    int bx,
                    int by,
...
{
    if (bxs < 0 || bys < 0 || bxs > ssize || bys > ssize || bxs * bys > MAXNETBLOCKSQR
        || OUTBORD (bx, by) || OUTBORD (bx + bxs - 1, by + bys - 1))
        return;
...
void restoreeditundo (ucharbuf &q)
{
    ...
    while (!q.overread () && (type = getuint (q)))
    {
        ...
        if (worldiodebug) switch (type)
            ...
            case 10:
            case 20:
                clientlogf ("  got %sdo x %d, y %d, xs %d, ys %d, remaining %d, overread %d",
    ...
    if (undos.length () || redos.length ())
        conoutf ("restored editing history: %d undos and %d redos", undos.length (), redos.length ());
}
...
#ifdef _DEBUG
    if (worldiodebug)
        clientlogf ("    compressing redo/undo x %d, y %d, xs %d, ys %d, compressed length "
                    "%d, cubes %d",
...
#ifdef _DEBUG
        if (worldiodebug)
            clientlogf ("  written undo x %d, y %d, xs %d, ys %d, compressed length %d",
...
#ifdef _DEBUG
        if (worldiodebug)
            clientlogf ("  written redo x %d, y %d, xs %d, ys %d, compressed length %d",
...
void paste ()
{
    ...
    loopv (sels)
    {
        ...
        loopvj (copybuffers)
        {
            ...
            sel.x = selx + dx;
            sel.y = sely + dy;
            if (!correctsel (sel) || sel.xs != copyblock->xs || sel.ys != copyblock->ys)
            {
...
void renderhudtexturepreview (int slot, int pos, bool highlight)
{
    ...
    int hs = (highlight ? 0 : (FONTH / 3) * 2), bs = VIRTW / 8 - hs, border = FONTH / (highlight ? 2 : 4),
        x = 2 * VIRTW - VIRTW / 8 - FONTH + hs / 2, y = VIRTH * (6 - pos) / 4, xs, ys;
...
void edittype (int type)
{
    ...
    loopv (sels)
    {
        block &sel = sels[i];
        if (type == CORNER
            && (sel.xs != sel.ys || (sel.xs != 1 && sel.xs != 2 && sel.xs != 4 && sel.xs != 8 && sel.xs != 16)
...
{
    loopselxy (sel, s->vdelta = sel.xs > sel.ys ?
...
void edittagxy (int orv, int andv, block &sel) { loopselxy (sel, s->tag = (s->tag & andv) | orv); }
...
void movemap (int *xop, int *yop, int *zop) // move whole map
{
    ...
    if (!worldbordercheck (MINBORD + max (-xo, 0), MINBORD + max (xo, 0), MINBORD + max (-yo, 0),
                           MINBORD + max (yo, 0), max (zo, 0), max (-zo, 0)))
    {
...
#undef GETA
            if (memcmp (s, &so, sizeof (sqr)))
            {
                ...
                {
                    if (!didwarn)
                        conoutf ("\f3selectionwalk: selected area %dx%d too big for multiplayer "
...
{                             // (hardcoded factor ENTSCALE10 for attr1 and ENTSCALE5 for attr2-4)
    ...
    do
    {
        ...
        {
            entity &e = ents[i];
            if ((e.type == CLIP || e.type == PLCLIP) && e.attr2 / ENTSCALE5 && e.attr3 / ENTSCALE5
                && e.attr4 / ENTSCALE5)
            {
                ...
                for (int xx = x1o; xx <= x2o; xx++)
                    for (int yy = y1o; yy <= y2o; yy++) // loop over outer rectangle to check, if a
                                                        // clip has the required height
                    {
                        ...
                        if ((z1 - floor > allowedspace || ceil - z2 > allowedspace) && !alreadytagged)
                            bigenough = false;
                ...
                if (bigenough)
                {
                    for (int xx = x1i; xx <= x2i; xx++)
                        for (int yy = y1i; yy <= y2i; yy++) // only convert inner rectangle to tag clips
                        {
