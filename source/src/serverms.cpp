...
ENetSocket connectmaster() {
  ...
  if (scl.maxclients > MAXCL) {
    logline(ACLOG_WARNING, "maxclient exceeded: cannot register");
  ...
  if (sock != ENET_SOCKET_NULL && serveraddress.host != ENET_HOST_ANY &&
      enet_socket_bind(sock, &serveraddress) < 0) {
    enet_socket_destroy(sock);
    sock = ENET_SOCKET_NULL;
  }
  if (sock == ENET_SOCKET_NULL ||
      connectwithtimeout(sock, mastername, masteraddress) < 0) {
    logline(ACLOG_WARNING, sock == ENET_SOCKET_NULL ? "could not open socket"
                                                    : "could not connect");
    return ENET_SOCKET_NULL;
  }
...
void flushmasterinput() {
  if (masterin.length() >= masterin.capacity()) masterin.reserve(4096);
...
// send alive signal to masterserver after 40 minutes of uptime and if currently
// in intermission (so theoretically <= 1 hour)
// TODO?: implement a thread to drop this "only in intermission" business, we'll
// need it once AUTH gets active!
static inline void updatemasterserver(int millis, int port) {
  if (!lastupdatemaster || ((millis - lastupdatemaster) > 40 * 60 * 1000 &&
    ...
    filtertext(servername, global_name, FTXT__GLOBALNAME, 20);
    if (mastername[0])
      requestmasterf("regserv %d %s %d\n", port,
                     servername[0] ? servername : "noname", AC_VERSION);
...
void serverms(int mode, int numplayers, int minremain, char *smapname,
              int millis, const ENetAddress &localaddr, int *mnum, int *msend,
              int *mrec, int *cnum, int *csend, int *crec,
  ...
  loopi(2) {
      ...
      switch (extcmd) {
        ...
        case EXT_PLAYERSTATS:  // playerstats
        {
          ...
          *csend += int(buf.dataLength = len + po.length());
          enet_socket_send(pongsock, &addr, &buf,
                           1);  // send all available player ids
          po.len = bpos;
    ...
    if (std)
      *msend += (int)buf.dataLength;
    else
      *csend += (int)buf.dataLength;
  }

  if (mastersock != ENET_SOCKET_NULL &&
      ENET_SOCKETSET_CHECK(sockset, mastersock))
    flushmasterinput();
}

// this function should be made better, because it is used just ONCE (no need of
// so much parameters)
void servermsinit(const char *master, const char *ip, int infoport,
                  bool listen) {
  ...
  if (listen) {
    ...
    if (pongsock != ENET_SOCKET_NULL &&
        enet_socket_bind(pongsock, &address) < 0) {
    ...
    if (lansock != ENET_SOCKET_NULL &&
        (enet_socket_set_option(lansock, ENET_SOCKOPT_REUSEADDR, 1) < 0 ||
         enet_socket_bind(lansock, &address) < 0)) {
      enet_socket_destroy(lansock);
      lansock = ENET_SOCKET_NULL;
    }
    if (lansock == ENET_SOCKET_NULL)
      logline(ACLOG_WARNING, "could not create LAN server info socket");
    else
