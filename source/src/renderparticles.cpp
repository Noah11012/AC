...
= { "SPARK",          "SMOKE",       "ECLOSEST",   "BLOOD",  "DEMOTRACK", "FIREBALL",
    "SHOTLINE",       "BULLETHOLE",  "BLOODSTAIN", "SCORCH", // 0..9
    "HUDMUZZLEFLASH", "MUZZLEFLASH", "ELIGHT",     "ESPAWN", "EAMMO",     "EPICKUP",
    "EMODEL",         "ECARROT",     "ELADDER",    "EFLAG",  "" }; // 10..19
    ...
    for (int i = MAXPARTYPES - 1; i >= 0; i--)
        if (typemask & (1 << parttypes[i].type) && parlist[i])
        {
            ...
            for (particle *p, **pp = &parlist[i]; (p = *pp);)
            {
                ...
                case PT_PART:
                    glTexCoord2i (0, 1);
                    glVertex3f (p->o.x + (-camright.x + camup.x) * sz, p->o.y + (-camright.y + camup.y) * sz,
                ...
                case PT_DECAL:
                {
                    ...
                    glColor4f (s->r / 127.5f, s->g / 127.5f, s->b / 127.5,
                               max (0.0f, min ((p->millis + p->fade - lastmillis) / 1000.0f, 0.7f)));
                    ...
                    glVertex3f (p->o.x + (-dx.x + dy.x) * pt.sz, p->o.y + (-dx.y + dy.y) * pt.sz,
                ...
                case PT_BULLETHOLE:
                {
                    ...
                    glTexCoord2f (0.5f + 0.5f * (-dx[tx] + dy[tx]), 0.5f + 0.5f * (-dx[ty] + dy[ty]));
                    glVertex3f (p->o.x + (-dx.x + dy.x) * pt.sz, p->o.y + (-dx.y + dy.y) * pt.sz,
                                p->o.z + (-dx.z + dy.z) * pt.sz);
                    glTexCoord2f (0.5f + 0.5f * (dx[tx] + dy[tx]), 0.5f + 0.5f * (dx[ty] + dy[ty]));
                    glVertex3f (p->o.x + (dx.x + dy.x) * pt.sz, p->o.y + (dx.y + dy.y) * pt.sz,
                ...
                case PT_BLOOD:
                {
                    ...
                    glVertex3f (p->o.x + (-camright.x + camup.x) * sz, p->o.y + (-camright.y + camup.y) * sz,
                                p->o.z + (-camright.z + camup.z) * sz);
                    glTexCoord2f (tx + tsz * tc[(o + 1) % 4][0], ty + tsz * tc[(o + 1) % 4][1]);
                    glVertex3f (p->o.x + (camright.x + camup.x) * sz, p->o.y + (camright.y + camup.y) * sz,
                                p->o.z + (camright.z + camup.z) * sz);
                    glTexCoord2f (tx + tsz * tc[(o + 2) % 4][0], ty + tsz * tc[(o + 2) % 4][1]);
                    glVertex3f (p->o.x + (camright.x - camup.x) * sz, p->o.y + (camright.y - camup.y) * sz,
                                p->o.z + (camright.z - camup.z) * sz);
                    glTexCoord2f (tx + tsz * tc[(o + 3) % 4][0], ty + tsz * tc[(o + 3) % 4][1]);
                    glVertex3f (p->o.x + (-camright.x - camup.x) * sz, p->o.y + (-camright.y - camup.y) * sz,
                                p->o.z + (-camright.z - camup.z) * sz);
                    xtraverts += 4;
                    break;
...
void particle_cube (int type, int num, int fade, int x, int y)
{
    ...
    loopi (num * fc / 12)
    {
        vec p (float(rnd (100)) / 100.0 + x, float(rnd (100)) / 100.0 + y, float(rnd (100) * fc) / 100 + floor),
        d (0, 0, 0);
        newparticle (p, d, rnd (fade * 3), type);
    }
}
...
bool addbullethole (dynent *d, const vec &from, const vec &to, float radius, bool noisy)
{
    ...
    if (surface.iszero () || (radius > 0 && (dist < mag - radius || dist > mag + radius)))
        return false;
    vec o (from);
    o.add (ray.mul (dist));
...
void addshotline (dynent *pl, const vec &from, const vec &to)
{
    ...
    bool fx = shotlinettl > 0 && (player1->isspectating () || !multiplayer (nullptr)); // allow fx only in spect mode and locally
    ...
    if (fd <= (float)bulletairsoundsourcerad || td <= (float)bulletairsounddestrad)
