...
static void subdivide(int depth, int face) {
  ...
  loopi(3) {
    ...
    hemiverts[vert] = vec(hemiverts[idx[i]])
                          .add(hemiverts[idx[(i + 1) % 3]])
                          .normalize();  // push on to unit sphere
    idx[3 + i] = vert;
...
static void inithemisphere(int hres, int depth) {
  ...
  hemiverts[heminumverts++] =
      vec(0.0f, 0.0f, 1.0f);  // build initial 'hres' sided pyramid
  loopi(hres) {
...
GLuint createexpmodtex(int size, float minval) {
  ...
  loop(y, size) loop(x, size) {
    float dx = 2 * float(x) / (size - 1) - 1,
          dy = 2 * float(y) / (size - 1) - 1;
...
void drawexplosion(bool inside, float r, float g, float b, float a) {
  ...
  loopi(!reflecting && inside ? 2 : 1) {
    ...
    if (inside) {
      if (!reflecting) {
        glCullFace(GL_BACK);
        glDrawElements(GL_TRIANGLES, heminumindices, GL_UNSIGNED_SHORT,
                       hemiindices);
        glCullFace(GL_FRONT);
      }
      glScalef(1, 1, -1);
...
void cleanupparticles() {
  loopi(2) if (expmodtex[i]) {
    glDeleteTextures(1, &expmodtex[i]);
    expmodtex[i] = 0;
  }
}
...
const char *particletypenames[MAXPARTYPES + 1] = {
    "SPARK",          "SMOKE",       "ECLOSEST", "BLOOD",
    "DEMOTRACK",      "FIREBALL",    "SHOTLINE", "BULLETHOLE",
    "BLOODSTAIN",     "SCORCH",  // 0..9
    "HUDMUZZLEFLASH", "MUZZLEFLASH", "ELIGHT",   "ESPAWN",
    "EAMMO",          "EPICKUP",     "EMODEL",   "ECARROT",
    "ELADDER",        "EFLAG",       ""};  // 10..19
...
static struct parttype {
  ...
  float sz;
    ...
    {PT_PART, 1.0f, 1.0f, 1.0f, 20, 0,
     0.08f},  // white: edit mode ent type : light
    {PT_PART, 0.0f, 1.0f, 0.0f, 20, 0,
     0.08f},  // green: edit mode ent type : spawn
    {PT_PART, 1.0f, 0.0f, 0.0f, 20, 0,
     0.08f},  // red: edit mode ent type : ammo
    {PT_PART, 1.0f, 1.0f, 0.0f, 20, 0,
     0.08f},  // yellow: edit mode ent type : pickup
    {PT_PART, 1.0f, 0.0f, 1.0f, 20, 0,
     0.08f},  // magenta: edit mode ent type : model, sound
    {PT_PART, 1.0f, 0.5f, 0.2f, 20, 0,
     0.08f},  // orange: edit mode ent type : "carrot"
    {PT_PART, 0.5f, 0.5f, 0.5f, 20, 0,
     0.08f},  // grey: edit mode ent type : ladder, (pl)clip
    {PT_PART, 0.0f, 1.0f, 1.0f, 20, 0,
     0.08f},  // turquoise: edit mode ent type : CTF-flag
};
...
void render_particles(int time, int typemask) {
  bool rendered = false;
  for (int i = MAXPARTYPES - 1; i >= 0; i--)
    if (typemask & (1 << parttypes[i].type) && parlist[i]) {
      if (!rendered) {
        rendered = true;
      ...
      for (particle *p, **pp = &parlist[i]; (p = *pp);) {
        switch (pt.type) {
          case PT_HUDFLASH:
          case PT_FLASH: {
            vec corners[4] = {vec(-camright.x + camup.x, -camright.y + camup.y,
                                  -camright.z + camup.z),
                              vec(camright.x + camup.x, camright.y + camup.y,
                                  camright.z + camup.z),
                              vec(camright.x - camup.x, camright.y - camup.y,
                                  camright.z - camup.z),
                              vec(-camright.x - camup.x, -camright.y - camup.y,
          ...
          case PT_PART:
            glTexCoord2i(0, 1);
            glVertex3f(p->o.x + (-camright.x + camup.x) * sz,
                       p->o.y + (-camright.y + camup.y) * sz,
                       p->o.z + (-camright.z + camup.z) * sz);
            glTexCoord2i(1, 1);
            glVertex3f(p->o.x + (camright.x + camup.x) * sz,
                       p->o.y + (camright.y + camup.y) * sz,
                       p->o.z + (camright.z + camup.z) * sz);
          ...
          case PT_FIREBALL: {
            sz = 1.0f + (pt.sz - 1.0f) * min(p->fade, lastmillis - p->millis) /
                            p->fade;
            glPushMatrix();
            glTranslatef(p->o.x, p->o.y, p->o.z);
            bool inside = p->o.dist(camera1->o) <=
                          sz * 1.25f;  // 1.25 is max wobble scale
            vec oc(p->o);
            oc.sub(camera1->o);
            if (reflecting && !refracting) oc.z = p->o.z - (waterlevel - 0.3f);
            glRotatef(
                inside ? camera1->yaw - 180 : atan2(oc.y, oc.x) / RAD - 90, 0,
                0, 1);
            glRotatef(
                (inside ? camera1->pitch : asin(oc.z / oc.magnitude()) / RAD) -
                    90,
                1, 0, 0);
          ...
          case PT_DECAL: {
            ...
            glColor4f(
                s->r / 127.5f, s->g / 127.5f, s->b / 127.5,
                max(0.0f,
                    min((p->millis + p->fade - lastmillis) / 1000.0f, 0.7f)));
            ...
            glTexCoord2i(tc[o % 4][0], tc[o % 4][1]);
            glVertex3f(p->o.x + (-dx.x + dy.x) * pt.sz,
                       p->o.y + (-dx.y + dy.y) * pt.sz,
                       p->o.z + (-dx.z + dy.z) * pt.sz);
            glTexCoord2i(tc[(o + 1) % 4][0], tc[(o + 1) % 4][1]);
            glVertex3f(p->o.x + (dx.x + dy.x) * pt.sz,
                       p->o.y + (dx.y + dy.y) * pt.sz,
                       p->o.z + (dx.z + dy.z) * pt.sz);
            glTexCoord2i(tc[(o + 2) % 4][0], tc[(o + 2) % 4][1]);
            glVertex3f(p->o.x + (dx.x - dy.x) * pt.sz,
                       p->o.y + (dx.y - dy.y) * pt.sz,
                       p->o.z + (dx.z - dy.z) * pt.sz);
            glTexCoord2i(tc[(o + 3) % 4][0], tc[(o + 3) % 4][1]);
            glVertex3f(p->o.x + (-dx.x - dy.x) * pt.sz,
                       p->o.y + (-dx.y - dy.y) * pt.sz,
                       p->o.z + (-dx.z - dy.z) * pt.sz);
            xtraverts += 4;
          ...
          case PT_BULLETHOLE: {
            float blend = max(
                0.0f, min((p->millis + p->fade - lastmillis) / 1000.0f, 1.0f));
            ...
            glTexCoord2f(0.5f + 0.5f * (-dx[tx] + dy[tx]),
                         0.5f + 0.5f * (-dx[ty] + dy[ty]));
            glVertex3f(p->o.x + (-dx.x + dy.x) * pt.sz,
                       p->o.y + (-dx.y + dy.y) * pt.sz,
                       p->o.z + (-dx.z + dy.z) * pt.sz);
            glTexCoord2f(0.5f + 0.5f * (dx[tx] + dy[tx]),
          ...
          case PT_BLOOD: {
            ...
            static const int tc[4][2] = {{0, 1}, {1, 1}, {1, 0}, {0, 0}};
            glTexCoord2f(tx + tsz * tc[o % 4][0], ty + tsz * tc[o % 4][1]);
            glVertex3f(p->o.x + (-camright.x + camup.x) * sz,
                       p->o.y + (-camright.y + camup.y) * sz,
                       p->o.z + (-camright.z + camup.z) * sz);
            glTexCoord2f(tx + tsz * tc[(o + 1) % 4][0],
                         ty + tsz * tc[(o + 1) % 4][1]);
            glVertex3f(p->o.x + (camright.x + camup.x) * sz,
                       p->o.y + (camright.y + camup.y) * sz,
                       p->o.z + (camright.z + camup.z) * sz);
            glTexCoord2f(tx + tsz * tc[(o + 2) % 4][0],
                         ty + tsz * tc[(o + 2) % 4][1]);
            glVertex3f(p->o.x + (camright.x - camup.x) * sz,
                       p->o.y + (camright.y - camup.y) * sz,
                       p->o.z + (camright.z - camup.z) * sz);
            glTexCoord2f(tx + tsz * tc[(o + 3) % 4][0],
                         ty + tsz * tc[(o + 3) % 4][1]);
            glVertex3f(p->o.x + (-camright.x - camup.x) * sz,
                       p->o.y + (-camright.y - camup.y) * sz,
                       p->o.z + (-camright.z - camup.z) * sz);
          ...
          case PT_STAIN: {
            float blend = max(
                0.0f, min((p->millis + p->fade - lastmillis) / 1000.0f, 1.0f));
            glColor3f(blend * (1 - pt.r), blend * (1 - pt.g),
                      blend * (1 - pt.b));
            int n = detrnd((size_t)p, 4), o = detrnd((size_t)p, 11);
            float tx = 0.5f * (n & 1), ty = 0.5f * ((n >> 1) & 1), tsz = 0.5f;
            static const int tc[4][2] = {{0, 1}, {1, 1}, {1, 0}, {0, 0}};
        ...
        if (time < 0)
          ...
          *pp = p->next;
          ...
          if (pt.gr)
            p->o.z -=
                ((lastmillis - p->millis) / 3.0f) * time / (pt.gr * 10000);
          ...
          if (pt.type == PT_BLOOD) {
            ...
            if (s->type == SPACE && p->o.z <= s->floor) {
              *pp = p->next;
              p->next = parempty;
              parempty = p;
              newparticle(vec(p->o.x, p->o.y, s->floor + 0.005f), vec(0, 0, 1),
                          bloodttl, 8);
              continue;
            }
          }
...
void particle_emit(int type, int *args, int basetime, int seed, const vec &p) {
  ...
  if (pt.type == PT_FIREBALL)
    particle_fireball(type, p);
  else if (pt.type == PT_FLASH || pt.type == PT_HUDFLASH) {
    if (lastmillis - basetime < args[0])
      particle_flash(type, args[1] > 0 ? args[1] / 100.0f : 1.0f, seed % 360,
                     p);
  } else
    particle_splash(type, args[0], args[1], p);
}
...
void particle_cube(int type, int num, int fade, int x, int y) {
  ...
  loopi(num * fc / 12) {
    vec p(float(rnd(100)) / 100.0 + x, float(rnd(100)) / 100.0 + y,
          float(rnd(100) * fc) / 100 + floor),
        d(0, 0, 0);
    newparticle(p, d, rnd(fade * 3), type);
  }
}
...
void particle_fireball(int type, const vec &o) {
  newparticle(o, vec(0, 0, 0), (int)((parttypes[type].sz - 1.0f) * 100.0f),
              type);
}
...
bool addbullethole(dynent *d, const vec &from, const vec &to, float radius,
                   bool noisy) {
  ...
  if (surface.iszero() ||
      (radius > 0 && (dist < mag - radius || dist > mag + radius)))
    return false;
  vec o(from);
  o.add(ray.mul(dist));
  o.add(vec(surface).normalize().mul(0.01f));
  newparticle(o, surface, bulletholettl, 7);
  if (noisy && bulletbouncesound && bulletbouncesoundrad && d != player1 &&
      o.dist(camera1->o) <= bulletbouncesoundrad) {
    audiomgr.playsound(o.z < waterlevel ? S_BULLETWATERHIT : S_BULLETHIT, &o,
                       SP_LOW);
  }
  return true;
}
...
void addshotline(dynent *pl, const vec &from, const vec &to) {
  ...
  if (fd <= (float)bulletairsoundsourcerad ||
      td <= (float)bulletairsounddestrad)
    return;  // ignore nearly fired/detonated shots
  vec soundpos(unitv);
  soundpos.mul(fd / (fd + td) * dist);
  soundpos.add(from);
  if (!bulletairsound || soundpos.dist(camera1->o) > bulletairsoundrad)
    return;  // outside player radius
