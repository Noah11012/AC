...
void neterr (const char *s)
{
    ...
    conoutf ("\f3wrote a network error report to packetlog.txt, please post this file "
                    ...
                    int revision) // forced map change from the server
{
    ...
    { /*conoutf("SANITY drop from EDITING");*/
        toggleedit (true);
    ...
    if (download > 0)
    {
        ...
        {
            if (loaded && !revmatch)
                conoutf ("\f3demo was recorded on map revision %d, you have map revision %d",
        ...
        else
        {
            ...
            if (autogetmap || getnewrev)
            {
                ...
                {
                    defformatstring (msg) ("map '%s' revision: local %d, provided by server %d",
                                           name, hdr.maprevision, revision);
            ...
            else
            {
                if (!loaded || download < 10)
                    ...
                    conoutf ("\"getmap\" to download a %s version of the current map from the "
                             "server",
...
#define DIST_ATT \
    ((e1.z + float(e1.attr1) / entscale[e1.type][0]) - (e2.z + float(e2.attr1) / entscale[e2.type][0]))
            float r2 = DIST (x) * DIST (x) + DIST (y) * DIST (y) + DIST_ATT * DIST_ATT;
#undef DIST_ATT
#undef DIST
            if (r2 == 0.0f)
            {
                conoutf ("\f3MAP CHECK FAIL: Items too close %s %s (%hd,%hd)", entnames[e1.type],
                         entnames[e2.type], e1.x, e1.y);
                item_fail = true;
                break;
            }
            r2 = 1 / r2;
        ...
        if (hdensity > 0.5f)
        {
            conoutf ("\f3MAP CHECK FAIL: Items too close %s %.2f (%hd,%hd)", entnames[e1.type],
                     hdensity, e1.x, e1.y);
            item_fail = true;
            break;
        }
...
#undef LOGTHISSWITCH
    ...
    map_quality = (!item_fail && F2F > MINFF && MA < MAXMAREA && Mh < MAXMHEIGHT && Hhits < MAXHHITS) ?
                  MAP_IS_GOOD :
                  MAP_IS_BAD;
...
void onChangeVote (int mod, int id) { exechook (HOOK_SP_MP, "onChangeVote", "%d %d", mod, id); }
...
void parsemessages (int cn, playerent *d, ucharbuf &p, bool demo = false)
{
    ...
    while (p.remaining ())
    {
        ...
        case SV_SERVINFO: // welcome message from the server
        {
            ...
            {
                conoutf ("\f3incompatible game protocol (local protocol: %d :: server "
                         "protocol: %d)",
                         CUR_PROTOCOL_VERSION, prot);
                conoutf ("\f3if this occurs a lot, obtain an upgrade from "
                         "\f1http://assault.cubers.net");
                if (watchingdemo)
                    conoutf ("breaking loop : \f3this demo is using a different protocol\f5 "
        ...
        case SV_TEAMTEXT:
        {
            ...
            if (!d) break;
            if (d->ignored)
                clientlogf ("ignored: %s%s %s", colorname (d), type == SV_TEAMTEXT ? ":" : "", text);
            else
            {
                if (m_teammode || team_isspect (player1->team))
                    conoutf (type == SV_TEAMTEXTME ? "\f1%s %s" : "%s:\f1 %s", colorname (d), highlight (text));
                else
                    conoutf (type == SV_TEAMTEXTME ? "\f0%s %s" : "%s:\f0 %s", colorname (d), highlight (text));
            }
            break;
        }
        ...
        case SV_TEXT:
            ...
            else if (d)
            {
                ...
                if (d->ignored && d->clientrole != CR_ADMIN)
                    clientlogf ("ignored: %s%s %s", colorname (d), type == SV_TEXT ? ":" : "", text);
                else
                    conoutf (type == SV_TEXTME ? "\f0%s %s" : "%s:\f0 %s", colorname (d), highlight (text));
            }
            else
                return;
            break;
        ...
        case SV_DAMAGE:
        {
            int tcn = getint (p), acn = getint (p), gun = getint (p), damage = getint (p),
                armour = getint (p), health = getint (p);
        ...
        case SV_RESUME:
        {
            loopi (MAXCLIENTS)
            {
                ...
                int state = getint (p), lifesequence = getint (p), primary = getint (p),
                    ...
                    deaths = getint (p), health = getint (p), armour = getint (p), points = getint (p);
                int teamkills = 0;
        ...
        case SV_EDITBLOCK:
        {
            int bx = getuint (p), by = getuint (p), bxs = getuint (p), bys = getuint (p),
                light = getuint (p);
        ...
        case SV_PONG:
        {
            ...
            addmsg (SV_CLIENTPING, "i",
                    player1->ping = max (0, (player1->ping * 5 + totalmillis - millis) / 6));
            break;
        }
            ...
            if (cl >= 0 && r >= 0)
            {
                ...
                if (pl)
                {
                    ...
                    if (pl->name[0])
                    {
                        ...
                        if (pl == player1)
                            conoutf ("you claimed %s status", r == CR_ADMIN ? "admin" : "master");
                        else
                            conoutf ("%s claimed %s status", colorname (pl), r == CR_ADMIN ? "admin" : "master");
                    }
                }
            }
            break;
            ...
            if (d)
            {
                ...
                if (m_teammode || team_isspect (fnt))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            case FTR_AUTOTEAM:
                                if (watchingdemo || team_isspect (player1->team))
                                    conoutf ("the server forced %s to team %s", colorname (d), nts);
                                else
                        ...
                        if (you && !team_isspect (d->team) && team_isspect (fnt) && d->state == CS_DEAD)
                            spectatemode (SM_FLY);
                    }
                }
                else if (d->team != fnt && ftr == FTR_PLAYERWISH && !team_isactive (d->team))
...
void setDemoTimestampFormat (char *fmt)
{
    ...
    if (fmt && fmt[0] != '\0')
    {
        ...
        copystring (demotimestampformat,
...
void receivefile (uchar *data, int len)
{
    ...
    case SV_RECVMAP:
    {
        ...
        if (MAXMAPSENDSIZE < mapsize + cfgsizegz || cfgsize > MAXCFGFILESIZE)
        { // sam's suggestion
