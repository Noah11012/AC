...
void neterr(const char *s) {
  ...
  conoutf(
      "\f3wrote a network error report to packetlog.txt, please post this file "
      "to the bugtracker now!");
...
bool changemapserv(char *name, int mode, int download,
                   int revision)  // forced map change from the server
{
  ...
  if (m_coop) {
    ...
    return true;
             ...
             CS_EDITING) { /*conoutf("SANITY drop from EDITING");*/
    toggleedit(true);
  }  // fix stuck-in-editmode bug
  ...
  if (download > 0) {
    ...
    if (watchingdemo) {
      if (loaded && !revmatch)
        conoutf(
            "\f3demo was recorded on map revision %d, you have map revision %d",
            revision, hdr.maprevision);
    } else {
      ...
      if (autogetmap || getnewrev) {
        if (!loaded || getnewrev)
          getmap();  // no need to ask
        else {
          defformatstring(msg)(
              "map '%s' revision: local %d, provided by server %d", name,
              hdr.maprevision, revision);
          alias("__getmaprevisions", msg, true);
          showmenu("getmap");
        }
      } else {
        if (!loaded || download < 10)
          conoutf("\"getmap\" to download the current map from the server");
        else
          conoutf(
              "\"getmap\" to download a %s version of the current map from the "
              "server",
              revision == 0 ? "different"
                            : (revision > hdr.maprevision ? "newer" : "older"));
      }
    }
  } else
    return true;
      ...
      case SV_POSC: {
        ...
        loopi(3) {
          ...
          if (!dr)
            ...
            d->vel.v[i] = dr * 0.05f + d->vel.v[i] * 0.95f;
          d->vel.v[i] += vel.v[i];
        ...
        if (d->state == CS_DEAD) {
          d->resetinterp();
          d->smoothmillis = 0;
        } else if (smoothmove && d->smoothmillis >= 0 &&
                   oldpos.dist(d->o) < smoothdist) {
          d->newpos = d->o;
        ...
        if (player1->isspectating() && player1->spectatemode == SM_NONE)
          togglespect();
        break;
      }
...
/// TODO: many functions and variables are redundant between client and
/// server... someone should redo the entire server code and unify client and
/// server.
...
#define DIST(x) (f0.pos.x - f1.pos.x)
    F2F = 1000;  //(!numflagspawn[0] || !numflagspawn[1]) ? 1000 * MINFF :
  ...
  loopv(ents) {
    ...
    loopvj(ents) {
      ...
      if (e2.type < I_CLIPS || e2.type > I_AKIMBO || i == j) continue;
        // only I_CLIPS, I_AMMO, I_GRENADE, I_HEALTH, I_HELMET, I_ARMOUR,
        // I_AKIMBO
#define DIST(x) (e1.x - e2.x)
#define DIST_ATT                                     \
  ((e1.z + float(e1.attr1) / entscale[e1.type][0]) - \
   (e2.z + float(e2.attr1) / entscale[e2.type][0]))
      float r2 = DIST(x) * DIST(x) + DIST(y) * DIST(y) + DIST_ATT * DIST_ATT;
#undef DIST_ATT
#undef DIST
      if (r2 == 0.0f) {
        conoutf("\f3MAP CHECK FAIL: Items too close %s %s (%hd,%hd)",
                entnames[e1.type], entnames[e2.type], e1.x, e1.y);
        item_fail = true;
        break;
      }
      r2 = 1 / r2;
    ...
    if (hdensity > 0.5f) {
      conoutf("\f3MAP CHECK FAIL: Items too close %s %.2f (%hd,%hd)",
              entnames[e1.type], hdensity, e1.x, e1.y);
      item_fail = true;
      break;
    }
    switch (e1.type) {
#define LOGTHISSWITCH(X)                                            \
  if (density > X) {                                                \
    conoutf("\f3MAP CHECK FAIL: Items too close %s %.2f (%hd,%hd)", \
            entnames[e1.type], density, e1.x, e1.y);                \
    item_fail = true;                                               \
    break;                                                          \
  }
      case I_CLIPS:
...
#undef LOGTHISSWITCH
  ...
  map_quality = (!item_fail && F2F > MINFF && MA < MAXMAREA &&
                 Mh < MAXMHEIGHT && Hhits < MAXHHITS)
                    ? MAP_IS_GOOD
                    : MAP_IS_BAD;
  if ((!connected || gamemode == GMODE_COOPEDIT) && map_quality == MAP_IS_BAD)
    map_quality = MAP_IS_EDITABLE;
...
char *strcaps(const char *s, bool on) {
  ...
  char *o = r;
  if (on)
    while (*s && o < &r[sizeof(r) - 1]) *o++ = toupper(*s++);
  else
    while (*s && o < &r[sizeof(r) - 1]) *o++ = tolower(*s++);
  *o = '\0';
  return r;
}
...
void showhudextras(char hudextras, char value) {
  ...
  switch (value) {
    ...
    case HE_COMBO5: {
      ...
      if (n > 3)
        outf("\f3%s", strcaps("monster combo!!!",
                              caps));  // I expect to never see this one
      else if (!n)
        outf("\f5%s", strcaps("combo", caps));
      else
        outf("\f5%s x%d", strcaps("multi combo", caps), n + 1);
      break;
    }
    case HE_TEAMWORK:
      outf("\f5%s", strcaps("teamwork done", caps));
      break;
    case HE_FLAGDEFENDED:
      outf("\f5%s", strcaps("you defended the flag", caps));
      break;
    case HE_FLAGCOVERED:
      outf("\f5%s", strcaps("you covered the flag", caps));
      break;
    case HE_COVER:
      if (teamworkid >= 0) {
    ...
    default: {
      if (value >= HE_NUM) {
        ...
        if (!p || p == player1)
          ...
          outf("\f4you replied to %s", p->name);
      } else
        outf("\f3Update your client!");
      break;
    }
  }
#undef SSPAM
  ...
  while (p.remaining()) {
    ...
    if (demo && watchingdemo && demoprotocol == 1132) {
      ...
      if (type == SV_SWITCHNAME)           // SV_SPECTCN removed
      {
        getint(p);
        continue;
      } else if (type > SV_SWITCHNAME)
        --type;
    }

#ifdef _DEBUG
    if (type != SV_POS && type != SV_CLIENTPING && type != SV_PING &&
        type != SV_PONG && type != SV_CLIENT) {
      DEBUGVAR(d);
    ...
    switch (type) {
      case SV_SERVINFO:  // welcome message from the server
      {
        ...
        if (prot != CUR_PROTOCOL_VERSION &&
            !(watchingdemo && prot == -PROTOCOL_VERSION)) {
          conoutf(
              "\f3incompatible game protocol (local protocol: %d :: server "
              "protocol: %d)",
              CUR_PROTOCOL_VERSION, prot);
          conoutf(
              "\f3if this occurs a lot, obtain an upgrade from "
              "\f1http://assault.cubers.net");
          if (watchingdemo)
            conoutf(
                "breaking loop : \f3this demo is using a different protocol\f5 "
                ": end it now!");  // SVN-WiP-bug: causes endless retry loop
                                   // else!
          else
            disconnect();
          return;
      ...
      case SV_VOICECOMTEAM: {
        ...
        if (!d || !(d->muted || d->ignored)) {
          if (voicecomsounds == 1 || (voicecomsounds == 2 && m_teammode))
      ...
      case SV_TEAMTEXT: {
        ...
        if (!d) break;
        if (d->ignored)
          clientlogf("ignored: %s%s %s", colorname(d),
                     type == SV_TEAMTEXT ? ":" : "", text);
        else {
          if (m_teammode || team_isspect(player1->team))
            conoutf(type == SV_TEAMTEXTME ? "\f1%s %s" : "%s:\f1 %s",
                    colorname(d), highlight(text));
          else
            conoutf(type == SV_TEAMTEXTME ? "\f0%s %s" : "%s:\f0 %s",
                    colorname(d), highlight(text));
        }
        break;
      }
      ...
      case SV_TEXT:
        if (cn == -1) {
          ...
          getstring(text, p);
          filtertext(text, text, FTXT__CHAT);
          if (d->ignored && d->clientrole != CR_ADMIN)
            clientlogf("ignored: %s%s %s", colorname(d),
                       type == SV_TEXT ? ":" : "", text);
          else
            conoutf(type == SV_TEXTME ? "\f0%s %s" : "%s:\f0 %s", colorname(d),
                    highlight(text));
        } else
          return;
        break;
      ...
      case SV_INITCLIENT:  // another client either connected or changed
                           // name/team
      {
        int cn = getint(p);
        ...
        //                FIXME: uncomment on protocol bump + etc.
        //                d->maxrolleffect = (float)clamp(getint(p), 0,
        //                ROLLEFFMAX);    FIXME: uncomment on protocol bump
      ...
      case SV_SPAWNSTATE: {
        ...
        addmsg(SV_SPAWN, "rii", player1->lifesequence,
               player1->weaponsel->type);
      ...
      case SV_AUTHCHAL: {
        ...
        //     if(a && a->lastauth && lastmillis - a->lastauth < 60*1000)
        {
          ...
          // conoutf("answering %u, challenge %s with %s", id, text,
          // buf.getbuf());
      ...
      case SV_GIBDAMAGE:
      case SV_DAMAGE: {
        int tcn = getint(p), acn = getint(p), gun = getint(p),
            damage = getint(p), armour = getint(p), health = getint(p);
      ...
      case SV_RESUME: {
        loopi(MAXCLIENTS) {
          ...
          int state = getint(p), lifesequence = getint(p), primary = getint(p),
              gunselect = getint(p), flagscore = getint(p), frags = getint(p),
      ...
      case SV_EDITXY:  // coop editing messages, should be extended to include
                       // all possible editing ops
      {
        int cmd = getint(p);
        int x = getint(p);
        int y = getint(p);
        ...
        int a1 = getint(p);
        int a2 = getint(p);
        if (m_coop && !OUTBORD(x, y) && xs > 0 && ys > 0 &&
            !OUTBORD(x + xs - 1, y + ys - 1)) {
          block b = {x, y, xs, ys};
          switch (cmd) {
            case EDITXY_HEIGHT:
              editheightxy(a1 != 0, a2, b);
              break;
            case EDITXY_TEX:
              edittexxy(a1, a2, b);
              break;
      ...
      case SV_EDITBLOCK: {
        int bx = getuint(p), by = getuint(p), bxs = getuint(p),
            bys = getuint(p), light = getuint(p);
      ...
      case SV_NEWMAP: {
        ...
        if (d && d != player1)
          conoutf(size >= 0 ? "%s started a new map of size %d"
                            : "%s enlarged the map to size %d",
                  colorname(d), sfactor);
        break;
      }
      ...
      case SV_PONG: {
        int millis = getint(p);
        addmsg(SV_CLIENTPING, "i",
               player1->ping =
                   max(0, (player1->ping * 5 + totalmillis - millis) / 6));
        break;
      }
      ...
      case SV_ARENAWIN: {
        ...
        if (m_botmode && acn == -2)
          hudoutf("the bots have won the round!");
        else if (acn == -1)
          hudoutf("everyone died!");
        else if (m_teammode)
          hudoutf("team %s has won the round!", team_string(alive->team));
        else if (alive == player1)
          hudoutf("you are the survivor!");
        else
          hudoutf("%s is the survivor!", colorname(alive));
      ...
      case SV_SPAWNDENY: {
        ...
        if (spawnpermission == SP_REFILLMATCH)
          hudoutf("\f3you can spawn now to refill your team");
        break;
      }
      case SV_FORCEDEATH: {
        int cn = getint(p);
      ...
      case SV_SERVOPINFO: {
        loopv(players) {
          if (players[i]) players[i]->clientrole = CR_DEFAULT;
        }
        ...
        if (cl >= 0 && r >= 0) {
          ...
          if (pl) {
            pl->clientrole = r;
            if (pl->name[0]) {
              // two messages required to allow for proper german translation -
              // is there a better way to do it?
              if (pl == player1)
                conoutf("you claimed %s status",
                        r == CR_ADMIN ? "admin" : "master");
              else
                conoutf("%s claimed %s status", colorname(pl),
                        r == CR_ADMIN ? "admin" : "master");
            }
          }
        }
      ...
      case SV_TEAMDENY: {
        ...
        if (m_teammode) {
          if (team_isvalid(t))
            conoutf("you can't change to team %s", team_string(t));
        } else {
          if (team_isspect(t))
            conoutf("you can't change to spectate mode");
          else if (player1->state != CS_ALIVE)
            conoutf("you can't change to active mode");
          else
            conoutf("you can't switch teams while being alive");
        }
        break;
      }
      ...
      case SV_SETTEAM: {
        ...
        if (d) {
          ...
          if (m_teammode || team_isspect(fnt)) {
            if (d->team == fnt) {
              if (you && ftr == FTR_AUTOTEAM)
                hudoutf("you stay in team %s", nts);
            } else {
              if (you && !watchingdemo) {
                switch (ftr) {
                  case FTR_PLAYERWISH:
                ...
                switch (ftr) {
                  case FTR_PLAYERWISH:
                    conoutf("player %s switched to team %s", pls,
                            nts);  // new message
                    break;
                  case FTR_AUTOTEAM:
                    if (watchingdemo || team_isspect(player1->team))
                      conoutf("the server forced %s to team %s", colorname(d),
                              nts);
                    else
                      hudoutf("the server forced %s to %s team", colorname(d),
                              et ? "the enemy" : "your");
                    break;
                }
              }
              if (you && !team_isspect(d->team) && team_isspect(fnt) &&
                  d->state == CS_DEAD)
                spectatemode(SM_FLY);
            }
          } else if (d->team != fnt && ftr == FTR_PLAYERWISH &&
                     !team_isactive(d->team))
            conoutf("%s changed to active play", you ? "you" : colorname(d));
          d->team = fnt;
...
void setDemoFilenameFormat(char *fmt) {
  ...
  if (fmt && fmt[0] != '\0') {
    ...
    copystring(
        demofilenameformat,
        DEFDEMOFILEFMT);  // reset to default if passed empty string - or should
                          // we output the current value in this case?
}
...
void setDemoTimestampFormat(char *fmt) {
  ...
  if (fmt && fmt[0] != '\0') {
    ...
    copystring(
        demotimestampformat,
        DEFDEMOTIMEFMT);  // reset to default if passed empty string - or should
                          // we output the current value in this case?
}
...
void getdemonameformat() {
  extern string demofilenameformat;
  result(demofilenameformat);
}
COMMAND(getdemonameformat, "");
void getdemotimeformat() {
  extern string demotimestampformat;
  result(demotimestampformat);
}
COMMAND(getdemotimeformat, "");
void getdemotimelocal() {
  extern int demotimelocal;
  intret(demotimelocal);
}
COMMAND(getdemotimelocal, "");
...
const char *parseDemoFilename(char *srvfinfo) {
  ...
  extern const char *getDemoFilename(int gmode, int mplay, int mdrop,
                                     int tstamp, char *srvmap);
...
void receivefile(uchar *data, int len) {
  ...
  switch (type) {
    ...
    case SV_RECVMAP: {
      ...
      if (MAXMAPSENDSIZE < mapsize + cfgsizegz ||
          cfgsize > MAXCFGFILESIZE) {  // sam's suggestion
...
void servertoclient(int chan, uchar *buf, int len,
                    bool demo)  // processes any updates from the server
{
  ucharbuf p(buf, len);
  switch (chan) {
    case 0:
...
void localservertoclient(int chan, uchar *buf, int len,
                         bool demo)  // processes any updates from the server
{
  //    pktlogger.queue(enet_packet_create (buf, len, 0));  // log local & demo
  //    packets
