...
void renderscore(playerent *d) {
  ...
  line.addcol(sc_name, "\fs%s%s\fr%s%s%c ", status, colorname(d), ign,
              flagicon ? " \a" : "", flagicon);
}
...
void renderteamscore(teamscore *t) {
  ...
  static color teamcolors[2] = {color(1.0f, 0, 0, 0.2f),
                                color(0, 0, 1.0f, 0.2f)};
...
void renderscores(void *menu, bool init) {
  ...
  if (m_teammode) {
    ...
    loopv(scores) if (scores[i]->team != TEAM_SPECT)
        teamscores[team_base(scores[i]->team)]
            .addplayer(scores[i]);
    loopv(discscores) if (discscores[i].team != TEAM_SPECT)
        teamscores[team_base(discscores[i].team)]
            .addscore(discscores[i]);

    int sort =
        teamscorecmp(&teamscores[TEAM_CLA], &teamscores[TEAM_RVSF]) < 0 ? 0 : 1;
    loopi(2) {
    ...
    winner =
        m_flags
            ? (teamscores[sort].flagscore >
                       teamscores[team_opposite(sort)].flagscore
                   ? sort
                   : -1)
            : (teamscores[sort].frags > teamscores[team_opposite(sort)].frags
                   ? sort
                   : -1);
  ...
  if (getclientmap()[0]) {
    ...
    formatstring(modeline)(
        "\"%s\" on map %s", modestr(gamemode, modeacronyms > 0),
...
const char *asciiscores(bool destjpg) {
  ...
  loopv(discscores) {
    ...
    if (destjpg)
      formatstring(text)("%s(disconnected)%s (%s%d/%d)\n", d.name,
                         m_teammode ? team : "", m_flags ? flags : "", d.frags,
                         d.deaths);
    else
  ...
  if (destjpg) {
    ...
    formatstring(text)("(%sfrags/deaths), %d minute%s remaining\n",
                       m_flags ? "flags/" : "", minutesremaining,
