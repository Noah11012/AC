...
{
    if (col_a->priority > col_b->priority)
        return 1;
    else if (col_a->priority < col_b->priority)
        return -1;
    return 0;
}
...
void calcteamscores (int scores[4])
{
    ...
    if (!watchingdemo && player1->team != TEAM_SPECT)
        teamscores[team_base (player1->team)].addplayer (player1);
    loopi (2)
    {
...
void renderdiscscores (int team)
{
    loopv (discscores) if (team == team_group (discscores[i].team))
    {
        ...
        if (multiplayer (nullptr) || watchingdemo) line.addcol (sc_score, "%d", max (d.points, 0));
...
void renderscore (playerent *d)
{
    ...
    if (d->clientrole == CR_ADMIN)
        status = d->state != CS_ALIVE ? "\f7" : "\f3";
    else if (d->state != CS_ALIVE)
        status = "\f4";
    if (team_isspect (d->team))
        copystring (lagping, "SPECT");
    ...
    if (m_flags) // show flag icon at flag carrier with use radaricons font
    {
        loopi (2)
        {
            if (flaginfos[i].state == CTFF_STOLEN && flaginfos[i].actor == d)
                flagicon = m_ktf ? 'L' : "DH"[i];
        }
    }
...
void renderscores (void *menu, bool init)
{
    ...
    if (m_teammode)
    {
        ...
        loopv (scores) if (scores[i]->team != TEAM_SPECT) teamscores[team_base (scores[i]->team)].addplayer (
        scores[i]);
        loopv (discscores) if (discscores[i].team != TEAM_SPECT) teamscores[team_base (discscores[i].team)]
        .addscore (discscores[i]);
    ...
    if (getclientmap ()[0])
    {
        ...
        formatstring (modeline) ("\"%s\" on map %s", modestr (gamemode, modeacronyms > 0),
                                 fldrprefix ? getclientmap () + strlen ("maps/") : getclientmap ());
    }

    extern int minutesremaining, gametimedisplay;
    extern string gtime;

    if ((gamemode > 1 || (gamemode == 0 && (multiplayer (nullptr) || watchingdemo))) && minutesremaining >= 0)
    {
        if (!minutesremaining)
        {
            ...
            else // Add the winning player
            {
                if (winner < 0)
                    ...
                    concatformatstring (modeline, ", \f1%s wins!", scores[0]->name);
            }
        }
        else
        {
            concatformatstring (modeline, ", %s", gtime);
            if (gametimedisplay == 2)
                concatformatstring (modeline, " / %d:00", gametimemaximum / 60000);
            else
                concatformatstring (modeline, " remaining");
        }
    }
        ...
        if (s)
        {
            if (servstate.mastermode > MM_OPEN)
            {
                if (servstate.mastermode == MM_MATCH)
                    concatformatstring (serverline, "M%d ", servstate.matchteamsize);
                else
                    concatstring (serverline, "P ");
            }
            ...
            // but we might want to consider wrapping the bottom-line to accomodate
            // longer descriptions - to a limit.
            string text;
            filtertext (text, s->sdesc, FTXT__SERVDESC);
            // for(char *p = text; (p = strchr(p, '\"')); *p++ = ' ');
            // text[30] = '\0'; // serverbrowser has less room - +8 chars here -
            // 2010AUG03 - seems it was too much, falling back to 30 (for now): TODO
            // get real width of menu as reference-width. FIXME: cutoff
    ...
    menureset (menu);
    loopv (scorelines) menuimagemanual (menu, nullptr, "radaricons", scorelines[i].getcols (),
                                        nullptr, scorelines[i].bgcolor);
...
static void addstr (char *&dest, const char *end, const char *src)
{
    size_t l = strlen (src);
    if (dest + l < end) copystring (dest, src, l + 1), dest += l;
}
...
const char *asciiscores (bool destjpg)
{
    ...
    copystring (buf, "Comment",
                MAXJPGCOM); // include PNG keyword and skip it (which looks like a
                            // hack because it is one)
    ...
    if (getclientmap ()[0])
    {
        formatstring (text) ("\n\"%s\" on map %s", modestr (gamemode, false), getclientmap ());
        addstr (t, e, text);
    }
    if (multiplayer (nullptr))
    {
    ...
    loopv (scores)
    {
        ...
        if (destjpg)
            formatstring (text) ("%s%s (%s%d/%d)\n", d->name, m_teammode ? team : "",
                                 m_flags ? flags : "", d->frags, d->deaths);
        else
            formatstring (text) ("%s %4d   %4d %2d%s %s%s\n", m_flags ? flags : "", d->frags,
                                 d->deaths, d->clientnum, m_teammode ? team : "", d->name,
                                 d->clientrole == CR_ADMIN ? " (admin)" : d == player1 ? " (you)" : "");
        addstr (t, e, text);
    }
    discscores.sort (discscorecmp);
    loopv (discscores)
    {
        ...
        if (destjpg)
            ...
            formatstring (text) ("%s %4d   %4d --%s %s(disconnected)\n", m_flags ? flags : "",
                                 d.frags, d.deaths, m_teammode ? team : "", d.name);
        addstr (t, e, text);
    }
    if (destjpg)
    {
...
void winners ()
{
    ...
    if (m_teammode)
    {
        ...
        if (!sort)
            copystring (winners, "0 1");
        else
            itoa (winners, sort < 0 ? 0 : 1);
    }
    else
    {
        loopv (scores)
        {
            if (!i || !scorecmp (&scores[i], &scores[i - 1]))
                concatformatstring (winners, "%s%d", i ? " " : "", scores[i]->clientnum);
            else
                break;
        }
    }
