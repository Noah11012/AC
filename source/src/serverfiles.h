...
#define FLOORPLANBUFSIZE \
    (sizeof (struct servsqr) << ((SERVERMAXMAPFACTOR)*2)) // that's 4MB for size 10 maps (or 1 MB
                                                          // for size 9 maps)
...
struct servermap // in-memory version of a map file on a server
{
    ...
    void load (void) // load map into memory and extract everything important about it
                     // (assumes struct to be zeroed: can only be called once)
    {
        static uchar *staticbuffer = NULL;
        if (!staticbuffer)
            staticbuffer = new uchar[FLOORPLANBUFSIZE]; // this buffer gets reused for every map
        ...
        if (cfgraw)
        {
            ...
            loopk (cfglen) if (cfgraw[k] > 0x7f || (cfgraw[k] < 0x20 && !isspace (cfgraw[k]))) err
        ...
        // floorplan
        {
            ...
            if (version > MAPVERSION || numents > MAXENTITIES || sfactor < SMALLEST_FACTOR
                || sfactor > SERVERMAXMAPFACTOR || f->read (&h->waterlevel, restofhead) != restofhead)
...
#undef SCALEATTR
            ...
            for (int j = mapdims.yspan; j > 0; j--, ss += linegap) loopirev (mapdims.xspan)
                {
                    ...
                    {
                        int diff = (ss->vdelta + ss[1].vdelta + ss[linelen].vdelta + ss[linelen + 1].vdelta + 15)
        ...
        {
            if (calcmapareastats (areastats, (servsqr *)staticbuffer, 1 << sfactor, mapdims) < 0)
        ...
        // create compact floorplan
        {
            ...
            if (compress2 (staticbuffer + layoutlen, &gzbufsize, staticbuffer, layoutlen, 9) != Z_OK)
                gzbufsize = 0;
    ...
    loadfailed:
        ...
        { // success
            if (readmaplog)
                readmaplog->printf ("read map '%s%s': cgz %d bytes, cfg %d bytes (%d gz), version "
                                    "%d, "
                                    "size %d, rev %d, "
...
void trymapfiles (const char *fpath,
                  const char *fname,
...
void tagmapfile (const char *fpath,
                 const char *fname,
...
int readmapsthread (void *logfileprefix)
{
    ...
    while (1)
    {
        ...
        DELETEP (readmaplog); // always close the logfile when done, so we can create a
...
struct servermapbuffer // sending of maps between clients
{
    ...
    bool sendmap (const char *nmapname, int nmapsize, int ncfgsize, int ncfgsizegz, uchar *ndata)
    {
        ...
        { // update mapbuffer only in coopedit
...
static const char *const maplocstr[]
= { "not found", "temporary", "custom", "local", "official", "void" };
...
bool mapisok (mapstats *ms)
{
    ...
    {
        logline (ACLOG_INFO, "MAP CHECK FAIL: The overall ceil height is too high (%.1f cubes)", Mheight);
        return false;
    }
    if (Mopen > MAXMAREA)
    {
        logline (ACLOG_INFO, "MAP CHECK FAIL: There is a big open area in this (hint: use more "
                             "solid walls)");
        return false;
    }
    if (SHhits > MAXHHITS)
    {
        logline (ACLOG_INFO, "MAP CHECK FAIL: Too high height in some parts of the map (%d hits)", SHhits);
        return false;
    }
    ...
    if (FlagFlag < MINFF)
    {
        logline (ACLOG_INFO, "MAP CHECK FAIL: The flags are too close to each other");
        return false;
    }
    ...
    for (int i = 0; i < ms->hdr.numents; i++)
    {
        ...
        for (int j = 0; j < ms->hdr.numents; j++)
        {
            ...
            if (r2 == 0.0f)
            {
                logline (ACLOG_INFO, "MAP CHECK FAIL: Items too close %s %s (%hd,%hd)", entnames[v],
                         entnames[w], p[0], p[1]);
                return false;
            }
            r2 = 1 / r2;
        ...
        if (hdensity > 0.5f)
        {
            logline (ACLOG_INFO, "MAP CHECK FAIL: Items too close %s %.2f (%hd,%hd)", entnames[v],
                     hdensity, p[0], p[1]);
            return false;
        }
        switch (v)
            ...
            if (l)
            {
                ...
                if (i > 2)
                {
                    ...
                    logline (ACLOG_VERBOSE,
                             ...
                             "skiplines:%d",
                             c.mapname, modestr (c.mode, false), c.time, c.vote, c.minplayer,
                             c.maxplayer, c.skiplines);
                }
                else
        ...
        loopi (3 * csl + 1)
        {
            ...
            if ((n >= c->minplayer || i >= csl) && (!c->maxplayer || n <= c->maxplayer || i >= 2 * csl))
            {
                ...
                if ((ms = getservermapstats (c->mapname)) && mapisok (ms))
                    ...
                    logline (ACLOG_INFO, "maprot error: map '%s' %s", c->mapname,
                             (ms ? "does not satisfy some basic requirements" : "not found"));
            }
            if (i >= 3 * csl)
                fatal ("maprot unusable"); // not a single map in rotation can be found...
        }
        if (!nochange)
        {
            curcfgset = ccs;
        ...
        loopv (ipranges)
        {
            ...
            if (ipranges[i].ur <= ipranges[i - 1].ur)
            {
                if (ipranges[i].lr == ipranges[i - 1].lr && ipranges[i].ur == ipranges[i - 1].ur)
                    logline (ACLOG_VERBOSE, " blacklist entry %s got dropped (double entry)",
                             iprtoa (ipranges[i]));
                else
                    logline (ACLOG_VERBOSE, " blacklist entry %s got dropped (already covered by %s)",
            ...
            {
                logline (ACLOG_VERBOSE, " blacklist entries %s and %s are joined due to overlap",
...
struct servernickblacklist : serverconfigfile
{
    ...
    void read ()
    {
            ...
            if (l)
            {
                ...
                { // accept nickname IP-range
                    ...
                    s = NULL;
                         ...
                         && (!strcmp (l, "block") || !strcmp (l, "b") || ic++
                             || !strcmp (l, "blocki") || !strcmp (l, "bi")))
                { // block nickname fragments (ic
                  // == ignore case)
                    bl.clear ();
                ...
                {
                    logline (ACLOG_INFO, " error in line %d, file %s: unknown keyword '%s'", line, filename, l);
                    errors++;
                }
                if (s && s[strspn (s, " ")])
                {
                    logline (ACLOG_INFO, " error in line %d, file %s: ignored '%s'", line, filename, s);
                    errors++;
                }
            }
            line++;
        ...
        logline (ACLOG_INFO, "read %d + %d entries from nickname blacklist file '%s', %d errors",
    ...
    const char *getinfocache (const char *fnbase, const char *lang)
    {
        ...
        loopv (serverinfotexts)
        {
            ...
            if (si.type == s->type && !strcmp (si.lang, s->lang))
            {
                ...
                { // re-read existing files after 15 minutes; search
