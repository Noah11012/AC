...
struct md3 : vertmodel
{
                 ...
                 float scale)
    {
        ...
        if (a)
            for (int i = 0; a[i].tag; i++) link (NULL, a[i].tag);
    ...
    bool load ()
    {
        ...
        else // md3 without configuration, try default tris and skin
        {
            ...
            {
                formatstring (name1) ("packages/models/%s/tris.md3",
                                      pname); // try md3 in parent folder (vert sharing)
...
void md3skin (char *objname, char *skin)
{
    ...
    if (!used)
    {
        ...
        conoutf ("mesh \"%s\" not found in model %s, skin %s not loaded%s", objname,
                 loadingmd3->loadname, skin, mdl.meshes.length () ? s : "");
        flagmapconfigerror (LWW_MODELERR);
        scripterr ();
    }
}
