...
void render_wall (sqr *o, sqr *s, int x1, int y1, int x2, int y2, int mip, sqr *d1, sqr *d2, bool topleft, int dir)
{
    ...
    if (SOLID (o) || o->type == SEMISOLID)
    {
        ...
        render_square (o->wtex, c1, c2, f1, f2, x1 << mip, y1 << mip, x2 << mip, y2 << mip,
                       1 << mip, d1, d2, topleft, dir);
        return;
    }
    {
        float f1 = s->floor;
        ...
        render_square (o->wtex, f1, f2, c1, c2, x1 << mip, y1 << mip, x2 << mip, y2 << mip,
                       1 << mip, d1, d2, topleft, dir);
    }
skip:
{
    ...
    render_square (o->utex, f1, f2, c1, c2, x1 << mip, y1 << mip, x2 << mip, y2 << mip, 1 << mip,
                   d1, d2, topleft, dir);
}
}
...
void render_seg_new (float vx, float vy, float vh, int mip, int x, int y, int xs, int ys)
{
    ...
    for (int oy = y; oy < ys; oy++) // first collect occlusion information for this block
    {
        ...
        for (int ox = x; ox < xs; ox++, s++)
            s->occluded
            = isoccluded (camera1->o.x, camera1->o.y, (float)(ox << mip), (float)(oy << mip), fsize);
    }
...
#define LOOPD                        \
    ...
    {
        LOOPD
        render_flatdelta (s->ftex, xx << mip, yy << mip, 1 << mip, df (s), df (t), df (u), df (v),
                          s, t, u, v, false);
...
if (!minimap)
    ...
    {
        LOOPD
        render_flatdelta (s->ctex, xx << mip, yy << mip, 1 << mip, dc (s), dc (t), dc (u), dc (v),
                          s, t, u, v, true);
        break;
    }
    }
...
if (s->type == CORNER)
{
    ...
    else
    {
        ...
        if (z->ceil - z->floor < t->ceil - t->floor)
        {
            if (wv)
            {
                render_wall (h1 = s, h2 = v, xx + 1, yy, xx, yy + 1, mip, t, v, false, 4);
                topleft = false;
            }
            else
            {
                render_wall (h1 = s, h2 = w, xx, yy, xx + 1, yy + 1, mip, s, u, false, 5);
            }
        }
        else
        {
            if (wv)
            {
                render_wall (h2 = s, h1 = v, xx + 1, yy + 1, xx, yy, mip, u, s, false, 6);
            }
            else
            {
                render_wall (h2 = s, h1 = w, xx, yy + 1, xx + 1, yy, mip, v, t, false, 7);
                topleft = false;
            }
        }
    }
...
void render_world (float vx, float vy, float vh, float changelod, int yaw, int pitch, float fov, float fovy, int w, int h)
{
    ...
    min_lod = minimap || (player1->isspectating () && player1->spectatemode == SM_OVERVIEW) ?
              MAX_LOD :
