...
void render_wall(sqr *o, sqr *s, int x1, int y1, int x2, int y2, int mip,
                 sqr *d1, sqr *d2, bool topleft, int dir) {
  if (minimap) return;
  if (SOLID(o) || o->type == SEMISOLID) {
    ...
    render_square(o->wtex, c1, c2, f1, f2, x1 << mip, y1 << mip, x2 << mip,
                  y2 << mip, 1 << mip, d1, d2, topleft, dir);
    return;
  }
  {
    float f1 = s->floor;
    ...
    render_square(o->wtex, f1, f2, c1, c2, x1 << mip, y1 << mip, x2 << mip,
                  y2 << mip, 1 << mip, d1, d2, topleft, dir);
  }
skip : {
  float f1 = o->ceil;
...
bool issemi(int mip, int x, int y, int x1, int y1, int x2, int y2) {
  ...
  switch (SWS(w, x + x1, y + y1, mfactor)->type) {
    case SEMISOLID:
      if (issemi(mip, x + x1, y + y1, x1, y1, x2, y2)) return true;
    case CORNER:
    case SOLID:
      break;
    default:
  ...
  switch (SWS(w, x + x2, y + y2, mfactor)->type) {
    case SEMISOLID:
      if (issemi(mip, x + x2, y + y2, x1, y1, x2, y2)) return true;
    case CORNER:
    case SOLID:
      break;
    default:
...
void render_seg_new(float vx, float vy, float vh, int mip, int x, int y, int xs,
  ...
  int lx = vxx - lodleft;  // these mark the rect inside the current rest that
       ...
       oy++)  // first collect occlusion information for this block
  {
    ...
    for (int ox = x; ox < xs; ox++, s++)
      s->occluded = isoccluded(camera1->o.x, camera1->o.y, (float)(ox << mip),
                               (float)(oy << mip), fsize);
  }
...
#define dc(x) s->ceil + (x->vdelta / 4.0f)
  ...
  // facilitate dynamic stripify) for each we skip occluded cubes (occlusion at
  // higher mip levels is a big time saver!). during the first loop (ceil) we
  // collect cubes that lie within the lower mip rect and are also deferred, and
  // render them recursively. Anything left (perfect mips and higher lods) we
  // render here.

#define LOOPH                                \
  {                                          \
    for (int yy = y; yy < ys; yy++) {        \
      sqr *s = SWS(w, x, yy, mfactor);       \
      for (int xx = x; xx < xs; xx++, s++) { \
...
#define LOOPD                     \
  ...
  LOOPH  // floors
    ...
    int start = xx;
    while (xx < xs - 1 && s[1].defer && !s[1].occluded) {
      xx++;
      s++;
      if (xx < rx) s->occluded = 1;
    }  // collect 2xN rect of lower mip
    render_seg_new(vx, vy, vh, mip - 1, start * 2, yy * 2, xx * 2 + 2,
                   yy * 2 + 2);
    continue;
  }
  rendered++;
  switch (s->type) {
    ...
    case CHF:
      if (s->floor <= vh && render_floor) {
        sqr *v = SWS(s, 0, 1, mfactor);
        render_flat(s->ftex, xx << mip, yy << mip, 1 << mip, s->floor, s, s + 1,
                    v + 1, v, false);
        if (s->floor < waterlevel && !reflecting)
    ...
    case FHF: {
      ...
      if (s->floor - s->vdelta / 4.0f < waterlevel && !reflecting)
        addwaterquad(xx << mip, yy << mip, 1 << mip);
      break;
    }
  }
  ...
  LOOPH  // ceils
    ...
    case FHF:
      if (s->ceil >= vh && render_ceil) {
        sqr *v = SWS(s, 0, 1, mfactor);
        render_flat(s->ctex, xx << mip, yy << mip, 1 << mip, s->ceil, s, s + 1,
                    v + 1, v, true);
      }
      break;
    case CHF: {
...
if (s->type == CORNER) {
  ...
  if (SOLID(z)) {
    if (SOLID(w)) {
      ...
      render_wall(v, h2 = s, xx, yy, xx + 1, yy + 1, mip, s, u, false, 5);
    }
  } else if (SOLID(t)) {
    if (SOLID(w)) {
      render_wall(w, h1 = s, xx + 1, yy + 1, xx, yy, mip, u, s, false, 6);
    } else if (SOLID(v)) {
      render_wall(v, h1 = s, xx, yy + 1, xx + 1, yy, mip, v, t, false, 7);
      topleft = false;
    }
  } else {
    normalwall = false;
    ...
    if (z->ceil - z->floor < t->ceil - t->floor) {
      if (wv) {
        render_wall(h1 = s, h2 = v, xx + 1, yy, xx, yy + 1, mip, t, v, false,
                    4);
        topleft = false;
      } else {
        render_wall(h1 = s, h2 = w, xx, yy, xx + 1, yy + 1, mip, s, u, false,
                    5);
      }
    } else {
      if (wv) {
        render_wall(h2 = s, h1 = v, xx + 1, yy + 1, xx, yy, mip, u, s, false,
                    6);
      } else {
        render_wall(h2 = s, h1 = w, xx, yy + 1, xx + 1, yy, mip, v, t, false,
                    7);
        topleft = false;
      }
    }
...
if (editmode && !showtagclipfocus && showtagclips)
  LOOPH  // tag clips
      if (mip) {
    int ix1 = xx << mip, ix2 = (xx + 1) << mip, iy1 = yy << mip,
        iy2 = (yy + 1) << mip;
    sqr *tt = S(ix1, iy1), *t;
    for (int iy = iy1; iy < iy2; iy++, tt += ssize) {
      t = tt;
      for (int ix = ix1; ix < ix2; ix++, t++) {
        if (t->tag & TAGANYCLIP && t->type != SOLID)
          tagclipcubes.add(ix + (iy << 16));
      }
    }
  }
else if (s->tag & TAGANYCLIP && s->type != SOLID)
  tagclipcubes.add(xx + (yy << 16));
}
}
}
}
...
void render_world(float vx, float vy, float vh, float changelod, int yaw,
  ...
  min_lod = minimap || (player1->isspectating() &&
                        player1->spectatemode == SM_OVERVIEW)
                ? MAX_LOD
                : MIN_LOD + iabs(pitch) / 12;
  yaw = 360 - yaw;
  ...
  {
    min_lod =
        max(min_lod, (int)(MIN_LOD + (10 - cdist) / 1.0f *
