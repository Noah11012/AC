...
struct cert
{
    ...
    *name,           // name of the cert (player name, clan name, etc), needs to be
                     // unique (in combination with type)
    *orgfilename;    // name of the file, the cert was loaded from - without
                     // path or extension
    ...
    *next;               // used to chain certs
    ...
    expired,      // true, if the cert type does expire and the cert is properly
                  // signed, but the expired-date is passed
...
extern void
menuitemmanual (void *menu, char *text, char *action = NULL, color *bgcolor = NULL, const char *desc = NULL);
extern void menuimagemanual (void *menu,
...
extern void *addmenu (const char *name,
...
struct mitem
{
    ...
    : parent (parent), bgcolor (bgcolor), greyedout (menugreyedout), mitemtype (type)
    {
    }
    virtual ~mitem () {}
...
struct serverinfo
{
    ...
    pongflags, getinfo, menuline_from, menuline_to;
    ENetAddress address;
...
struct httpget
{
    ...
    const char *useragent, *referrer; // can be set manually with "newstrings",
    ...
    int get (const char *url1,
             uint timeout,
             uint totaltimeout,
             int range = 0,
             bool head = false); // get web ressource
};
...
extern void
framedquadtexture (GLuint tex, int x, int y, int xs, int ys, int border = 0, int backlight = 255, bool blend = false);
...
extern Texture *
textureload (const char *name, int clamp = 0, bool mipmap = true, bool canreduce = false, float scale = 1.0f, bool trydl = false);
...
extern void
render_flatdelta (int wtex, int x, int y, int size, float h1, float h2, float h3, float h4, sqr *l1, sqr *l2, sqr *l3, sqr *l4, bool isceil);
extern void render_square (int wtex,
                           float floor1,
                           float floor2,
                           float ceil1,
                           float ceil2,
                           ...
                           int dir);
extern void
render_tris (int x, int y, int size, bool topleft, sqr *h1, sqr *h2, sqr *s, sqr *t, sqr *u, sqr *v);
...
extern void
dodamage (int damage, playerent *pl, playerent *actor, int gun = -1, bool gib = false, bool local = true);
...
struct votedisplayinfo
{
    ...
    votedisplayinfo () : owner (NULL), result (VOTE_NEUTRAL), millis (0), localplayervoted (false)
    {
        loopi (VOTE_NUM) stats[i] = VOTE_NEUTRAL;
    }
};

extern votedisplayinfo *
...
extern void
adddynlight (physent *owner, const vec &o, int reach, int expire, int fade, uchar r, uchar g = 0, uchar b = 0);
...
// worldrender
extern void
render_world (float vx, float vy, float vh, float changelod, int yaw, int pitch, float fov, float fovy, int w, int h);
...
extern void popfont ();
extern void
draw_text (const char *str, int left, int top, int r = 255, int g = 255, int b = 255, int a = 255, int cursor = -1, int maxwidth = -1);
...
// editing
#define EDITSEL(x) \
    if (noteditmode (x) || noselection ()) return
#define EDITSELMP(x) \
    if (noteditmode (x) || noselection () || multiplayer (x)) return
#define EDITMP(x) \
    if (noteditmode (x) || multiplayer (x)) return
#define EDIT(x) \
    if (noteditmode (x)) return
...
extern void
show_out_of_renderloop_progress (float bar1, const char *text1, float bar2 = 0, const char *text2 = NULL);
...
extern float
fvariable (const char *name, float min, float cur, float max, float *storage, void (*fun) (), bool persist);
extern char *
svariable (const char *name, const char *cur, char **storage, void (*fun) (), void (*getfun) (), bool persist);
...
extern bool exechook (int context, const char *ident, const char *body, ...)
PRINTFARGS (3, 4); // execute cubescript hook if available and allowed in
...
extern void putgzbuf (vector<uchar> &d,
                      vector<uchar> &s); // zips a vector into a stream, stored in another vector
...
extern void
extinfo_statsbuf (ucharbuf &p, int pid, int bpos, ENetSocket &pongsock, ENetAddress &addr, ENetBuffer &buf, int len, int *csend);
...
extern bool
initlogging (const char *identity, int facility_, int consolethres, int filethres, int syslogthres, bool logtimestamp);
...
{
    int uprate, serverport, syslogfacility, filethres, syslogthres, maxdemos, maxclients,
    kickthreshold, banthreshold, verbose, incoming_limit, afk_limit, ban_time, demotimelocal;
    const char *ip, *master, *logident, *serverpassword, *adminpasswd, *demopath, *maprot, *pwdfile,
    *blfile, *nbfile, *infopath, *motdpath, *forbidden, *demofilenameformat, *demotimestampformat;
    ...
    servercommandline ()
    : uprate (0),
      serverport (CUBE_DEFAULT_SERVER_PORT),
      syslogfacility (6),
      filethres (-1),
      syslogthres (-1),
      maxdemos (5),
    ...
    bool checkarg (const char *arg)
    {
        ...
        case 'u':
            ...
            break;
        case 'f':
            if (ai > 0 && ai < 65536) serverport = ai;
            break;
        case 'i':
            ip = a;
            break;
        ...
        case 'l':
            ...
            break;
        case 'F':
            if (isdigit (*a) && ai >= 0 && ai <= 7) syslogfacility = ai;
            break;
        case 'T':
            logtimestamp = true;
            break;
        case 'L':
            ...
            break;
        case 'A':
            if (*a) adminonlymaps.add (a);
            break;
        case 'c':
            if (ai > 0) maxclients = min (ai, MAXCLIENTS);
            break;
        case 'k':
        {
            if (arg[2] == 'A' && arg[3] != '\0')
