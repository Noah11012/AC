...
void writepngchunk (stream *f, const char *type, const void *data = nullptr, uint len = 0)
{
    f->putbig<uint> (len);
    f->write (type, 4);
...
void screenshot (const char *filename)
{
    ...
    if (filename && filename[0])
        ...
        formatstring (buf) ("screenshots/%s_%s_%s%s", timestring (), behindpath (getclientmap ()),
                            modestr (gamemode, true), getscrext ());
    else
...
void mapshot ()
{
    defformatstring (buf) ("screenshots" PATHDIVS "mapshot_%s_%s%s", behindpath (getclientmap ()),
                           timestring (), getscrext ());
...
void screenshotpreview (int *res)
{
    ...
    defformatstring (buf) ("packages" PATHDIVS "maps" PATHDIVS "%spreview" PATHDIVS "%s.jpg",
        ...
        if (modes && modes != (SDL_Rect **)-1)
        {
            for (int i = 0; modes[i]; i++)
            {
                if (lw != modes[i]->w && lh != modes[i]->h)
                    concatformatstring (res, "%s%d %d", *res ? " " : "", (lw = modes[i]->w),
                                        (lh = modes[i]->h));
            }
        }
        result (res);
    ...
    void setupscreen (int &usedcolorbits, int &useddepthbits, int &usedfsaa)
    {
        ...
        if (colorbits && modes)
            hasbpp = SDL_VideoModeOK (modes != (SDL_Rect **)-1 ? modes[0]->w : scr_w,
                                      modes != (SDL_Rect **)-1 ? modes[0]->h : scr_h, colorbits,
                                      SDL_OPENGL | flags)
                     == colorbits;
    ...
    void checkinput ()
    {
        ...
        while (events.length () || SDL_PollEvent (&event))
        {
            ...
            case SDL_ACTIVEEVENT:
                ...
                if (event.active.state & SDL_APPACTIVE) minimized = !event.active.gain;
#if 0
...
#define AUTOSTARTPATH "config" PATHDIVS "autostart" PATHDIVS
    ...
    void createconfigtemplates (const char *templatezip) // create customisable config files in
                                                         // homedir - only if missing
    {
        ...
        if (mz)
        {
            loopv (files)
            {
                const char *filename = behindpath (files[i]); // only look for config files in the zip
    ...
    void initclientlog () // rotate old logfiles and create new one
    {
        ...
        if (clientlogfile)
        {
            ...
            conoutf ("could not create logfile \"%s\"", findfile ("clientlog.txt", "w"));
    ...
    void sanitychecks ()
    {
        ASSERT ((GMMASK__BOT ^ GMMASK__MP ^ GMMASK__TEAM ^ GMMASK__FFA ^ GMMASK__TEAMSPAWN ^ GMMASK__FFASPAWN)
                == GMMASK__ALL);
...
#define initlog(s) clientlogf ("init: " s)
            ...
            if (!havehomedir ()) sethomedir (DEFAULTPROFILEPATH);
            entropy_init (time (nullptr) + (uint) (size_t)&initscript + (uint) (size_t)entropy_init);
            initclientlog ();
...
#define SDLVERSIONSTRING          \
    ...
    "." STRINGIFY (SDL_MINOR_VERSION) "." STRINGIFY (SDL_PATCHLEVEL)
            ...
            if (SDL_COMPILEDVERSION != SDL_VERSIONNUM (sdlver->major, sdlver->minor, sdlver->patch))
                clientlogf ("SDL: compiled version " SDLVERSIONSTRING ", linked version %u.%u.%u",
    ...
    if(highprocesspriority) setprocesspriority(true);
            ...
            if (!notexture)
                fatal ("could not find core textures (hint: run AssaultCube from the "
            ...
            initlog ("cfg");
            extern void *scoremenu, *servmenu, *searchmenu, *serverinfomenu, *kickmenu, *banmenu,
            *forceteammenu, *giveadminmenu, *docmenu, *applymenu, *downloaddemomenu;
            ...
            serverinfomenu = addmenu ("serverinfo", "extended server information (F5: refresh)",
            ...
            static char resdata[]
            = { 112, 97,  99,  107, 97,  103, 101, 115, 47,  116, 101, 120, 116, 117, 114, 101, 115,
                47,  107, 117, 114, 116, 47,  107, 108, 105, 116, 101, 50,  46,  106, 112, 103, 0 };
...
#endif
            if (scl.logident[0])
            { // "-Nxxx" sets the number of client log lines to
              // xxx (after init)
            ...
            for (;;)
            {
                ...
                if (frames > 3 && !minimized)
                {
                    gl_drawframe (screen->w, screen->h,
                                  fps < lowfps ? fps / lowfps : (fps > highfps ? fps / highfps : 1.0f),
                                  fps, elapsed);
