...
hashtable<const char *, docident> docidents; // manage globally instead of a section tree to ensure
                                             // uniqueness
...
void adddocident (char *name, char *desc)
{
    ...
    DEBUGCODE (if (strlen (desc) > 111)
               clientlogf ("docident: very long description for ident %s (%d)", name, (int)strlen (desc)));
}
...
void adddocargument (char *token, char *desc, char *values, char *vararg)
{
    ...
    if (*token)
        loopv (lastident->arguments) if (!strcmp (token, lastident->arguments[i].token))
        clientlogf ("docargument: double token %s in reference %s", token, lastident->name);
...
void adddocref (char *refident)
{
    ...
    loopv (lastident->references) if (!strcasecmp (lastident->references[i], refident))
    clientlogf ("docref: double reference %s in %s", refident, lastident->name);
#endif
...
{
    stream *f = openfile (path (allidents < 0 ? "docs/autogenerated_base_reference.xml" : "docs/autogenerated_todo_reference.xml",
                                true),
                          "w");
    if (!f) return;
    ...
    if (allidents < 0)
    {
        ...
        f->printf ("<cuberef xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" "
    ...
    loopv (inames)
    {
        ...
        {
        case ID_VAR:
            f->printf ("\t\t\t\t\t<value %s description=\"TODO\" minValue=\"%d\" "
                       "maxValue=\"%d\" defaultValue=\"%d\" %s/>\n",
                       id->minval > id->maxval ? "" : "token=\"N\"", id->minval, id->maxval,
                       *id->storage.i, id->minval > id->maxval ? "readOnly=\"true\"" : "");
            break;
        case ID_FVAR:
            f->printf ("\t\t\t\t\t<value %s description=\"TODO\" minValue=\"%s\" "
                       "maxValue=\"%s\" defaultValue=\"%s\" %s/>\n",
                       id->minvalf > id->maxvalf ? "" : "token=\"N\"", floatstr (id->minvalf),
                       floatstr (id->maxvalf), floatstr (*id->storage.f),
...
void docwritetodoref (int *allidents) { docwriteref (*allidents ? 1 : 0, "", "", ""); }
        ...
        loopi (MAXWORDS)
        {
            ...
            { // find matching ) or ]
                ...
                while (brak.length ())
                {
                    ...
                    { // unmatched braces, interpret from there (in first
                      // pass)
                        *unmatched = right;
    ...
    if (curident)
    {
        ...
        if (curident->arguments.length ()) // args
        {
            ...
            loopvj (curident->arguments)
            {
                ...
                formatstring (doclines.add (heaplines.add (newstringbuf ()))) ("\f%c%-8s%s",
                                                                               j == carg ? '4' : '5',
                                                                               a->token, a->desc);
    ...
    if (csident && docidentverbose) // cs ident details
    {
        ...
        case ID_VAR:
            if (csident->maxval < csident->minval)
                ...
                concatformatstring (buf, "integer variable, current %d, min %d, max %d, default %d",
                                    *csident->storage.i, csident->minval, csident->maxval, csident->defaultval);
            break;
        case ID_FVAR:
            if (csident->maxvalf < csident->minvalf)
                ...
                concatformatstring (buf, "float variable, current %s, min %s, max %s, default %s",
        ...
        case ID_COMMAND:
            ...
            break;
        case ID_ALIAS:
            concatformatstring (buf, "alias, %s",
                                csident->istemp ?
                                "temp" :
                                (csident->isconst ? "const" : (csident->persist ? "persistent" : "non-persistent")));
            break;
        }
...
int msectionsort (const msection *a, const msection *b) { return strcmp (a->name, b->name); }
...
void renderdocsection (void *menu, bool init)
{
    ...
    loopv (sections)
    {
        ...
        loopv (msections)
        {
            menuitemmanual (menu, msections[i].name, msections[i].cmd, nullptr, msections[i].desc);
        }
        return;
    }
}
