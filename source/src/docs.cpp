...
void adddocident(char *name, char *desc) {
  ...
  lastident = &c;
  DEBUGCODE(if (strlen(desc) > 111)
                clientlogf("docident: very long description for ident %s (%d)",
                           name, (int)strlen(desc)));
}
...
void adddocargument(char *token, char *desc, char *values, char *vararg) {
  ...
  if (*token)
    loopv(
        lastident->arguments) if (!strcmp(token, lastident->arguments[i].token))
        clientlogf("docargument: double token %s in reference %s", token,
                   lastident->name);
...
void adddocref(char *refident) {
  ...
  loopv(lastident->references) if (!strcasecmp(lastident->references[i],
                                               refident))
      clientlogf("docref: double reference %s in %s", refident,
                 lastident->name);
#endif
...
void docinvalid() {
  ...
  enumerate(
      docidents, docident, d,
      if (!strchr(d.name, ' ') && !identexists(d.name)) inames.add(d.name););
  inames.sort(stringsort);
  ...
  enumerate(docidents, docident, d, {
    loopv(d.references) if (!identexists(d.references[i])) {
      bool add = true;
      loopvj(inames) if (!strcasecmp(inames[j], d.references[i])) add =
          false;  // don't list references to nonexisting but documented entries
  ...
  loopvj(inames) {
    ...
    if ((r = cvecstr(srch, search, &rline))) {
      ...
      if (!*silent) {
        ...
        copystring(match, srch[rline] + (pos < showchars - 10 ? 0 : pos - 10),
                   showchars);
...
void docwriteref(int allidents, const char *ref, const char *schemalocation,
                 const char *transformation) {
  stream *f =
      openfile(path(allidents < 0 ? "docs/autogenerated_base_reference.xml"
                                  : "docs/autogenerated_todo_reference.xml",
                    true),
               "w");
  if (!f) return;
  ...
  if (allidents < 0)
    ...
    listundoneidents(inames,
                     allidents);  // 0: todo builtin, 1: todo builtin + alias
  inames.sort(stringsort);

  if (allidents < 0) {
    f->printf("<?xml version=\"1.0\" encoding=\"utf-8\"?>\n");
    f->printf("<?xml-stylesheet type=\"text/xsl\" href=\"%s\"?>\n",
              transformation && strlen(transformation)
                  ? transformation
                  : "transformations/cuberef2xhtml.xslt");
    f->printf(
        "<cuberef xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" "
        "name=\"%s\" version=\"v0.1\" xsi:schemaLocation=\"%s\" "
        "xmlns=\"http://cubers.net/Schemas/CubeRef\">\n",
        ref && strlen(ref) ? ref : "Unnamed Reference",
        schemalocation && strlen(schemalocation)
            ? schemalocation
            : "http://cubers.net/Schemas/CubeRef schemas/cuberef.xsd");
    f->printf("\t%s\n", desc);
    f->printf("\t<sections>\n");
  ...
  loopv(inames) {
    ...
    switch (id->type) {
      case ID_VAR:
        f->printf(
            "\t\t\t\t\t<value %s description=\"TODO\" minValue=\"%d\" "
            "maxValue=\"%d\" defaultValue=\"%d\" %s/>\n",
            id->minval > id->maxval ? "" : "token=\"N\"", id->minval,
            id->maxval, *id->storage.i,
            id->minval > id->maxval ? "readOnly=\"true\"" : "");
        break;
      case ID_FVAR:
        f->printf(
            "\t\t\t\t\t<value %s description=\"TODO\" minValue=\"%s\" "
            "maxValue=\"%s\" defaultValue=\"%s\" %s/>\n",
            id->minvalf > id->maxvalf ? "" : "token=\"N\"",
            floatstr(id->minvalf), floatstr(id->maxvalf),
            floatstr(*id->storage.f),
...
void scrolldoc(int i) {
  docskip += i;
  if (docskip < 0) docskip = 0;
}

int docs_parsecmd(const char *p, const char *pos, const char *w[MAXWORDS],
                  int wl[MAXWORDS], bool outer, int *unmatched) {
      ...
      if (*p == '"') {              // find matching "
        ...
        while (brak.length()) {
          ...
          if (c == left && !quot)
            ...
            brak.drop();
          else if (!c) {  // unmatched braces, interpret from there (in first
                          // pass)
            *unmatched = right;
...
void renderdoc(int x, int y, int doch) {
  ...
  int cmdpos;
  const char *exp =
      getcurcommand(&cmdpos);  // get command buffer and cursor position
  if (!exp || *exp != '/') return;
  ...
  {
    carg = docs_parsecmd(exp + 1, pos, w, wl, i != 0,
                         &unmatched);  // break into words
    if (!w[0]) return;
  ...
  if (curident) {
    if (!curident->label) {
      ...
      loopvj(curident->arguments)
          concatformatstring(buf, " %s", curident->arguments[j].token);
    ...
    {
      if (carg >= curident->arguments.length() &&
          curident->arguments.last().vararg)
        carg = curident->arguments.length() - 1;  // vararg spans the line

      loopvj(curident->arguments) {
        docargument *a = &curident->arguments[j];
        formatstring(doclines.add(heaplines.add(newstringbuf())))(
            "\f%c%-8s%s", j == carg ? '4' : '5', a->token, a->desc);
    ...
    {
      doclines.add(curident->keylines.length() == 1 ? "Default key:"
                                                    : "Default keys:");
      loopvj(curident->keylines)
          doclines.add(curident->keylines[j]);  // stored preformatted
      doclines.add(NULL);
    }
    ...
    if (docrefvisible && curident->references.length())  // references
    {
      if (!curident->related) {
        string refs = "";
        loopvj(curident->references)
            concatformatstring(refs, ", %s", curident->references[j]);
    ...
    switch (csident->type) {
      case ID_VAR:
        if (csident->maxval < csident->minval)
          concatformatstring(buf, "integer variable, current %d, read-only",
                             *csident->storage.i);
        else
          concatformatstring(
              buf, "integer variable, current %d, min %d, max %d, default %d",
              *csident->storage.i, csident->minval, csident->maxval,
              csident->defaultval);
        break;
      case ID_FVAR:
        if (csident->maxvalf < csident->minvalf)
          concatformatstring(buf, "float variable, current %s, read-only",
                             floatstr(*csident->storage.f));
        else
          concatformatstring(
              buf, "float variable, current %s, min %s, max %s, default %s",
              floatstr(*csident->storage.f), floatstr(csident->minvalf),
              floatstr(csident->maxvalf), floatstr(csident->defaultvalf));
        break;
      case ID_SVAR:
        concatstring(buf, "string variable");
        break;
      case ID_COMMAND:
        concatformatstring(buf, "builtin command%s",
                           strchr("sif", *csident->sig) ? ", arguments:" : "");
      ...
      case ID_ALIAS:
        concatformatstring(
            ...
            csident->istemp
                ...
                : (csident->isconst
                       ? "const"
                       : (csident->persist ? "persistent" : "non-persistent")));
        break;
    }
  ...
  for (int j = offset; j < doclines.length(); j++) {
    ...
    if (str)
      text_bounds(*str == '~' ? str + 1 : str, width, height,
                  *str == '~' ? -1 : VIRTW * 4 / 3);
  ...
  for (int j = offset; j < maxl; j++) {
    ...
    if (str) {
      ...
      draw_text(text, x, cury, 0xFF, 0xFF, 0xFF, 0xFF, -1,
                str == text ? VIRTW * 4 / 3 : -1);
      int width, height;
