...
bool valid_client (int cn) { return clients.inrange (cn) && clients[cn]->type != ST_EMPTY; }
...
void sendpacket (int n, int chan, ENetPacket *packet, int exclude, bool demopacket)
{
    if (n < 0)
    {
        ...
        loopv (clients) if (i != exclude && (clients[i]->type != ST_TCPIP || clients[i]->isauthed))
...
bool buildworldstate ()
{
    ...
    loopv (clients)
    {
        ...
        {
            packet
            = enet_packet_create (&ws.positions[pkt[i].posoff < 0 ? 0 : pkt[i].posoff + pkt[i].poslen],
...
int numactiveclients ()
{
    ...
    loopv (clients) if (clients[i]->type != ST_EMPTY && clients[i]->isauthed
                        && clients[i]->isonrightmap && team_isactive (clients[i]->team)) num++;
    return num;
}
...
int *numteamclients (int exclude = -1)
{
    ...
    loopv (clients) if (i != exclude && clients[i]->type != ST_EMPTY && clients[i]->isauthed
                        && clients[i]->isonrightmap && team_isvalid (clients[i]->team))
    num[clients[i]->team]++;
    return num;
}
...
void changemastermode (int newmode)
{
    if (mastermode != newmode)
    {
        ...
        if (mastermode != MM_MATCH)
        {
            loopv (clients) if (clients[i]->type != ST_EMPTY && clients[i]->isauthed)
            {
                if (clients[i]->team == TEAM_CLA_SPECT || clients[i]->team == TEAM_RVSF_SPECT)
...
int findcnbyaddress (ENetAddress *address)
{
    loopv (clients)
    {
        if (clients[i]->type == ST_TCPIP && clients[i]->peer->address.host == address->host
...
savedscore *findscore (client &c, bool insert)
{
    ...
    mastermode == MM_MATCH ? 0xFFFF0000 : 0xFFFFFFFF); // in match mode, reconnecting from
    ...
    if (!insert)
    {
        loopv (clients)
        {
            ...
            if (o.clientnum != c.clientnum && o.peer->address.host == c.peer->address.host
    ...
    loopv (savedscores)
    {
        ...
        if (!strcmp (sc.name, c.name) && (sc.ip & mask) == (c.peer->address.host & mask))
            return &sc;
    }
...
void sendextras ()
{
    ...
    {
        client &c = *clients[i];
        if (c.type != ST_TCPIP || !c.isauthed || !(c.md.updated && c.md.upmillis < gamemillis))
            continue;
        if (c.md.combosend)
        {
...
void sendservmsg (const char *msg, int cn = -1) { sendf (cn, 1, "ris", SV_SERVMSG, msg); }
...
void sendspawn (client *c)
{
    ...
    gs.spawnstate (smode);
    gs.lifesequence++;
    sendf (c->clientnum, 1, "ri7vv", SV_SPAWNSTATE, gs.lifesequence, gs.health, gs.armour,
           gs.primary, gs.gunselect, m_arena ? c->spawnindex : -1, NUMGUNS, gs.ammo, NUMGUNS, gs.mag);
...
void enddemorecord ()
{
    ...
    formatstring (d.info) ("%s: %s, %s, %.2f%s", asctimestr (), modestr (gamemode), smapname,
    ...
    if (scl.demopath[0])
    {
        formatstring (msg) ("%s%s.dmo", scl.demopath,
...
void setupdemorecord ()
{
    ...
    defformatstring (desc) ("%s, %s, %s %s", modestr (gamemode, false), behindpath (smapname),
                            asctimestr (), servdesc_current);
    if (strlen (desc) > DHDR_DESCCHARS)
        formatstring (desc) ("%s, %s, %s %s", modestr (gamemode, true), behindpath (smapname),
                             asctimestr (), servdesc_current);
...
void setupdemoplayback ()
{
    ...
    else
    {
        ...
        if (hdr.version != DEMO_VERSION)
            formatstring (msg) ("demo \"%s\" requires an %s version of AssaultCube", file,
                                hdr.version < DEMO_VERSION ? "older" : "newer");
        else if (hdr.protocol != PROTOCOL_VERSION
                 && !(hdr.protocol < 0 && hdr.protocol == -PROTOCOL_VERSION) && hdr.protocol != 1132)
            formatstring (msg) ("demo \"%s\" requires an %s version of AssaultCube", file,
                                hdr.protocol < PROTOCOL_VERSION ? "older" : "newer");
        else if (hdr.protocol == 1132)
            sendservmsg ("WARNING: using experimental compatibility mode for older demo "
...
bool flagdistance (sflaginfo &f, int cn)
{
    ...
    {
        c.farpickups++;
        logline (ACLOG_INFO, "[%s] %s %s the %s flag at distance %.2f (%d)", c.hostname, c.name,
                 (pdist == 2 ? "tried to touch" : "touched"), team_string (&f == sflaginfos + 1),
                 dist, c.farpickups);
...
void flagaction (int flag, int action, int actor)
{
    ...
    bool deadactor = valid_client (actor) ?
                     clients[actor]->state.state != CS_ALIVE || team_isspect (clients[actor]->team) :
                     true;
    int abort = 0;
    int score = 0;
    int message = -1;
    ...
    if (m_ctf || m_htf)
    {
        ...
        case FA_STEAL:
        {
            if (deadactor || f.state != (action == FA_STEAL ? CTFF_INBASE : CTFF_DROPPED)
        ...
        case FA_SCORE: // ctf: f = carried by actor flag,  htf: f = hunted flag
            ...
            if (m_ctf)
            {
                if (f.state != CTFF_STOLEN || f.actor_cn != actor || of.state != CTFF_INBASE
    ...
    if (valid_client (actor))
    {
        ...
        case FM_SCORE:
            if (m_htf)
                logline (ACLOG_INFO, "[%s] %s hunted the flag for %s, new score %d", c.hostname,
                         c.name, team_string (c.team), c.state.flagscore);
            else
                logline (ACLOG_INFO, "[%s] %s scored with the flag for %s, new score %d",
                         c.hostname, c.name, team_string (c.team), c.state.flagscore);
            break;
        case FM_KTFSCORE:
            logline (ACLOG_INFO, "[%s] %s scored, carrying for %d seconds, new score %d",
                     c.hostname, c.name, (gamemillis - f.stolentime) / 1000, c.state.flagscore);
            break;
        case FM_SCOREFAIL:
    ...
    else
    {
        ...
        case FM_RESET:
            ...
            break;
        default:
            logline (ACLOG_INFO, "flagaction %d with invalid actor cn %d, flag %d, message %d",
    ...
    sendflaginfo (flag);
    if (message >= 0) flagmessage (flag, message, valid_client (actor) ? actor : -1);
}
...
{
    loopv (clients) if (clients[i]->type != ST_EMPTY) { clients[i]->spawnindex = -1; }
    if (m_teammode)
    {
        distributeteam (0);
        distributeteam (1);
...
{
    client *c = clients[n];
    int waitspawn = min (c->ping, 200) + c->state.spawn; // flowtron to Brahma: can't this be
...
void arenacheck ()
{
    ...
    if (m_botmode && clients[0]->type == ST_LOCAL)
    {
        ...
        if (enemies && (!alive_enemies || player1->state == CS_DEAD))
        {
            sendf (-1, 1, "ri2", SV_ARENAWIN,
                   ...
                   (alive && alive->type == ENT_BOT ? -2 : player1->state == CS_ALIVE ? player1->clientnum : -1));
    ...
    loopv (clients)
    {
        ...
        if (c.state.state == CS_ALIVE
            || ((c.state.state == CS_DEAD || c.state.state == CS_SPECTATE) && c.state.lastspawn >= 0))
        {
            if (!alive)
                alive = &c;
...
void sendvoicecomteam (int sound, int sender)
{
    ...
    loopv (clients) if (i != sender)
    {
        if (clients[i]->team == clients[sender]->team || !m_teammode) sendpacket (i, 1, packet);
    }
}
...
{
    const char *hn = sender >= 0 && clients[sender]->type == ST_TCPIP ? clients[sender]->hostname : nullptr;
    if (!sents.inrange (i))
    {
        if (hn && !m_coop)
            logline (ACLOG_INFO, "[%s] tried to pick up entity #%d - doesn't exist on this map", hn, i);
        return false;
    }
    ...
    if (sender >= 0)
    {
        ...
        if (cl->type == ST_TCPIP)
        {
            if (cl->state.state != CS_ALIVE || !cl->state.canpickup (e.type) || (m_arena && !free_items (sender)))
                return false;
            ...
            if (pdist)
            {
                cl->farpickups++;
                if (!m_demo)
                    logline (ACLOG_INFO, "[%s] %s %s up entity #%d (%s), distance %.2f (%d)",
                             cl->hostname, cl->name, (pdist == 2 ? "tried to pick" : "picked"), i,
                             entnames[e.type], dist, cl->farpickups);
                if (pdist == 2) return false;
            }
        }
...
void serverdamage (client *target, client *actor, int damage, int gun, bool gib, const vec &hitpush = vec (0, 0, 0))
{
    ...
    if (m_arena && gun == GUN_GRENADE && arenaroundstartmillis + 2000 > gamemillis && target != actor)
        return;
    ...
    if (damage < INT_MAX)
    {
        ...
        sendf (-1, 1, "ri7", gib ? SV_GIBDAMAGE : SV_DAMAGE, target->clientnum, actor->clientnum,
               gun, damage, ts.armour, ts.health);
        if (target != actor)
        {
            ...
            if (!hitpush.iszero ())
            {
                ...
                sendf (target->clientnum, 1, "ri6", SV_HITPUSH, gun, damage, int(v.x * DNF),
                       int(v.y * DNF), int(v.z * DNF));
            }
        }
    }
    if (ts.health <= 0)
    {
        ...
        sendf (-1, 1, "ri5", gib ? SV_GIBDIED : SV_DIED, target->clientnum, actor->clientnum,
               actor->state.frags, gun);
        ...
        if (isdedicated && actor->type == ST_TCPIP && tk)
        {
            if (actor->state.frags < scl.banthreshold ||
                ...
                (actor->state.teamkills >= -scl.banthreshold && actor->state.teamkills * 30 * 1000 > gamemillis
                 && actor->state.frags < 4 * actor->state.teamkills))
            {
                addban (actor, DISC_AUTOBAN);
                     ...
                     (actor->state.teamkills >= -scl.kickthreshold && actor->state.teamkills * 60 * 1000 > gamemillis
                      && actor->state.frags < 4 * actor->state.teamkills))
...
bool updatedescallowed () { return scl.servdesc_pre[0] || scl.servdesc_suf[0]; }
...
int canspawn (client *c) // beware: canspawn() doesn't check m_arena!
{
    if (!c || c->type == ST_EMPTY || !c->isauthed || !team_isvalid (c->team)
        || (c->type == ST_TCPIP
            && (c->state.lastdeath > 0 ? gamemillis - c->state.lastdeath : servmillis - c->connectmillis)
...
void autospawncheck ()
{
    ...
    {
        client *cl = clients[i];
        if ((cl->state.state == CS_DEAD || cl->state.state == CS_SPECTATE) && canspawn (cl) == SP_OK
            && !cl->autospawn)
        {
            sendspawn (cl);
            cl->autospawn = true;
...
bool updateclientteam (int cln, int newteam, int ftr)
{
    ...
    if (mastermode == MM_OPEN && cl.state.forced && ftr == FTR_PLAYERWISH && newteam < TEAM_SPECT
        && team_base (cl.team) != team_base (newteam))
    ...
    if (newteam == TEAM_ANYACTIVE) // when spawning from spect
    {
        ...
        else
        {
            if (autoteam && teamsizes[TEAM_CLA] != teamsizes[TEAM_RVSF])
                newteam = teamsizes[TEAM_CLA] < teamsizes[TEAM_RVSF] ? TEAM_CLA : TEAM_RVSF;
            else
            { // join weaker team
                int teamscore[2] = { 0, 0 }, sum = calcscores ();
                loopv (clients) if (clients[i]->type != ST_EMPTY && i != cln && clients[i]->isauthed
                                    && clients[i]->team != TEAM_SPECT)
                {
                    teamscore[team_base (clients[i]->team)] += clients[i]->at3_score;
                }
                newteam = sum > 200 ? (teamscore[TEAM_CLA] < teamscore[TEAM_RVSF] ? TEAM_CLA : TEAM_RVSF) :
                                      rnd (2);
            }
        }
    }
    if (ftr == FTR_PLAYERWISH)
    {
        if (mastermode == MM_MATCH && matchteamsize && m_teammode)
        {
            if (newteam != TEAM_SPECT && (team_base (newteam) != team_base (cl.team) || !m_teammode))
        ...
        if (team_isactive (newteam))
        {
            ...
            {
                if (m_teammode && autoteam && teamsizes[newteam] > teamsizes[team_opposite (newteam)])
                    return false; // don't switch to an already bigger team
            }
        }
        else if (mastermode != MM_MATCH || !m_teammode)
            newteam = TEAM_SPECT; // only match mode (team) has more than one spect team
    }
    if (cl.team == newteam && ftr != FTR_AUTOTEAM) return true; // no change
    if (cl.team != newteam) sdropflag (cl.clientnum);
    if (ftr != FTR_INFO && (team_isspect (newteam) || (team_isactive (newteam) && team_isactive (cl.team))))
        forcedeath (&cl);
...
void shuffleteams (int ftr = FTR_AUTOTEAM)
{
    ...
    { // random
        int teamsize[2] = { 0, 0 };
        loopv (clients) if (clients[i]->type != ST_EMPTY && clients[i]->isonrightmap
                            && !team_isspect (clients[i]->team)) // only shuffle active players
        {
            sums += rnd (1000);
            team = sums & 1;
...
bool balanceteams (int ftr) // pro vs noobs never more
{
    ...
    { // the h score team has less or the same player number, so, lets
      // exchange
        loopv (clients) if (clients[i]->type != ST_EMPTY)
        {
            client *c = clients[i]; // loop for h
...
bool refillteams (bool now, int ftr) // force only minimal amounts of players
{
    ...
    if (diffnum > 1)
    {
        if (now || gamemillis - lasttime_eventeams > 8000 + allplayers * 1000 || diffnum > 2 + allplayers / 10)
        {
            ...
            loopv (clients) if (clients[i]->type != ST_EMPTY && clients[i]->team != bigteam) clients[i]
            ->at3_dontmove
            = true; // dont move small team players
            while (diffnum > 1 && moveable[bigteam] > 0)
            {
                ...
                loopv (clients) if (clients[i]->type != ST_EMPTY && !clients[i]->at3_dontmove) // try all still movable players
                {
                    ...
                    int forcedelay = clients[i]->at3_lastforce ?
                ...
                if (updateclientteam (pick, !bigteam, ftr))
                {
                    ...
                    clients[pick]->at3_lastforce = gamemillis; // try not to force this player again
...
void startgame (const char *newname, int newmode, int newtime, bool notify)
{
    ...
    else
    {
        ...
        if (ms)
        {
            ...
            sendservmsg ("\f3server error: map not found - please start another map or send "
        ...
        defformatstring (gsmsg) ("Game start: %s on %s, %d players, %d minutes, mastermode %d, ",
        ...
        if (ms)
            concatformatstring (gsmsg, "(map rev %d/%d, %s, 'getmap' %sprepared)", smapstats.hdr.maprevision,
                                smapstats.cgzsize, maplocstr[maploc], mapbuffer.available () ? "" : "not ");
        else
            concatformatstring (gsmsg, "error: failed to preload map (map: %s)", maplocstr[maploc]);
        logline (ACLOG_INFO, "\n%s", gsmsg);
...
struct voteinfo
{
    ...
    void evaluate (bool forceend = false)
    {
        ...
        loopv (clients) if (clients[i]->type
...
#define yes_condition                                                                   \
    ((min_time && stats[VOTE_YES] - stats[VOTE_NO] > 0.34f * total && totalclients > 4) \
     || stats[VOTE_YES] > requiredcount * total)
#define no_condition                                                                              \
    (forceend || !valid_client (owner) || stats[VOTE_NO] >= stats[VOTE_YES] + stats[VOTE_NEUTRAL] \
     || adminvote == VOTE_NO)
#define boot_condition \
    (!boot || (boot && valid_client (num1) && clients[num1]->peer->address.host == host))
...
void scallvotesuc (voteinfo *v)
{
    ...
    logline (ACLOG_INFO, "[%s] client %s called a vote: %s", clients[v->owner]->hostname,
             clients[v->owner]->name, v->action && *v->action->desc ? v->action->desc : "[unknown]");
}
...
void scallvoteerr (voteinfo *v, int error)
{
    ...
    logline (ACLOG_INFO, "[%s] client %s failed to call a vote: %s (%s)",
             ...
             v->action && *v->action->desc ? v->action->desc : "[unknown]", voteerrorstr (error));
}
                ...
                ENetPacket *msg) // true if a regular vote was called
{
    ...
    else if ((c->lastvotecall && servmillis - c->lastvotecall < 60 * 1000 && c->role != CR_ADMIN
              && numclients () > 1)
             || c->nvotes > 3)
        error = VOTEE_MAX;
             ...
             && !is_lagging (b) && !b->mute && b->spamcount < 2)
    {
         ...
         * to kick? */
        if (!isteam (c->team, b->team) && b->state.frags < (b->state.deaths > 0 ? b->state.deaths : 1) * 3)
            error = VOTEE_WEAK;
        /** same team, with low tk, not lagging, and not spamming... so, why to
         * kick? */
        else if (isteam (c->team, b->team) && b->state.teamkills < c->state.teamkills)
            error = VOTEE_WEAK;
    }
...
{ // FIXME, it would be far smart if the msg buffer
...
void changeclientrole (int client, int role, char *pwd, bool force)
{
    ...
    if (force || role == CR_DEFAULT
        || (role == CR_ADMIN && pwd && pwd[0]
            && passwords.check (clients[client]->name, pwd, clients[client]->salt, &pd) && !pd.denyadmin))
    {
        ...
        sendserveropinfo (-1);
        if (pd.line > -1)
            logline (ACLOG_INFO, "[%s] player %s used admin password in line %d", clients[client]->hostname,
                     clients[client]->name[0] ? clients[client]->name : "[unnamed]", pd.line);
        logline (ACLOG_INFO, "[%s] set role of player %s to %s", clients[client]->hostname,
                 clients[client]->name[0] ? clients[client]->name : "[unnamed]",
                 role == CR_ADMIN ? "admin" : "normal player"); // flowtron : connecting players haven't
...
const char *disc_reason (int reason)
{
    ...
    return reason >= 0 && (size_t)reason < sizeof (disc_reasons) / sizeof (disc_reasons[0]) ?
           disc_reasons[reason] :
           "unknown";
}
...
void disconnect_client (int n, int reason)
{
    ...
    if (reason >= 0)
        logline (ACLOG_INFO, "[%s] disconnecting client %s (%s) cn %d, %d seconds played%s",
                 c.hostname, c.name, disc_reason (reason), n, sp, scoresaved);
    else
        logline (ACLOG_INFO, "[%s] disconnected client %s cn %d, %d seconds played%s", c.hostname,
                 c.name, n, sp, scoresaved);
    totalclients--;
    c.peer->data = (void *)-1;
...
{
    sendf (broadcast ? -1 : c.clientnum, 1, "ri3i9ivvi", SV_RESUME, c.clientnum, c.state.state,
           c.state.lifesequence, c.state.primary, c.state.gunselect, c.state.flagscore,
           c.state.frags, c.state.deaths, c.state.health, c.state.armour, c.state.points,
           c.state.teamkills, NUMGUNS, c.state.ammo, NUMGUNS, c.state.mag, -1);
}
...
void sendservinfo (client &c)
{
    sendf (c.clientnum, 1, "ri5", SV_SERVINFO, c.clientnum,
           isdedicated ? SERVER_PROTOCOL_VERSION : PROTOCOL_VERSION, c.salt, scl.serverpassword[0] ? 1 : 0);
}
...
int checktype (int type, client *cl)
{
    ...
    = { SV_SERVINFO,    SV_WELCOME,      SV_INITCLIENT, SV_POSN,         SV_CDIS,     SV_GIBDIED,
        SV_DIED,        SV_GIBDAMAGE,    SV_DAMAGE,     SV_HITPUSH,      SV_SHOTFX,   SV_AUTHREQ,
        SV_AUTHCHAL,    SV_SPAWNSTATE,   SV_SPAWNDENY,  SV_FORCEDEATH,   SV_RESUME,   SV_DISCSCORES,
        SV_TIMEUP,      SV_ITEMACC,      SV_MAPCHANGE,  SV_ITEMSPAWN,    SV_PONG,     SV_SERVMSG,
        SV_ITEMLIST,    SV_FLAGINFO,     SV_FLAGMSG,    SV_FLAGCNT,      SV_ARENAWIN, SV_SERVOPINFO,
        SV_CALLVOTESUC, SV_CALLVOTEERR,  SV_VOTERESULT, SV_SETTEAM,      SV_TEAMDENY, SV_SERVERMODE,
        SV_IPLIST,      SV_SENDDEMOLIST, SV_SENDDEMO,   SV_DEMOPLAYBACK, SV_CLIENT,   SV_HUDEXTRAS,
        SV_POINTS };
    // only allow edit messages in coop-edit mode
    static int edittypes[]
    = { SV_EDITENT, SV_EDITXY, SV_EDITARCH, SV_EDITBLOCK, SV_EDITD, SV_EDITE, SV_NEWMAP };
    if (cl)
    {
        loopi (sizeof (servtypes) / sizeof (int)) if (type == servtypes[i]) return -1;
        loopi (sizeof (edittypes) / sizeof (int)) if (type == edittypes[i]) return smode == GMODE_COOPEDIT ?
        type :
        -1;
...
void process (ENetPacket *packet, int sender, int chan)
{
    ...
    if (cl && !cl->isauthed)
    {
        ...
        else
        {
            ...
            { // nickname matches whitelist, but IP is
            ...
            { // nickname matches blacklist
                logline (ACLOG_INFO, "[%s] '%s' matches nickname blacklist line %d%s", cl->hostname,
                         cl->name, bl, tags);
                disconnect_client (sender, DISC_BADNICK);
            }
            else if (passwords.check (cl->name, cl->pwd, cl->salt, &pd,
                                      (cl->type == ST_TCPIP ? cl->peer->address.host : 0))
                     && (!pd.denyadmin || (banned && !srvfull && !srvprivate)) && bantype != BAN_MASTER) // pass admins always through
            { // admin (or deban) password match
                ...
                logline (ACLOG_INFO, "[%s] %s logged in using the admin password in line %d%s",
            ...
            { // server password required
                ...
                {
                    cl->isauthed = true;
                    logline (ACLOG_INFO, "[%s] %s client logged in (using serverpassword)%s",
        ...
        if (cl->type == ST_TCPIP)
        {
            loopv (clients) if (i != sender)
            {
                ...
                if (dup->type == ST_TCPIP && dup->peer->address.host == cl->peer->address.host
        ...
        sendinitclient (*cl);
        if (clientrole != CR_DEFAULT) changeclientrole (sender, clientrole, nullptr, true);
    ...
    while ((curmsg = p.length ()) < p.maxlen)
    {
            ...
            if (*text)
            {
                ...
                if (!spamdetect (cl, text) && canspeech) // team chat
                {
                    logline (ACLOG_INFO, "[%s] %s%s says to team %s: '%s'", cl->hostname,
                             type == SV_TEAMTEXTME ? "(me) " : "", cl->name, team_string (cl->team), text);
                    sendteamtext (text, sender, type);
                }
                else
                {
                    logline (ACLOG_INFO, "[%s] %s%s says to team %s: '%s', %s", cl->hostname,
                             type == SV_TEAMTEXTME ? "(me) " : "", cl->name, team_string (cl->team),
                             text, canspeech ? "SPAM detected" : "Forbidden speech");
                    if (canspeech)
                    {
        ...
        case SV_TEXT:
        {
            ...
            if (*text)
            {
                ...
                {
                    if (mastermode != MM_MATCH || !matchteamsize || team_isactive (cl->team)
                        || (cl->team == TEAM_SPECT && cl->role == CR_ADMIN)) // common chat
                    {
        ...
        case SV_TEXTPRIVATE:
        {
            ...
            if (*text)
            {
                ...
                if (!spamdetect (cl, text) && canspeech)
                {
                    bool allowed = !(mastermode == MM_MATCH && cl->team != target->team)
                                   && cl->role >= roleconf ('t');
                    logline (ACLOG_INFO, "[%s] %s says to %s: '%s' (%s)", cl->hostname, cl->name,
                             target->name, text, allowed ? "allowed" : "disallowed");
                    if (allowed)
                ...
                {
                    logline (ACLOG_INFO, "[%s] %s says to %s: '%s', %s", cl->hostname, cl->name,
                             target->name, text, canspeech ? "SPAM detected" : "Forbidden speech");
                    if (canspeech)
                    {
                        sendservmsg ("\f3Please do not spam; your message was not delivered.", sender);
                        if (cl->spamcount > SPAMMAXREPEAT + 2)
        ...
        case SV_MAPIDENT:
        {
            ...
            { // here any game really starts for a client: spawn, if
              ...
              // already running
                ...
                if (cl->loggedwrongmap)
                    logline (ACLOG_INFO, "[%s] %s is now on the right map: revision %d/%d",
            ...
            {
                forcedeath (cl);
                logline (ACLOG_INFO, "[%s] %s is on the wrong map: revision %d/%d", cl->hostname,
                         cl->name, rev, gzs);
        ...
        case SV_SWITCHNAME:
        {
            ...
            if (namechanged)
            {
                ...
                case NWL_PWDFAIL:
                case NWL_IPFAIL:
                    logline (ACLOG_INFO, "[%s] '%s' matches nickname whitelist: wrong IP/PWD",
                             cl->hostname, cl->name);
                    disconnect_client (sender, DISC_BADNICK);
                    break;
                ...
                case NWL_UNLISTED:
                {
                    ...
                    if (l >= 0)
                    {
                        logline (ACLOG_INFO, "[%s] '%s' matches nickname blacklist line %d",
        ...
        {
            int t = getint (p);
            if (!updateclientteam (sender, team_isvalid (t) ? t : TEAM_SPECT, FTR_PLAYERWISH))
        ...
        case SV_TRYSPAWN:
        {
            ...
            if (!m_arena && sp < SP_OK_NUM && gamemillis > cl->state.lastspawn + 1000)
                sendspawn (cl);
            break;
        }
...
#define seteventmillis(event)                                                                   \
    {                                                                                           \
        ...
        {                                                                                       \
            ...
            loopk (3)
            {
                shot.shot.from[k] = cl->state.o.v[k] + (k == 2 ? (((cl->f >> 7) & 1) ? 2.2f : 4.2f) : 0);
            }
            loopk (3)
            {
        ...
        case SV_POSC:
        {
            ...
            if (((cl->f >> 6) & 1) != (cl->state.lifesequence & 1)
                || usefactor != (smapstats.hdr.sfactor < 7 ? 7 : smapstats.hdr.sfactor))
                break;
            cl->state.o[0] = xt / DMF;
            cl->state.o[1] = yt / DMF;
            cl->state.o[2] = zt / DMF;
        ...
        case SV_SENDMAP:
        {
            ...
            else if (mp == MAP_TEMP && revision >= mapbuffer.revision && !strchr (scl.mapperm, 'u')
            ...
            else if (mp == MAP_TEMP && revision < mapbuffer.revision && !strchr (scl.mapperm, 'r')
                     && cl->role < CR_ADMIN) // default: only admins can revert maps
                                             // to older revisions
            {
                reject = "no permission to revert revision";
                sendservmsg ("\f3map revert to older revision rejected: you need to be admin "
            ...
            else
            {
                if (mapbuffer.sendmap (sentmap, mapsize, cfgsize, cfgsizegz, &p.buf[p.len]))
                {
                    ...
                    logline (ACLOG_INFO, "[%s] %s sent map %s, rev %d, %d + %d(%d) bytes written",
                             clients[sender]->hostname, clients[sender]->name, sentmap, revision,
                             mapsize, cfgsize, cfgsizegz);
                    defformatstring (msg) ("%s (%d) up%sed map %s, rev %d%s", clients[sender]->name,
        ...
        case SV_REMOVEMAP:
        {
            ...
            int reqrole = strchr (scl.mapperm, 'D') ?
                          CR_ADMIN :
...
void localclienttoserver (int chan, ENetPacket *packet) { process (packet, 0, chan); }
...
void checkintermission ()
{
    ...
    {
        minremain
        = (gamemillis >= gamelimit || forceintermission) ? 0 : (gamelimit - gamemillis + 60000 - 1) / 60000;
        sendf (-1, 1, "ri3", SV_TIMEUP,
               (gamemillis >= gamelimit || forceintermission) ? gamelimit : gamemillis, gamelimit);
    }
...
void loggamestatus (const char *reason)
{
    ...
    logline (ACLOG_INFO, "");
    logline (ACLOG_INFO, "Game status: %s on %s, %s, %s, %d clients%c %s", modestr (gamemode),
             smapname, reason ? reason : text, mmfullname (mastermode), totalclients,
             custom_servdesc ? ',' : '\0', servdesc_current);
    if (!scl.loggamestatus) return;
    logline (ACLOG_INFO, "cn name             %s%s score frag death %sping role    host",
             m_teammode ? "team " : "", m_flags ? "flag " : "", m_teammode ? "tk " : "");
    loopv (clients)
    {
        client &c = *clients[i];
        ...
        if (m_teammode)
            concatformatstring (text, "%-4s ", team_string (c.team, true)); // teamname (abbreviated)
        ...
        logline (ACLOG_INFO, "%s%5d %s  %s", text, c.ping, c.role == CR_ADMIN ? "admin " : "normal",
                 c.hostname);
    ...
    if (mastermode == MM_MATCH)
    {
        loopv (savedscores)
        {
            ...
            if (sc.valid)
            {
                ...
                logline (ACLOG_INFO, "   %-16s %s%4d %5d%s    - disconnected", sc.name, text,
                         sc.frags, sc.deaths, m_teammode ? "  -" : "");
    ...
    if (m_teammode)
    {
        loopi (2) logline (ACLOG_INFO, "Team %4s:%3d players,%5d frags%c%5d flags", team_string (i),
                           pnum[i], fragscore[i], m_flags ? ',' : '\0', flagscore[i]);
    }
    logline (ACLOG_INFO, "");
}
                                ...
                                // loop in sp, or dedicated server loop
{
    ...
    if (minremain > 0)
    {
        ...
        if (m_flags) loopi (2)
            {
                sflaginfo &f = sflaginfos[i];
                if (f.state == CTFF_DROPPED && gamemillis - f.lastupdate > (m_ctf ? 30000 : 10000))
        ...
        //        if(m_lms) lmscheck();
        sendextras ();
        if (scl.afk_limit && mastermode == MM_OPEN && next_afk_check < servmillis && gamemillis > 20 * 1000)
            check_afk ();
    }
    ...
    if (forceintermission
        || ((smode > 1 || (gamemode == 0 && nonlocalclients)) && gamemillis - diff > 0
    ...
    serverms (smode, numclients (), minremain, smapname, servmillis, serverhost->address, &mnum,
              &msend, &mrec, &cnum, &csend, &crec, SERVER_PROTOCOL_VERSION);

    if (autoteam && m_teammode && !m_arena && !interm && servmillis - lastfillup > 5000 && refillteams ())
        lastfillup = servmillis;
    ...
    if (servmillis - laststatus > 60 * 1000) // display bandwidth stats, useful for server ops
    {
        ...
        if (nonlocalclients || serverhost->totalSentData || serverhost->totalReceivedData)
        {
            ...
            logline (ACLOG_INFO,
                     ...
                     serverhost->totalReceivedData / 60.0f / 1024, mnum, msend, mrec, cnum, csend, crec);
    ...
    while (!serviced)
    {
        ...
        case ENET_EVENT_TYPE_CONNECT:
        {
            ...
            char hn[1024];
            copystring (c.hostname,
                        (enet_address_get_host_ip (&c.peer->address, hn, sizeof (hn)) == 0) ? hn : "unknown");
...
void extping_serverinfo (ucharbuf &pi, ucharbuf &po)
{
    ...
    if (buf)
    {
        for (const char *c = buf; *c && po.remaining () > MAXINFOLINELEN + 10; c += strlen (c) + 1)
            sendstring (c, po);
        sendstring ("", po);
    }
}
...
void extinfo_cnbuf (ucharbuf &p, int cn)
{
    ...
    {
        loopv (clients) if (clients[i]->type != ST_EMPTY) putint (p, clients[i]->clientnum);
    }
...
#ifndef STANDALONE
void localdisconnect () { loopv (clients) if (clients[i]->type == ST_LOCAL) clients[i]->zap (); }
...
void quitproc (int param)
{
    ...
    exit (param == 2 ? EXIT_SUCCESS : EXIT_FAILURE); // 3 is the only reply on Win32 apparently,
                                                     // SIGINT == 2 == Ctrl-C
}
...
void initserver (bool dedicated, int argc, char **argv)
{
    ...
    if (scl.logident[0])
        ...
        formatstring (identity) ("%s#%d", scl.ip[0] ? scl.ip : "local", scl.serverport);
    int conthres = scl.verbose > 1 ? ACLOG_DEBUG : (scl.verbose ? ACLOG_VERBOSE : ACLOG_INFO);
    if (dedicated
        && !initlogging (identity, scl.syslogfacility, conthres, scl.filethres, scl.syslogthres, scl.logtimestamp))
        printf ("WARNING: logging not started!\n");
    logline (ACLOG_INFO, "logging local AssaultCube server (version %d, protocol %d/%d) now..",
    ...
    if ((isdedicated = dedicated))
    {
        ...
        if (scl.demopath[0])
            logline (ACLOG_VERBOSE, "all recorded demos will be written to: \"%s\"", scl.demopath);
        if (scl.voteperm[0])
        ...
        logline (ACLOG_VERBOSE, "maxclients: %d, kick threshold: %d, ban threshold: %d",
        ...
        if (scl.serverpassword[0])
            logline (ACLOG_VERBOSE, "server password: \"%s\"", hiddenpwd (scl.serverpassword));
#ifdef ACAC
