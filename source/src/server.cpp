...
int interm = 0;
static int minremain = 0, gamemillis = 0, gamelimit = 0,
           /*lmsitemtype = 0,*/ nextsendscore = 0;
mapstats smapstats;
vector<server_entity> sents;
char *maplayout = nullptr, *testlayout = nullptr;
...
void poll_serverthreads()  // called once per mainloop-timeslice
{
  ...
  switch (stage) {
    ...
    {
      if (startnewservermapsepoch || readmapsthread_sem->getvalue())
        fatal("thread management mishap");  // zero tolerance...
    ...
    {
      auto *fresh = (servermap *)servermapdropbox;
      if (fresh) {
        if (fresh == servermapdropbox) {
          // got new servermap...
          loopv(servermaps) {
            if (!strcmp(servermaps[i]->fname,
                        fresh->fname))  // we don't check paths here - map
                                        // filenames have to be unique
            {                           // found map of same name
              logline(ACLOG_INFO, "marked servermap %s%s for deletion",
                      servermaps[i]->fpath, servermaps[i]->fname);
              servermapstodelete.add(
                  servermaps.remove(i));  // mark old version for deletion
            }
          }
          if (fresh->isok) {
            servermaps.add(fresh);
            logline(ACLOG_INFO, "added servermap %s%s", fresh->fpath,
                    fresh->fname);
          }
          servermapdropbox = nullptr;
        }
        readmapsthread_sem->post();
...
#endif
    {
      if (servmillis - lastworkerthreadstart > 60 * 1000)
        stage = 0;
      else if (numclients() == 0) {  // empty server and nothing to do:
        loopvrev(servermapstodelete) delete servermapstodelete.remove(
            i);  // delete outdated servermaps
      }
      break;
    }
  }
...
void cleanworldstate(ENetPacket *packet) {
  loopv(worldstates) {
    ...
    if (ws->positions.inbuf(packet->data) || ws->messages.inbuf(packet->data))
      ws->uses--;
    else
      continue;
    if (!ws->uses) {
...
void sendpacket(int n, int chan, ENetPacket *packet, int exclude,
  ...
  if (n < 0) {
    ...
    loopv(clients) if (i != exclude &&
                       (clients[i]->type != ST_TCPIP || clients[i]->isauthed))
  ...
  loopv(clients) {
    ...
    if (psize && (pkt[i].posoff < 0 || psize - pkt[i].poslen > 0)) {
      packet = enet_packet_create(
          &ws.positions[pkt[i].posoff < 0 ? 0 : pkt[i].posoff + pkt[i].poslen],
    ...
    if (msize && (pkt[i].msgoff < 0 || msize - pkt[i].msglen > 0)) {
      packet = enet_packet_create(
          &ws.messages[pkt[i].msgoff < 0 ? 0 : pkt[i].msgoff + pkt[i].msglen],
          pkt[i].msgoff < 0 ? msize : msize - pkt[i].msglen,
          (reliablemessages ? ENET_PACKET_FLAG_RELIABLE : 0) |
              ENET_PACKET_FLAG_NO_ALLOCATE);
...
int numactiveclients() {
  int num = 0;
  loopv(clients) if (clients[i]->type != ST_EMPTY && clients[i]->isauthed &&
                     clients[i]->isonrightmap &&
                     team_isactive(clients[i]->team)) num++;
  return num;
}
...
int *numteamclients(int exclude = -1) {
  ...
  loopv(clients) if (i != exclude && clients[i]->type != ST_EMPTY &&
                     clients[i]->isauthed && clients[i]->isonrightmap &&
                     team_isvalid(clients[i]->team)) num[clients[i]->team]++;
  return num;
}

int sendservermode(bool send = true) {
  int sm = (autoteam ? AT_ENABLED : AT_DISABLED) |
           ((mastermode & MM_MASK) << 2) | (matchteamsize << 4);
  ...
  if (mastermode == MM_MATCH && matchteamsize && m_teammode) {
    ...
    loopv(clients) if (clients[i]->type != ST_EMPTY && clients[i]->isauthed &&
      ...
      if (team_isactive(clients[i]->team)) {
        if (++size[clients[i]->team] > matchteamsize)
          updateclientteam(i, team_tospec(clients[i]->team), FTR_SILENTFORCE);
      }
    }
  }
}
    ...
    if (mastermode != MM_MATCH) {
      loopv(clients) if (clients[i]->type != ST_EMPTY && clients[i]->isauthed) {
        if (clients[i]->team == TEAM_CLA_SPECT ||
            clients[i]->team == TEAM_RVSF_SPECT)
          updateclientteam(i, TEAM_SPECT, FTR_SILENTFORCE);
      }
    } else if (matchteamsize)
      changematchteamsize(matchteamsize);
    sendservermode();
  }
}
...
int findcnbyaddress(ENetAddress *address) {
  loopv(clients) {
    if (clients[i]->type == ST_TCPIP &&
        clients[i]->peer->address.host == address->host &&
        clients[i]->peer->address.port == address->port)
      return i;
  }
  return -1;
}
...
savedscore *findscore(client &c, bool insert) {
  ...
  enet_uint32 mask = ENET_HOST_TO_NET_32(
      mastermode == MM_MATCH ? 0xFFFF0000
                             : 0xFFFFFFFF);  // in match mode, reconnecting from
                                             // /16 subnet is allowed
  if (!insert) {
    loopv(clients) {
      client &o = *clients[i];
      if (o.type != ST_TCPIP || !o.isauthed) continue;
      if (o.clientnum != c.clientnum &&
          o.peer->address.host == c.peer->address.host &&
...
void sendextras() {
  ...
  loopv(clients) {
    client &c = *clients[i];
    if (c.type != ST_TCPIP || !c.isauthed ||
        !(c.md.updated && c.md.upmillis < gamemillis))
      continue;
    if (c.md.combosend) {
      sendf(c.clientnum, 1, "ri2", SV_HUDEXTRAS,
            min(c.md.combo, c.md.combofrags) - 1 + HE_COMBO);
...
void sendspawn(client *c) {
  ...
  gs.spawnstate(smode);
  gs.lifesequence++;
  sendf(c->clientnum, 1, "ri7vv", SV_SPAWNSTATE, gs.lifesequence, gs.health,
        gs.armour, gs.primary, gs.gunselect, m_arena ? c->spawnindex : -1,
...
const char *currentserver(int i) {
  ...
  switch (i) {
    case 1: {
      copystring(r, scl.ip[0] ? scl.ip : "local");
      break;
    }  // IP
    case 2: {
      copystring(r, scl.logident[0] ? scl.logident : "local");
      break;
    }  // HOST
    case 3: {
      formatstring(r)("%d", scl.serverport);
      break;
    }  // PORT
    // the following are used by a client, a server will simply return empty
    // strings for them
    case 4:
    case 5:
    case 6:
...
// these are actually the values used by the client, the server ones are in
// "scl".
...
const char *getDemoFilename(int gmode, int mplay, int mdrop, int tstamp,
                            char *srvmap) {
  // we use the following internal mapping of formatchars:
  // %g : gamemode (int)      %G : gamemode (chr)             %F : gamemode
  // (full) %m : minutes remaining   %M : minutes played %s : seconds remaining
  // %S : seconds played %h : IP of server        %H : hostname of server %n :
  // mapName %w : timestamp "when"
  static string dmofn;
  copystring(dmofn, "");
      ...
      case '%': {
        if (cc < (mc - 1)) {
          ...
          switch (DEMOFORMAT[cc + 1]) {
            case 'F':
              formatstring(cfspp)("%s", fullmodestr(gmode));
              break;
            case 'g':
              formatstring(cfspp)("%d", gmode);
              break;
            case 'G':
              formatstring(cfspp)("%s", acronymmodestr(gmode));
              break;
            case 'h':
              formatstring(cfspp)("%s", currentserver(1));
              break;  // client/server have different implementations
            case 'H':
              formatstring(cfspp)("%s", currentserver(2));
              break;  // client/server have different implementations
            case 'm':
              formatstring(cfspp)("%d", mdrop / 60);
              break;
            case 'M':
              formatstring(cfspp)("%d", mplay / 60);
              break;
            case 'n':
              formatstring(cfspp)("%s", srvmap);
              break;
            case 's':
              formatstring(cfspp)("%d", mdrop);
              break;
            case 'S':
              formatstring(cfspp)("%d", mplay);
              break;
            case 'w': {
              time_t t = tstamp;
              struct tm *timeinfo;
            ...
            default:
              logline(ACLOG_INFO, "bad formatstring: demonameformat @ %d", cc);
              cc -= 1;
              break;  // don't drop the bad char
          }
...
void enddemorecord() {
  ...
  int mr = gamemillis >= gamelimit
               ? 0
               : (gamelimit - gamemillis + 60000 - 1) / 60000;
  demofile &d = demofiles.add();

  // 2010oct10:ft: suggests : formatstring(d.info)("%s, %s, %.2f%s",
  // modestr(gamemode), smapname, len > 1024*1024 ? len/(1024*1024.f) :
  // len/1024.0f, len > 1024*1024 ? "MB" : "kB"); // the datetime bit is pretty
  // useless in the servmesg, no?!
  formatstring(d.info)(
      "%s: %s, %s, %.2f%s", asctimestr(), modestr(gamemode), smapname,
      len > 1024 * 1024 ? len / (1024 * 1024.f) : len / 1024.0f,
      len > 1024 * 1024 ? "MB" : "kB");
  if (mr) {
    concatformatstring(d.info, ", %d mr", mr);
    concatformatstring(d.file, "_%dmr", mr);
  }
  defformatstring(msg)(
  ...
  // formatstring(d.file)("%s_%s_%s", timestring(), behindpath(smapname),
  // modestr(gamemode, true)); // 20100522_10.08.48_ac_mines_DM.dmo
  // formatstring(d.file)("%s_%s_%s", modestr(gamemode, true),
  // behindpath(smapname), timestring( true, "%Y.%m.%d_%H%M")); //
  // DM_ac_mines.2010.05.22_1008.dmo
  ...
  if (scl.demopath[0]) {
    formatstring(msg)(
        "%s%s.dmo", scl.demopath,
        getDemoFilename(gamemode, mPLAY, mDROP, iTIME, iMAPN));  // d.file);
    path(msg);
    stream *demo = openfile(msg, "wb");
    if (demo) {
      auto wlen = (int)demo->write(d.data, d.len);
      delete demo;
...
void setupdemorecord() {
  if (numlocalclients() || !m_mp(gamemode) || gamemode == GMODE_COOPEDIT)
    return;

  defformatstring(demotmppath)("demos/demorecord_%s_%d",
                               scl.ip[0] ? scl.ip : "local", scl.serverport);
  ...
  defformatstring(desc)("%s, %s, %s %s", modestr(gamemode, false),
                        behindpath(smapname), asctimestr(), servdesc_current);
  if (strlen(desc) > DHDR_DESCCHARS)
    formatstring(desc)("%s, %s, %s %s", modestr(gamemode, true),
                       behindpath(smapname), asctimestr(), servdesc_current);
  ...
  loopv(clients) {
    ...
    if (strlen(hdr.plist) + strlen(ci->name) < DHDR_PLISTCHARS - 2) {
      strcat(hdr.plist, bl);
      strcat(hdr.plist, ci->name);
    }
    bl = " ";
  }
...
void senddemo(int cn, int num) {
  ...
  if (scl.demo_interm && (!interm || totalclients > 2) && !is_admin) {
    sendservmsg(
        "\f3sorry, but this server only sends demos at intermission.\n wait "
        "for the end of this game, please",
        cn);
    return;
  }
  ...
  if (!demofiles.inrange(num - 1)) {
    if (demofiles.empty())
      sendservmsg("no demos available", cn);
    else {
  ...
  loopv(d.clientssent) if (d.clientssent[i].ip == cl->peer->address.host &&
                           d.clientssent[i].clientnum == cl->clientnum) {
...
void setupdemoplayback() {
  ...
  if (!demoplayback)
    formatstring(msg)("could not read demo \"%s\"", file);
  else if (demoplayback->read(&hdr, sizeof(demoheader)) != sizeof(demoheader) ||
    ...
    if (hdr.version != DEMO_VERSION)
      formatstring(msg)("demo \"%s\" requires an %s version of AssaultCube",
                        file, hdr.version < DEMO_VERSION ? "older" : "newer");
    else if (hdr.protocol != PROTOCOL_VERSION &&
             !(hdr.protocol < 0 && hdr.protocol == -PROTOCOL_VERSION) &&
             hdr.protocol != 1132)
      formatstring(msg)("demo \"%s\" requires an %s version of AssaultCube",
                        file,
                        hdr.protocol < PROTOCOL_VERSION ? "older" : "newer");
    else if (hdr.protocol == 1132)
      sendservmsg(
          "WARNING: using experimental compatibility mode for older demo "
          "protocol, expect breakage");
  ...
  if (demoplayback->read(&nextplayback, sizeof(nextplayback)) !=
      sizeof(nextplayback)) {
    enddemoplayback();
    return;
  }
...
bool flagdistance(sflaginfo &f, int cn) {
  ...
  if (pdist) {
    ...
    logline(ACLOG_INFO, "[%s] %s %s the %s flag at distance %.2f (%d)",
            c.hostname, c.name, (pdist == 2 ? "tried to touch" : "touched"),
            team_string(&f == sflaginfos + 1), dist, c.farpickups);
    if (pdist == 2) return false;
  }
  return lagging ? false : true;  // today I found a lag hacker :: Brahma,
                                  // 19-oct-2010... lets test it a bit
}
...
void flagmessage(int flag, int message, int actor, int cn = -1) {
  if (message == FM_KTFSCORE)
    sendf(cn, 1, "riiiii", SV_FLAGMSG, flag, message, actor,
          (gamemillis - sflaginfos[flag].stolentime) / 1000);
  else
...
void flagaction(int flag, int action, int actor) {
  ...
  bool deadactor = valid_client(actor)
                       ? clients[actor]->state.state != CS_ALIVE ||
                             team_isspect(clients[actor]->team)
                       : true;
  int abort = 0;
  int score = 0;
  int message = -1;
  ...
  if (m_ctf || m_htf) {
    switch (action) {
      ...
      case FA_STEAL: {
        if (deadactor ||
            f.state != (action == FA_STEAL ? CTFF_INBASE : CTFF_DROPPED) ||
      ...
      case FA_DROP:
        if (f.state != CTFF_STOLEN || f.actor_cn != actor) {
          abort = 12;
          break;
        }
      ...
      case FA_RETURN:
        ...
        message = FM_RETURN;
        break;
      case FA_SCORE:  // ctf: f = carried by actor flag,  htf: f = hunted flag
                      // (run over by actor)
        if (m_ctf) {
          if (f.state != CTFF_STOLEN || f.actor_cn != actor ||
              of.state != CTFF_INBASE || !flagdistance(of, actor)) {
            abort = 14;
            break;
          }
          score = 1;
        ...
        {
          if (f.state != CTFF_DROPPED || !flagdistance(f, actor)) {
            abort = 15;
            break;
          }
  ...
  } else if (m_ktf)  // f: active flag, of: idle flag
  {
    switch (action) {
      case FA_STEAL:
        if (deadactor || f.state != CTFF_INBASE || !flagdistance(f, actor)) {
          abort = 20;
          break;
        }
        f.state = CTFF_STOLEN;
      ...
      case FA_SCORE:  // f = carried by actor flag
        if (actor != -1 || f.state != CTFF_STOLEN) {
          abort = 21;
          break;
        }  // no client msg allowed here
        if (valid_client(f.actor_cn) &&
            clients[f.actor_cn]->state.state == CS_ALIVE &&
            !team_isspect(clients[f.actor_cn]->team)) {
          actor = f.actor_cn;
          score = 1;
          message = FM_KTFSCORE;
          break;
      ...
      case FA_LOST:
        if (actor == -1) actor = f.actor_cn;
      case FA_DROP:
        if (f.actor_cn != actor || f.state != CTFF_STOLEN) {
          abort = 22;
          break;
        }
      case FA_RESET:
  ...
  if (abort) {
    logline(ACLOG_DEBUG,
            "aborting flagaction(flag %d, action %d, actor %d), reason %d, "
            "resending flag states",
            flag, action, actor,
            abort);  // FIXME: remove this logline after some time - it will
                     // only show a few bad ping effects
    sendflaginfo();
    return;
  }
  if (score) {
  ...
  if (valid_client(actor)) {
    ...
    switch (message) {
      case FM_PICKUP:
        logline(ACLOG_INFO, "[%s] %s %s the flag", c.hostname, c.name,
                action == FA_STEAL ? "stole" : "picked up");
        break;
      case FM_DROP:
      case FM_LOST:
        logline(ACLOG_INFO, "[%s] %s %s the flag", c.hostname, c.name,
                message == FM_LOST ? "lost" : "dropped");
        break;
      case FM_RETURN:
      ...
      case FM_SCORE:
        if (m_htf)
          logline(ACLOG_INFO, "[%s] %s hunted the flag for %s, new score %d",
                  c.hostname, c.name, team_string(c.team), c.state.flagscore);
        else
          logline(ACLOG_INFO,
                  "[%s] %s scored with the flag for %s, new score %d",
                  c.hostname, c.name, team_string(c.team), c.state.flagscore);
        break;
      case FM_KTFSCORE:
        logline(ACLOG_INFO,
                "[%s] %s scored, carrying for %d seconds, new score %d",
                c.hostname, c.name, (gamemillis - f.stolentime) / 1000,
                c.state.flagscore);
        break;
      case FM_SCOREFAIL:
        ...
        break;
      default:
        logline(ACLOG_INFO, "flagaction %d, actor %d, flag %d, message %d",
                action, actor, flag, message);
        break;
    }
    ...
    switch (message) {
      case FM_RESET:
        logline(ACLOG_INFO, "the server reset the flag for team %s",
                team_string(flag));
        break;
      default:
        logline(ACLOG_INFO,
                "flagaction %d with invalid actor cn %d, flag %d, message %d",
                action, actor, flag, message);
        break;
    }
  }
...
void htf_forceflag(int flag) {
  ...
  loopv(clients) if (clients[i]->type != ST_EMPTY) {
    if (clients[i]->state.state == CS_ALIVE &&
        team_base(clients[i]->team) == flag) {
  ...
  if (clientnumbers.length()) {
    ...
    logline(ACLOG_INFO, "[%s] %s got forced to pickup the flag", cl->hostname,
            cl->name);
  }
  f.lastupdate = gamemillis;
}
...
int cmpscore(const int *a, const int *b) {
  return clients[*a]->at3_score - clients[*b]->at3_score;
}
vector<int> tdistrib;
...
bool free_items(int n) {
  client *c = clients[n];
  int waitspawn =
      min(c->ping, 200) + c->state.spawn;  // flowtron to Brahma: can't this be
                                           // removed now? (re: rev. 5270)
...
void arenacheck() {
  if (!m_arena || interm || gamemillis < arenaround || !numactiveclients())
    return;
  ...
  if (m_botmode && clients[0]->type == ST_LOCAL) {
    ...
    playerent *alive = nullptr;
    loopv(players) if (players[i] &&
                       (!m_teammode ||
                        players[i]->team == team_opposite(player1->team))) {
      enemies++;
    ...
    if (enemies && (!alive_enemies || player1->state == CS_DEAD)) {
      sendf(-1, 1, "ri2", SV_ARENAWIN,
            m_teammode
                ? (alive ? alive->clientnum : -1)
                : (alive && alive->type == ENT_BOT
                       ? -2
                       : player1->state == CS_ALIVE ? player1->clientnum : -1));
  ...
  loopv(clients) {
    client &c = *clients[i];
    if (c.type == ST_EMPTY || !c.isauthed || !c.isonrightmap ||
        team_isspect(c.team))
      continue;
    if (c.state.state == CS_ALIVE ||
        ((c.state.state == CS_DEAD || c.state.state == CS_SPECTATE) &&
         c.state.lastspawn >= 0)) {
      if (!alive)
        alive = &c;
      else if (!m_teammode || alive->team != c.team)
        return;
  ...
  if (autoteam && m_teammode && mastermode != MM_MATCH) {
    ...
    if ((!ntc[0] || !ntc[1]) && (ntc[0] > 1 || ntc[1] > 1))
      refillteams(true, FTR_AUTOTEAM);
  }
...
#define SPAMREPEATINTERVAL \
  20                     // detect doubled lines only if interval < 20 seconds
...
bool spamdetect(client *cl,
                char *text)  // checks doubled lines and average typing speed
{
  ...
  if (text[0] && !strcmp(text, cl->lastsaytext) &&
      servmillis - cl->lastsay < SPAMREPEATINTERVAL * 1000) {
  ...
  if (cl->saychars > (SPAMCHARPERMINUTE * SPAMCHARINTERVAL) / 60) spam = true;
  return spam;
}
...
// ||||                                           | | | | | | | | |      C:
// normal chat
// ||||   team modes:                chat goes to | | | | | | | | |      T: team
// chat XX     -->   RVSF players                >-----/ | | | | | | | |      c:
// normal chat in all mastermodes except 'match' XX X   -->   RVSF spect players
// >-------/ | | | | | | |      t: all chat in mastermode 'match', otherwise
// only team chat X X    -->   CLA players                 >---------/ | | | | |
// | X XX   -->   CLA spect players           >-----------/ | | | | | X  X   -->
// SPECTATORs                  >-------------/ | | | |
...
// purpose:
//  a) give spects a possibility to chat without annoying the players (even in
//  ffa), b) no hidden messages from spects to active teams, c) no spect talk to
...
void sendteamtext(char *text, int sender, int msgtype) {
  ...
  loopv(clients) if (i != sender) {
    ...
    if ((rt == TEAM_SPECT &&
         clients[i]->role == CR_ADMIN) ||  // spect-admin reads all
        (team_isactive(st) &&
         st == team_group(rt)) ||  // player to own team + own spects
        (team_isspect(st) &&
         team_isspect(rt)))  // spectator to other spectators
...
int spawntime(int type) {
  int np = numplayers();
  np = np < 3 ? 4 : (np > 4 ? 2 : 3);  // Some spawn times are dependent on the
                                       // number of players.
  int sec = 0;
  switch (type) {
      // Please update ./ac_website/htdocs/docs/introduction.html if these times
      // change.
    case I_CLIPS:
    case I_AMMO:
      sec = np * 2;
...
bool serverpickup(int i, int sender)  // server side item pickup, acknowledge
                                      // first client that gets it
{
  const char *hn = sender >= 0 && clients[sender]->type == ST_TCPIP
                       ? clients[sender]->hostname
                       : nullptr;
  if (!sents.inrange(i)) {
    if (hn && !m_coop)
      logline(ACLOG_INFO,
              "[%s] tried to pick up entity #%d - doesn't exist on this map",
              hn, i);
    return false;
  }
  server_entity &e = sents[i];
  if (!e.spawned) {
    if (!e.legalpickup && hn && !m_demo)
      logline(ACLOG_INFO,
              "[%s] tried to pick up entity #%d (%s) - can't be picked up in "
              "this gamemode or at all",
              hn, i, entnames[e.type]);
    return false;
  }
  if (sender >= 0) {
    client *cl = clients[sender];
    if (cl->type == ST_TCPIP) {
      if (cl->state.state != CS_ALIVE || !cl->state.canpickup(e.type) ||
          (m_arena && !free_items(sender)))
        return false;
      ...
      if (pdist) {
        ...
        if (!m_demo)
          logline(
              ACLOG_INFO, "[%s] %s %s up entity #%d (%s), distance %.2f (%d)",
              cl->hostname, cl->name, (pdist == 2 ? "tried to pick" : "picked"),
              i, entnames[e.type], dist, cl->farpickups);
        if (pdist == 2) return false;
      }
    }
    sendf(-1, 1, "ri3", SV_ITEMACC, i, sender);
    cl->state.pickup(sents[i].type);
    if (m_lss && sents[i].type == I_GRENADE)
      cl->state.pickup(sents[i].type);  // get two nades at lss
  }
  e.spawned = false;
...
void serverdamage(client *target, client *actor, int damage, int gun, bool gib,
                  const vec &hitpush = vec(0, 0, 0)) {
  if (!m_demo && !m_coop && !validdamage(target, actor, damage, gun, gib))
    return;
  if (m_arena && gun == GUN_GRENADE &&
      arenaroundstartmillis + 2000 > gamemillis && target != actor)
    return;
  clientstate &ts = target->state;
  ts.dodamage(damage, gun);
  if (damage < INT_MAX) {
    actor->state.damage += damage;
    sendf(-1, 1, "ri7", gib ? SV_GIBDAMAGE : SV_DAMAGE, target->clientnum,
          actor->clientnum, gun, damage, ts.armour, ts.health);
    if (target != actor) {
  ...
  if (ts.health <= 0) {
    ...
    if (target != actor) {
      if (!isteam(target->team, actor->team))
        actor->state.frags +=
            gib && gun != GUN_GRENADE && gun != GUN_SHOTGUN ? 2 : 1;
      else {
        actor->state.frags--;
    ...
    sendf(-1, 1, "ri5", gib ? SV_GIBDIED : SV_DIED, target->clientnum,
          actor->clientnum, actor->state.frags, gun);
    if ((suic || tk) && (m_htf || m_ktf) && targethasflag >= 0) {
      actor->state.flagscore--;
      sendf(-1, 1, "riii", SV_FLAGCNT, actor->clientnum,
            actor->state.flagscore);
    }
    ...
    if (!suic)
      logline(ACLOG_INFO, "[%s] %s %s%s %s", actor->hostname, actor->name,
              valid_weapon(gun) ? killmessages[gib ? 1 : 0][gun] : "smurfed",
              tk ? " their teammate" : "", target->name);
    ...
    if (isdedicated && actor->type == ST_TCPIP && tk) {
      if (actor->state.frags < scl.banthreshold ||
          /** teamkilling more than 6 (defaults), more than 2 per minute and
             less than 4 frags per tk */
        ...
        addban(actor, DISC_AUTOBAN);
                 ...
                 /** teamkilling more than 5 (defaults), more than 1 tk per
                    minute and less than 4 frags per tk */
  ...
  } else if (target != actor && isteam(target->team, actor->team))
    check_ffire(target, actor, damage);  // friendly fire counter
}

#include "serverevents.h"

bool updatedescallowed() {
  return scl.servdesc_pre[0] || scl.servdesc_suf[0];
}
...
void updatesdesc(const char *newdesc, ENetAddress *caller = nullptr) {
  if (!newdesc || !newdesc[0] || !updatedescallowed()) {
    ...
    formatstring(servdesc_current)("%s%s%s", scl.servdesc_pre, newdesc,
                                   scl.servdesc_suf);
...
int canspawn(client *c)  // beware: canspawn() doesn't check m_arena!
{
  if (!c || c->type == ST_EMPTY || !c->isauthed || !team_isvalid(c->team) ||
      (c->type == ST_TCPIP &&
       (c->state.lastdeath > 0 ? gamemillis - c->state.lastdeath
                               : servmillis - c->connectmillis) <
           (m_arena ? 0 : (m_flags ? 5000 : 2000))) ||
      (c->type == ST_TCPIP &&
       (servmillis - c->connectmillis < 1000 + c->state.reconnections * 2000 &&
        gamemillis > 10000 && totalclients > 3 && !team_isspect(c->team))))
    return SP_OK_NUM;  // equivalent to SP_DENY
  if (!c->isonrightmap) return SP_WRONGMAP;
  if (mastermode == MM_MATCH && matchteamsize) {
    if (c->team == TEAM_SPECT || (team_isspect(c->team) && !m_teammode))
      return SP_SPECT;
    if (c->team == TEAM_CLA_SPECT || c->team == TEAM_RVSF_SPECT) {
      if (numteamclients()[team_base(c->team)] >= matchteamsize)
        return SP_SPECT;
      else
        return SP_REFILLMATCH;
    }
  }
  return SP_OK;
}
...
bool updateclientteam(int cln, int newteam, int ftr) {
  ...
  if (mastermode == MM_OPEN && cl.state.forced && ftr == FTR_PLAYERWISH &&
      newteam < TEAM_SPECT && team_base(cl.team) != team_base(newteam))
    return false;                 // no free will changes to forced people
  if (newteam == TEAM_ANYACTIVE)  // when spawning from spect
  {
    if (mastermode == MM_MATCH && cl.team < TEAM_SPECT) {
      newteam = team_base(cl.team);
    } else {
      if (autoteam && teamsizes[TEAM_CLA] != teamsizes[TEAM_RVSF])
        newteam =
            teamsizes[TEAM_CLA] < teamsizes[TEAM_RVSF] ? TEAM_CLA : TEAM_RVSF;
      else {  // join weaker team
        int teamscore[2] = {0, 0}, sum = calcscores();
        loopv(clients) if (clients[i]->type != ST_EMPTY && i != cln &&
                           clients[i]->isauthed &&
                           clients[i]->team != TEAM_SPECT) {
          teamscore[team_base(clients[i]->team)] += clients[i]->at3_score;
        }
        newteam = sum > 200
                      ? (teamscore[TEAM_CLA] < teamscore[TEAM_RVSF] ? TEAM_CLA
                                                                    : TEAM_RVSF)
                      : rnd(2);
      }
    }
  ...
  if (ftr == FTR_PLAYERWISH) {
    if (mastermode == MM_MATCH && matchteamsize && m_teammode) {
      if (newteam != TEAM_SPECT &&
          (team_base(newteam) != team_base(cl.team) || !m_teammode))
        return false;  // no switching sides in match mode when teamsize is set
    }
    if (team_isactive(newteam)) {
      if (!m_teammode && cl.state.state == CS_ALIVE)
        return false;  // no comments
      if (mastermode == MM_MATCH) {
        if (m_teammode && matchteamsize && teamsizes[newteam] >= matchteamsize)
          return false;  // ensure maximum team size
      } else {
        if (m_teammode && autoteam &&
            teamsizes[newteam] > teamsizes[team_opposite(newteam)])
          return false;  // don't switch to an already bigger team
      }
    } else if (mastermode != MM_MATCH || !m_teammode)
      newteam =
          TEAM_SPECT;  // only match mode (team) has more than one spect team
  }
  if (cl.team == newteam && ftr != FTR_AUTOTEAM) return true;  // no change
  if (cl.team != newteam) sdropflag(cl.clientnum);
  if (ftr != FTR_INFO && (team_isspect(newteam) ||
                          (team_isactive(newteam) && team_isactive(cl.team))))
    forcedeath(&cl);
  sendf(-1, 1, "riii", SV_SETTEAM, cln,
        newteam | ((ftr == FTR_SILENTFORCE ? FTR_INFO : ftr) << 4));
  if (ftr != FTR_INFO && !team_isspect(newteam) && team_isspect(cl.team))
    sendspawn(&cl);
...
int calcscores()  // skill eval
{
  ...
  loopv(clients) if (clients[i]->type != ST_EMPTY) {
    ...
    sum += clients[i]->at3_score =
        cs.points > 0 ? sqrtf((float)cs.points) : -sqrtf((float)-cs.points);
  }
  return sum;
}
...
void shuffleteams(int ftr = FTR_AUTOTEAM) {
  ...
  if (gamemillis < 2 * 60 * 1000) {  // random
    ...
    loopv(clients) if (clients[i]->type != ST_EMPTY &&
                       clients[i]->isonrightmap &&
                       !team_isspect(
                           clients[i]->team))  // only shuffle active players
    {
      sums += rnd(1000);
      team = sums & 1;
...
bool balanceteams(int ftr)  // pro vs noobs never more
{
  ...
  loopv(clients) if (clients[i]->type != ST_EMPTY) {
    ...
    if (c->isauthed && team_isactive(c->team)) {
      ...
      c->eff_score = c->state.points * 60 * 1000 / time + c->state.points / 6 +
                     c->state.flagscore * flagmult;
  ...
  if (tscore[h] > 3 * tscore[l] && tscore[h] > 150 * nplayers) {
    //        sendf(-1, 1, "ri2", SV_SERVERMODE, sendservermode(false) |
    //        AT_SHUFFLE);
    shuffleteams();
    return true;
  }
  ...
  if (tsize[h] - tsize[l] >
      0)  // the h team has more players, so we will force only one player
  {
    loopv(clients) if (clients[i]->type != ST_EMPTY) {
      client *c = clients[i];  // loop for h
      // client from the h team, not forced in this game, and without the flag
      if (c->isauthed && c->team == h && !c->state.forced &&
          clienthasflag(i) < 0) {
        // do not exchange in the way that weaker team becomes the stronger or
        // the change is less than 20% effective
        if (2 * c->eff_score <= diffscore && 10 * c->eff_score >= diffscore &&
            c->eff_score > besth) {
          besth = c->eff_score;
          hid = i;
        }
  ...
  } else {  // the h score team has less or the same player number, so, lets
            // exchange
    loopv(clients) if (clients[i]->type != ST_EMPTY) {
      client *c = clients[i];  // loop for h
      if (c->isauthed && c->team == h && !c->state.forced &&
        ...
        loopvj(clients) if (clients[j]->type != ST_EMPTY && j != i) {
          ...
          if (cj->isauthed && cj->team == l && !cj->state.forced &&
              clienthasflag(j) < 0) {
            int pairdiff = 2 * (c->eff_score - cj->eff_score);
            if (pairdiff <= diffscore && 5 * pairdiff >= diffscore &&
                pairdiff > bestdiff) {
              bestdiff = pairdiff;
              bestpair[h] = i;
              bestpair[l] = j;
    ...
    if (bestpair[h] >= 0 && bestpair[l] >= 0) {
      ...
      clients[bestpair[h]]->at3_lastforce =
          clients[bestpair[l]]->at3_lastforce = gamemillis;
      clients[bestpair[h]]->state.forced = clients[bestpair[l]]->state.forced =
          true;
      return true;
    }
  }
...
bool refillteams(bool now, int ftr)  // force only minimal amounts of players
{
  ...
  if (diffnum > 1) {
    if (now || gamemillis - lasttime_eventeams > 8000 + allplayers * 1000 ||
        diffnum > 2 + allplayers / 10) {
      // time to even out teams
      loopv(clients) if (clients[i]->type != ST_EMPTY &&
                         clients[i]->team != bigteam) clients[i]
          ->at3_dontmove = true;  // dont move small team players
      while (diffnum > 1 && moveable[bigteam] > 0) {
        ...
        loopv(
            clients) if (clients[i]->type != ST_EMPTY &&
                         !clients[i]
                              ->at3_dontmove)  // try all still movable players
        {
          int fit = targetscore - clients[i]->at3_score;
          if (fit < 0) fit = -(fit * 15) / 10;  // avoid too good players
          int forcedelay =
              clients[i]->at3_lastforce
                  ? (1000 - (gamemillis - clients[i]->at3_lastforce) / (5 * 60))
                  : 0;
          if (forcedelay > 0)
            fit += (fit * forcedelay) / 600;  // avoid lately forced players
        ...
        if (updateclientteam(pick, !bigteam, ftr)) {
          ...
          clients[pick]->at3_lastforce =
              gamemillis;  // try not to force this player again for the next 5
                           // minutes
          switched = true;
        }
      }
  ...
  if (diffnum < 2) {
    if ((gamemillis - lastbalance) > waitbalance &&
        (gamelimit - gamemillis) > 4 * 60 * 1000) {
      if (balanceteams(ftr)) {
...
void startgame(const char *newname, int newmode, int newtime, bool notify) {
  if (!newname || !*newname || (newmode == GMODE_DEMO && isdedicated))
    fatal("startgame() abused");
  if (newmode == GMODE_DEMO) {
    ...
    if (custom_servdesc && findcnbyaddress(&servdesc_caller) < 0) {
      ...
      if (notify) {
        ...
        logline(ACLOG_INFO, "server description reset to '%s'",
                servdesc_current);
      }
    }
    ...
    if (ms) {
      ...
      loopi(2) {
        ...
        if (smapstats.flags[i] == 1)  // don't check flag positions, if there is
                                      // more than one flag per team
        {
      ...
      loopi(smapstats.hdr.numents) {
        ...
        server_entity se = {e.type,
                            false,
                            false,
                            false,
                            0,
                            smapstats.entposs[i * 3],
                            smapstats.entposs[i * 3 + 1]};
        sents.add(se);
        if (e.fitsmode(smode)) sents[i].spawned = sents[i].legalpickup = true;
      }
      mapbuffer.setrevision();
      logline(ACLOG_INFO,
              "Map height density information for %s: H = %.2f V = %d, A = %d "
              "and MA = %d",
              smapname, Mheight, Mvolume, Marea, Mopen);
      items_blocked = false;
    } else if (isdedicated)
      sendservmsg(
          "\f3server error: map not found - please start another map or send "
          "this map to the server");
    if (notify) {
      // change map
      sendf(-1, 1, "risiii", SV_MAPCHANGE, smapname, smode,
            mapbuffer.available(), mapbuffer.revision);
      if (smode > 1 || (smode == 0 && numnonlocalclients() > 0))
        sendf(-1, 1, "ri3", SV_TIMEUP, gamemillis, gamelimit);
    }
    ...
    defformatstring(gsmsg)(
        "Game start: %s on %s, %d players, %d minutes, mastermode %d, ",
        modestr(smode), smapname, numclients(), minremain, mastermode);
    if (mastermode == MM_MATCH)
      concatformatstring(gsmsg, "teamsize %d, ", matchteamsize);
    if (ms)
      concatformatstring(gsmsg, "(map rev %d/%d, %s, 'getmap' %sprepared)",
                         smapstats.hdr.maprevision, smapstats.cgzsize,
                         maplocstr[maploc],
                         mapbuffer.available() ? "" : "not ");
    else
      concatformatstring(gsmsg, "error: failed to preload map (map: %s)",
                         maplocstr[maploc]);
    logline(ACLOG_INFO, "\n%s", gsmsg);
...
void cleargbans() { gbans.shrink(0); }
...
void addgban(const char *name) {
  ...
  loopvrev(clients) {
    ...
    if (checkgban(c.peer->address.host))
      disconnect_client(c.clientnum, DISC_BANREFUSE);
  }
}
...
int serveroperator() {
  loopv(clients) if (clients[i]->type != ST_EMPTY &&
                     clients[i]->role > CR_DEFAULT) return i;
  return -1;
}

void sendserveropinfo(int receiver) {
  int op = serveroperator();
  sendf(receiver, 1, "riii", SV_SERVOPINFO, op,
        op >= 0 ? clients[op]->role : -1);
}
...
struct voteinfo {
  int boot{0};
  int owner{0}, callmillis{0}, result{VOTE_NEUTRAL}, num1, num2, type;
  char text[MAXTRANS];
  ...
  void end(int result) {
    if (action && !action->isvalid())
      result = VOTE_NO;  // don't perform() invalid votes
  ...
  bool isvalid() {
    return valid_client(owner) && action != nullptr && action->isvalid();
  }
  ...
  void evaluate(bool forceend = false) {
    ...
    loopv(clients) if (
        clients[i]->type !=
        ST_EMPTY /*&& clients[i]->connectmillis < callmillis*/)  // new
                                                                 // connected
                                                                 // people will
                                                                 // vote now
    {
    ...
    bool admin = clients[owner]->role == CR_ADMIN ||
                 (!isdedicated && clients[owner]->type == ST_LOCAL);
...
#define yes_condition                                               \
  ((min_time && stats[VOTE_YES] - stats[VOTE_NO] > 0.34f * total && \
    totalclients > 4) ||                                            \
   stats[VOTE_YES] > requiredcount * total)
#define no_condition                                          \
  (forceend || !valid_client(owner) ||                        \
   stats[VOTE_NO] >= stats[VOTE_YES] + stats[VOTE_NEUTRAL] || \
   adminvote == VOTE_NO)
#define boot_condition \
  (!boot ||            \
   (boot && valid_client(num1) && clients[num1]->peer->address.host == host))
    if ((yes_condition || admin || adminvote == VOTE_YES) && boot_condition)
      end(VOTE_YES);
    else if (no_condition || (min_time && !boot_condition))
      end(VOTE_NO);
    else
      return;
...
bool svote(int sender, int vote,
           ENetPacket *msg)  // true if the vote was placed successfully
{
  if (!curvote || !valid_client(sender) || vote < VOTE_YES || vote > VOTE_NO)
    return false;
  if (clients[sender]->vote != VOTE_NEUTRAL) {
    ...
    logline(ACLOG_DEBUG, "[%s] client %s voted %s", clients[sender]->hostname,
            clients[sender]->name, vote == VOTE_NO ? "no" : "yes");
    curvote->evaluate();
    return true;
  }
}
...
void scallvotesuc(voteinfo *v) {
  ...
  logline(ACLOG_INFO, "[%s] client %s called a vote: %s",
          clients[v->owner]->hostname, clients[v->owner]->name,
          v->action && *v->action->desc ? v->action->desc : "[unknown]");
}

void scallvoteerr(voteinfo *v, int error) {
  if (!valid_client(v->owner)) return;
  sendf(v->owner, 1, "ri2", SV_CALLVOTEERR, error);
  logline(ACLOG_INFO, "[%s] client %s failed to call a vote: %s (%s)",
          clients[v->owner]->hostname, clients[v->owner]->name,
          v->action && *v->action->desc ? v->action->desc : "[unknown]",
          voteerrorstr(error));
}
...
bool scallvote(voteinfo *v,
               ENetPacket *msg)  // true if a regular vote was called
{
  if (!v) return false;
  int area = isdedicated ? EE_DED_SERV : EE_LOCAL_SERV;
  int error = -1;
  client *c = clients[v->owner],
         *b = (v->boot && valid_client(cn2boot) ? clients[cn2boot] : NULL);
  v->host = v->boot && b ? b->peer->address.host : 0;

  int time = servmillis - c->lastvotecall;
  if (c->nvotes > 0 && time > 4 * 60 * 1000)
    c->nvotes -= time / (4 * 60 * 1000);
  if (c->nvotes < 0 || c->role == CR_ADMIN) c->nvotes = 0;
  c->nvotes++;

  if (!v || !v->isvalid() || (v->boot && (!b || cn2boot == v->owner)))
    error = VOTEE_INVALID;
  else if (v->action->role > c->role)
    error = VOTEE_PERMISSION;
  else if (!(area & v->action->area))
    error = VOTEE_AREA;
  else if (curvote && curvote->result == VOTE_NEUTRAL)
    error = VOTEE_CUR;
  else if (v->type == SA_MAP && v->num1 >= GMODE_NUM && map_queued)
    error = VOTEE_NEXT;
  else if (c->role == CR_DEFAULT && v->action->isdisabled())
    error = VOTEE_DISABLED;
  else if ((c->lastvotecall && servmillis - c->lastvotecall < 60 * 1000 &&
            c->role != CR_ADMIN && numclients() > 1) ||
           c->nvotes > 3)
    error = VOTEE_MAX;
  else if (((v->boot == 1 && c->role < roleconf('w')) ||
            (v->boot == 2 && c->role < roleconf('X'))) &&
           !is_lagging(b) && !b->mute && b->spamcount < 2) {
    /** not same team, with low ratio, not lagging, and not spamming... so, why
     * to kick? */
    if (!isteam(c->team, b->team) &&
        b->state.frags < (b->state.deaths > 0 ? b->state.deaths : 1) * 3)
      error = VOTEE_WEAK;
    /** same team, with low tk, not lagging, and not spamming... so, why to
     * kick? */
    else if (isteam(c->team, b->team) &&
             b->state.teamkills < c->state.teamkills)
      error = VOTEE_WEAK;
  }
  ...
  if (error >= 0) {
    ...
    if (!v->gonext)
      sendpacket(
          -1, 1, msg,
          v->owner);  // FIXME in fact, all votes should go to the server,
                      // registered, and then go back to the clients
    else
      callvotepacket(-1,
                     v);  // also, no vote should exclude the caller... these
                          // would provide many code advantages/facilities
    scallvotesuc(v);      // but we cannot change the vote system now for
                          // compatibility issues... so, TODO
    return true;
  }
}
...
void callvotepacket(
    int cn,
    voteinfo *v = curvote) {  // FIXME, it would be far smart if the msg buffer
                              // from SV_CALLVOTE was simply saved
  int n_yes = 0, n_no = 0;
  loopv(clients) if (clients[i]->type != ST_EMPTY) {
    if (clients[i]->vote == VOTE_YES)
      n_yes++;
    else if (clients[i]->vote == VOTE_NO)
      n_no++;
  }
...
void changeclientrole(int client, int role, char *pwd, bool force) {
  ...
  pd.line = -1;
  if (force || role == CR_DEFAULT ||
      (role == CR_ADMIN && pwd && pwd[0] &&
       passwords.check(clients[client]->name, pwd, clients[client]->salt,
                       &pd) &&
       !pd.denyadmin)) {
    ...
    sendserveropinfo(-1);
    if (pd.line > -1)
      logline(ACLOG_INFO, "[%s] player %s used admin password in line %d",
              clients[client]->hostname,
              clients[client]->name[0] ? clients[client]->name : "[unnamed]",
              pd.line);
    logline(ACLOG_INFO, "[%s] set role of player %s to %s",
            clients[client]->hostname,
            clients[client]->name[0] ? clients[client]->name : "[unnamed]",
            role == CR_ADMIN
                ? "admin"
                : "normal player");  // flowtron : connecting players haven't
                                     // got a name yet (connectadmin)
    if (role > CR_DEFAULT) sendiplist(client);
  } else if (pwd && pwd[0])
    disconnect_client(client, DISC_SOPLOGINFAIL);  // avoid brute-force
...
const char *disc_reason(int reason) {
  static const char *disc_reasons[] = {
      ...
      "error - end of packet",
      "error - client num",
      "vote-kicked from the server",
      "vote-banned from the server",
      "error - tag type",
      "connection refused - you have been banned from this server",
      "incorrect password",
      "unsuccessful administrator login",
      "the server is FULL - try again later",
      "servers mastermode is \"private\" - wait until the servers mastermode "
      "is \"open\"",
      "auto-kick - your score dropped below the servers threshold",
      "auto-ban - your score dropped below the servers threshold",
      "duplicate connection",
      "inappropriate nickname",
      "error - packet flood",
      "auto-kick - excess spam detected",
      "auto-kick - inactivity detected",
      "auto-kick - team killing detected",
      "auto-kick - abnormal client behavior detected"};
  return reason >= 0 &&
                 (size_t)reason < sizeof(disc_reasons) / sizeof(disc_reasons[0])
             ? disc_reasons[reason]
             : "unknown";
}
...
void disconnect_client(int n, int reason) {
  ...
  if (reason >= 0)
    logline(ACLOG_INFO,
            "[%s] disconnecting client %s (%s) cn %d, %d seconds played%s",
            c.hostname, c.name, disc_reason(reason), n, sp, scoresaved);
  else
    logline(ACLOG_INFO,
            "[%s] disconnected client %s cn %d, %d seconds played%s",
            c.hostname, c.name, n, sp, scoresaved);
  totalclients--;
  c.peer->data = (void *)-1;
...
void tryauth(client *cl, const char *user) {
  ...
  if (!requestmasterf("reqauth %u %s\n", cl->authreq, cl->authname)) {
    cl->authreq = 0;
    sendf(cl->clientnum, 1, "ris", SV_SERVMSG,
...
void sendiplist(int receiver, int cn) {
  ...
  packetbuf p(MAXTRANS, ENET_PACKET_FLAG_RELIABLE);
  putint(p, SV_IPLIST);
  loopv(clients) if (valid_client(i) && clients[i]->type == ST_TCPIP &&
                     i != receiver &&
                     (clients[i]->clientnum == cn || cn == -1)) {
    putint(p, i);
    putint(p, isbigendian() ? endianswap(clients[i]->peer->address.host)
                            : clients[i]->peer->address.host);
  }
  putint(p, -1);
...
void sendresume(client &c, bool broadcast) {
  sendf(broadcast ? -1 : c.clientnum, 1, "ri3i9ivvi", SV_RESUME, c.clientnum,
        c.state.state, c.state.lifesequence, c.state.primary, c.state.gunselect,
        c.state.flagscore, c.state.frags, c.state.deaths, c.state.health,
        c.state.armour, c.state.points, c.state.teamkills, NUMGUNS,
        c.state.ammo, NUMGUNS, c.state.mag, -1);
}
...
bool restorescore(client &c) {
  ...
  if (sc && sc->valid) {
    sc->restore(c.state);
    sc->valid = false;
    if (c.connectmillis - c.state.lastdisc < 5000)
...
void sendservinfo(client &c) {
  sendf(c.clientnum, 1, "ri5", SV_SERVINFO, c.clientnum,
        isdedicated ? SERVER_PROTOCOL_VERSION : PROTOCOL_VERSION, c.salt,
        scl.serverpassword[0] ? 1 : 0);
}
...
void welcomepacket(packetbuf &p, int n) {
  ...
  if (smapname[0] && !m_demo) {
    ...
    if (smode > 1 || (smode == 0 && numnonlocalclients() > 0)) {
      putint(p, SV_TIMEUP);
      putint(p, (gamemillis >= gamelimit || forceintermission) ? gamelimit
                                                               : gamemillis);
      putint(p, gamelimit);
  ...
  const char *motd =
      scl.motd[0] ? scl.motd : infofiles.getmotd(c ? c->lang : "");
  if (motd) {
    putint(p, SV_TEXT);
    sendstring(motd, p);
  }
...
int checktype(int type, client *cl) {
  ...
  static int servtypes[] = {
      SV_SERVINFO,   SV_WELCOME,      SV_INITCLIENT,  SV_POSN,
      SV_CDIS,       SV_GIBDIED,      SV_DIED,        SV_GIBDAMAGE,
      SV_DAMAGE,     SV_HITPUSH,      SV_SHOTFX,      SV_AUTHREQ,
      SV_AUTHCHAL,   SV_SPAWNSTATE,   SV_SPAWNDENY,   SV_FORCEDEATH,
      SV_RESUME,     SV_DISCSCORES,   SV_TIMEUP,      SV_ITEMACC,
      SV_MAPCHANGE,  SV_ITEMSPAWN,    SV_PONG,        SV_SERVMSG,
      SV_ITEMLIST,   SV_FLAGINFO,     SV_FLAGMSG,     SV_FLAGCNT,
      SV_ARENAWIN,   SV_SERVOPINFO,   SV_CALLVOTESUC, SV_CALLVOTEERR,
      SV_VOTERESULT, SV_SETTEAM,      SV_TEAMDENY,    SV_SERVERMODE,
      SV_IPLIST,     SV_SENDDEMOLIST, SV_SENDDEMO,    SV_DEMOPLAYBACK,
      SV_CLIENT,     SV_HUDEXTRAS,    SV_POINTS};
  // only allow edit messages in coop-edit mode
  static int edittypes[] = {SV_EDITENT, SV_EDITXY, SV_EDITARCH, SV_EDITBLOCK,
                            SV_EDITD,   SV_EDITE,  SV_NEWMAP};
  if (cl) {
    loopi(sizeof(servtypes) / sizeof(int)) if (type == servtypes[i]) return -1;
    loopi(sizeof(edittypes) /
          sizeof(int)) if (type == edittypes[i]) return smode == GMODE_COOPEDIT
        ? type
        : -1;
...
void process(ENetPacket *packet, int sender, int chan) {
  ...
  if (cl && !cl->isauthed) {
    ...
    if (chan == 0)
      return;
    else if (chan != 1 || getint(p) != SV_CONNECT)
      ...
      if (matchreconnect &&
          !banned) {  // former player reconnecting to a server in match mode
        cl->isauthed = true;
        logline(ACLOG_INFO, "[%s] %s logged in (reconnect to match)%s",
                cl->hostname, cl->name, tags);
      } else if (wl == NWL_IPFAIL ||
                 wl == NWL_PWDFAIL) {  // nickname matches whitelist, but IP is
                                       // not in the required range or PWD
                                       // doesn't match
        logline(ACLOG_INFO, "[%s] '%s' matches nickname whitelist: wrong %s%s",
                cl->hostname, cl->name, wl == NWL_IPFAIL ? "IP" : "PWD", tags);
        disconnect_client(sender, DISC_BADNICK);
      } else if (bl > 0) {  // nickname matches blacklist
        logline(ACLOG_INFO, "[%s] '%s' matches nickname blacklist line %d%s",
                cl->hostname, cl->name, bl, tags);
        disconnect_client(sender, DISC_BADNICK);
      } else if (passwords.check(
                     cl->name, cl->pwd, cl->salt, &pd,
                     (cl->type == ST_TCPIP ? cl->peer->address.host : 0)) &&
                 (!pd.denyadmin || (banned && !srvfull && !srvprivate)) &&
                 bantype != BAN_MASTER)  // pass admins always through
      {                                  // admin (or deban) password match
        cl->isauthed = true;
        if (!pd.denyadmin && wantrole == CR_ADMIN) clientrole = CR_ADMIN;
        if (bantype == BAN_VOTE) {
          loopv(bans) if (bans[i].address.host == cl->peer->address.host) {
            bans.remove(i);
            concatstring(tags, ", ban removed");
            break;
          }  // remove admin bans
        }
        if (srvfull) {
          loopv(clients) if (i != sender && clients[i]->type == ST_TCPIP) {
            disconnect_client(i, DISC_MAXCLIENTS);  // disconnect someone else
                                                    // to fit maxclients again
            break;
          }
        }
        logline(ACLOG_INFO,
                "[%s] %s logged in using the admin password in line %d%s",
                cl->hostname, cl->name, pd.line, tags);
      } else if (scl.serverpassword[0] &&
                 !(srvprivate || srvfull ||
                   banned)) {  // server password required
        if (!strcmp(genpwdhash(cl->name, scl.serverpassword, cl->salt),
                    cl->pwd)) {
          cl->isauthed = true;
          logline(ACLOG_INFO,
                  "[%s] %s client logged in (using serverpassword)%s",
                  cl->hostname, cl->name, tags);
        } else
        ...
        cl->isauthed = true;
        logline(ACLOG_INFO, "[%s] %s logged in (default)%s", cl->hostname,
                cl->name, tags);
      }
    }
    ...
    if (cl->type == ST_TCPIP) {
      loopv(clients) if (i != sender) {
        ...
        if (dup->type == ST_TCPIP &&
            dup->peer->address.host == cl->peer->address.host &&
            dup->peer->address.port == cl->peer->address.port)
    ...
    if (restorescore(*cl)) {
      sendresume(*cl, true);
      senddisconnectedscores(-1);
    } else if (cl->type == ST_TCPIP)
      senddisconnectedscores(sender);
    sendinitclient(*cl);
    if (clientrole != CR_DEFAULT)
      changeclientrole(sender, clientrole, nullptr, true);
    if (curvote && curvote->result == VOTE_NEUTRAL)
      callvotepacket(cl->clientnum);

    // send full IPs to admins
    loopv(clients) {
      if (clients[i] && clients[i]->clientnum != cl->clientnum &&
          (clients[i]->role == CR_ADMIN || clients[i]->type == ST_LOCAL))
        sendiplist(clients[i]->clientnum, cl->clientnum);
    }
  }

  if (!cl) {
    logline(ACLOG_ERROR, "<NULL> client in process()");
    return;
...
#define QUEUE_MSG                                                    \
  {                                                                  \
    if (cl->type == ST_TCPIP)                                        \
      while (curmsg < p.length()) cl->messages.add(p.buf[curmsg++]); \
  }
...
#ifdef _DEBUG
    if (type != SV_POS && type != SV_POSC && type != SV_CLIENTPING &&
        type != SV_PING && type != SV_CLIENT) {
      DEBUGVAR(cl->name);
    ...
    switch (type) {
      ...
      case SV_TEAMTEXT:
        ...
        if (*text) {
          ...
          {
            logline(ACLOG_INFO, "[%s] %s%s says to team %s: '%s'", cl->hostname,
                    type == SV_TEAMTEXTME ? "(me) " : "", cl->name,
                    team_string(cl->team), text);
            sendteamtext(text, sender, type);
          } else {
            logline(ACLOG_INFO, "[%s] %s%s says to team %s: '%s', %s",
                    cl->hostname, type == SV_TEAMTEXTME ? "(me) " : "",
                    cl->name, team_string(cl->team), text,
                    canspeech ? "SPAM detected" : "Forbidden speech");
            if (canspeech) {
              sendservmsg(
                  "\f3Please do not spam; your message was not delivered.",
                  sender);
              if (cl->spamcount > SPAMMAXREPEAT + 2)
                disconnect_client(cl->clientnum, DISC_ABUSE);
            } else {
              sendservmsg(
                  "\f3Watch your language! Your message was not delivered.",
                  sender);
      ...
      case SV_TEXT: {
        ...
        if (*text) {
          ...
          if (!spamdetect(cl, text) && canspeech) {
            if (mastermode != MM_MATCH || !matchteamsize ||
                team_isactive(cl->team) ||
                (cl->team == TEAM_SPECT &&
                 cl->role == CR_ADMIN))  // common chat
            {
              logline(ACLOG_INFO, "[%s] %s%s says: '%s'", cl->hostname,
                      type == SV_TEXTME ? "(me) " : "", cl->name, text);
              if (cl->type == ST_TCPIP)
                while (mid1 < mid2) cl->messages.add(p.buf[mid1++]);
              QUEUE_STR(text);
            } else  // spect chat
            {
              logline(ACLOG_INFO, "[%s] %s%s says to team %s: '%s'",
                      cl->hostname, type == SV_TEAMTEXTME ? "(me) " : "",
                      cl->name, team_string(cl->team), text);
              sendteamtext(text, sender,
                           type == SV_TEXTME ? SV_TEAMTEXTME : SV_TEAMTEXT);
            }
          } else {
            logline(ACLOG_INFO, "[%s] %s%s says: '%s', %s", cl->hostname,
                    type == SV_TEXTME ? "(me) " : "", cl->name, text,
                    canspeech ? "SPAM detected" : "Forbidden speech");
            if (canspeech) {
              sendservmsg(
      ...
      case SV_TEXTPRIVATE: {
        ...
        if (*text) {
          ...
          if (!spamdetect(cl, text) && canspeech) {
            bool allowed =
                !(mastermode == MM_MATCH && cl->team != target->team) &&
                cl->role >= roleconf('t');
            logline(ACLOG_INFO, "[%s] %s says to %s: '%s' (%s)", cl->hostname,
                    cl->name, target->name, text,
                    allowed ? "allowed" : "disallowed");
            if (allowed)
              sendf(target->clientnum, 1, "riis", SV_TEXTPRIVATE, cl->clientnum,
                    text);
          } else {
            logline(ACLOG_INFO, "[%s] %s says to %s: '%s', %s", cl->hostname,
                    cl->name, target->name, text,
      ...
      case SV_VOICECOMTEAM: {
        ...
        {
          if (!gamesound_isvoicecom(s))
            cl->mute = servmillis + 10000;  // vc is invalid
          else if (cl->lastvc + 4000 < servmillis) {
            if (cl->spam > 0) cl->spam -= (servmillis - cl->lastvc) / 4000;
          }  // no vc in the last 4 seconds
          else
            cl->spam++;  // the guy is spamming
          if (cl->spam < 0) cl->spam = 0;
          cl->lastvc = servmillis;  // register
          if (cl->spam > 4) {
            cl->mute = servmillis + 10000;
            break;
          }  // 5 vcs in less than 20 seconds... shut up please
          if (m_teammode)
            checkteamplay(s,
                          sender);  // finally here we check the teamplay comm
          if (type == SV_VOICECOM) {
            QUEUE_MSG;
          } else
            sendvoicecomteam(s, sender);
        }
      } break;
      ...
      case SV_MAPIDENT: {
        ...
        int rev = getint(p);
        if (!isdedicated ||
            (smapstats.cgzsize == gzs &&
             smapstats.hdr.maprevision ==
                 rev)) {  // here any game really starts for a client: spawn, if
                          // it's a new game - don't spawn if the game was
                          // already running
          cl->isonrightmap = true;
          int sp = canspawn(cl);
          sendf(sender, 1, "rii", SV_SPAWNDENY, sp);
          cl->spawnperm = sp;
          if (cl->loggedwrongmap)
            logline(ACLOG_INFO,
                    "[%s] %s is now on the right map: revision %d/%d",
                    cl->hostname, cl->name, rev, gzs);
          bool spawn = false;
          if (team_isspect(cl->team)) {
            if (numclients() < 2 && !m_demo &&
                mastermode != MM_MATCH)  // spawn on empty servers
            {
          ...
          forcedeath(cl);
          logline(ACLOG_INFO, "[%s] %s is on the wrong map: revision %d/%d",
                  cl->hostname, cl->name, rev, gzs);
      ...
      case SV_SWITCHNAME: {
        ...
        if (namechanged)
          logline(ACLOG_INFO, "[%s] %s changed name to %s", cl->hostname,
                  cl->name, text);
        copystring(cl->name, text, MAXNAMELEN + 1);
        if (namechanged) {
          // very simple spam detection (possible FIXME: centralize spam
          // detection)
          if (cl->type == ST_TCPIP &&
              (servmillis - cl->lastprofileupdate < 1000)) {
            ++cl->fastprofileupdates;
            if (cl->fastprofileupdates == 3)
              sendservmsg("\f3Please do not spam", sender);
            if (cl->fastprofileupdates >= 5) {
              disconnect_client(sender, DISC_ABUSE);
              break;
            }
          } else if (servmillis - cl->lastprofileupdate > 10000)
            cl->fastprofileupdates = 0;
          ...
          switch (nickblacklist.checkwhitelist(*cl)) {
            ...
            case NWL_IPFAIL:
              logline(ACLOG_INFO,
                      "[%s] '%s' matches nickname whitelist: wrong IP/PWD",
                      cl->hostname, cl->name);
              disconnect_client(sender, DISC_BADNICK);
              break;
            ...
            case NWL_UNLISTED: {
              ...
              if (l >= 0) {
                logline(ACLOG_INFO,
                        "[%s] '%s' matches nickname blacklist line %d",
                        cl->hostname, cl->name, l);
                disconnect_client(sender, DISC_BADNICK);
              }
              break;
            }
      ...
      case SV_SWITCHTEAM: {
        int t = getint(p);
        if (!updateclientteam(sender, team_isvalid(t) ? t : TEAM_SPECT,
                              FTR_PLAYERWISH))
          sendf(sender, 1, "rii", SV_TEAMDENY, t);
        break;
      }
      ...
      case SV_TRYSPAWN: {
        ...
        if (!m_arena && sp < SP_OK_NUM &&
            gamemillis > cl->state.lastspawn + 1000)
          sendspawn(cl);
        break;
      }
      ...
      case SV_SPAWN: {
        ...
        if ((cl->state.state != CS_ALIVE && cl->state.state != CS_DEAD &&
             cl->state.state != CS_SPECTATE) ||
            ls != cl->state.lifesequence || cl->state.lastspawn < 0 ||
            !valid_weapon(gunselect) || gunselect == GUN_CPISTOL)
          break;
...
#define seteventmillis(event)                                              \
  {                                                                        \
    event.id = getint(p);                                                  \
    if (!cl->timesync ||                                                   \
        (cl->events.length() == 1 && cl->state.waitexpired(gamemillis))) { \
      cl->timesync = true;                                                 \
      cl->gameoffset = gamemillis - event.id;                              \
      event.millis = gamemillis;                                           \
    } else                                                                 \
      event.millis = cl->gameoffset + event.id;                            \
  }
        seteventmillis(shot.shot);
        shot.shot.gun = getint(p);
        loopk(3) {
          shot.shot.from[k] = cl->state.o.v[k] +
                              (k == 2 ? (((cl->f >> 7) & 1) ? 2.2f : 4.2f) : 0);
        }
        loopk(3) {
          float v = getint(p) / DMF;
          shot.shot.to[k] = ((k < 2 && v < 0.0f) ? 0.0f : v);
        }
        int hits = getint(p);
        int tcn = -1;
        loopk(hits) {
      ...
      case SV_EXPLODE:  // Brahma says: FIXME handle explosion by location and
                        // deal damage from server
      {
      ...
      case SV_AUTHT: {
        /*                int n = getint(p);
                        loopi(n) getint(p);*/
        //                 if (cl) disconnect_client(cl->clientnum, DISC_TAGT);
        //                 // remove this in the future, when auth is complete
        break;
      }
        // :for AUTH
      ...
      case SV_POS: {
        ...
        if (cl->type == ST_TCPIP &&
            (cl->state.state == CS_ALIVE || cl->state.state == CS_EDITING)) {
      ...
      case SV_POSC: {
        ...
        if (!cl->isonrightmap || p.remaining() || p.overread()) {
          p.flags = 0;
          break;
        }
        if (((cl->f >> 6) & 1) != (cl->state.lifesequence & 1) ||
            usefactor !=
                (smapstats.hdr.sfactor < 7 ? 7 : smapstats.hdr.sfactor))
          break;
        cl->state.o[0] = xt / DMF;
        cl->state.o[1] = yt / DMF;
        cl->state.o[2] = zt / DMF;
      ...
      case SV_SENDMAP: {
        ...
        if (p.remaining() < mapsize + cfgsizegz ||
            MAXMAPSENDSIZE < mapsize + cfgsizegz) {
          p.forceoverread();
          break;
        }
        ...
        if (readonlymap(mp)) {
          reject = "map is ro";
          defformatstring(msg)("\f3map upload rejected: map %s is readonly",
                               sentmap);
          sendservmsg(msg, sender);
        } else if (scl.incoming_limit &&
                   (scl.incoming_limit << 20) <
                       incoming_size + mapsize + cfgsizegz) {
          reject = "server incoming reached its limits";
          sendservmsg(
              "\f3server does not support more incomings: limit reached",
              sender);
        } else if (mp == MAP_NOTFOUND && strchr(scl.mapperm, 'C') &&
                   cl->role < CR_ADMIN) {
          reject = "no permission for initial upload";
          sendservmsg("\f3initial map upload rejected: you need to be admin",
                      sender);
        } else if (mp == MAP_TEMP && revision >= mapbuffer.revision &&
                   !strchr(scl.mapperm, 'u') &&
                   cl->role < CR_ADMIN)  // default: only admins can update maps
        {
          reject = "no permission to update";
          sendservmsg("\f3map update rejected: you need to be admin", sender);
        } else if (mp == MAP_TEMP && revision < mapbuffer.revision &&
                   !strchr(scl.mapperm, 'r') &&
                   cl->role < CR_ADMIN)  // default: only admins can revert maps
                                         // to older revisions
        {
          reject = "no permission to revert revision";
          sendservmsg(
              "\f3map revert to older revision rejected: you need to be admin "
              "to upload an older map",
              sender);
        } else {
          if (mapbuffer.sendmap(sentmap, mapsize, cfgsize, cfgsizegz,
                                &p.buf[p.len])) {
            incoming_size += mapsize + cfgsizegz;
            logline(ACLOG_INFO,
                    "[%s] %s sent map %s, rev %d, %d + %d(%d) bytes written",
                    clients[sender]->hostname, clients[sender]->name, sentmap,
                    revision, mapsize, cfgsize, cfgsizegz);
            defformatstring(msg)(
                "%s (%d) up%sed map %s, rev %d%s", clients[sender]->name,
                sender, mp == MAP_NOTFOUND ? "load" : "dat", sentmap, revision,
                /*strcmp(sentmap, behindpath(smapname)) || smode ==
                   GMODE_COOPEDIT ? "" :*/
                "\f3 (restart game to use new map version)");
            sendservmsg(msg);
          } else {
      ...
      case SV_RECVMAP: {
        if (mapbuffer.available()) {
          ...
          sendservmsg("no map to get", cl->clientnum);
        break;
      }
      ...
      case SV_REMOVEMAP: {
        ...
        int reqrole =
            strchr(scl.mapperm, 'D')
                ? CR_ADMIN
                : (strchr(scl.mapperm, 'd') ? CR_DEFAULT : CR_ADMIN + 100);
        if (cl->role < reqrole)
          reject = "no permission";
        else if (readonlymap(mp))
          reject = "map is readonly";
        else if (mp == MAP_NOTFOUND)
          ...
          sendservmsg(msg, sender);
          logline(ACLOG_INFO, "[%s] deleted map %s", clients[sender]->hostname,
                  rmmap);
        }
        if (reject) {
          logline(ACLOG_INFO, "[%s] deleting map %s failed: %s",
                  clients[sender]->hostname, rmmap, reject);
      ...
      case SV_FLAGACTION: {
        int action = getint(p);
        int flag = getint(p);
        if (!m_flags || flag < 0 || flag > 1 || action < 0 || action > FA_NUM)
          break;
      ...
      case SV_SETADMIN: {
        ...
        if (claim) {
          ...
          changeclientrole(sender, CR_DEFAULT);
        break;
      }

      case SV_CALLVOTE: {
        auto *vi = new voteinfo;
        vi->boot = 0;
        vi->type = getint(p);
        switch (vi->type) {
          case SA_MAP: {
            ...
            if (m_isdemo(mode))
              filtertext(text, text, FTXT__DEMONAME);
            else
            ...
            if (vi->gonext) {
              ...
              if (c) {
                ...
                fatal("unable to get next map in maprot");
            } else {
            ...
            if (mode == GMODE_DEMO)
              vi->action = new demoplayaction(newstring(text));
            else {
              char *vmap = newstring(*vi->text ? behindpath(vi->text) : "");
              vi->action =
                  new mapaction(vmap, qmode, time, sender, qmode != mode);
            }
            break;
          }
          case SA_KICK: {
      ...
      case SV_VOTE: {
        int n = getint(p);
        MSG_PACKET(msg);
        ++msg->referenceCount;  // need to increase reference count in case a
                                // vote disconnects a player after packet is
                                // queued to prevent double-freeing by packetbuf
        svote(sender, n, msg);
        --msg->referenceCount;
        break;
      }
      ...
      case SV_EXTENSION: {
        ...
        //
        // rules:
        // 1. extensions MUST NOT modify gameplay or the behavior of the game in
        // any way
        // 2. extensions may ONLY be used to extend or automate server
        // administration tasks
        // 3. extensions may ONLY operate on the server and must not send any
        // additional data to the connected clients
        // 4. extensions not adhering to these rules may cause the hosting
        // server being banned from the masterserver
        //
        // also note that there is no guarantee that custom extensions will work
        // in future AC versions

        getstring(text, p, 64);
        char *ext = text;   // extension specifier in the form of
                            // OWNER::EXTENSION, see sample below
        ...
        if (!strcmp(ext, "driAn::writelog")) {
          ...
          // usage:       /serverextension driAn::writelog "your log message
          // here.." note:        There is a 49 character limit. The server will
          // ignore messages with 50+ characters.

          getstring(text, p, n);
          if (valid_client(sender) && clients[sender]->role == CR_ADMIN) {
            logline(ACLOG_INFO, "[%s] %s writes to log: %s", cl->hostname,
                    cl->name, text);
        ...
        // add other extensions here
          ...
          for (; n > 0; n--) getint(p);  // ignore unknown extensions
      ...
      default: {
        ...
        if (size <= 0) {
          if (sender >= 0) disconnect_client(sender, DISC_TAGT);
          return;
        }
...
void checkintermission() {
  if (minremain > 0) {
    minremain = (gamemillis >= gamelimit || forceintermission)
                    ? 0
                    : (gamelimit - gamemillis + 60000 - 1) / 60000;
    sendf(
        -1, 1, "ri3", SV_TIMEUP,
        (gamemillis >= gamelimit || forceintermission) ? gamelimit : gamemillis,
        gamelimit);
  }
...
void sendworldstate() {
  ...
  if (demorecord)
    recordpackets = true;  // enable after 'old' worldstate is sent
}
...
void loggamestatus(const char *reason) {
  ...
  logline(ACLOG_INFO, "Game status: %s on %s, %s, %s, %d clients%c %s",
          modestr(gamemode), smapname, reason ? reason : text,
          mmfullname(mastermode), totalclients, custom_servdesc ? ',' : '\0',
          servdesc_current);
  if (!scl.loggamestatus) return;
  logline(ACLOG_INFO,
          "cn name             %s%s score frag death %sping role    host",
          m_teammode ? "team " : "", m_flags ? "flag " : "",
          m_teammode ? "tk " : "");
  loopv(clients) {
    client &c = *clients[i];
  ...
  if (mastermode == MM_MATCH) {
    loopv(savedscores) {
      ...
      if (sc.valid) {
        if (m_teammode)
          formatstring(text)("%-4s ", team_string(sc.team, true));
        else
          text[0] = '\0';
        if (m_flags) concatformatstring(text, "%4d ", sc.flagscore);
        logline(ACLOG_INFO, "   %-16s %s%4d %5d%s    - disconnected", sc.name,
                text, sc.frags, sc.deaths, m_teammode ? "  -" : "");
  ...
  if (m_teammode) {
    loopi(2) logline(ACLOG_INFO, "Team %4s:%3d players,%5d frags%c%5d flags",
                     team_string(i), pnum[i], fragscore[i],
                     m_flags ? ',' : '\0', flagscore[i]);
  }
  logline(ACLOG_INFO, "");
}
...
void linequalitystats(int elapsed) {
  static unsigned int chokes[MAXCLIENTS + 1] = {0}, spent[MAXCLIENTS + 1] = {0},
                                          chokes_raw[MAXCLIENTS + 1] = {0},
                                          spent_raw[MAXCLIENTS + 1] = {0};
  if (elapsed) {  // collect data
    int c1 = 0, c2 = 0, r1 = 0, numc = 0;
    loopv(clients) {
      client &c = *clients[i];
      if (c.type != ST_TCPIP) continue;
      numc++;
      enet_uint32 &rtt = c.peer->lastRoundTripTime,
                  &throttle = c.peer->packetThrottle;
    ...
    chokes_raw[numc] +=
        ((c1 >= t ? c1 + c2 : 0) + (r1 >= t ? r1 : 0)) * elapsed;
  } else {  // calculate compressed statistics
...
void serverslice(uint timeout)  // main server update, called from cube main
                                // loop in sp, or dedicated server loop
{
  ...
  if (minremain > 0) {
    ...
    if (m_flags) loopi(2) {
        ...
        if (f.state == CTFF_DROPPED &&
            gamemillis - f.lastupdate > (m_ctf ? 30000 : 10000))
          flagaction(i, FA_RESET, -1);
        if (m_htf && f.state == CTFF_INBASE &&
            gamemillis - f.lastupdate > (smapstats.hasflags ? 10000 : 1000)) {
          htf_forceflag(i);
        }
        if (m_ktf && f.state == CTFF_STOLEN &&
            gamemillis - f.lastupdate > 15000) {
          flagaction(i, FA_SCORE, -1);
        }
        if (f.state == CTFF_INBASE || f.state == CTFF_STOLEN)
          ktfflagingame = true;
      }
    if (m_ktf && !ktfflagingame)
      flagaction(rnd(2), FA_RESET, -1);  // ktf flag watchdog
    if (m_arena)
      arenacheck();
    else if (m_autospawn)
      autospawncheck();
    //        if(m_lms) lmscheck();
    sendextras();
    if (scl.afk_limit && mastermode == MM_OPEN && next_afk_check < servmillis &&
        gamemillis > 20 * 1000)
      check_afk();
  }
  ...
  if (forceintermission || ((smode > 1 || (gamemode == 0 && nonlocalclients)) &&
                            gamemillis - diff > 0 &&
                            gamemillis / 60000 != (gamemillis - diff) / 60000))
    checkintermission();
  if (m_demo && !demoplayback)
    maprot.restart();
  else if (interm &&
           ((scl.demo_interm && sending_demo) ? gamemillis > (interm << 1)
                                              : gamemillis > interm)) {
    sending_demo = false;
    ...
    // start next game
    if (nextmapname[0])
      startgame(nextmapname, nextgamemode);
    else
      maprot.next();
    nextmapname[0] = '\0';
    map_queued = false;
  ...
  serverms(smode, numclients(), minremain, smapname, servmillis,
           serverhost->address, &mnum, &msend, &mrec, &cnum, &csend, &crec,
           SERVER_PROTOCOL_VERSION);

  if (autoteam && m_teammode && !m_arena && !interm &&
      servmillis - lastfillup > 5000 && refillteams())
    lastfillup = servmillis;

  static unsigned int lastThrottleEpoch = 0;
  if (serverhost->bandwidthThrottleEpoch != lastThrottleEpoch) {
    if (lastThrottleEpoch)
      linequalitystats(serverhost->bandwidthThrottleEpoch - lastThrottleEpoch);
    lastThrottleEpoch = serverhost->bandwidthThrottleEpoch;
  }

  if (servmillis - laststatus >
      60 * 1000)  // display bandwidth stats, useful for server ops
  {
    laststatus = servmillis;
    rereadcfgs();
    if (nonlocalclients || serverhost->totalSentData ||
        serverhost->totalReceivedData) {
      if (nonlocalclients) loggamestatus(nullptr);
      logline(ACLOG_INFO,
              "Status at %s: %d remote clients, %.1f send, %.1f rec (K/sec);"
              " Ping: #%d|%d|%d; CSL: #%d|%d|%d (bytes)",
              timestring(true, "%d-%m-%Y %H:%M:%S"), nonlocalclients,
              serverhost->totalSentData / 60.0f / 1024,
              serverhost->totalReceivedData / 60.0f / 1024, mnum, msend, mrec,
              cnum, csend, crec);
  ...
  while (!serviced) {
    ...
    switch (event.type) {
      ...
      case ENET_EVENT_TYPE_RECEIVE: {
        auto cn = (int)(size_t)event.peer->data;
...
int getpongflags(enet_uint32 ip) {
  ...
  loopv(bans) if (bans[i].address.host == ip) {
    flags |= 1 << PONGFLAG_BANNED;
    break;
  }
...
void extping_namelist(ucharbuf &p) {
  loopv(clients) {
    if (clients[i]->type == ST_TCPIP && clients[i]->isauthed)
      sendstring(clients[i]->name, p);
  }
  sendstring("", p);
}
...
void extping_serverinfo(ucharbuf &pi, ucharbuf &po) {
  char lang[3];
  lang[0] = tolower(getint(pi));
  lang[1] = tolower(getint(pi));
  lang[2] = '\0';
  const char *reslang = lang,
             *buf = infofiles.getinfo(lang);          // try client language
  if (!buf) buf = infofiles.getinfo(reslang = "en");  // try english
  sendstring(buf ? reslang : "", po);
  if (buf) {
    for (const char *c = buf; *c && po.remaining() > MAXINFOLINELEN + 10;
         c += strlen(c) + 1)
      sendstring(c, po);
    sendstring("", po);
  }
...
void extinfo_statsbuf(ucharbuf &p, int pid, int bpos, ENetSocket &pongsock,
                      ENetAddress &addr, ENetBuffer &buf, int len, int *csend) {
  loopv(clients) {
    ...
    // "team_string(clients[i]->team)" sometimes return NULL according to RK,
    // causing the server to crash. WTF ?
    ...
    putint(p, ismatch ? 0
                      : clients[i]->state.damage * 100 /
                            max(clients[i]->state.shotdamage, 1));  // Accuracy
    ...
    putint(
        p,
        clients[i]->state.state);  // State (Alive,Dead,Spawning,Lagged,Editing)
    uint ip = clients[i]->peer->address.host;  // only 3 byte of the ip address
                                               // (privacy protected)
    p.put((uchar *)&ip, 3);
...
void extinfo_teamscorebuf(ucharbuf &p) {
  ...
  int teamsizes[TEAM_NUM] = {0}, fragscores[TEAM_NUM] = {0},
      flagscores[TEAM_NUM] = {0};
  loopv(clients) if (clients[i]->type != ST_EMPTY &&
                     team_isvalid(clients[i]->team)) {
...
void quitproc(int param) {
  ...
  exit(param == 2 ? EXIT_SUCCESS
                  : EXIT_FAILURE);  // 3 is the only reply on Win32 apparently,
                                    // SIGINT == 2 == Ctrl-C
}
...
void initserver(bool dedicated, int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!scl.checkarg(argv[i])) {
      ...
      else if (strncmp(argv[i], "assaultcube://", 13))
        printf("WARNING: unknown commandline argument\n");
    }
  }
  ...
  string identity;
  if (scl.logident[0])
    filtertext(identity, scl.logident, FTXT__LOGIDENT);
  else
    formatstring(identity)("%s#%d", scl.ip[0] ? scl.ip : "local",
                           scl.serverport);
  int conthres = scl.verbose > 1 ? ACLOG_DEBUG
                                 : (scl.verbose ? ACLOG_VERBOSE : ACLOG_INFO);
  if (dedicated &&
      !initlogging(identity, scl.syslogfacility, conthres, scl.filethres,
                   scl.syslogthres, scl.logtimestamp))
    printf("WARNING: logging not started!\n");
  logline(ACLOG_INFO,
          "logging local AssaultCube server (version %d, protocol %d/%d) now..",
          AC_VERSION, SERVER_PROTOCOL_VERSION, EXT_VERSION);

  copystring(servdesc_current, scl.servdesc_full);
  servermsinit(scl.master ? scl.master : AC_MASTER_URI, scl.ip,
               CUBE_SERVINFO_PORT(scl.serverport), dedicated);

  if ((isdedicated = dedicated)) {
    ENetAddress address = {ENET_HOST_ANY, (enet_uint16)scl.serverport};
    if (scl.ip[0] && enet_address_set_host(&address, scl.ip) < 0)
      logline(ACLOG_WARNING, "server ip not resolved!");
    serverhost =
        enet_host_create(&address, scl.maxclients + 1, 3, 0, scl.uprate);
    ...
    logline(ACLOG_VERBOSE, "holding up to %d recorded demos in memory",
            scl.maxdemos);
    if (scl.demopath[0])
      logline(ACLOG_VERBOSE, "all recorded demos will be written to: \"%s\"",
              scl.demopath);
    if (scl.voteperm[0])
      logline(ACLOG_VERBOSE, "vote permission string: \"%s\"", scl.voteperm);
    if (scl.mapperm[0])
      logline(ACLOG_VERBOSE, "map permission string: \"%s\"", scl.mapperm);
    logline(ACLOG_VERBOSE, "server description: \"%s\"", scl.servdesc_full);
    if (scl.servdesc_pre[0] || scl.servdesc_suf[0])
      logline(ACLOG_VERBOSE, "custom server description: \"%sCUSTOMPART%s\"",
              scl.servdesc_pre, scl.servdesc_suf);
    logline(ACLOG_VERBOSE,
            "maxclients: %d, kick threshold: %d, ban threshold: %d",
            scl.maxclients, scl.kickthreshold, scl.banthreshold);
    if (scl.master)
      logline(ACLOG_VERBOSE, "master server URL: \"%s\"", scl.master);
    if (scl.serverpassword[0])
      logline(ACLOG_VERBOSE, "server password: \"%s\"",
              hiddenpwd(scl.serverpassword));
#ifdef ACAC
...
#endif
    // kill -2 / Ctrl-C - see
    // http://msdn.microsoft.com/en-us/library/xdkz3x12%28v=VS.100%29.aspx (or
    // VS-2008?) for caveat (seems not to pertain to AC - 2011feb05:ft)
    if (signal(SIGINT, quitproc) == SIG_ERR)
      logline(ACLOG_INFO, "Cannot handle SIGINT!");
    // kill -15 / probably process-manager on Win32 *shrug*
    if (signal(SIGTERM, quitproc) == SIG_ERR)
      logline(ACLOG_INFO, "Cannot handle SIGTERM!");
#ifndef WIN32
    // kill -1
    if (signal(SIGHUP, quitproc) == SIG_ERR)
      logline(ACLOG_INFO, "Cannot handle SIGHUP!");
// kill -9 is uncatchable - http://en.wikipedia.org/wiki/SIGKILL
// if (signal(SIGKILL, quitproc) == SIG_ERR) logline(ACLOG_INFO, "Cannot handle
// SIGKILL!");
#endif
    logline(ACLOG_INFO, "dedicated server started, waiting for clients...");
    logline(
        ACLOG_INFO,
        "Ctrl-C to exit");  // this will now actually call the atexit-hooks
                            // below - thanks to SIGINT hooked above - noticed
                            // and signal-code-docs found by SKB:2011feb05:ft:
    atexit(enet_deinitialize);
    atexit(cleanupserver);
    enet_time_set(0);

    // start file-IO threads
    readmapsthread_sem = new sl_semaphore(0, nullptr);
