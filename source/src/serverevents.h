...
void processevent (client *c, explodeevent &e)
{
    ...
    for (int i = 1; i < c->events.length () && c->events[i].type == GE_HIT; i++)
    {
        ...
        if (target->type == ST_EMPTY || target->state.state != CS_ALIVE
            || h.lifesequence != target->state.lifesequence || h.dist < 0 || h.dist > EXPDAMRAD)
            continue;

        int j = 1;
        for (j = 1; j < i; j++)
            if (c->events[j].hit.target == h.target) break;
        if (j < i) continue;
...
void processevent (client *c, shotevent &e)
{
    ...
    if (!gs.isalive (gamemillis) || !valid_weapon (e.gun) || wait < gs.gunwait[e.gun] || gs.mag[e.gun] <= 0)
        return;
    ...
    sendf (-1, 1, "ri6x", SV_SHOTFX, c->clientnum, e.gun,
           ...
           int(e.to[0] * DMF), int(e.to[1] * DMF), int(e.to[2] * DMF), c->clientnum);
    gs.shotdamage
    += guns[e.gun].damage
       * (e.gun == GUN_SHOTGUN ? SGMAXDMGLOC : 1); // 2011jan17:ft: so accuracy stays correct, since
                                                   // SNIPER:headshot also "exceeds expectations" we
                                                   // use SGMAXDMGLOC instead of SGMAXDMGABS!
    switch (e.gun)
    {
    case GUN_GRENADE:
        gs.grenades.add (e.id);
        break;
    default:
    {
        ...
        for (int i = 1; i < c->events.length () && c->events[i].type == GE_HIT; i++)
        {
            ...
            client *target = clients[h.target];
            if (target->type == ST_EMPTY || target->state.state != CS_ALIVE
                || h.lifesequence != target->state.lifesequence)
                continue;
            ...
            if (e.gun == GUN_SHOTGUN)
            {
                ...
                int numhits_c = (h.info & 0x0000FF00) >> 8, numhits_m = (h.info & 0x00FF0000) >> 16,
                    numhits_o = (h.info & 0xFF000000) >> 24;
                tothits_c += numhits_c;
                tothits_m += numhits_m;
                tothits_o += numhits_o;
                rays = numhits_c + numhits_m + numhits_o;

                if (rays < 1 || tothits_c > SGRAYS || tothits_m > SGRAYS || tothits_o > SGRAYS || bonusdist > SGDMGBONUS)
                    continue;

                gib = rays == maxrays;
                float fdamage
                = (SGDMGTOTAL / (21 * 100.0f))
                  * (numhits_o * SGCOdmg / 10.0f + numhits_m * SGCMdmg / 10.0f + numhits_c * SGCCdmg / 10.0f);
...
void processevent (client *c, suicideevent &e) { serverdamage (c, c, INT_MAX, GUN_KNIFE, false); }
...
{
    clientstate &gs = c->state;
    if (!gs.isalive (gamemillis) || !valid_weapon (e.gun) || !reloadable_gun (e.gun) || gs.ammo[e.gun] <= 0)
        return;
    ...
    if (gs.gunwait[e.gun] && wait < gs.gunwait[e.gun])
...
void processevents ()
{
    loopv (clients)
    {
        ...
        while (c->events.length ())
        {
            ...
            {
            case GE_SHOT:
                processevent (c, e.shot);
                break;
            case GE_EXPLODE:
                processevent (c, e.explode);
                break;
            case GE_AKIMBO:
                processevent (c, e.akimbo);
                break;
            case GE_RELOAD:
                processevent (c, e.reload);
                break;
            // untimed events
            case GE_SUICIDE:
                processevent (c, e.suicide);
                break;
            case GE_PICKUP:
                processevent (c, e.pickup);
                break;
            }
            clearevent (c);
        }
