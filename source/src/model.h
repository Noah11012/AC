...
struct animstate // used for animation blending of animated characters
{
    ...
    {
        return frame == o.frame && range == o.range && basetime == o.basetime && speed == o.speed;
    }
    bool operator!= (const animstate &o) const
    {
        return frame != o.frame || range != o.range || basetime != o.basetime || speed != o.speed;
...
struct model
{
    ...
    // model() : cullface(true), vertexlight(false), alphatest(0.9f),
    // translucency(0.25f), scale(1), radius(0), shadowdist(0), translate(0, 0, 0),
    // cachelimit(8), batch(-1) {}
    model ()
    : cullface (true),
      vertexlight (false),
      alphablend (false),
    ...
    virtual void
    rendershadow (int anim, int varseed, float speed, int basetime, const vec &o, float yaw, modelattach *a = NULL)
    {
    }
