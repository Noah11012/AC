...
{
    int hwlimit = hwtexsize, sizelimit = mipmap && maxtexsize ? min (maxtexsize, hwlimit) : hwlimit;
...
void createtexture (int tnum, int w, int h, void *pixels, int clamp, bool mipmap, bool canreduce, GLenum format)
{
    ...
    if (hasAF && min (aniso, hwmaxaniso) > 0 && mipmap)
        glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, min (aniso, hwmaxaniso));
    glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, bilinear ? GL_LINEAR : GL_NEAREST);
    glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                     mipmap ?
                     (trilinear ? (bilinear ? GL_LINEAR_MIPMAP_LINEAR : GL_NEAREST_MIPMAP_LINEAR) :
...
SDL_Surface *fixsurfaceformat (SDL_Surface *s)
{
    ...
    case 3:
        ...
        break;
    case 4:
        if (s->format->Rmask != rgbamasks[0] || s->format->Gmask != rgbamasks[1]
...
GLuint loadsurface (const char *texname,
                    int &xs,
                    int &ys,
                    int &bpp,
                    int clamp = 0,
...
void _texture (Slot &s, float *scale, char *name)
{
    if (name)
    {
        filtertext (s.name, parentdir (name),
...
COMMANDF (texture, "fs", (float *scale, char *name) {
    ...
    {
        conoutf ("\f3texture slot #%d \"%s\" error: scale factor %.7g out of range 0..4",
...
void draw_envbox_face (float s0,
                       float t0,
                       float x0,
                       float y0,
                       float z0,
...
void draw_envbox (int w)
{
    ...
    draw_envbox_face (0.0f, 0.0f, -w, -w, -w, 1.0f, 0.0f, -w, w, -w, 1.0f, vclip, -w, w, z, 0.0f,
                      vclip, -w, -w, z, sky[0] ? sky[0] : notexture);

    draw_envbox_face (1.0f, vclip, +w, -w, z, 0.0f, vclip, +w, w, z, 0.0f, 0.0f, +w, w, -w, 1.0f,
                      0.0f, +w, -w, -w, sky[1] ? sky[1] : notexture);

    draw_envbox_face (1.0f, vclip, -w, -w, z, 0.0f, vclip, w, -w, z, 0.0f, 0.0f, w, -w, -w, 1.0f,
                      0.0f, -w, -w, -w, sky[2] ? sky[2] : notexture);

    draw_envbox_face (1.0f, vclip, +w, w, z, 0.0f, vclip, -w, w, z, 0.0f, 0.0f, -w, w, -w, 1.0f,
                      0.0f, +w, w, -w, sky[3] ? sky[3] : notexture);

    if (!zclip)
        draw_envbox_face (0.0f, 1.0f, -w, w, w, 0.0f, 0.0f, +w, w, w, 1.0f, 0.0f, +w, -w, w, 1.0f,
                          1.0f, -w, -w, w, sky[4] ? sky[4] : notexture);

    draw_envbox_face (0.0f, 1.0f, +w, w, -w, 0.0f, 0.0f, -w, w, -w, 1.0f, 0.0f, -w, -w, -w, 1.0f,
                      1.0f, +w, -w, -w, sky[5] ? sky[5] : notexture);
...
void parsetmufunc (tmufunc &f, const char *s)
{
    ...
    while (*s) switch (tolower (*s++))
        ...
        case '~':
            ...
            break;
        case 'a':
            f.ops[arg] = f.ops[arg] == GL_ONE_MINUS_SRC_COLOR ? GL_ONE_MINUS_SRC_ALPHA : GL_SRC_ALPHA;
            break;
        case '=':
...
void committmufunc (bool rgb, tmufunc &dst, tmufunc &src)
{
    if (dst.combine != src.combine)
        glTexEnvi (GL_TEXTURE_ENV, rgb ? GL_COMBINE_RGB_ARB : GL_COMBINE_ALPHA_ARB, src.combine);
    loopi (3)
    {
        if (dst.sources[i] != src.sources[i])
            glTexEnvi (GL_TEXTURE_ENV, (rgb ? GL_SOURCE0_RGB_ARB : GL_SOURCE0_ALPHA_ARB) + i, src.sources[i]);
        if (dst.ops[i] != src.ops[i])
            glTexEnvi (GL_TEXTURE_ENV, (rgb ? GL_OPERAND0_RGB_ARB : GL_OPERAND0_ALPHA_ARB) + i, src.ops[i]);
    }
    if (dst.scale != src.scale)
        glTexEnvi (GL_TEXTURE_ENV, rgb ? GL_RGB_SCALE_ARB : GL_ALPHA_SCALE, src.scale);
}
...
void gettexturelist (char *_filter,
                     char *_exclude,
                     char *_exts) // create a list of texture paths and filenames
{
    ...
    loopv (files)
    {
        ...
        if (cutprefix)
            cvecprintf (res, " \"%s\"",
                        int(strlen (p)) > filter_n[0] ? p + filter_n[0] : ""); // if there's exactly one filter string, add column
...
void textureslotusagegeometry (int *used, int *visible = nullptr, int *mostvisible = nullptr)
{
    sqr *s = world;
    int *mostw = mostvisible ? new int[1024] : nullptr, *mostc = mostvisible ? mostw + 256 : nullptr,
        *mostf = mostvisible ? mostw + 512 : nullptr, *mostu = mostvisible ? mostw + 768 : nullptr;
...
void textureslotusagelist (char *what)
{
    ...
    { // if not only models, count map geometry
        textureslotusagegeometry (used, visible, onlymostvisible ? mostvisible : nullptr);
    }
    vector<char> res;
    if (onlymostvisible)
        cvecprintf (res, "wall %d floor %d ceiling %d \"upper wall\" %d ", mostvisible[0],
                    mostvisible[1], mostvisible[2], mostvisible[3]);
    else
...
void textureslotusage (int *n) // returns all mapmodel entity indices that use a certain texture
                                        ...
                                        // or "purge" is specified
{
    ...
    {
        conoutf ("texture slots below #%d should usually not be deleted", DEFAULT_CEIL + 1);
        return;
    }
    ...
    loopi (256) if (usedslots[i] && s->inrange (i))
    {
        ...
        if (!found)
        {
            if (slots.length () < 255)
            {
                ...
                conoutf ("\f3failed to add texture \"%s\" (scale %.7g): no texture slot "
                         "available",
                         os.name, os.scale);
        }
    }
...
int tempslotunsort (temptexslot *a, temptexslot *b) { return a->oldslot[0] - b->oldslot[0]; }
...
void sorttextureslots (char **args, int numargs)
{
    ...
    if (!nomerge) loopvrev (tempslots) if (i > 0)
        {
            ...
            if (s1.oldslot[0] > 4 && s0.oldslot[0] > 0 && !strcmp (s0.s.name, s1.s.name)
                && s0.s.scale == s1.s.scale && (mergeused || !s0.used || !s1.used))
            {
    ...
    loopv (presort) for (int n = i + 1; n < presort.length (); n++) if (presort[i] == presort[n])
    ...
    conoutf ("%d texture slots%s, %d %sslots merged", tempslots.length (), nosort ? "" : " sorted",
             slots.length () - tempslots.length (), mergeused ? "" : "unused ");
