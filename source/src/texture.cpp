...
void scaletexture(uchar *src, uint sw, uint sh, uint bpp, uchar *dst, uint dw,
                  uint dh) {
  if (sw == dw * 2 && sh == dh * 2) {
    switch (bpp) {
      case 1:
        return halvetexture1(src, sw, sh, dst);
      case 2:
        return halvetexture2(src, sw, sh, dst);
      case 3:
        return halvetexture3(src, sw, sh, dst);
      case 4:
        return halvetexture4(src, sw, sh, dst);
    }
  } else if (sw < dw || sh < dh || sw & (sw - 1) || sh & (sh - 1)) {
    switch (bpp) {
      case 1:
        return scaletexture1(src, sw, sh, dst, dw, dh);
      case 2:
        return scaletexture2(src, sw, sh, dst, dw, dh);
      case 3:
        return scaletexture3(src, sw, sh, dst, dw, dh);
      case 4:
        return scaletexture4(src, sw, sh, dst, dw, dh);
    }
  } else {
    switch (bpp) {
      case 1:
        return shifttexture1(src, sw, sh, dst, dw, dh);
      case 2:
        return shifttexture2(src, sw, sh, dst, dw, dh);
      case 3:
        return shifttexture3(src, sw, sh, dst, dw, dh);
      case 4:
        return shifttexture4(src, sw, sh, dst, dw, dh);
    }
  }
}
...
void resizetexture(int w, int h, bool mipmap, bool canreduce, GLenum target,
                   int &tw, int &th) {
...
void uploadtexture(GLenum target, GLenum internal, int tw, int th,
...
void createtexture(int tnum, int w, int h, void *pixels, int clamp, bool mipmap,
                   bool canreduce, GLenum format) {
  glBindTexture(GL_TEXTURE_2D, tnum);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                  clamp & 1 ? GL_CLAMP_TO_EDGE : GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                  clamp & 2 ? GL_CLAMP_TO_EDGE : GL_REPEAT);
  if (hasAF && min(aniso, hwmaxaniso) > 0 && mipmap)
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT,
                    min(aniso, hwmaxaniso));
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                  bilinear ? GL_LINEAR : GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                  mipmap ? (trilinear ? (bilinear ? GL_LINEAR_MIPMAP_LINEAR
  ...
  uploadtexture(GL_TEXTURE_2D, format, tw, th, format, GL_UNSIGNED_BYTE, pixels,
                w, h, mipmap);
}
...
bool checkgrayscale(SDL_Surface *s) {
  ...
  if (s->format->palette) {
    ...
    loopi(256) if (colors[i].r != i || colors[i].g != i ||
                   colors[i].b != i) return false;
  }
  return true;
}
...
int fixcl(SDL_Surface *s, int threshold) {
  ...
  if (s->format->palette || s->format->BytesPerPixel < 3)
    s = (SDL_Surface *)s->pixels;
  for (int y = 0; y < s->h; y++) {
    uchar *pix = ((uchar *)s->pixels) + y * s->pitch + a0;
    w = 0;
    for (int x = 0; x < s->w; x++) {
      w += iabs(((pix[0] * pix[0]) | (pix[1] * pix[1]) | (pix[2] * pix[2])) -
                ((pix[0] * pix[1] * pix[2]) / 222));
...
SDL_Surface *fixsurfaceformat(SDL_Surface *s) {
  ...
  switch (s->format->BytesPerPixel) {
    case 1:
      if (!checkgrayscale(s))
        return s->format->colorkey ? forcergbasurface(s) : forcergbsurface(s);
      break;
    case 3:
      if (s->format->Rmask != rgbmasks[0] || s->format->Gmask != rgbmasks[1] ||
          s->format->Bmask != rgbmasks[2])
        return forcergbsurface(s);
      break;
    case 4:
      if (s->format->Rmask != rgbamasks[0] ||
          s->format->Gmask != rgbamasks[1] ||
          s->format->Bmask != rgbamasks[2] || s->format->Amask != rgbamasks[3])
...
GLuint loadsurface(const char *texname, int &xs, int &ys, int &bpp,
                   int clamp = 0, bool mipmap = true, bool canreduce = false,
                   float scale = 1.0f, bool trydl = false) {
  const char *file = texname;
  if (texname[0] == '<') {
    file = strchr(texname, '>');
    if (!file) {
      if (!silent_texture_load) conoutf("could not load texture %s", texname);
      return 0;
    }
    file++;
  }
  ...
  if (strstr(texname, "playermodel")) {
    fixcl(s, 45);
  } else if (strstr(texname, "skin") && strstr(texname, "weapon")) {
    fixcl(s, 44);
  }
  ...
  if (max(s->w, s->h) > (1 << 12)) {
    SDL_FreeSurface(s);
    conoutf("texture size exceeded %dx%d pixels: %s", 1 << 12, 1 << 12,
            texname);
    return 0;
  }

  if (texname[0] == '<') {
    const char *cmd = &texname[1],
               *arg1 = strchr(
                   cmd, ':');  //, *arg2 = arg1 ? strchr(arg1, ',') : NULL;
    if (!arg1) arg1 = strchr(cmd, '>');
    if (!strncmp(cmd, "decal", arg1 - cmd)) {
      s = texdecal(s);
      format = texformat(s->format->BitsPerPixel);
    }
  }
...
// each texture slot can have multiple texture frames, of which currently only
...
Texture *textureload(const char *name, int clamp, bool mipmap, bool canreduce,
  ...
  if (t) return t;
  int xs, ys, bpp;
  GLuint id = loadsurface(pname + TEXSCALEPREFIXSIZE, xs, ys, bpp, clamp,
                          mipmap, canreduce, scale, trydl);
...
COMMANDF(texturereset, "", () {
  if (execcontext == IEXC_MAPCFG) slots.setsize(0);
});

void _texture(Slot &s, float *scale, char *name) {
  if (name) {
    filtertext(
        s.name, parentdir(name),
        FTXT__MEDIAFILEPATH);  // filter parts separately, because filename may
                               // (legally) contain "<decal>"
    if (s.name[0] == '.' && s.name[1] == '/')
      memmove(s.name, s.name + 2, sizeof(s.name) - 2);
...
COMMANDF(texture, "fs", (float *scale, char *name) {
  ...
  if (*scale < 0.0f || *scale > 4.0f) {
    conoutf(
        "\f3texture slot #%d \"%s\" error: scale factor %.7g out of range 0..4",
        slots.length() - 1, name, *scale);
    flagmapconfigerror(LWW_CONFIGERR * 4);
    scripterr();
  }
...
void cleanuptextures() {
  enumerate(textures, Texture, t, if (t.id) {
...
bool reloadtexture(Texture &t) {
  ...
  int xs = 1, ys = 1, bpp = 0;
  t.id =
      loadsurface(t.name, xs, ys, bpp, t.clamp, t.mipmap, t.canreduce, t.scale);
  t.xs = xs;
  t.ys = ys;
  t.bpp = bpp;
  return t.id != 0;
...
bool loadskymap(bool reload) {
  ...
  if (!reload) {
    ...
    if (!strncmp(loadsky, legacyprefix, n)) {  // remove the prefix
      char *t = loadsky;
  ...
  loopi(6) {
    defformatstring(name)("packages/%s%s_%s.jpg", legacyprefix, loadsky,
                          side[i]);
    sky[i] = textureload(name, 3);
    if (sky[i] == notexture && !reload && autodownload &&
        execcontext == IEXC_MAPCFG) {
...
void loadnotexture(char *c) {
  ...
  *mapconfigdata.notexturename = '\0';
  if (c[0]) {
    ...
    if (noworldtexture == notexture) {
      conoutf("could not load alternative texture '%s'.", p);
      flagmapconfigerror(LWW_CONFIGERR * 8);
      scripterr();
    }
  }
  flagmapconfigchange();
...
void draw_envbox(int w) {
  ...
  float zclip = skyclip && skyfloor >= camera1->o.z
                    ? 0.5f + float(skyfloor - camera1->o.z) / w
                    : 0.0f,
        vclip = 1 - zclip, z = 2 * w * (vclip - 0.5f);

  if (vclip < 0) return;

  glDepthMask(GL_FALSE);

  draw_envbox_face(0.0f, 0.0f, -w, -w, -w, 1.0f, 0.0f, -w, w, -w, 1.0f, vclip,
                   -w, w, z, 0.0f, vclip, -w, -w, z,
                   sky[0] ? sky[0] : notexture);

  draw_envbox_face(1.0f, vclip, +w, -w, z, 0.0f, vclip, +w, w, z, 0.0f, 0.0f,
                   +w, w, -w, 1.0f, 0.0f, +w, -w, -w,
                   sky[1] ? sky[1] : notexture);

  draw_envbox_face(1.0f, vclip, -w, -w, z, 0.0f, vclip, w, -w, z, 0.0f, 0.0f, w,
                   -w, -w, 1.0f, 0.0f, -w, -w, -w, sky[2] ? sky[2] : notexture);

  draw_envbox_face(1.0f, vclip, +w, w, z, 0.0f, vclip, -w, w, z, 0.0f, 0.0f, -w,
                   w, -w, 1.0f, 0.0f, +w, w, -w, sky[3] ? sky[3] : notexture);

  if (!zclip)
    draw_envbox_face(0.0f, 1.0f, -w, w, w, 0.0f, 0.0f, +w, w, w, 1.0f, 0.0f, +w,
                     -w, w, 1.0f, 1.0f, -w, -w, w, sky[4] ? sky[4] : notexture);

  draw_envbox_face(0.0f, 1.0f, +w, w, -w, 0.0f, 0.0f, -w, w, -w, 1.0f, 0.0f, -w,
                   -w, -w, 1.0f, 1.0f, +w, -w, -w, sky[5] ? sky[5] : notexture);
  ...
  {                                                                \
    GL_MODULATE, {0, 0, 0, 0},                                     \
        {GL_MODULATE,                                              \
         ...
         1},                                                       \
    {                                                              \
      GL_MODULATE, {GL_TEXTURE, GL_PREVIOUS_ARB, GL_CONSTANT_ARB}, \
          {GL_SRC_ALPHA, GL_SRC_ALPHA, GL_SRC_ALPHA}, 1            \
    }                                                              \
  }

#define MAXTMUS 4

tmu tmus[MAXTMUS] = {INVALIDTMU, INVALIDTMU, INVALIDTMU, INVALIDTMU};
...
void parsetmufunc(tmufunc &f, const char *s) {
  ...
  while (*s) switch (tolower(*s++)) {
      case 't':
        f.sources[++arg] = GL_TEXTURE;
        f.ops[arg] = GL_SRC_COLOR;
        break;
      case 'p':
        f.sources[++arg] = GL_PREVIOUS_ARB;
        f.ops[arg] = GL_SRC_COLOR;
        break;
      case 'k':
        f.sources[++arg] = GL_CONSTANT_ARB;
        f.ops[arg] = GL_SRC_COLOR;
        break;
      case 'c':
        f.sources[++arg] = GL_PRIMARY_COLOR_ARB;
        f.ops[arg] = GL_SRC_COLOR;
        break;
      case '~':
        f.ops[arg] = GL_ONE_MINUS_SRC_COLOR;
        break;
      case 'a':
        f.ops[arg] = f.ops[arg] == GL_ONE_MINUS_SRC_COLOR
                         ? GL_ONE_MINUS_SRC_ALPHA
                         : GL_SRC_ALPHA;
        break;
      case '=':
        f.combine = GL_REPLACE;
        break;
      case '*':
        f.combine = GL_MODULATE;
        break;
      case '+':
        f.combine = GL_ADD;
        break;
      case '-':
        f.combine = GL_SUBTRACT_ARB;
        break;
      case ',':
      case '@':
        f.combine = GL_INTERPOLATE_ARB;
        break;
      case '.':
        f.combine = GL_DOT3_RGB_ARB;
        break;
      case 'x':
        while (!isdigit(*s)) s++;
        f.scale = *s++ - '0';
...
void committmufunc(bool rgb, tmufunc &dst, tmufunc &src) {
  if (dst.combine != src.combine)
    glTexEnvi(GL_TEXTURE_ENV, rgb ? GL_COMBINE_RGB_ARB : GL_COMBINE_ALPHA_ARB,
              src.combine);
  loopi(3) {
    if (dst.sources[i] != src.sources[i])
      glTexEnvi(GL_TEXTURE_ENV,
                (rgb ? GL_SOURCE0_RGB_ARB : GL_SOURCE0_ALPHA_ARB) + i,
                src.sources[i]);
    if (dst.ops[i] != src.ops[i])
      glTexEnvi(GL_TEXTURE_ENV,
                (rgb ? GL_OPERAND0_RGB_ARB : GL_OPERAND0_ALPHA_ARB) + i,
                src.ops[i]);
  }
  if (dst.scale != src.scale)
    glTexEnvi(GL_TEXTURE_ENV, rgb ? GL_RGB_SCALE_ARB : GL_ALPHA_SCALE,
              src.scale);
}

void committmu(int n, tmu &f) {
  if (n >= maxtmus) return;
  if (tmus[n].mode != f.mode)
    glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, f.mode);
  if (memcmp(tmus[n].color, f.color, sizeof(f.color)))
    glTexEnvfv(GL_TEXTURE_ENV, GL_TEXTURE_ENV_COLOR, f.color);
...
void setuptmu(int n, const char *rgbfunc, const char *alphafunc) {
  ...
  if (rgbfunc)
    parsetmufunc(f.rgb, rgbfunc);
  else
    f.rgb = init.rgb;
  if (alphafunc)
    parsetmufunc(f.alpha, alphafunc);
  else
    f.alpha = init.alpha;
...
void gettexturelist(
    char *_filter, char *_exclude,
    char *_exts)  // create a list of texture paths and filenames
{
  ...
  explodelist(_exts, exts);  // file needs to have one of those extensions (or
                             // ".jpg" by default)
  ...
  loopvrev(files) {
    ...
    if (!strncmp(files[i], "packages/textures/", pn)) {
      ...
      loopvj(exclude) if (!strncmp(s, exclude[j], exclude_n[j])) excluded =
          true;
      int n = strlen(s);
      loopvj(exts) if (n > exts_n[j] && !strcmp(s + n - exts_n[j], exts[j]))
          extmatch = true;
    }
  ...
  loopv(files) {
    ...
    if (cutprefix)
      cvecprintf(res, " \"%s\"",
                 int(strlen(p)) > filter_n[0]
                     ? p + filter_n[0]
                     : "");  // if there's exactly one filter string, add column
                             // with that string omitted
    if (cutext) {
      int n = strlen(b);
      if (n > exts_n[0]) ((char *)b)[n - exts_n[0]] = '\0';
      cvecprintf(
          res, " \"%s\"",
          b);  // if there's only one allowed extension and it does not start
               // with '.', add column of filenames without extension
    }
    res.add('\n');
  }
  filter.deletearrays();
...
void textureslotusagegeometry(int *used, int *visible = nullptr,
  ...
  loopirev(cubicsize) {
    ...
    if (visible && !(s->visible & INVISIBLE)) {  // cube not invisible
      if (!(s->visible & INVISWTEX)) {           // lower wall visible
        visible[s->wtex]++;
        if (mostw) mostw[s->wtex]++;
      }
      if (s->type != SOLID) {
        if (!(s->visible & INVISUTEX)) {  // upper wall visible
          visible[s->utex]++;
  ...
  if (mostvisible)
    loopk(4) {  // find most visible texures (for all four geometry uses)
...
void textureslotusagelist(char *what) {
  ...
  if (strcmp(what,
             "onlygeometry")) {  // if not only geometry, count map model usage
  ...
  vector<char> res;
  if (onlymostvisible)
    cvecprintf(res, "wall %d floor %d ceiling %d \"upper wall\" %d ",
               mostvisible[0], mostvisible[1], mostvisible[2], mostvisible[3]);
  else
    loopi(256) cvecprintf(res, "%d %d ", used[i], visible[i]);
  resultcharvector(res, -1);
}
...
bool testworldtexusage(int n) {
  sqr *s = world;
  loopirev(cubicsize) {
    if (s->wtex == n || s->ctex == n || s->ftex == n || s->utex == n)
      return true;
    s++;
  }
  return false;
...
void textureslotusage(
    int *n)  // returns all mapmodel entity indices that use a certain texture
             // to skin the model if the texture is used in map geometry, at
             // least a space is returned if the texture is unused, an empty
             // string is returned
{
  string res = "";
  loopv(ents) if (ents[i].type == MAPMODEL && ents[i].attr4 == *n)
      concatformatstring(res, "%s%d", i ? " " : "", i);
  if (!*res) {  // not used to skin a mapmodel: check world usage...
...
void deletetextureslot(int *n, char *opt,
                       char *_replace)  // delete texture slot - only if unused
                                        // or "purge" is specified
{
  EDITMP("deletetextureslot");
  if (!slots.inrange(*n)) return;
  bool purgeall = !strcmp(opt, "purge");
  bool mmused = false;
  loopv(ents) if (ents[i].type == MAPMODEL && ents[i].attr4 == *n) mmused =
      true;
  if (!purgeall && *n <= DEFAULT_CEIL) {
    conoutf("texture slots below #%d should usually not be deleted",
            DEFAULT_CEIL + 1);
    return;
  }
  if (!purgeall && (mmused || testworldtexusage(*n))) {
    conoutf("texture slot #%d is in use: can't delete", *n);
    return;
  }
  ...
  block bb = {clmapdims.x1 - 1, clmapdims.y1 - 1, clmapdims.xspan + 2,
              clmapdims.yspan + 2};
  remip(bb);
  loopv(ents) if (ents[i].type ==
                  MAPMODEL) {  // check, if mapmodels use the texture
    entity &e = ents[i];
    if (e.attr4 == *n) {  // use default model texture instead
      e.attr4 = 0;
      deld++;
    } else if (e.attr4 > *n)
      e.attr4--;  // adjust models in higher slots
  }
  ...
  if (replace != 255)
    formatstring(m)(" (%d uses changed to use slot #%d)", deld, replace);
...
void edittextureslot(int *n, char *scale,
...
void gettextureorigin(char *fname) {
  ...
  switch (findfilelocation) {
    case FFL_ZIP:
      ...
      break;
    case FFL_WORKDIR:
      res = fileexists(s, "r") ? "official" : "<file not found>";
      break;
    case FFL_HOME:
      res = "custom";
      break;
    default:
      formatstring(s)("package dir #%d", findfilelocation);
      break;
  }
  result(res);
}
...
void textureslotbyname(char *name)  // returns the slot(s) that a certain
                                    // texture file is configured in
{
  string res = "";
  loopv(slots) if (!strcmp(name, slots[i].name))
      concatformatstring(res, "%s%d", *res ? " " : "", i);
  result(res);
}
COMMAND(textureslotbyname, "s");

// helper functions to allow copy&paste between different maps while keeping the
...
void texconfig_delete(void *s) { delete (vector<Slot> *)s; }

uchar *texconfig_paste(
    void *_s,
    ...
    if (!found) {
      if (slots.length() < 255) {
        ...
        if (!(hdr.flags & MHF_AUTOMAPCONFIG))
          automapconfig();  // make sure, the new texture slots will be saved
                            // with the map
      } else
        conoutf(
            "\f3failed to add texture \"%s\" (scale %.7g): no texture slot "
            "available",
            os.name, os.scale);
    }
  }
...
void sorttextureslots(char **args, int numargs) {
  ...
  loopi(numargs) if (args[i][0]) {
    if (!strcasecmp(args[i], "nomerge"))
      ...
      presort.add(ATOI(args[i]));
    else {
      conoutf("sorttextureslots: unknown argument \"%s\"", args[i]);
      unknownarg = true;
    }
  }
  ...
  if (!nomerge) loopvrev(tempslots) if (i > 0) {
      ...
      if (s1.oldslot[0] > 4 && s0.oldslot[0] > 0 &&
          !strcmp(s0.s.name, s1.s.name) && s0.s.scale == s1.s.scale &&
          (mergeused || !s0.used || !s1.used)) {
  ...
  // sort special textures (like skymap) back front - also, revert sorting, if
  // it's unwanted
  if (nosort) tempslots.sort(tempslotunsort);
  loopv(presort) for (int n = i + 1; n < presort.length();
                      n++) if (presort[i] == presort[n])
      presort.remove(n);  // first occurrence is kept
  loopvk(presort) {
    loopv(tempslots) {
      bool yep = false;
      loopvj(tempslots[i].oldslot) if (tempslots[i].oldslot[j] == presort[k])
          yep = true;
      if (yep && i > k) {
  ...
  conoutf("%d texture slots%s, %d %sslots merged", tempslots.length(),
          nosort ? "" : " sorted", slots.length() - tempslots.length(),
          mergeused ? "" : "unused ");
...
#ifdef _DEBUG
void resettexturelists() { loopk(3) loopi(256) hdr.texlists[k][i] = i; }
