...
class winserviceinstaller
{
    ...
    public:
    winserviceinstaller (LPCSTR name, LPCSTR displayName, LPCSTR path)
    : name (name), displayName (displayName), path (path), scm (NULL)
    {
    }

    ~winserviceinstaller () { CloseManager (); }
    ...
    {
        if (!scm) return -1;
        SC_HANDLE svc = CreateService (scm, name, displayName, SERVICE_ALL_ACCESS,
