...
const char *setnames(const char *name) {
  ...
  formatstring(ocgzname)("packages" PATHDIVS "maps" PATHDIVS "official" PATHDIVS
                         "%s.cgz",
                         mapname);
  formatstring(omcfname)("packages" PATHDIVS "maps" PATHDIVS "official" PATHDIVS
                         "%s.cfg",
                         mapname);
  formatstring(bakname)("packages" PATHDIVS "maps" PATHDIVS "%s_%s.BAK",
                        mapname, nt);
  formatstring(cbakname)("packages" PATHDIVS "maps" PATHDIVS "%s.cfg_%s.BAK",
                         mapname, nt);
  return cgzname;
}
...
void rlencodecubes(vector<uchar> &f, sqr *s, int len,
...
bool rldecodecubes(ucharbuf &f, sqr *s, int len, int version,
                                 ...
                                 // (version is only relevant, if < 6)
{
  ...
  while (s < e) {
    ...
    switch (type) {
      case -1: {
        if (!silent)
          conoutf("while reading map at %d: unexpected end of file",
                  int(cubicsize - (e - s)));
        f.forceoverread();
        silent = true;
        sqrdefault(s);
      ...
      default: {
        if (type < 0 || type >= MAXTYPE) {
          if (!silent)
            conoutf("while reading map at %d: type %d out of range",
                    int(cubicsize - (e - s)), type);
          f.forceoverread();
          continue;
        }
        sqrdefault(s);
...
// the format and handling is explicitly designed to handle yet unknown header
...
const char *hx_names[] = {"unused",   "mapinfo", "modeinfo",      "artist",
                          "editundo", "config",  "vantage point", "unknown"};
...
void deleteheaderextra(int n) {
  if (headerextras.inrange(n)) delete headerextras.remove(n);
}

void listheaderextras() {
  loopv(headerextras)
      conoutf("extra header record %d: %s, %d bytes", i,
              hx_name(headerextras[i]->flags), headerextras[i]->len);
  if (!headerextras.length()) conoutf("no extra header records found");
}
COMMAND(listheaderextras, "");

int findheaderextra(int type) {
  loopv(headerextras) if ((headerextras[i]->flags & HX_TYPEMASK) ==
                          type) return i;
  return -1;
}

void unpackheaderextra(
    uchar *buf,
    int len)  // break the extra data from the mapheader into its pieces
{
  ucharbuf p(buf, len);
  ...
  while (true) {
    ...
    DEBUGCODE(clientlogf(
        " found headerextra \"%s\", %d bytes%s", hx_name(flags & HX_TYPEMASK),
        len, flags & HX_FLAG_PERSIST ? ", persistent" : ""));  // debug info
    headerextras.add(new headerextra(len, flags, p.subbuf(len).buf));
  }
}

void parseheaderextra(
    bool clearnonpersist = true,
    int ignoretypes = 0)  // parse all headerextra packets, delete the
                          // nonpersistent ones (like editundos)
{
  ...
  loopv(headerextras) {
    ...
    bool deletethis =
        false;  // (set deletethis for headers that persist outside headerextras
                // and get reinserted by packheaderextras())
    ...
    if (deletethis ||
        (clearnonpersist && !(headerextras[i]->flags & HX_FLAG_PERSIST)))
      delete headerextras.remove(i--);
  }
}

ucharbuf packheaderextras(
    int ignoretypes =
        0)  // serialise all extra data packets to save with the map header
{
  ...
  if (!(ignoretypes & (1 << HX_MAPINFO))) {
    ...
    if (mapinfo_license[0]) {
      ...
      headerextras.add(new headerextra(
          tmp.length(), HX_MAPINFO | HX_FLAG_PERSIST, tmp.getbuf()));
      tmp.setsize(0);
    }
  }
  loopv(headerextras) if (!(ignoretypes &
                            (1 << (headerextras[i]->flags &
                                   HX_TYPEMASK)))) {  // copy existing
                                                      // persistent hx packets
    addhxpacket(buf, headerextras[i]->len, headerextras[i]->flags,
                headerextras[i]->data);
  }
...
bool embedconfigfile() {
  ...
  if (unsavededits) {
    conoutf(
        "There are unsaved edits: they need to be saved or discarded before a "
        "config file can be embedded.");
    return false;
  }
...
void extractconfigfile() {
  ...
  if (unsavededits) {
    conoutf(
        "There are unsaved edits: they need to be saved or discarded before "
        "the config file can be extracted.");
    return;
  }
  setnames(getclientmap());
  ...
  if (n < 0)
    ...
    backup(
        mcfname,
        cbakname);  // don't overwrite an existing config file, just rename it
    stream *f = openfile(mcfname, "w");
    if (f) {
      f->write(headerextras[n]->data,
               headerextras[n]->len > 0 ? headerextras[n]->len - 1 : 0);
      delete f;
...
void automapconfig() {
  conoutf("\"automatic embedded map config data\" %senabled",
          hdr.flags & MHF_AUTOMAPCONFIG ? "was already " : "");
  hdr.flags |= MHF_AUTOMAPCONFIG;
}
COMMAND(automapconfig, "");

COMMANDF(getautomapconfig, "",
         () { intret((hdr.flags & MHF_AUTOMAPCONFIG) != 0); });

void flagmapconfigchange() {  // if changes are tracked, because automapcfg is
                              // enabled and the change is not read from a
                              // config file -> set unsaved edits flag
...
void getcurrentmapconfig(vector<char> &f, bool onlysounds) {
  if (!onlysounds) {
    ...
    loopi(256) {
      ...
      if (!mmi) break;
      cvecprintf(f, "mapmodel %d %d %d %s \"%s\"", mmi->rad, mmi->h, mmi->zoff,
                 mmi->scale == 1.0f ? "0" : floatstr(mmi->scale, true),
                 mmshortname(mmi->name));
...
COMMANDF(clearvantagepoint, "", () {
  if (!noteditmode("clearvantagepoint") && !multiplayer() &&
      clearvantagepoint())
    conoutf("cleared vantage point");
});

void setvantagepoint() {
  if (multiplayer("setvantagepoint") ||
      (!player1->isspectating() && noteditmode("setvantagepoint")))
    return;
  clearvantagepoint();
  short p[5];
  storeposition(p);
  ...
  if (!OUTBORD(p[0] / DMF, p[1] / DMF) && iabs(p[2]) < 127 * int(DMF) &&
      p[3] >= 0 && p[3] < 360 && iabs(p[4]) <= 90) {
    headerextras.add(new headerextra(
        buf.length(), HX_VANTAGEPOINT | HX_FLAG_PERSIST, buf.getbuf()));
...
COMMANDF(getvantagepoint, "", () {
  ...
  if (getvantagepoint(p))
    formatstring(res)("%s %s %s %d %d", floatstr(p[0] / DMF),
                      floatstr(p[1] / DMF), floatstr(p[2] / DMF), p[3], p[4]);
  result(res);
});
...
bool gotovantagepoint() {
  ...
  // if there is no (valid) vantage point set, we go to the first playerstart
  // instead
...
COMMANDF(gotovantagepoint, "", () {
  intret((editmode || (!multiplayer(NULL) && player1->isspectating())) &&
                 gotovantagepoint()
             ? 1
             : 0);
});
...
void setmapinfo(const char *newlicense, const char *newcomment) {
  string s;
  filtertext(s, newlicense ? newlicense : mapinfo_license,
             FTXT__MAPINFO_LICENSE, 20);
  mapinfo_license = exchangestr(mapinfo_license, s);
  filtertext(s, newcomment ? newcomment : mapinfo_comment,
             FTXT__MAPINFO_COMMENT, 60);
...
void rebuildtexlists()  // checks the texlists, if they still contain all
                        // possible textures
{
  short h[256];
  int mk[3], mt = 0;
  ...
  if (mt)
    conoutf("WARNING: rebuildtexlists() fixed %d|%d|%d missing entries", mk[0],
            mk[1], mk[2]);
}

static const uchar sortenttab[] = {  // 1..9: used in network protocol, 10..19:
                                     // sorted lights, 20..29: misc, 30..39:
                                     // clipped ents at the end of the list
    99,  // NOTUSED                          // entity slot not in use in map
         // (usually seen at deleted entities)
    10,  // LIGHT (colored: 11, shadow: 12)  // lightsource, attr1 = radius,
         // attr2 = intensity (or attr2..4 = r-g-b)
    ...
    30,   // MAPMODEL (clipped: 31)           // attr1 = angle, attr2 = idx,
          // attr3 = elevation, attr4 = texture, attr5 = pitch, attr6 = roll
    21,   // CARROT                           // attr1 = tag, attr2 = type
    3,    // LADDER                           // attr1 = height
    1,    // CTF_FLAG                         // attr1 = angle, attr2 = red/blue
    20,   // SOUND                            // attr1 = idx, attr2 = radius,
          // attr3 = size, attr4 = volume
    32,   // CLIP                             // attr1 = elevation, attr2 =
          // xradius, attr3 = yradius, attr4 = height, attr6 = slope, attr7 =
          // shape
    33};  // PLCLIP                           // attr1 = elevation, attr2 =
          // xradius, attr3 = yradius, attr4 = height, attr6 = slope, attr7 =
          // shape
...
int cmp_npe(const numbered_persistent_entity *a,
            const numbered_persistent_entity *b) {
  ASSERT(a->n != b->n);
  return a->n - b->n;
}
...
void save_world(char *mname, bool skipoptimise, bool addcomfort) {
  ...
  if (!validmapname(mname)) {
    conoutf(
        "\f3Invalid map name. It must only contain letters, digits, '-', '_' "
        "and be less than %d characters long",
        MAXMAPNAMELEN);
    return;
  }
  if (!skipoptimise) {  // optimisation works on the real world geometry, not on
                        // a copy
    voptimize();
    mapmrproper(false);
  ...
  if (autoembedconfig) {  // write embedded map config
    backup(mcfname,
           cbakname);  // rename possibly existing external map config file
  ...
  strncpy(hdr.head, "ACMP", 4);  // ensure map now declares itself as an
  ...
  loopv(ents) if (ents[i].type != NOTUSED &&
                  ents[i].type !=
                      DUMMYENT)  // omit deleted entities and dummies
  {
    hdr.numents++;
    npe.e = ents[i];
    ...
    if (t == LIGHT) {
      ...
      if (mmi && mmi->h)
        nt++;  // sort clipped mapmodels right after unclipped mapmodels, but
               // before clips and plclips
    }
    npe.n =
        i | (nt << 16);  // numbers have to be unique for the sort to be stable
    sortedents.add(npe);
  }
  sortedents.sort(cmp_npe);
  if (hdr.numents > MAXENTITIES) {
    conoutf("too many map entities (%d), only %d will be written to file",
  ...
  header tmp = hdr;
  ucharbuf hx = packheaderextras(
      addcomfort ? 0 : (1 << HX_EDITUNDO));  // if addcomfort -> add undos/redos
  int writeextra = 0;
  if (hx.maxlen)
    tmp.headersize += writeextra = clamp(hx.maxlen, 0, MAXHEADEREXTRA);
  tmp.maprevision += advancemaprevision;
  DEBUG("version " << tmp.version << " headersize " << tmp.headersize
                   << " entities " << tmp.numents << " factor " << tmp.sfactor
                   << " revision " << tmp.maprevision);
  ...
  rlencodecubes(rawcubes, world, cubicsize,
                skipoptimise);  // if skipoptimize -> keep properties of solid
                                // cubes (forces format 10)
  f->write(rawcubes.getbuf(), rawcubes.length());
  delete f;
  unsavededits = 0;
  conoutf("wrote map file %s %s%s%s", cgzname,
          skipoptimise ? "without optimisation" : "(optimised)",
          addcomfort ? " (including editing history)" : "",
          autoembedconfig ? " (including map config)" : "");
}
VARP(preserveundosonsave, 0, 0, 1);
COMMANDF(savemap, "s", (char *name) {
  save_world(name, preserveundosonsave && editmode,
             preserveundosonsave && editmode);
});
COMMANDF(savemapoptimised, "s",
         (char *name) { save_world(name, false, false); });

void save_world9(char *mname) {
  if (unsavededits) {
    conoutf(
        "\f3There are unsaved changes to the map. Please save them to a "
        "lossless format first (\"savemap\").");
    return;
  }
  if (!*mname) {
  ...
  if (findheaderextra(HX_CONFIG) >= 0) {  // extract embedded config file first
    string clientmapbak;
...
void showmapdims() {
  conoutf("  min X|Y|Z: %3d : %3d : %3d", clmapdims.x1, clmapdims.y1,
          clmapdims.minfloor);
  conoutf("  max X|Y|Z: %3d : %3d : %3d", clmapdims.x2, clmapdims.y2,
          clmapdims.maxceil);
  conoutf("delta X|Y|Z: %3d : %3d : %3d", clmapdims.xspan, clmapdims.yspan,
...
COMMANDF(getmaptimestamp, "s", (char *fmt) {
  result(hdr.timestamp ? timestring((time_t)hdr.timestamp, *fmt != 'U',
                                    *fmt == 'U' ? fmt + 1 : fmt)
                       : "no timestamp stored");
});
...
VARF(mapoverride_nowaterreflect, 0, 0, 1,
     changemapflag(mapoverride_nowaterreflect, MHF_DISABLEWATERREFLECT));
VARF(mapoverride_limitwaveheight, 0, 0, 1,
     changemapflag(mapoverride_limitwaveheight, MHF_LIMITWATERWAVEHEIGHT));
VARF(mapoverride_nostencilshadows, 0, 0, 1,
     changemapflag(mapoverride_nostencilshadows, MHF_DISABLESTENCILSHADOWS));
...
int load_world(char *mname)  // still supports all map formats that have existed
                             // since the earliest cube betas!
{
  ...
  if (tmp.sfactor < SMALLEST_FACTOR || tmp.sfactor > LARGEST_FACTOR ||
      tmp.numents > MAXENTITIES) {
  ...
  DEBUG("version " << tmp.version << " headersize " << tmp.headersize
                   << " headerextrasize "
                   << tmp.headersize - int(sizeof(header)) << " entities "
                   << tmp.numents << " factor " << tmp.sfactor << " revision "
                   << tmp.maprevision);
  clearheaderextras();
  ...
  mapoverride_nostencilshadows =
      (hdr.flags & MHF_DISABLESTENCILSHADOWS) ? 1 : 0;
...
#define SCALEATTR(x)                                                         \
  ...
  if (ov != e.attr##x) {                                                     \
    conoutf(                                                                 \
        "overflow during conversion of attr%d of entity #%d (%s): value %d " \
        "can no longer be represented - pls fix manually before saving the " \
        "map",                                                               \
        x, i, entnames[e.type], oe.attr##x);                                 \
    e.attr##x = oe.attr##x;                                                  \
    res |= LWW_ENTATTROVERFLOW;                                              \
    defformatstring(desc)(                                                   \
        "%s: attr%d was %d, value can no longer be represented",             \
        entnames[e.type], x, oe.attr##x);                                    \
...
#undef SCALEATTR
  ...
  if (!mapinfo.numelems ||
      (mapinfo.access(mname) && !cmpf(cgzname, mapinfo[mname])))
  ...
  res |= rldecodecubes(uf, world, cubicsize, hdr.version, false)
             ? 0
             : LWW_DECODEERR;  // decode file
  c2skeepalive();
  ...
  c2skeepalive();
  calclight();
  conoutf("read map %s rev %d (%d milliseconds)", cgzname, hdr.maprevision,
          watch.elapsed());
  conoutf("%s", hdr.maptitle);
  mapconfigerror = 0;
  pushscontext(IEXC_MAPCFG);  // untrusted altogether
  if (hdr.flags &
      MHF_AUTOMAPCONFIG) {  // full featured embedded config: no need to read
                            // any other map config files
    copystring(lastloadedconfigfile, "");
  } else {  // map config from files
  ...
  if (downloaded > 0)
    clientlogf("downloaded content (%d KB in %d seconds)", downloaded / 1024,
               watch.elapsed() / 1000);
  ...
  watch.start();
  loopi(256) if (texuse[i] && lookupworldtexture(i, false) == notexture &&
                 (!_ignoreillegalpaths || gettextureslot(i))) res |=
      LWW_MISSINGMEDIA * 2;
  clientlogf("loaded textures (%d milliseconds)",
             texloadtime + watch.elapsed());
  c2skeepalive();
  watch.start();
  res |= preload_mapmodels(false) ? 0 : LWW_MISSINGMEDIA * 4;
  clientlogf("loaded mapmodels (%d milliseconds)",
             mdlloadtime + watch.elapsed());
  c2skeepalive();
  watch.start();
  res |= audiomgr.preloadmapsounds(false)
             ? 0
             : LWW_MISSINGMEDIA *
                   8;  // also set, if soundvol == 0, because no sounds are
                       // loaded (or checked) in that case
  clientlogf("loaded mapsounds (%d milliseconds)",
             audioloadtime + watch.elapsed());
  c2skeepalive();

  // determine index of first clipped entity
  clentstats.firstclip =
      max(0, ents.length() - 1);  // fallback for "no clipped entities at all"
  loopv(ents) {
    entity &e = ents[i];
    mapmodelinfo *mmi;
    if (e.type == CLIP || e.type == PLCLIP ||
        (e.type == MAPMODEL && (mmi = getmminfo(e.attr2)) && mmi->h)) {
  ...
  if (res)
    conoutf(
        "\f3unresolved problems occurred during load_world(), warning: 0x%x",
        res);
  return res;  // negative: error (no map loaded), zero: no problem, positive:
               // some problems (bitmask value)
}
                  ...
                  const char *ext)  // extract demo and map descriptions
{
  ...
  if (!strcmp(ext, "dmo")) {
    ...
    formatstring(text)("%s%s",
                       (hdr.version > MAPVERSION) ? "(incompatible file) " : "",
                       hdr.maptitle);
    text[DHDR_DESCCHARS - 1] = '\0';
    return newstring(text);
  }
...
void hexbinwrite(stream *f, void *data, int len,
                 bool ascii = true)  // write block of binary data as hex values
...
COMMANDF(hexbinchunk, "v",
         (char **args, int numargs)  // read up to 24 bytes from the command's
                                     // arguments to the hexbin buffer
         {
           if (!hexbinenabled) {
...
//
// the general assumption is, that ram is cheap, and that a copy of a size-9 map
// should be below 10MB (including undo data)

#define XMAPVERSION 1001001

VARP(persistentxmaps, 0, 1,
     1);  // save all xmaps on exit, restore them at game start
...
struct xmap {
  ...
  xmap(const char *nnick)  // take the current map and store it in an xmap
  {
    ...
    copystring(mcfname,
               lastloadedconfigfile);  // may have been "official" or not
    if (hdr.flags & MHF_AUTOMAPCONFIG) getcurrentmapconfig(mapconfig, false);
    storeposition(position);
    vector<uchar>
        tmp;  // add undo/redo data in compressed form as temporary headerextra
  ...
  void restore()  // overwrite the current map with the contents of an xmap
  {
    ...
    parseheaderextra(true, mapconfig.length()
                               ? (1 << HX_CONFIG)
                               : 0);  // don't execute headerextra config, if we
                                      // already executed an autostored config
    popscontext();
    loadskymap(true);
    startmap(name, false,
             true);  // "start" but don't respawn: basically just set clientmap
  ...
  void write(stream *f)  // write xmap as cubescript to a file
  {
    f->printf("restorexmap version %d %d %d\n", XMAPVERSION,
              isbigendian() ? 1 : 0,
              int(sizeof(world)));  // it is a non-portable binary file format,
                                    // so we have to check...
    ...
    uLongf
        worldsize = cubicsize * sizeof(sqr),
        gzbufsize =
            (worldsize * 11) /
    ...
    loopv(headerextras) {
      ...
      f->printf("restorexmap headerextra %d  // %s\n", headerextras[i]->flags,
                hx_name(headerextras[i]->flags));
    }
    ...
    loopi(all)  // entities are stored as plain text - you may edit them
    {
      ...
      f->printf(
          "restorexmap %sent %d  %d %d %d  %d %d %d %d %d %d %d // %s\n",
          i >= el ? "del" : "", e.type, e.x, e.y, e.z, e.attr1, e.attr2,
          e.attr3, e.attr4, e.attr5, e.attr6, e.attr7,
          e.type >= 0 && e.type < MAXENTTYPES ? entnames[e.type] : "unknown");
    }
    if (mapconfig.length()) {
    ...
    loopv(todoents) f->printf("restorexmap todoent %d %s\n", todoents[i],
                              escapestring(todoentdescs[i]));
    f->printf(
        "restorexmap position %d %d %d %d %d  // EOF, don't touch this\n\n",
...
bool validxmapname(const char *nick) {
  if (validmapname(nick)) return true;
...
void xmapbackup(
    ...
    const char *nick)  // throw away existing backup and make a new one
{
  xmapdelete(bak);
  ...
  if (unsavededits)
    conoutf(
        "\f3stored backup of unsaved edits on map '%s'; to restore, type "
        "\"/xmap_restore\"",
        bak->name);
}
...
const char *xmapdescstring(xmap *xm, bool shortform = false) {
  ...
  static int toggle;
  toggle = !toggle;
  formatstring(s[toggle])(
      "\"%s\": %s rev %d, %d ents%c size %d, hdrs %d, %d undo steps", xm->nick,
      xm->name, xm->hdr.maprevision, xm->ents.length(), shortform ? '\0' : ',',
      xm->hdr.sfactor, xm->headerextras.length() - 1, xm->numundo);
  return s[toggle];
}
...
COMMANDF(getxmaplist, "s",
         (char *what)  // return list of xmap nicks and descriptions
         {
           vector<char> res;
           if (strcasecmp(what, "bak"))
             loopv(xmaps)
                 cvecprintf(res, "%s %s\n", escapestring(xmaps[i]->nick),
                            escapestring(xmapdescstring(xmaps[i])));
           else if (bak)
             cvecprintf(res, "%s %s", escapestring(bak->nick),
                        escapestring(xmapdescstring(bak)));
           resultcharvector(res, -1);
         });

COMMANDF(xmap_store, "s",
         (const char *nick)  // store current map in an xmap buffer
         {
...
COMMANDF(xmap_delete, "s",
         (const char *nick)  // delete xmap buffer
         {
           if (!*nick) return;
           int i;
...
COMMANDF(xmap_delete_backup, "", () {
  if (bak) {
    xmapdelete(bak);
    conoutf("deleted backup xmap");
  }
});

COMMANDF(xmap_keep_backup, "s",
         (const char *nick)  // move backup xmap to more permanent position
         {
           if (!bak) {
           ...
           loopi(42) if (getxmapbynick(bak->nick, NULL, false))
               concatstring(bak->nick, "_");  // evade existing nicknames
           xmaps.add(bak);
           bak = NULL;
           conoutf("stored backup xmap \"%s\" as xmap %s", bakdesc,
                   xmapdescstring(xmaps.last()));
         });

COMMANDF(xmap_rename, "ss",
         (const char *oldnick, const char *newnick)  // rename xmap buffer
         {
           if (!*oldnick || !*newnick) return;
           xmap *xm = getxmapbynick(oldnick);
           if (!xm || !validxmapname(newnick) ||
               getxmapbynick(newnick, NULL, false))
             return;
           copystring(xm->nick, newnick);
         });

COMMANDF(xmap_restore, "s",
         (const char *nick)  // use xmap as current map
         {
           EDITMP("xmap_restore");
           if (*nick) {
             ...
             xmapbackup(
                 "restore_xmap_",
                 nick);  // keep backup, so we can restore from the restore :)
             xm->restore();
             unsavededits = 0;
...
void restorexmap(char **args,
                 int numargs)  // read an xmap from a cubescript file
{
  const char *cmdnames[] = {"version", "names",       "sizes",    "header",
                            "world",   "headerextra", "ent",      "delent",
                            "config",  "todoent",     "position", ""};
  ...
  switch (cmd) {
    case 0:  // version
      if (ATOI(args[1]) != XMAPVERSION ||
          ATOI(args[2]) != (isbigendian() ? 1 : 0) ||
          ATOI(args[3]) != int(sizeof(world))) {
    ...
    case 4:  // world
    {
      ...
      if (uncompress((uchar *)xmjigsaw->world, &rawsize, hexbin.getbuf(),
                     hexbin.length()) != Z_OK ||
          rawsize - rawworldsize != 0)
        abort = true;
      break;
    }
    case 5:  // headerextra
      xmjigsaw->headerextras.add(
          new headerextra(hexbin.length(), ATOI(args[1]), hexbin.getbuf()));
      break;
    case 6:  // ent
    case 7:  // delent
    {
      persistent_entity &e =
          cmd == 6 ? xmjigsaw->ents.add() : xmjigsaw->delents.add();
      int a[11];
    ...
    case 9:  // todoent
      ...
      break;
    case 10:  // position - this is also the last command and will finish the
              // xmap
    {
...
void writeallxmaps()  // at game exit, write all xmaps to cubescript files in
                      // mapediting/xmaps
{
...
int loadallxmaps()  // at game start, load all xmaps from mapediting/xmaps
{
  ...
  loopv(xmapnames) {
    ...
    backup(filename, xmapfilename(xmapnames[i],
                                  "backups/"));  // move to backup folder - we
                                                 // will write new files at exit
    delete[] xmapnames[i];
  }
