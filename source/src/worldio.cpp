...
void rlencodecubes (vector<uchar> &f,
                    sqr *s,
                    int len,
...
#define c(f) (s->f == t->f)
    while (len-- > 0)
    {
        ...
        {
            if (t && c (type) && c (floor) && c (ceil) && c (ctex) && c (ftex) && c (utex)
                && c (wtex) && c (vdelta) && c (tag))
            {
                sc++;
            }
            else
...
const char *hx_names[]
= { "unused", "mapinfo", "modeinfo", "artist", "editundo", "config", "vantage point", "unknown" };
...
{
    loopv (headerextras) conoutf ("extra header record %d: %s, %d bytes", i,
...
void unpackheaderextra (uchar *buf,
                        int len) // break the extra data from the mapheader into its pieces
{
    ...
    while (true)
    {
        ...
        DEBUGCODE (clientlogf (" found headerextra \"%s\", %d bytes%s", hx_name (flags & HX_TYPEMASK),
                               len, flags & HX_FLAG_PERSIST ? ", persistent" : "")); // debug info
        headerextras.add (new headerextra (len, flags, p.subbuf (len).buf));
    }
}

void parseheaderextra (bool clearnonpersist = true,
                                            ...
                                            // nonpersistent ones (like editundos)
{
    ...
    loopv (headerextras)
    {
        ...
        bool deletethis = false; // (set deletethis for headers that persist outside headerextras
...
bool embedconfigfile ()
{
    ...
    if (unsavededits)
    {
        conoutf ("There are unsaved edits: they need to be saved or discarded before a "
    ...
    if (buf)
    {
        ...
        headerextras.add (new headerextra (len + 1, HX_CONFIG | HX_FLAG_PERSIST, nullptr))->data = buf;
...
void extractconfigfile ()
{
    ...
    if (unsavededits)
    {
        conoutf ("There are unsaved edits: they need to be saved or discarded before "
...
{ // if changes are tracked, because automapcfg is
    ...
    if (!onlysounds)
    {
        ...
        loopi (256)
        {
            ...
            cvecprintf (f, "mapmodel %d %d %d %s \"%s\"", mmi->rad, mmi->h, mmi->zoff,
                        mmi->scale == 1.0f ? "0" : floatstr (mmi->scale, true), mmshortname (mmi->name));
...
COMMANDF (clearvantagepoint, "", () {
    if (!noteditmode ("clearvantagepoint") && !multiplayer () && clearvantagepoint ())
...
void setvantagepoint ()
{
    ...
    if (!OUTBORD (p[0] / DMF, p[1] / DMF) && iabs (p[2]) < 127 * int(DMF) && p[3] >= 0 && p[3] < 360
        && iabs (p[4]) <= 90)
    {
...
bool getvantagepoint (short p[5])
{
    ...
    if (n >= 0)
    {
        ...
        if (!OUTBORD (p[0] / DMF, p[1] / DMF) && iabs (p[2]) < 127 * int(DMF) && p[3] >= 0
            && p[3] < 360 && iabs (p[4]) <= 90)
...
COMMANDF (getvantagepoint, "", () {
    ...
    if (getvantagepoint (p))
        formatstring (res) ("%s %s %s %d %d", floatstr (p[0] / DMF), floatstr (p[1] / DMF),
                            floatstr (p[2] / DMF), p[3], p[4]);
    result (res);
});
...
COMMANDF (gotovantagepoint, "", () {
    intret ((editmode || (!multiplayer (NULL) && player1->isspectating ())) && gotovantagepoint () ? 1 : 0);
});
...
void save_world (char *mname, bool skipoptimise, bool addcomfort)
{
    ...
    {
        conoutf ("\f3Invalid map name. It must only contain letters, digits, '-', '_' "
                 "and be less than %d characters long",
                 MAXMAPNAMELEN);
        return;
    }
    if (!skipoptimise)
    { // optimisation works on the real world geometry, not on
      // a copy
        voptimize ();
        mapmrproper (false);
    ...
    { // write embedded map config
        ...
        headerextras.add (new headerextra (ec.length (), HX_CONFIG | HX_FLAG_PERSIST, nullptr))->data = ecu;
    }
    ...
    DEBUG ("version " << tmp.version << " headersize " << tmp.headersize << " entities "
                      << tmp.numents << " factor " << tmp.sfactor << " revision " << tmp.maprevision);
    ...
    delete f;
    unsavededits = 0;
    conoutf ("wrote map file %s %s%s%s", cgzname, skipoptimise ? "without optimisation" : "(optimised)",
...
COMMANDF (savemap, "s", (char *name) {
    save_world (name, preserveundosonsave && editmode, preserveundosonsave && editmode);
});
...
void save_world9 (char *mname)
{
    if (unsavededits)
    {
        conoutf ("\f3There are unsaved changes to the map. Please save them to a "
...
COMMANDF (getmaptimestamp, "s", (char *fmt) {
    result (hdr.timestamp ? timestring ((time_t)hdr.timestamp, *fmt != 'U', *fmt == 'U' ? fmt + 1 : fmt) :
                            "no timestamp stored");
});
                             ...
                             // since the earliest cube betas!
{
    ...
    DEBUG ("version " << tmp.version << " headersize " << tmp.headersize << " headerextrasize "
                      << tmp.headersize - int(sizeof (header)) << " entities " << tmp.numents
                      << " factor " << tmp.sfactor << " revision " << tmp.maprevision);
    clearheaderextras ();
    if (tmp.headersize > sizeof_header)
    {
        ...
        if (extrasize)
        { // map file actually has extra header data that we want
          // too preserve
...
#define SCALEATTR(x)                                                                     \
    ...
    if (ov != e.attr##x)                                                                 \
    {                                                                                    \
        conoutf ("overflow during conversion of attr%d of entity #%d (%s): value %d "    \
                 "can no longer be represented - pls fix manually before saving the "    \
                 "map",                                                                  \
                 x, i, entnames[e.type], oe.attr##x);                                    \
        e.attr##x = oe.attr##x;                                                          \
        res |= LWW_ENTATTROVERFLOW;                                                      \
        defformatstring (desc) ("%s: attr%d was %d, value can no longer be represented", \
...
#undef SCALEATTR
    ...
    if (!mapinfo.numelems || (mapinfo.access (mname) && !cmpf (cgzname, mapinfo[mname])))
    ...
    { // full featured embedded config: no need to read
    ...
    res |= audiomgr.preloadmapsounds (false) ?
           0 :
           LWW_MISSINGMEDIA * 8; // also set, if soundvol == 0, because no sounds are
...
char *getfiledesc (const char *dir,
                   const char *name,
...
struct xmap
{
    ...
    void restore () // overwrite the current map with the contents of an xmap
    {
        ...
        parseheaderextra (true, mapconfig.length () ? (1 << HX_CONFIG) :
    ...
    void write (stream *f) // write xmap as cubescript to a file
    {
        ...
        uLongf worldsize = cubicsize * sizeof (sqr),
               gzbufsize = (worldsize * 11) / 10; // gzip the world, so the file will only be big instead of huge
        auto *gzbuf = new uchar[gzbufsize];
        ...
        loopi (all) // entities are stored as plain text - you may edit them
        {
            ...
            f->printf ("restorexmap %sent %d  %d %d %d  %d %d %d %d %d %d %d // %s\n", i >= el ? "del" : "",
                       e.type, e.x, e.y, e.z, e.attr1, e.attr2, e.attr3, e.attr4, e.attr5, e.attr6,
                       e.attr7, e.type >= 0 && e.type < MAXENTTYPES ? entnames[e.type] : "unknown");
        }
        if (mapconfig.length ())
        {
...
void xmapbackup (const char *nickprefix,
                 const char *nick) // throw away existing backup and make a new one
{
    ...
    bak = new xmap (text);
    if (unsavededits)
        conoutf ("\f3stored backup of unsaved edits on map '%s'; to restore, type "
                 "\"/xmap_restore\"",
                 bak->name);
}
...
const char *xmapdescstring (xmap *xm, bool shortform = false)
{
    ...
    static int toggle;
    toggle = !toggle;
    formatstring (s[toggle]) ("\"%s\": %s rev %d, %d ents%c size %d, hdrs %d, %d undo steps", xm->nick,
...
{
    const char *cmdnames[] = { "version", "names",  "sizes",  "header",  "world",    "headerextra",
                               "ent",     "delent", "config", "todoent", "position", "" };
    ...
    {
    case 0: // version
        if (ATOI (args[1]) != XMAPVERSION || ATOI (args[2]) != (isbigendian () ? 1 : 0)
    ...
    case 4: // world
    {
        ...
        if (uncompress ((uchar *)xmjigsaw->world, &rawsize, hexbin.getbuf (), hexbin.length ()) != Z_OK
