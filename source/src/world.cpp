...
void remip (const block &b, int level)
{
    ...
    for (int y = s.y; y < s.y + s.ys; y += 2)
    {
        ...
        {
            *r = *o[0];
            r->type = o[0]->type == o[1]->type && o[0]->type == o[2]->type && o[0]->type == o[3]->type ?
                      o[0]->type :
                      SEMISOLID;
            if (!SOLID (r))
            {
                ...
                loopi (4) if (!SOLID (o[i]))
                {
                    ...
                    {
                        if (o[i]->type == FHF)
                            fh -= o[i]->vdelta / 4 + 2; // crap hack, needed for rendering large mips next to hfs
                        if (o[i]->type == CHF)
                            ch += o[i]->vdelta / 4 + 2; // FIXME: needs to somehow take into account
                                                        // middle vertices on higher mips
                    }
                    if (fh < floor)
...
COMMANDF (toggleclosestentpin, "", () {
    pinnedclosestent = ents.inrange ((pinnedent = pinnedclosestent ? -1 : closestent ()));
});
...
int closestent () // used for delent and edit mode ent display
{
    ...
    int best = -1,
        enttypemask = clenttype == NOTUSED ? ((~edithideentmask) << 1) :
    ...
    if (pinnedclosestent)
    {
        if (ents.inrange (pinnedent) && (enttypemask & (1 << ents[pinnedent].type)))
            ...
            pinnedclosestent = false; // release lock when ent is deleted or no longer selectable
    }
    if (best < 0)
    ...
    if (best >= 0)
    { // if closest ent shares location with other ents, pick one
      // with clentsel
        vector<int> bs;
        loopv (ents)
        {
...
void undelent (char *index)
{
    ...
    if (deleted_ents.inrange (n))
    {
        ...
        if (OUTBORD (e.x, e.y))
            conoutf ("failed to undelete %s entity (coordinates outside map borders)", entnames[t]);
        else
        {
            newentity (-1, e.x, e.y, e.z, entnames[t], float(e.attr1) / entscale[t][0],
                       float(e.attr2) / entscale[t][1], float(e.attr3) / entscale[t][2],
                       float(e.attr4) / entscale[t][3]);
            *((persistent_entity *)&ents.last ()) = e;
            if (changedents.find (ents.length () - 1) == -1)
                changedents.add (ents.length () - 1); // tag ent for sync because of attr5..7
                ...
                float v4f) // add an entity or overwrite an existing one
{
    ...
    switch (type)
    { // MAPMODEL, PLAYERSTART and CTF-FLAG use the current camera
    ...
    int v1 = v1f * entscale[type][0], v2 = v2f * entscale[type][1], v3 = v3f * entscale[type][2],
        v4 = v4f * entscale[type][3];
    ...
    clampentityattributes (e);
    syncentchanges (true);
    addmsg (SV_EDITENT, "ri9i3", index < 0 ? ents.length () : index, type, e.x, e.y, e.z, e.attr1,
                       ...
                       int z2) // check for solid world border
{
    loop (x, ssize) loop (y, ssize)
    {
        ...
        {
            if (S (x, y)->type != SOLID && (S (x, y)->ceil > 126 - z1 || S (x, y)->floor < -127 + z2))
                return false;
        }
        else
        {
                              ...
                              // enlarge old world by 1, factor -2 will shrink old world by 1
{
    ...
    if (factor == -2
        && !worldbordercheck (ssize / 4 + MINBORD, ssize / 4 + MINBORD, ssize / 4 + MINBORD,
                              ssize / 4 + MINBORD, 0, 0))
    {
...
COMMANDF (calcmipstats, "", () {
    ...
    conoutf ("current mips: %d / %d / %d / %d / %d / %d / %d", st[0], st[1], st[2], st[3], st[4],
             st[5], st[6]);
});
    ...
    for (int mip = SMALLEST_FACTOR; mip > 0; mip--)
    {
        ...
        for (int y = 1; y < bs - 1; y++)
            for (int x = 1; x < bs - 1; x++)
            {
                ...
                if (s->type == CORNER
                    && (mip == SMALLEST_FACTOR || SWS (wmip[mip + 1], x / 2, y / 2, mfactor - 1)->type != CORNER))
                { // highest corner mip (this one is rendered) - mark
                  // helper cubes
    ...
    for (int mip = SMALLEST_FACTOR; mip > 0; mip--)
    {
        ...
        for (int y = 1; y < bs - 1; y++)
            for (int x = 1; x < bs - 1; x++)
            {
                ...
                if (s->type == CORNER
                    && (mip == SMALLEST_FACTOR || SWS (wmip[mip + 1], x / 2, y / 2, mfactor - 1)->type != CORNER))
                { // highest mip - mark the important cube like the
                  // highest mip
...
#undef cc
    ...
    auto *worldbackup = (sqr *)memcpy (new sqr[ssize * ssize], world, ssize * ssize * sizeof (sqr));
    r = world;
    ...
    for (int mip = 2; mip > 0; mip--)
    {
        ...
        for (int y = 0; y < bs; y++)
            for (int x = 0; x < bs; x++)
            {
                ...
                if (wdiff == 1)
                    domip0 (if (rs & INVISWTEX) s->wtex = wtex; rs &= ~(INVISWTEX | INVISIBLE););
...
#define c(f) (s->f == r->f)
    loopi (cubicsize)
    {
        ...
        {
            if (r && c (type) && c (floor) && c (ceil) && c (ctex) && c (ftex) && c (vdelta) && c (tag)
                && ((s->visible & INVISWTEX) || c (wtex)) && ((s->visible & INVISUTEX) || c (utex)))
            {
                s->wtex = r->wtex;
                s->utex = r->utex;
            }
...
#undef c
    ...
    if (manual)
    {
        ...
        conoutf ("before: %d / %d / %d / %d / %d / %d / %d", sta[0], sta[1], sta[2], sta[3], sta[4],
                 sta[5], sta[6]);
        conoutf ("idealized: %d / %d / %d / %d / %d / %d / %d", stb[0], stb[1], stb[2], stb[3],
                 stb[4], stb[5], stb[6]);
        conoutf ("final count: %d / %d / %d / %d / %d / %d / %d", stc[0], stc[1], stc[2], stc[3],
                 stc[4], stc[5], stc[6]);
        unsavededits++;
    }
}
