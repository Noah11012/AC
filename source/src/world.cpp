...
void remip(const block &b, int level) {
  ...
  for (int y = s.y; y < s.y + s.ys; y += 2) {
    ...
    sqr *r = SWS(v, s.x / 2, y / 2,
                 vfactor);  // the target cube in the higher mip level

    for (int x = s.x; x < s.x + s.xs; x += 2) {
      *r = *o[0];
      r->type = o[0]->type == o[1]->type && o[0]->type == o[2]->type &&
                        o[0]->type == o[3]->type
                    ? o[0]->type
                    : SEMISOLID;
      if (!SOLID(r)) {
        ...
        loopi(4) if (!SOLID(o[i])) {
          ...
          if (r->type == SEMISOLID) {
            if (o[i]->type == FHF)
              fh -=
                  o[i]->vdelta / 4 +
                  2;  // crap hack, needed for rendering large mips next to hfs
            if (o[i]->type == CHF)
              ch += o[i]->vdelta / 4 +
                    2;  // FIXME: needs to somehow take into account middle
                        // vertices on higher mips
          }
          if (fh < floor)
            floor = fh;  // take lowest floor and highest ceil, so we never have
                         // to see missing lower/upper from the side
          if (ch > ceil) ceil = ch;
        }
        r->floor = floor;
        r->ceil = ceil;
      ...
      if (r->type == CORNER)
        goto mip;  // special case: don't ever split even if textures etc are
                   // different
      r->defer = 1;
      if (level) {
        loopi(4) if (o[i]->defer) goto c;
      }  // if any of the constituents is not perfect, then this one isn't
         // either
      if (SOLID(r)) {
        loopi(3) {
          if (o[i]->wtex != o[3]->wtex)
            goto c;  // on an all solid cube, only thing that needs to be equal
                     // for a perfect mip is the wall texture
        }
      } else {
        loopi(3) {
          sqr *q = o[i + 1];
          if (*((uint32_t *)q) !=
                  *((uint32_t *)o[0])  // type, floor, ceil, wtex
              || iabs(q->b - o[0]->b) > lighterr || q->ftex != o[0]->ftex ||
              q->ctex != o[0]->ctex ||
              iabs(q->r - o[0]->r) >
                  lighterr  // perfect mip even if light is not exactly equal
              || iabs(q->g - o[0]->g) > lighterr || q->utex != o[0]->utex)
            goto c;
        }
        if (r->type == CHF || r->type == FHF)  // can make a perfect mip out of
                                               // a hf if slopes lie on one line
        {
          if (o[0]->vdelta - o[1]->vdelta !=
                  o[1]->vdelta - SWS(w, x + 2, y, wfactor)->vdelta ||
              o[0]->vdelta - o[2]->vdelta !=
                  o[2]->vdelta - SWS(w, x + 2, y + 2, wfactor)->vdelta ||
              o[0]->vdelta - o[3]->vdelta !=
                  o[3]->vdelta - SWS(w, x, y + 2, wfactor)->vdelta ||
              o[3]->vdelta - o[2]->vdelta !=
                  o[2]->vdelta - SWS(w, x + 2, y + 1, wfactor)->vdelta ||
              o[1]->vdelta - o[2]->vdelta !=
                  o[2]->vdelta - SWS(w, x + 1, y + 2, wfactor)->vdelta)
            goto c;
        }
      }
    mip:
...
COMMANDF(toggleclosestentpin, "", () {
  pinnedclosestent =
      ents.inrange((pinnedent = pinnedclosestent ? -1 : closestent()));
});
...
bool intersectangular(const vec &from, vec ray, vec ent, float maxangle,
                      float &dist) {
  ray.sub(from);
  ent.sub(from);
...
int closestent()  // used for delent and edit mode ent display
{
  ...
  int best = -1,
      enttypemask = clenttype == NOTUSED
                        ? ((~edithideentmask) << 1)
                        : (1 << clenttype);  // bitmask of selectable ent types
  float bdist = 99999, dist,
        maxangle = pointingatents ? cosf(pointatentmaxangle * RAD) : -SQRT3;
  if (pinnedclosestent) {
    if (ents.inrange(pinnedent) && (enttypemask & (1 << ents[pinnedent].type)))
      best = pinnedent;
    else
      pinnedclosestent =
          false;  // release lock when ent is deleted or no longer selectable
  }
  if (best < 0) {  // find entity of closest distance in desired solid angle
    loopv(ents) {
      entity &e = ents[i];
      if (!(enttypemask & (1 << e.type))) continue;
      if (intersectangular(camera1->o, worldpos, vec(e.x, e.y, e.z), maxangle,
                           dist) &&
          dist < bdist) {
        best = i;
        bdist = dist;
  ...
  if (best >= 0) {  // if closest ent shares location with other ents, pick one
                    // with clentsel
    vector<int> bs;
    loopv(ents) {
      entity &e = ents[i];
...
void entproperty(int *prop, float *famount, int *unscaled) {
  ...
  *prop %= 100;
  ...
  amount *=
      int(*famount *
          (*prop >= 0 && *prop < 7 && !*unscaled ? entscale[t][*prop] : 1));
  ...
  if (changedents.find(n) == -1)
    changedents.add(n);  // apply ent changes later (reduces network traffic)
  unsavededits++;
}
...
#define CHKATTR(x)                                  \
  case x - 1:                                       \
    res = floatstr(float(e.attr##x) / scale, true); \
    break
      CHKATTR(1);
      CHKATTR(2);
      CHKATTR(3);
...
void undelent(char *index) {
  ...
  if (deleted_ents.inrange(n)) {
    ...
    if (OUTBORD(e.x, e.y))
      conoutf("failed to undelete %s entity (coordinates outside map borders)",
              entnames[t]);
    else {
      newentity(-1, e.x, e.y, e.z, entnames[t], float(e.attr1) / entscale[t][0],
                float(e.attr2) / entscale[t][1],
                float(e.attr3) / entscale[t][2],
                float(e.attr4) / entscale[t][3]);
      *((persistent_entity *)&ents.last()) = e;
      if (changedents.find(ents.length() - 1) == -1)
        changedents.add(ents.length() -
                        1);  // tag ent for sync because of attr5..7
...
void getdeletedentities() {
  ...
  loopv(deleted_ents) {
    ...
    int t = e.type < MAXENTTYPES ? e.type : NOTUSED;
    cvecprintf(res, "%s %d %d %d  %s\n", entnames[t], e.x, e.y, e.z,
               formatentityattributes(e));
  }
...
void newentity(int index, int x, int y, int z, const char *what, float v1f,
               float v2f, float v3f,
               float v4f)  // add an entity or overwrite an existing one
{
  int type = findtype(what);
  if (type == NOTUSED) return;
  if (index >= 0 && ents[index].type == SOUND)
    deletesoundentity(ents[index]);  // overwriting sound entity
  switch (type) {  // MAPMODEL, PLAYERSTART and CTF-FLAG use the current camera
                   // direction as value for attr1, so attr234 need to be moved
    case MAPMODEL:
      v4f = v3f;
      v3f = v2f;
    case PLAYERSTART:
  ...
  int v1 = v1f * entscale[type][0], v2 = v2f * entscale[type][1],
      v3 = v3f * entscale[type][2], v4 = v4f * entscale[type][3];
  ...
  clampentityattributes(e);
  syncentchanges(true);
  addmsg(SV_EDITENT, "ri9i3", index < 0 ? ents.length() : index, type, e.x, e.y,
         e.z, e.attr1, e.attr2, e.attr3, e.attr4, e.attr5, e.attr6, e.attr7);
  e.spawned = true;
  int oldtype = type;
  if (index < 0)
    ents.add(e);
  ...
  if (index >= 0 || type != DUMMYENT)
    unsavededits++;  // no need to save dummies
}
...
void addentity(char *what)  // type
{
  ...
  EDITMP("addentity");
  if (type == NOTUSED) return;
  entity e((int)camera1->o.x, (int)camera1->o.y, (int)camera1->o.z, type, 0, 0,
           0, 0);
  e.spawned = true;
  intret(ents.length());
  ents.add(e);
...
void editentity(char **args, int numargs)  // index x y z a1 a2 a3 a4 ...
{
  ...
  if (numargs > 0) {
    ...
    for (int i = 1; i < numargs; i++)
      if (*args[i])
        edit = true;  // only arguments other than empty strings can edit
                      // anything - otherwise we're just browsing
    int t = e.type < MAXENTTYPES ? e.type : NOTUSED;
    if ((edit) && !multiplayer("editentity")) {
      if (e.type == SOUND) {  // disable sound /before/ changing it
        entityreference entref(&e);
        location *loc = audiomgr.locations.find(e.attr1, &entref, mapsounds);
        if (loc) loc->drop();
      }
    ...
    formatstring(res)("%s %d %d %d  %s", entnames[t], e.x, e.y, e.z,
                      formatentityattributes(e));
  }
  result(res);
}
...
void cleartodoentities(const char *n) {
  ...
  if (!n || !*n || ni < 0) {
    ...
    loopvrev(todoents) if (todoents[i] == ni) {
      todoents.remove(i);
...
COMMANDF(addtodoentity, "is", (int *n, const char *desc) {
  if (ents.inrange(*n)) addtodoentity(*n, desc);
});

void enumtodoentities() {
  vector<char> res;
  loopv(todoents) cvecprintf(res, "%d %s\n", todoents[i],
                             escapestring(todoentdescs[i], true));
  resultcharvector(res, -1);
}
...
int findentity(int type, int index, uchar attr2) {
  for (int i = index; i < ents.length(); i++)
    if (ents[i].type == type && ents[i].attr2 == attr2) return i;
...
bool worldbordercheck(int x1, int x2, int y1, int y2, int z1,
                      int z2)  // check for solid world border
{
  loop(x, ssize) loop(y, ssize) {
    if (x >= x1 && x < ssize - x2 && y >= y1 && y < ssize - y2) {
      if (S(x, y)->type != SOLID &&
          (S(x, y)->ceil > 126 - z1 || S(x, y)->floor < -127 + z2))
        return false;
    } else {
...
bool empty_world(
    int factor,
    bool force)  // main empty world creation routine, if passed factor -1 will
                 // enlarge old world by 1, factor -2 will shrink old world by 1
{
  if (!force && noteditmode("empty world")) return false;
  if (factor < -2) return false;
  if (factor == -1 && multiplayer() && sfactor > 9) return false;
  if (factor == -2 &&
      !worldbordercheck(ssize / 4 + MINBORD, ssize / 4 + MINBORD,
                        ssize / 4 + MINBORD, ssize / 4 + MINBORD, 0, 0)) {
  ...
  setupworld(factor);
  loop(x, ssize) loop(y, ssize) { sqrdefault(S(x, y)); }
  ...
  if (!copy) {
    ...
    conoutf("new map size: %d", sfactor);
  freeblock(ow);
  return true;
}
...
void mapenlarge() {
  if (empty_world(-1, false)) addmsg(SV_NEWMAP, "ri", -1);
}
void mapshrink() {
  if (empty_world(-2, false)) addmsg(SV_NEWMAP, "ri", -2);
}
void newmap(int *i) {
  if (m_botmode) {
    conoutf("newmap not supported in bot mode");
    return;
  }
...
void countperfectmips(int mip, int xx, int yy, int bs, int *stats) {
  ...
  if (!bs) bs = 1 << mfactor;
  for (int y = yy; y < yy + bs; y++)
    for (int x = xx; x < xx + bs; x++) {
      sqr *r = SWS(wmip[mip], x, y, mfactor);
      if (r->defer && mip > 0)
        countperfectmips(mip - 1, x * 2, y * 2, 2, stats);
      else
        stats[mip]++;
    }
}
...
COMMANDF(calcmipstats, "", () {
  ...
  conoutf("current mips: %d / %d / %d / %d / %d / %d / %d", st[0], st[1], st[2],
          st[3], st[4], st[5], st[6]);
});
...
void calcworldvisibility() {
  ...
  r = world;
  loopirev(ssize * ssize)(r++)->visible = INVISUTEX | INVISWTEX;
  ...
  for (int y = 0; y < ssize; y++)
    for (int x = 0; x < ssize; x++) {
      s = S(x, y);
      uchar &rs = s->visible;
      if (x < 1 || x > ssize - 2 || y < 1 || y > ssize - 2)
        continue;  // we need 1 cube safety-space - all outside is invisible
                   // anyway
      o[0] = s - ssize;
      o[1] = s + ssize;
      o[2] = s + 1;
      o[3] = s - 1;
      loopi(4) if (!SOLID(o[i])) {
        if (SOLID(s) || s->floor > o[i]->floor ||
            (o[i]->type == FHF && (s->type != FHF || s->floor != o[i]->floor)))
          rs &= ~INVISWTEX;  // wall texture visible
        if (!SOLID(s) &&
            (s->ceil < o[i]->ceil ||
             (o[i]->type == CHF && (s->type != CHF || s->ceil != o[i]->ceil))))
          rs &= ~INVISUTEX;  // upper wall texture visible
      }
      if (SOLID(s) && SOLID(o[0]) && SOLID(o[1]) && SOLID(o[2]) && SOLID(o[3]))
        rs |= INVISIBLE;
    }

  // corners are rendered at the highest possible mip - make sure, the right
  // helper cubes are marked visible (including actually hidden ones) (this is a
  // lot of effort, but there's no easier way... savemap's toptimize() does it
  // wrong, for example
  for (int mip = SMALLEST_FACTOR; mip > 0; mip--) {
    ...
    for (int y = 1; y < bs - 1; y++)
      for (int x = 1; x < bs - 1; x++) {
        ...
        if (s->type == CORNER &&
            (mip == SMALLEST_FACTOR ||
             SWS(wmip[mip + 1], x / 2, y / 2, mfactor - 1)->type !=
                 CORNER)) {  // highest corner mip (this one is rendered) - mark
                             // helper cubes
          ...
          if (SOLID(z) || SOLID(t)) {
            if (SOLID(w))
              S(x << mip, (y - 1) << mip)->visible &= ~(INVISWTEX | INVISIBLE);
            else if (SOLID(v))
              S(x << mip, (y + 1) << mip)->visible &= ~(INVISWTEX | INVISIBLE);
          }
        }
  ...
  // corners are rendered at the highest possible mip - make sure, the right
  // cubes of the lot is marked visible (including actually hidden ones)
  for (int mip = SMALLEST_FACTOR; mip > 0; mip--) {
    int mfactor = sfactor - mip, bs = 1 << mfactor;
    for (int y = 1; y < bs - 1; y++)
      for (int x = 1; x < bs - 1; x++) {
        ...
        if (s->type == CORNER &&
            (mip == SMALLEST_FACTOR ||
             SWS(wmip[mip + 1], x / 2, y / 2, mfactor - 1)->type !=
                 CORNER)) {  // highest mip - mark the important cube like the
                             // highest mip
...
void mapmrproper(bool manual) {
  int sta[SMALLEST_FACTOR + 1] = {0}, stb[SMALLEST_FACTOR + 1] = {0},
                            stc[SMALLEST_FACTOR + 1] = {0};
  sqr *s, *r;
  ...
  // corners are rendered at the highest possible mip - squash data of lower
  // mips (except vdelta and tag)
  for (int mip = SMALLEST_FACTOR; mip > 0; mip--) {
    int mfactor = sfactor - mip, bs = 1 << mfactor;
    for (int y = 1; y < bs - 1; y++)
      for (int x = 1; x < bs - 1; x++) {
...
#define domip0(b)                                        \
  for (int yy = y << mip; yy < (y + 1) << mip; yy++)     \
    for (int xx = x << mip; xx < (x + 1) << mip; xx++) { \
      s = S(xx, yy);                                     \
      uchar &rs = s->visible;                            \
      b;                                                 \
    }
  for (int mip = 2; mip > 0; mip--) {
    ...
    for (int y = 0; y < bs; y++)
      for (int x = 0; x < bs; x++) {
        ...
        domip0(if (!(rs & INVISWTEX) && s->wtex != wtex) {
          wtex = s->wtex;
          wdiff++;
        ...
        if (wdiff == 1)
          domip0(if (rs & INVISWTEX) s->wtex = wtex;
                 rs &= ~(INVISWTEX | INVISIBLE););
        if (udiff == 1)
          domip0(if (rs & INVISUTEX) s->utex = utex; rs &= ~INVISUTEX;);
      }
  }
...
#define c(f) (s->f == r->f)
  loopi(cubicsize) {
    if (SOLID(s)) {
      ...
      if (r && c(type) && c(floor) && c(ceil) && c(ctex) && c(ftex) &&
          c(vdelta) && c(tag) && ((s->visible & INVISWTEX) || c(wtex)) &&
          ((s->visible & INVISUTEX) || c(utex))) {
        s->wtex = r->wtex;
        s->utex = r->utex;
      } else
        r = s;
...
#undef c
  ...
  if (manual) {
    ...
    conoutf("before: %d / %d / %d / %d / %d / %d / %d", sta[0], sta[1], sta[2],
            sta[3], sta[4], sta[5], sta[6]);
    conoutf("idealized: %d / %d / %d / %d / %d / %d / %d", stb[0], stb[1],
            stb[2], stb[3], stb[4], stb[5], stb[6]);
    conoutf("final count: %d / %d / %d / %d / %d / %d / %d", stc[0], stc[1],
            stc[2], stc[3], stc[4], stc[5], stc[6]);
    unsavededits++;
  }
}
...
void mapareacheck(
    char *what)  // "vdelta" | "steepest" | "total" | "pprest" | "pp"(default)
{
  mapareastats_s ms;
  mapdim_s md;
  stopwatch ti;
  ...
  } else if (!strcasecmp(what, "steepest"))
    cvecprintf(res, "%d.5 %d.5 %d ", ms.steepest % ssize, ms.steepest / ssize,
               world[ms.steepest % cubicsize].floor);
  else if (!strcasecmp(what, "total"))
    cvecprintf(res, "%d ", ms.total);
  else if (!strcasecmp(what, "pprest"))
    cvecprintf(res, "%d ", ms.rest);
  else  // default
  {
    loopi(MAS_GRID2) {
      int x = ms.ppp[i] % ssize, y = ms.ppp[i] / ssize,
          z = OUTBORD(x, y) ? 0 : S(x, y)->floor;
      cvecprintf(res, "%d %d %d  %d %d %.5g\n", x, y, z, ms.ppa[i], ms.ppv[i],
                 ms.ppa[i] ? float(ms.ppv[i]) / ms.ppa[i] : 0.0f);
    }
  }
  resultcharvector(res, -1);
}
