...
{
    const char *mdlname
    = entmdlnames[isitem (e.type) && !(m_lss && e.type == I_GRENADE) ? e.type - I_CLIPS + 1 : 0]; // render double nades in lss
    ...
    rendermodel (mdlname, ANIM_MAPMODEL | ANIM_LOOP | ANIM_DYNALLOC, 0, 0,
                 vec (e.x, e.y, z + S (e.x, e.y)->floor + float(e.attr1) / ENTSCALE10), 0, yaw, 0);
}
...
void renderclip (int type, int x, int y, float xs, float ys, float h, float elev, float tilt, int shape)
{
    ...
    if (h < 0.1f) h = 0.1f;
    vec bbmin (x - xs, y - ys, S (x, y)->floor + elev), bbmax (x + xs, y + ys, bbmin.z + h);
    vec bb[4];
    loopi (4) bb[i] = bbmin;
...
void rendermapmodels ()
{
    loopv (ents)
    {
        ...
        if (e.type == MAPMODEL)
        {
            ...
            if (!mmi) continue;
            rendermodel (mmi->name, ANIM_MAPMODEL | ANIM_LOOP, e.attr4, 0,
                         vec (e.x, e.y, S (e.x, e.y)->floor + mmi->zoff + float(e.attr3) / ENTSCALE5),
                         e.attr6, float(e.attr1) / ENTSCALE10, float(e.attr5) / ENTSCALE10, 10.0f,
                         0, nullptr, nullptr, mmi->scale);
        }
    }
}
...
void renderentarrow (const entity &e, const vec &dir, float radius)
{
    ...
    spoke.normalize ();
    spoke.mul (arrowsize);
    glDisable (GL_TEXTURE_2D); // this disables reaction to light, but also emphasizes
    ...
    loopv (ents)
    {
        ...
        else if (editmode)
        {
            if (e.type == CTF_FLAG)
            {
                ...
                rendermodel (path, ANIM_FLAG | ANIM_LOOP, 0, 0, vec (e.x, e.y, (float)S (e.x, e.y)->floor),
            ...
            else if ((e.type == CLIP || e.type == PLCLIP) && showclips && !stenciling)
            {
                renderclip (e.type, e.x, e.y, float(e.attr2) / ENTSCALE5,
                            ...
                            float(e.attr1) / ENTSCALE10, float(e.attr6) / (4 * ENTSCALE10), e.attr7);
            }
            ...
            else if (e.type == PLAYERSTART && showplayerstarts)
            {
                ...
                rendermodel ("playermodels", ANIM_IDLE, -(int)textureload (skin)->id, 1.5f, o, 0,
                             e.attr1 / ENTSCALE10 + 90, 0);
            }
        }
    ...
    if (m_flags && !editmode) loopi (2)
        {
            ...
            case CTFF_STOLEN:
                if (f.actor && f.actor != player1)
                {
                    ...
                    defformatstring (path) ("pickups/flags/small_%s%s", m_ktf ? "" : team_basestring (i),
                                            m_htf ? "_htf" : m_ktf ? "ktf" : "");
                    rendermodel (path, ANIM_FLAG | ANIM_START | ANIM_DYNALLOC, 0, 0,
                                 vec (f.actor->o).add (vec (0, 0, 0.3f + (sinf (lastmillis / 100.0f) + 1) / 10)),
            ...
            case CTFF_DROPPED:
            {
                ...
                entity &e = *f.flagent;
                defformatstring (path) ("pickups/flags/%s%s", m_ktf ? "" : team_basestring (i),
                                        m_htf ? "_htf" : m_ktf ? "ktf" : "");
                if (f.flagent->spawned)
                    rendermodel (path, ANIM_FLAG | ANIM_LOOP, 0, 0,
                                 vec (f.pos.x, f.pos.y,
                                      f.state == CTFF_INBASE ? (float)S (int(f.pos.x), int(f.pos.y))->floor :
                                                               f.pos.z),
                                 0, float(e.attr1) / ENTSCALE10, 0, 120.0f);
                break;
            }
        ...
        if (m_ctf)
        {
            if (own) // it's the own flag
            {
                if (f.state == CTFF_DROPPED)
                    flagreturn (flag);
                else if (f.state == CTFF_INBASE && of.state == CTFF_STOLEN && of.actor == d && of.ack)
                    flagscore (of.team);
            }
            else
                flagpickup (flag);
...
void entstats_ ()
{
    ...
    {
    case MAPMODEL:
        conoutf (" %d %s, %d clipped, %s%d unconfigured", es.entcnt[i], entnames[i], clipents,
                 xmodels ? "\f3" : "", xmodels);
        break;
    case SOUND:
        conoutf (" %d %s, %s%d unconfigured", es.entcnt[i], entnames[i], xsounds ? "\f3" : "", xsounds);
        break;
    case PLAYERSTART:
        conoutf (" %d %s, %d CLA, %d RVSF, %d FFA%c \f3unknown %d", es.entcnt[i], entnames[i],
                 es.spawns[0], es.spawns[1], es.spawns[2], es.unknownspawns ? ',' : '\0', es.unknownspawns);
        break;
    case CTF_FLAG:
        conoutf (" %d %s, %d CLA, %d RVSF%c \f3unknown %d", es.entcnt[i], entnames[i], es.flags[0],
                 es.flags[1], es.unknownflags ? ',' : '\0', es.unknownflags);
        break;
    case CLIP:
    case PLCLIP:
    ...
    conoutf (" map capabilities: has ffa spawns %d, has team spawns %d, has flags %d",
...
void syncentchanges (bool force)
{
    ...
    loopv (changedents) if (ents.inrange (changedents[i]))
    {
        ...
        addmsg (SV_EDITENT, "ri9i3", changedents[i], e.type, e.x, e.y, e.z, e.attr1, e.attr2,
...
#define AA(x) floatstr (float(e.attr##x) / entscale[t][x - 1], true)
    formatstring (res) ("%s%s %s%s %s%s %s%s %s%s %s%s %s", AA (1), c, AA (2), c, AA (3), c, AA (4),
                        c, AA (5), c, AA (6), c, AA (7));
#undef AA
    return res;
}
