...
{
    return (cl->spj > 50 || cl->ping > 500 || cl->ldt > 80); // do not change this except if you
...
// FIXME
/* There are smarter ways to implement this function, but most probably they
    ...
    for (int y = (inversed_y ? ls - 1 : 0); (inversed_y ? y >= 0 : y < ls); (inversed_y ? y-- : y++))
    {
        ...
        { // if the line has the minimun threshold of emptiness
            ...
            { // if the last line was saved
                if (2 * oxi + MINELINE < 2 * xf && 2 * xi + MINELINE < 2 * oxf)
...
const char *medal_messages[] = { "defended the flag", "covered the flag stealer", "defended the dropped flag",
                                 "covered the flag keeper", "covered teammate" };
enum
{
    CTFLDEF,
    CTFLCOV,
...
#define BONUSPT \
    target->state.points / 400               // bonus (for killing high level enemies :: beware
                                             // with exponential behavior!)
#define FLBONUSPT target->state.points / 300 // bonus if flag team mode
#define TMBONUSPT \
    target->state.points / 200       // bonus if team mode (to give some extra reward
                                     // for playing tdm modes)
...
void computeteamwork (int team, int exclude) // testing
{
    loopv (clients)
    {
        ...
        if (i == exclude || actor->type == ST_EMPTY || actor->team != team
            || actor->state.state != CS_ALIVE || actor->md.linkmillis < gamemillis)
            continue;
        vec position;
...
float a2c = 0, c2t = 0, a2t = 0; // distances: actor to covered, covered to
                                 // target and actor to target
...
{
    return actor->md.linked >= 0 && actor->md.linked == cn && gamemillis < actor->md.linkmillis
           && valid_client (actor->md.linked);
}
...
/* this function is managed to the PUBS, id est, many people playing in an open
 * server */
void check_afk ()
{
    next_afk_check = servmillis + 7 * 1000;
    /* if we have few people (like 2x2), or it is not a teammode with the server
     * not full: do nothing! */
    ...
    loopv (clients)
    {
        ...
        if ((c.state.state == CS_DEAD && !m_arena && c.state.lastdeath + 45 * 1000 < gamemillis)
            || (c.state.state == CS_ALIVE && c.upspawnp) /*||
             ...
             only kick spectator if server is full - 2011oct16:flowtron: mmh, that
             seems reasonable enough .. still, kicking spectators for inactivity seems
             harsh! disabled ATM, kick them manually if you must.
             */
        )
        {
...
/** This function counts how much non-killing-damage the player does to any
   teammates The damage limit is 100 hp per minute, which is about 2 tks per
   minute in a normal game In normal games, the players go over 6 tks only in
   the worst cases */
...
/**
If you read README.txt you must know that AC does not have cheat protection
implemented. However this file is the sketch to a very special kind of cheat
detection tools in server side.

This is not based in program tricks, i.e., encryption, secret bytes, nor
monitoring/scanning tools.

The idea behind these cheat detections is to check (or reproduce) the client
data, and verify if this data is expected or possible. Also, there is no need to
check all clients all time, and one coding this kind of check must pay a special
attention to the lag effect and how it can affect the data observed. This is not
a trivial task, and probably it is the main reason why such tools were never
implemented.
