...
void audiomanager::initsound() {
  ...
  if (device) {
    ...
    if (context) {
      ...
      // backend infos
      conoutf("Sound: %s / %s (%s)", alcGetString(device, ALC_DEVICE_SPECIFIER),
...
void audiomanager::musicpreload(int id) {
  ...
  if (musicvol && (id >= M_FLAGGRAB && id <= M_LASTMINUTE2) &&
    ...
    setmusicvol(musicvol);  // call "musicpreload -1" to ensure musicdonecmd
                            // runs - but it should w/o that
}

void audiomanager::musicsuggest(
    int id, int millis,
    bool rndofs)  // play bg music if nothing else is playing
{
  ...
  if (gamemusic->open(name)) {
    ...
    if (rndofs)
      gamemusic->seek(millis ? (double)rnd(millis) / 2.0f : (double)lastmillis);
...
int audiomanager::findsound(const char *name, int vol,
                            vector<soundconfig> &sounds) {
  loopv(sounds) {
    if (!strcmp(sounds[i].buf->name, name) && (!vol || sounds[i].vol == vol))
      return i;
  }
  return -1;
}

int audiomanager::addsound(const char *name, int vol, int maxuses, bool loop,
                           vector<soundconfig> &sounds, bool load,
                           int audibleradius) {
  if (nosound) return -1;
...
void audiomanager::applymapsoundchanges()  // during map editing, drop all
                                           // mapsounds so they can be re-added
{
  loopv(locations) {
...
void audiomanager::updateplayerfootsteps(playerent *p) {
  ...
  bool inrange =
      footsteps && (local || (camera1->o.dist(p->o) < footstepradius));
  ...
  if (!footsteps || (local && !localfootsteps) || !inrange ||
    ...
    loopi(sizeof(locs) / sizeof(locs[0])) {
      location *l = locs[i];
      if (!l) continue;
      if (l->playmillis + minplaytime > totalmillis)
        continue;  // tolerate short interruptions by enforcing a minimal
                   // playtime
      l->drop();
    }
  } else {
    ...
    float grounddist = 0;
    if (!((int(p->o.x) | int(p->o.y)) & ~(ssize - 1)))
      grounddist = waterlevel - S((int)p->o.x, (int)p->o.y)->floor;
    bool water = p->o.z - p->eyeheight + 0.25f < waterlevel;
    if (water && grounddist > p->eyeheight)
      return;  // don't play step sound when jumping into water
...
void soundtest() { loopi(S_NULL) audiomgr.playsound(i, rnd(SP_HIGH + 1)); }
...
soundconfig::soundconfig(sbuffer *b, int vol, int maxuses, bool loop,
  ...
  this->model = audibleradius > 0
                    ? DM_LINEAR
                    : DM_DEFAULT;  // use linear model when radius is configured
  uses = 0;
  muted = false;
}

void soundconfig::onattach() { uses++; }

void soundconfig::ondetach() { uses--; }
...
void audiomanager::updateaudio() {
  ...
  // water
  bool underwater =
      /*alive &&*/ firstperson && waterlevel > player1->o.z + player1->aboveeye;
...
const vec &camerareference::currentposition() const { return camera1->o; }

bool camerareference::nodistance() { return true; }
...
const vec &physentreference::currentposition() const { return phys->o; }

bool physentreference::nodistance() { return phys == camera1; }
...
const vec &staticreference::currentposition() const { return pos; }

bool staticreference::nodistance() { return false; }
...
const char *soundprioritynames[] = {"LOW", "NORMAL", "HIGH", "HIGHEST",
                                    ""};  // keep in sync with enum { SP_LOW =
                                          // 0, SP_NORMAL, SP_HIGH, SP_HIGHEST }

COMMANDF(sound, "is", (int *n, char *priorityname) {
  int priority =
      getlistindex(priorityname, soundprioritynames, true, SP_NORMAL);
  audiomgr.playsound(*n, priority);
});

COMMANDF(applymapsoundchanges, "", () {
  if (m_coop || !multiplayer("applymapsoundchanges"))
...
COMMANDF(registersound, "siii",
         (char *name, int *vol, int *loop, int *audibleradius) {
           intret(audiomgr.addsound(name, *vol, -1, *loop != 0, gamesounds,
                                    true, *audibleradius));
         });
...
void registerdefaultsounds() {
  loopi(S_NULL) {
    ...
    audiomgr.addsound(soundcfg[i].name, soundcfg[i].vol, -1,
                      soundcfg[i].loop != 0, gamesounds, true,
                      soundcfg[i].audibleradius);
  }
}

const char *soundcategories[SC_NUM + 1] = {
    "PAIN",     "OWNPAIN", "WEAPON", "PICKUP", "MOVEMENT", "BULLET", "OTHER",
    "VOICECOM", "TEAM",    "PUBLIC", "FFA",    "FLAGONLY", ""};
...
void enumsounds(char *what) {
  ...
  loopv(w) {
    if (w[i][0] == '!')
      nflags |= 1 << getlistindex(w[i] + 1, soundcategories, false, SC_NUM);
    else
      flags |= 1 << getlistindex(w[i], soundcategories, false, SC_NUM);
    delstring(w[i]);
  }
  vector<char> res;
  loopi(S_NULL) if ((soundcfg[i].flags & flags) &&
                    !(soundcfg[i].flags & nflags))
      cvecprintf(res, "%d %s\n", i, escapestring(soundcfg[i].desc));
  resultcharvector(res, -1);
}
COMMAND(enumsounds, "s");

const char *mapsoundbasepath = "packages/audio/",
           *mapsoundfinalpath = "ambience/";
const int mapsoundbasepath_n = strlen(mapsoundbasepath),
          mapsoundfinalpath_n = strlen(mapsoundfinalpath);

COMMANDF(mapsound, "si", (char *name, int *maxuses) {
  filtertext(name, name, FTXT__MEDIAFILEPATH);
  defformatstring(stripped)("%s%s", mapsoundbasepath,
  ...
  if (mapconfigdata.mapsoundlines.length() > 254) {
    ...
    if (!strncmp(name, mapsoundfinalpath, mapsoundfinalpath_n))
      name += mapsoundfinalpath_n;  // base path for mapsoundlines is
                                    // "packages/audio/ambience"
...
void mapsoundslotusage(
    int *n)  // returns all entity indices that use a mapsound of slot n
{
  string res = "";
  loopv(ents) if (ents[i].type == SOUND && ents[i].attr1 == *n)
...
void deletemapsoundslot(
    int *n,
    char *opt)  // delete mapsound slot - only if unused or "purge" is specified
{
  ...
  if (!purgeall && slotused) {
    conoutf("mapsound slot #%d is in use: can't delete", *n);
    return;
  }
  audiomgr.mapsoundreset();
  int deld = 0;
  loopv(ents) if (ents[i].type == SOUND) {
    ...
    if (e.attr1 == *n) {  // delete entity
      ...
      e.attr1--;  // adjust models in higher slots
  }
...
void editmapsoundslot(int *n, char *name,
                      char *maxuses)  // edit slot parameters != ""
{
  string res = "";
      ...
      if (*name)
        copystring(msl.name,
                   strncmp(name, mapsoundfinalpath, mapsoundfinalpath_n)
                       ? name
                       : name + mapsoundfinalpath_n);
      reloadmapsoundconfig();
      mapsoundchanged = 1;
      unsavededits++;
      hdr.flags |= MHF_AUTOMAPCONFIG;  // requires automapcfg
    }
    formatstring(res)("\"%s\" %d", msl.name,
                      msl.maxuses);  // give back all current attributes
  }
  result(res);
}
...
void mapsoundslotbyname(
    char *name)  // returns the slot(s) that a certain mapsound file is used in
{
  string res = "";
  loopv(mapconfigdata
            .mapsoundlines) if (!strcmp(name,
                                        mapconfigdata.mapsoundlines[i].name))
...
void sortmapsoundslots(char **args, int numargs) {
  ...
  loopi(numargs) if (args[i][0]) {
    if (!strcasecmp(args[i], "nomerge"))
      ...
      conoutf("sortmapsoundslots: unknown argument \"%s\"", args[i]);
      unknownarg = true;
    }
  }
  ...
  loopv(ents) if (ents[i].type == SOUND && tempslots.inrange(ents[i].attr1))
      tempslots[ents[i].attr1]
          .used = true;
  ...
  loopv(ents) if (ents[i].type == SOUND) ents[i].attr1 =
      newslot[ents[i].attr1 & 255];

  conoutf("%d mapsound slots%s, %d %sslots merged", tempslots.length(),
          nosort ? "" : " sorted",
          mapconfigdata.mapsoundlines.length() - tempslots.length(),
          mergeused ? "" : "unused ");

  // rewrite mapmodel slot list
  mapconfigdata.mapsoundlines.shrink(tempslots.length());
  loopv(tempslots) { mapconfigdata.mapsoundlines[i] = tempslots[i].m; }
  reloadmapsoundconfig();
  unsavededits++;
  mapsoundchanged = 1;
  hdr.flags |= MHF_AUTOMAPCONFIG;  // requires automapcfg
