...
void audiomanager::musicsuggest (int id,
                                 int millis,
...
void audiomanager::updateplayerfootsteps (playerent *p)
{
    ...
    if (!footsteps || (local && !localfootsteps) || !inrange || p->state != CS_ALIVE
        || lastmillis - p->lastpain < 300 || (!p->onfloor && p->timeinair > 50)
        || (!p->move && !p->strafe) || p->inwater)
    {
...
void audiomanager::unmuteallsounds () { loopv (gamesounds) gamesounds[i].muted = false; }
...
worldobjreference *camerareference::clone () const { return new camerareference (*this); }

const vec &camerareference::currentposition () const { return camera1->o; }

bool camerareference::nodistance () { return true; }

bool camerareference::operator== (const worldobjreference &other) { return type == other.type; }
...
worldobjreference *physentreference::clone () const { return new physentreference (*this); }
...
worldobjreference *entityreference::clone () const { return new entityreference (*this); }
...
worldobjreference *staticreference::clone () const { return new staticreference (*this); }
...
const char *soundprioritynames[]
= { "LOW", "NORMAL", "HIGH", "HIGHEST", "" }; // keep in sync with enum { SP_LOW =
...
void registerdefaultsounds ()
{
    loopi (S_NULL)
    {
        ...
        audiomgr.addsound (soundcfg[i].name, soundcfg[i].vol, -1, soundcfg[i].loop != 0, gamesounds,
                           true, soundcfg[i].audibleradius);
    }
}
...
void enumsounds (char *what)
{
    ...
    vector<char> res;
    loopi (S_NULL) if ((soundcfg[i].flags & flags) && !(soundcfg[i].flags & nflags))
...
COMMANDF (music, "sis", (char *name, int *millis, char *cmd) { audiomgr.music (name, *millis, cmd); });
                       ...
                       char *maxuses) // edit slot parameters != ""
{
    ...
    if (mapconfigdata.mapsoundlines.inrange (*n))
    {
        ...
        { // change attributes
            ...
            if (*name)
                copystring (msl.name, strncmp (name, mapsoundfinalpath, mapsoundfinalpath_n) ?
                                      name :
...
{
    string res = "";
    loopv (mapconfigdata.mapsoundlines) if (!strcmp (name, mapconfigdata.mapsoundlines[i].name))
...
int tempmsunsort (tempmsslot *a, tempmsslot *b) { return a->oldslot[0] - b->oldslot[0]; }
...
void sortmapsoundslots (char **args, int numargs)
{
    ...
    loopv (ents) if (ents[i].type == SOUND && tempslots.inrange (ents[i].attr1)) tempslots[ents[i].attr1]
    .used
    = true;
    ...
    conoutf ("%d mapsound slots%s, %d %sslots merged", tempslots.length (), nosort ? "" : " sorted",
             mapconfigdata.mapsoundlines.length () - tempslots.length (), mergeused ? "" : "unused ");
