...
bool multiplayer (const char *op)
{
    ...
    if (curpeer && op)
        conoutf ("%s%s%s not available in multiplayer", *op ? "\"" : "", *op ? op : "operation",
                 *op ? "\"" : "");
...
void connectserv_ (const char *servername, int serverport = 0, const char *password = nullptr, int role = CR_DEFAULT)
{
...
void whereami () { conoutf ("you are at (%.2f,%.2f)", player1->o.x, player1->o.y); }
...
void _toserver (char *text, int msg, int msgt)
{
    ...
    if (servstate.mastermode == MM_MATCH && servstate.matchteamsize && !team_isactive (player1->team)
    ...
    if (*text)
    {
        if (msg == SV_TEXTME)
            ...
            conoutf ("%s:\f%d %s", colorname (player1), toteam ? 1 : 0, highlight (text));
...
void c2sinfo (playerent *d) // send update to the server
{
    ...
    if (d->state == CS_ALIVE || d->state == CS_EDITING)
    {
        ...
        y = (int)(d->o.y * DMF), z = (int)((d->o.z - d->eyeheight) * DMF),
            ya = (int)((512 * d->yaw) / 360.0f), pi = (int)((127 * d->pitch) / 90.0f),
            r = (int)(31 * d->roll / 20), dxt = (int)(d->vel.x * DVELF),
            dyt = (int)(d->vel.y * DVELF), dzt = (int)(d->vel.z * DVELF), dx = dxt - d->vel_t.i[0],
        ...
        f = (d->strafe & 3) | ((d->move & 3) << 2) | (((int)d->onfloor) << 4)
            | (((int)d->onladder) << 5) | ((d->lifesequence & 1) << 6) | (((int)d->crouching) << 7),
            g = (dx ? 1 : 0) | ((dy ? 1 : 0) << 1) | ((dz ? 1 : 0) << 2) | ((r ? 1 : 0) << 3)
                | (((int)d->scoping) << 4) | (((int)d->shoot) << 5);
        d->vel_t.i[0] = dxt;
        d->vel_t.i[1] = dyt;
        d->vel_t.i[2] = dzt;
        int usefactor = sfactor < 7 ? 7 : sfactor, sizexy = 1 << (usefactor + 4);
        if (cn >= 0 && cn < 32 && usefactor <= 7 + 3 && // map size 7..10
            x >= 0 && x < sizexy && y >= 0 && y < sizexy && z >= -2047 && z <= 2047 && ya >= 0
            && ya < 512 && pi >= -128 && pi <= 127 && r >= -32 && r <= 31 && dx >= -8 && dx <= 7 && // FIXME
            dy >= -8 && dy <= 7 && dz >= -8 && dz <= 7)
        { // compact POS packet
    ...
    while (clienthost != nullptr && enet_host_service (clienthost, &event, 0) > 0)
        ...
        case ENET_EVENT_TYPE_RECEIVE:
        {
            ...
            if (discmillis)
                ...
                servertoclient (event.channelID, event.packet->data, (int)event.packet->dataLength);
            // destroyed in logger
        ...
        case ENET_EVENT_TYPE_DISCONNECT:
        {
            ...
            else
            {
                if (!discmillis || event.data)
                    conoutf ("\f3server network error, disconnecting (%s) ...", disc_reason (event.data));
                disconnect ();
            }
            return;
        }
...
bool securemapcheck (const char *map, bool msg)
{
    ...
    loopv (securemaps) if (!strcmp (securemaps[i], map))
    {
        if (msg)
        {
            conoutf ("\f3Map \f4%s\f3 is secured. This means you CAN'T send, receive or "
                     "overwrite it.",
                     map);
            if (connected)
            {
                conoutf ("\f3If you get this error often, you (or the server) may be "
                         "running an outdated game.");
                conoutf ("\f3You can check for updates at "
