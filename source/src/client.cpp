...
void throttle() {
  ...
  enet_peer_throttle_configure(curpeer, throttle_interval * 1000,
...
void connectserv_(const char *servername, int serverport = 0,
  ...
  if (servername) {
    ...
    conoutf("\f2attempting to %sconnect to \f5%s\f4:%d\f2",
            role == CR_DEFAULT ? "" : "\f8admin\f2", servername, serverport);
...
void _toserver(char *text, int msg, int msgt) {
  bool toteam =
      text && text[0] == '%' && (m_teammode || team_isspect(player1->team));
  if (!toteam && text[0] == '%' && strlen(text) > 1)
    text++;  // convert team-text to normal-text if no team-mode is active
  if (toteam) text++;
  filtertext(text, text, FTXT__CHAT);
  trimtrailingwhitespace(text);
  if (servstate.mastermode == MM_MATCH && servstate.matchteamsize &&
      !team_isactive(player1->team) &&
      !(player1->team == TEAM_SPECT && player1->clientrole == CR_ADMIN))
    toteam = true;  // spect chat
  if (*text) {
    if (msg == SV_TEXTME)
      conoutf("\f%d%s %s", toteam ? 1 : 0, colorname(player1), highlight(text));
    else
      conoutf("%s:\f%d %s", colorname(player1), toteam ? 1 : 0,
              highlight(text));
    addmsg(toteam ? msgt : msg, "rs", text);
  }
}

void toserver(char *text) { _toserver(text, SV_TEXT, SV_TEAMTEXT); }

void toserverme(char *text) { _toserver(text, SV_TEXTME, SV_TEAMTEXTME); }
...
void current_version(char *text) {
  ...
  if (version && AC_VERSION < version) {
    ...
    conoutf(
        "\f3YOU MUST UPDATE ASSAULTCUBE\nplease visit "
        "\f2http://assault.cubers.net \f3for more information");
  }
}
...
void c2sinfo(playerent *d)  // send update to the server
{
  ...
  if (d->state == CS_ALIVE || d->state == CS_EDITING) {
    packetbuf q(100);
    int cn = d->clientnum,
        x = (int)(d->o.x * DMF),  // quantize coordinates to 1/16th of a cube,
                                  // between 1 and 3 bytes
        y = (int)(d->o.y * DMF), z = (int)((d->o.z - d->eyeheight) * DMF),
        ya = (int)((512 * d->yaw) / 360.0f),
        pi = (int)((127 * d->pitch) / 90.0f), r = (int)(31 * d->roll / 20),
        dxt = (int)(d->vel.x * DVELF), dyt = (int)(d->vel.y * DVELF),
        dzt = (int)(d->vel.z * DVELF), dx = dxt - d->vel_t.i[0],
        dy = dyt - d->vel_t.i[1], dz = dzt - d->vel_t.i[2],
        // pack rest in 1 int: strafe:2, move:2, onfloor:1, onladder: 1
        f = (d->strafe & 3) | ((d->move & 3) << 2) | (((int)d->onfloor) << 4) |
            (((int)d->onladder) << 5) | ((d->lifesequence & 1) << 6) |
            (((int)d->crouching) << 7),
        g = (dx ? 1 : 0) | ((dy ? 1 : 0) << 1) | ((dz ? 1 : 0) << 2) |
            ((r ? 1 : 0) << 3) | (((int)d->scoping) << 4) |
            (((int)d->shoot) << 5);
    d->vel_t.i[0] = dxt;
    d->vel_t.i[1] = dyt;
    d->vel_t.i[2] = dzt;
    int usefactor = sfactor < 7 ? 7 : sfactor, sizexy = 1 << (usefactor + 4);
    if (cn >= 0 && cn < 32 && usefactor <= 7 + 3 &&  // map size 7..10
        x >= 0 && x < sizexy && y >= 0 && y < sizexy && z >= -2047 &&
        z <= 2047 && ya >= 0 && ya < 512 && pi >= -128 && pi <= 127 &&
        r >= -32 && r <= 31 && dx >= -8 && dx <= 7 &&  // FIXME
        dy >= -8 && dy <= 7 && dz >= -8 && dz <= 7) {  // compact POS packet
      ...
      b.putbits(1, z < 0 ? 1 : 0);
      if (z < 0)
        z = -z;  // z is encoded with 3..10 bits minimum (fitted to byte
                 // boundaries), or full 11 bits if necessary
  ...
  if (sendmapidenttoserver || messages.length() ||
      totalmillis - lastping > 250) {
    ...
    while (i <
           messages
               .length())  // send messages collected during the previous frames
    {
...
int getbuildtype() {
  return (isbigendian() ? 0x80 : 0) |
         (adler((unsigned char *)guns, sizeof(guns)) % 31 << 8) |
#ifdef WIN32
         0x40 |
#endif
#ifdef __APPLE__
    ...
    switch (event.type) {
      ...
      case ENET_EVENT_TYPE_RECEIVE: {
        ...
        if (discmillis)
          ...
          servertoclient(event.channelID, event.packet->data,
      ...
      case ENET_EVENT_TYPE_DISCONNECT: {
        ...
        if (event.peer == connpeer) {
          ...
          if (!discmillis || event.data)
            conoutf("\f3server network error, disconnecting (%s) ...",
                    disc_reason(event.data));
          disconnect();
        }
        return;
      }
...
void securemap(char *map) {
  if (map) securemaps.add(newstring(map));
}
bool securemapcheck(const char *map, bool msg) {
  if (strstr(map, "maps/") == map || strstr(map, "maps\\") == map)
    map += strlen("maps/");
  loopv(securemaps) if (!strcmp(securemaps[i], map)) {
    if (msg) {
      conoutf(
          "\f3Map \f4%s\f3 is secured. This means you CAN'T send, receive or "
          "overwrite it.",
          map);
      if (connected) {
        conoutf(
            "\f3If you get this error often, you (or the server) may be "
            "running an outdated game.");
        conoutf(
            "\f3You can check for updates at "
            "\f1http://assault.cubers.net/download.html");
      }
    }
    return true;
  }
...
void sendmap(char *mapname) {
  ...
  if (gamemode == GMODE_COOPEDIT && !strcmp(getclientmap(), mapname))
    save_world(mapname, true, false);  // skip optimisations, don't add undos
  ...
  putint(p, cfgsizegz);
  putint(p, revision);
  if (MAXMAPSENDSIZE - p.length() < mapsize + cfgsizegz ||
      cfgsize > MAXCFGFILESIZE) {
...
void getmap(char *name, char *callback) {
  if ((!name || !*name) || (connected && !strcmp(name, getclientmap()))) {
...
void getdemo(int *idx, char *dsp) {
  ...
  if (dsp && dsp[0])
    formatstring(demosubpath)("%s/", dsp);
  else
    copystring(demosubpath, "");
  if (*idx <= 0)
    conoutf("getting demo...");
  else
    conoutf("getting demo %d...", *idx);
...
void shiftgametime(int newmillis) {
  if (!watchingdemo) {
    conoutf("You have to be watching a demo to change game time");
    return;
  }

  newmillis = max(0, newmillis);
  if (newmillis > gametimemaximum) {
    conoutf("Invalid time specified");
    return;
  }
