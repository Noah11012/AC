...
{
  char *name{NULL}, *fullname{NULL};  // name from zip, full path name
  uint header{0}, offset, size{0}, compressedsize{0};
  ziparchive *location{NULL};
...
static bool findzipdirectory(stream *f, zipdirectoryheader &hdr) {
  ...
  int len = 0, offset = f->tell(),
      end = max(offset - 0xFFFF - ZIP_DIRECTORY_SIZE, 0);
  const auto signature = lilswap<uint>(ZIP_DIRECTORY_SIGNATURE);

  while (offset > end) {
    int carry = min(len, ZIP_DIRECTORY_SIZE - 1),
        next = min((int)sizeof(buf) - carry, offset - end);
    offset -= next;
    memmove(&buf[next], buf, carry);
    if (next + carry < ZIP_DIRECTORY_SIZE || !f->seek(offset, SEEK_SET) ||
        (int)f->read(buf, next) != next)
      return false;
    len = next + carry;
    uchar *search = &buf[next - 1];
    for (; search >= buf; search--)
      if (*(uint *)search == signature) break;
    if (search >= buf) {
      src = search;
      break;
    }
  ...
  if (hdr.signature != ZIP_DIRECTORY_SIGNATURE ||
      hdr.disknumber != hdr.directorydisk || hdr.diskentries != hdr.entries)
    return false;
...
static bool readzipdirectory(stream *f, int entries, int offset, int size,
                             vector<zipfile> &files) {
  ...
  loopi(entries) {
    ...
    if (!hdr.namelength /*|| !hdr.uncompressedsize */ ||
        (hdr.compression &&
         (hdr.compression != Z_DEFLATED || !hdr.compressedsize))) {
...
#ifndef STANDALONE
    if (dbgzip)
      conoutf("file %s, size %d, compress %d, flags %x", name,
              hdr.uncompressedsize, hdr.compression, hdr.flags);
#endif
...
static bool readlocalfileheader(stream *f, ziplocalfileheader &h, uint offset) {
  ...
  if (f->read(buf, ZIP_LOCAL_FILE_SIZE) != ZIP_LOCAL_FILE_SIZE) return false;
  uchar *src = buf;
...
struct zipstream : stream {
  ...
  void readbuf(uint size = BUFSIZE) {
    ...
    int n = arch->owner == this
                ? (int)arch->data->read(zfile.next_in + zfile.avail_in,
                                        min(size, remaining))
                : 0;
    zfile.avail_in += n;
    reading += n;
  ...
  bool open(ziparchive *a, zipfile *zf) {
    if (zf->offset == ~0U) {
      ...
      zf->offset =
          zf->header + ZIP_LOCAL_FILE_SIZE + h.namelength + h.extralength;
    }
  ...
  void stopreading() {
    ...
    if (dbgzip)
      conoutf(
          info->compressedsize ? "%s: zfile.total_out %d, info->size %u"
                               : "%s: reading %d, info->size %u",
          info->name,
          int(info->compressedsize ? zfile.total_out : reading - info->offset),
          info->size);
#endif
  ...
  long size() override { return info->size; }
  bool end() override { return reading < 0 || ended; }
  long tell() override {
    return reading >= 0 ? (info->compressedsize ? zfile.total_out
                                                : reading - info->offset)
                        : -1;
  }

  bool seek(long pos, int whence) override {
  ...
  int read(void *buf, int len) override {
    ...
    if (!info->compressedsize) {
      ...
      auto n = (int)arch->data->read(
          buf, min(len, int(info->size + info->offset - reading)));
      reading += n;
      if (n < len) ended = true;
      return n;
    }
...
static vector<ziparchive *>
    archives;  // list of mounted zip files, highest priority first
static hashtable<const char *, zipfile *>
    zipfiles;  // table of all files in all mounted zips
static hashtable<const char *, memfile> memfiles;  // table of all memfiles

static void mountzip(ziparchive &arch, const char *mountdir,
                     const char *stripdir, bool allowconfig) {
  string mdir = "", fname;
  if (mountdir) {
  ...
  loopv(arch.files) {  // build full path names - and check them
    zipfile *zf = &arch.files[i];
    formatstring(fname)("%s%s", mdir,
                        striplen && !strncmp(zf->name, stripdir, striplen)
                            ? zf->name + striplen
                            : zf->name);
    path(fname);
    unixpath(fname);
    ...
    if (strcmp(fname, zf->fullname) ||
        (strncmp("packages/", fname, 9) &&
         (!allowconfig ||
          strncmp("config/", fname, 7)))) {  // path is not allowed (illegal
                                             // characters or illegal path)
...
void clearmemfiles()  // clear list of memfiles and free all associated buffers
                      // (unless there are files still open)
{
  bool inuse = false;
  enumerate(memfiles, memfile, mf, {
    if (mf.refcnt != 1) inuse = true;
  });
  if (!inuse) {
    enumerate(memfiles, memfile, mf, {
      delstring(mf.name);
...
void listmemfiles() {
  enumerate(memfiles, memfile, mf, {
    conoutf("\"%s\", %x, len %d, refcnt %d", mf.name,
            int(int(size_t(mf.buf)) & 0xfffffff), mf.len, mf.refcnt);
  });
}
COMMAND(listmemfiles, "");
#endif

// manage zipped mod packages in subdirectory "mods/" including description and
// preview images zip packages are restricted to contain files below "packages/"
// only - unless the zip filename starts with "###", which additionally allows
// files below "config/"
...
const char *fixzipname(char *name)  // check, if valid path + filename, based on
                                    // mods/, return NULL if invalid
{
  string tmp;
  unixpath(name);  // changes "name"!
  filtertext(tmp, name, FTXT__MEDIAFILEPATH);
  const char *bp = behindpath(tmp), *pd = parentdir(tmp);
  static defformatstring(pname)(
      "%s%s%s%s", pd, bp != tmp ? "/" : "",
      !strncmp("###", behindpath(name), 3) ? "###" : "", bp);
  if (strlen(name) > 40 || strchr(name, '.') || strcmp(pname, name) ||
      *name == '/')
    return nullptr;  // illegal filename
  formatstring(pname)("mods/%s.zip", name);
  return path(pname);
}

bool validzipmodname(
    char *name)  // check for valid _downloadable_ zip mod name (no '#'
                 // allowed), also changes backslashes to slashes
{
  return fixzipname(name) && !strchr(name, '#');
}

VAR(zipcachemaxsize, 0, 512,
    1024);  // load zip files smaller than 512K straight into RAM
...
void addzipmod(char *name) {
  ...
  if (zipsize > 0 && zipsize < zipcachemaxsize * 1024) {
    ...
    delete arch->data;
    arch->data = openmemfile(buf, zipsize, nullptr);
  }
  zipdirectoryheader h;
  if (!findzipdirectory(arch->data, h) ||
      !readzipdirectory(arch->data, h.entries, h.offset, h.size, arch->files)) {
    conoutf("could not read directory in zip %s", pname);
    delete arch;
    return;
  }
  mountzip(*arch, nullptr, nullptr, !strncmp(behindpath(name), "###", 3));
  archives.add(arch);
  rebuildzipfilehashtable();
  DEBUGCODE(clientlogf("added zipmod %s, %d bytes, %d files", pname, zipsize,
                       arch->files.length()));
}
COMMAND(addzipmod, "s");
...
void zipmodremove(char *name) {
  const char *pname = fixzipname(name);
  ziparchive *exists = pname ? findzip(name) : nullptr;
  if (!exists) {
...
const char
    ...
    *modrev =
        "revision_";  // special files in zip containing description or preview

void zipmodgetdesc(
    char *name)  // open zip, read desc.txt and mount preview picture
{
  const char *pname = fixzipname(name);
  stream *f = pname ? openfile(pname, "rb") : nullptr, *zf = nullptr;
  ...
  if (!f || !mz)
    ...
    loopv(files) {
      if (!strcmp(files[i], desctxt)) {
        ...
        auto *v = new vector<uchar>;
        zf = openvecfile(v);
        int got = zipmanualread(
            mz, i, zf, 128 * 1024);  // max. file size for preview.jpg is 128k
        formatstring(tmp)("packages/modpreviews/%s.jpg", name);
        addmemfile(tmp, v->getbuf(), got);
        if (got == 128 * 1024)
          conoutf(
              "\f3preview.jpg in zip file %s exceeds maximum file size: file "
              "truncated",
              pname);
      }
      DELETEP(zf);
...
{
  const char *pname = fixzipname(name);
  stream *f = pname ? openfile(pname, "rb") : nullptr;
  vector<const char *> files;
  vector<char> res;
  void *mz = zipmanualopen(f, files);
  if (!f || !mz)
...
void zipmodgetrevision(char *name)  // open zip, read list of files
{
  ...
  if (!f || !mz)
    ...
    loopv(files) if (!strncasecmp(files[i], modrev, modrevlen)) res =
        atoi(files[i] + modrevlen);
    zipmanualclose(mz);
  }
  intret(res);
}
...
void *zipmanualopen(stream *f, vector<const char *> &files) {
  ...
  if (!findzipdirectory(f, h) ||
      !readzipdirectory(f, h.entries, h.offset, h.size, arch->files)) {
    delete arch;
    delete f;
    return nullptr;
  }
...
stream *zipmanualstream(void *a, int n) {
  auto *arch = (ziparchive *)a;
  if (arch->files.inrange(n)) {
    auto *s = new zipstream;
...
bool findzipfile(const char *name) {
  return zipfiles.access(name) != nullptr || memfiles.access(name) != nullptr;
}

stream *openzipfile(const char *name, const char *mode) {
  for (; *mode; mode++)
    if (*mode == 'w' || *mode == 'a') return nullptr;
...
void listzipfiles(const char *dir, const char *ext,
                  vector<char *> &files)  // (does not list memfiles)
{
  ...
  enumerate(zipfiles, zipfile *, zf, {
    ...
    if (!ext)
      ...
      if (namelength > 0 && name[namelength] == '.' &&
          strncmp(name + namelength + 1, ext, extsize - 1) == 0)
        files.add(newstring(name, namelength));
    }
  });
}

void listzipdirs(const char *dir,
                 vector<char *> &subdirs)  // (does not list memfiles)
{
  auto dirsize = (int)strlen(dir);
  enumerate(zipfiles, zipfile *, zf, {
    if (strncmp(zf->fullname, dir, dirsize)) continue;
    const char *name = zf->fullname + dirsize;
    if (name[0] == PATHDIV) name++;
