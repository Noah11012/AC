...
static bool findzipdirectory (stream *f, zipdirectoryheader &hdr)
{
    ...
    if (hdr.signature != ZIP_DIRECTORY_SIGNATURE || hdr.disknumber != hdr.directorydisk
        || hdr.diskentries != hdr.entries)
        return false;
...
#ifndef STANDALONE
        if (dbgzip)
            conoutf ("file %s, size %d, compress %d, flags %x", name, hdr.uncompressedsize,
                     hdr.compression, hdr.flags);
#endif
...
struct zipstream : stream
{
    ...
    void readbuf (uint size = BUFSIZE)
    {
        ...
        int n = arch->owner == this ?
                (int)arch->data->read (zfile.next_in + zfile.avail_in, min (size, remaining)) :
                0;
        zfile.avail_in += n;
        reading += n;
    }
...
#ifndef STANDALONE
        if (dbgzip)
            conoutf (info->compressedsize ? "%s: zfile.total_out %d, info->size %u" : "%s: reading %d, info->size %u",
                     info->name, int(info->compressedsize ? zfile.total_out : reading - info->offset),
                     info->size);
#endif
    ...
    long tell () override
    {
        return reading >= 0 ? (info->compressedsize ? zfile.total_out : reading - info->offset) : -1;
    }
    ...
    int read (void *buf, int len) override
    {
        ...
        if (!info->compressedsize)
        {
            ...
            auto n = (int)arch->data->read (buf, min (len, int(info->size + info->offset - reading)));
            reading += n;
            if (n < len) ended = true;
            return n;
        }
...
static void mountzip (ziparchive &arch, const char *mountdir, const char *stripdir, bool allowconfig)
{
    ...
    { // build full path names - and check them
        ...
        formatstring (fname) ("%s%s", mdir,
                              striplen && !strncmp (zf->name, stripdir, striplen) ? zf->name + striplen :
                                                                                    zf->name);
        path (fname);
        unixpath (fname);
        ...
        if (strcmp (fname, zf->fullname)
            || (strncmp ("packages/", fname, 9) && (!allowconfig || strncmp ("config/", fname, 7))))
        { // path is not allowed (illegal
...
{
    enumerate (memfiles, memfile, mf, {
        conoutf ("\"%s\", %x, len %d, refcnt %d", mf.name, int(int(size_t (mf.buf)) & 0xfffffff),
                 mf.len, mf.refcnt);
    });
}
                                    ...
                                    // mods/, return NULL if invalid
{
    ...
    static defformatstring (pname) ("%s%s%s%s", pd, bp != tmp ? "/" : "",
...
bool validzipmodname (char *name) // check for valid _downloadable_ zip mod name (no '#'
...
const char *desctxt = "desc.txt", *previewjpg = "preview.jpg",
           *modrev = "revision_"; // special files in zip containing description or preview
    ...
    else
    {
        loopv (files)
        {
            ...
            else if (!strcmp (files[i], previewjpg))
            {
                ...
                if (got == 128 * 1024)
                    conoutf ("\f3preview.jpg in zip file %s exceeds maximum file size: file "
                             "truncated",
                             pname);
            }
            DELETEP (zf);
...
void listzipfiles (const char *dir,
                   const char *ext,
