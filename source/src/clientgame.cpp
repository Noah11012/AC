...
bool duplicatename(playerent *d, char *name = nullptr) {
  ...
  loopv(players) if (players[i] && d != players[i] &&
                     !strcmp(name, players[i]->name)) return true;
  global_name =
      player1->name;  // this certainly is not the best place to put this
  return false;
}
...
char *colorping(int ping) {
  static string cping;
  formatstring(cping)("\fs\f%d%d\fr", ping <= 500 ? 0 : ping <= 1000 ? 2 : 3,
                      ping);
  return cping;
}
...
void listignored() {
  string pl;
  pl[0] = '\0';
  loopv(players) if (players[i] && players[i]->ignored)
      concatformatstring(pl, ", %s", colorname(players[i]));
  if (*pl)
    conoutf("ignored players: %s", pl + 2);
  else
    conoutf("no players are ignored");
}

void clearignored(int *cn) {
  loopv(players) if (players[i] && (*cn < 0 || *cn == i)) players[i]->ignored =
      false;
}
...
void listmuted() {
  string pl;
  pl[0] = '\0';
  loopv(players) if (players[i] && players[i]->muted)
      concatformatstring(pl, ", %s", colorname(players[i]));
  if (*pl)
    conoutf("muted players: %s", pl + 2);
  else
    conoutf("no players are muted");
}

void clearmuted(char *cn) {
  loopv(players) if (players[i] && (*cn < 0 || *cn == i)) players[i]->muted =
      false;
}
...
void newname(const char *name) {
  if (name[0]) {
    ...
    conoutf("your name is: %s", player1->name);
}
...
void newteam(char *name) {
  if (*name) {
    ...
    if (!team_isvalid(nt)) {
      conoutf("\f3\"%s\" is not a valid team name or number", name);
      return;
    }
    if (player1->state == CS_EDITING)
      conoutf("you can't change team while editing");
    else
      addmsg(SV_SWITCHTEAM, "ri", nt);
  } else
    conoutf("your team is: %s", team_string(player1->team));
}
...
void curmodeattr(char *attr) {
  if (!strcmp(attr, "team")) {
    intret(m_teammode);
    return;
  } else if (!strcmp(attr, "arena")) {
    intret(m_arena);
    return;
  } else if (!strcmp(attr, "flag")) {
    intret(m_flags);
    return;
  } else if (!strcmp(attr, "bot")) {
    intret(m_botmode);
    return;
  }
  intret(0);
...
void stopdemo() {
  if (watchingdemo)
    ...
    conoutf("not playing a demo");
}
COMMAND(stopdemo, "");

// macros for playerinfo() & teaminfo(). Use this to replace pstats_xxx ?
#define ATTR_INT(name, attribute) \
  if (!strcmp(attr, #name)) {     \
    intret(attribute);            \
    return;                       \
  }
#define ATTR_FLOAT(name, attribute) \
  if (!strcmp(attr, #name)) {       \
    floatret(attribute);            \
    return;                         \
  }
#define ATTR_STR(name, attribute) \
  if (!strcmp(attr, #name)) {     \
    result(attribute);            \
    return;                       \
  }
...
void playerinfo(int *cn, const char *attr) {
  ...
  if (p == player1 ||
      ...
      player1->team == TEAM_SPECT || m_coop) {
  ...
  if (addr[3] != 0 || player1->clientrole == CR_ADMIN)
    formatstring(addrstr)("%d.%d.%d.%d", addr[0], addr[1], addr[2],
                          addr[3]);  // full IP
  else
    formatstring(addrstr)("%d.%d.%d.x", addr[0], addr[1],
                          addr[2]);  // censored IP
  ATTR_STR(ip, addrstr);
...
const char *currentserver(int i)  // [client version]
{
  ...
  // using the curpeer directly we can get the info of our currently connected
  // server
  string r;
  r[0] = '\0';
  ...
  if (curpeer) {
    switch (i) {
      ...
      {
        char hn[1024];
        formatstring(r)("%s", (enet_address_get_host(&curpeer->address, hn,
                                                     sizeof(hn)) == 0)
                                  ? hn
                                  : "unknown");
        break;
      ...
      case 4:  // STATE
      {
        const char *statenames[] = {"disconnected",
                                    "connecting",
                                    ...
                                    "zombie"};
        if (curpeer->state >= 0 &&
            curpeer->state < int(sizeof(statenames) / sizeof(statenames[0])))
          copystring(r, statenames[curpeer->state]);
        break;  // 5 == Connected (compare ../enet/include/enet/enet.h +165)
      }
      // CAUTION: the following are only filled if the serverbrowser was used or
      // the scoreboard shown SERVERNAME
      case 5: {
        serverinfo *si = getconnectedserverinfo();
        if (si) copystring(r, si->name);
        break;
      }
      // DESCRIPTION (3)
      case 6: {
        serverinfo *si = getconnectedserverinfo();
        if (si) copystring(r, si->sdesc);
        break;
      }
      case 7: {
        serverinfo *si = getconnectedserverinfo();
        if (si) copystring(r, si->description);
        break;
      }
      // CAUTION: the following is only the last full-description _seen_ in the
      // serverbrowser!
      case 8: {
        serverinfo *si = getconnectedserverinfo();
        if (si) copystring(r, si->full);
        break;
      }
...
void deathstate(playerent *pl) {
  ...
  if (pl == player1) {
    ...
    if (pl->team == TEAM_SPECT)
      spectatemode(SM_FLY);
    else if (team_isspect(pl->team))
      spectatemode(SM_FOLLOW1ST);
  } else
    pl->resetinterp();
}

void spawnstate(
    playerent *d)  // reset player state not persistent accross spawns
{
  d->respawn();
...
playerent *newplayerent()  // create a new blank player
{
  ...
  setskin(d, rnd(6));
  weapon::equipplayer(
      d);  // flowtron : avoid overwriting d->spawnstate(gamemode) stuff from
           // the following line (this used to be called afterwards)
  spawnstate(d);
  return d;
}
...
botent *newbotent()  // create a new blank player
{
  ...
  while (lukin) {
    ...
    loopv(players) if (!used && players[i]) if (players[i]->clientnum == nextcn)
        used = true;
    if (!used)
      lukin = false;
    else
  ...
  loopv(players) if (players[i] && players[i]->type == ENT_PLAYER) {
    ...
    if (d->state == CS_ALIVE || d->state == CS_EDITING) {
      if (smoothmove && d->smoothmillis > 0)
        ...
        moveplayer(d, 1, false);
    } else if (d->state == CS_DEAD && lastmillis - d->lastpain < 2000)
      moveplayer(d, 1, true);
  }
}
...
bool showhudtimer(int maxsecs, int startmillis, const char *msg, bool flash) {
  ...
  if (nextticks < maxticks)
    hudeditf(HUDMSG_TIMER | HUDMSG_OVERWRITE, "%s", flash ? str : str + 2);
  else
    hudeditf(HUDMSG_TIMER, "%s", msg);
  return true;
}
...
void showrespawntimer() {
  ...
  if (m_arena) {
    ...
    showhudtimer(5, arenaintermission, "FIGHT!",
                 lastspawnattempt >= arenaintermission &&
                     lastmillis < lastspawnattempt + 100);
  } else if (player1->state == CS_DEAD && m_flags &&
             (!player1->isspectating() ||
              player1->spectatemode == SM_DEATHCAM)) {
    int secs = 5;
    showhudtimer(secs, player1->lastdeath, "READY!",
                 lastspawnattempt >= arenaintermission &&
                     lastmillis < lastspawnattempt + 100);
  }
}
...
float nearestenemy(const vec &place, int team) {
  ...
  loopv(players) {
    ...
    if (dist < nearestenemydist || nearestenemydist == -1)
      nearestenemydist = dist;
  }
  if (nearestenemydist >= SECURESPAWNDIST || nearestenemydist < 0)
    return -1;
  else
...
void findplayerstart(playerent *d, bool mapcenter, int arenaspawn) {
  ...
  if (!mapcenter) {
    ...
    } else if ((m_teammode || m_arena) && !m_ktf)  // ktf uses ffa spawns
    {
      ...
      loopi(r) {
        spawncycle =
            clentstats.hasffaspawns
                ? findentity(PLAYERSTART, spawncycle + 1, 100)
                : findentity(PLAYERSTART,
                             spawncycle + 1);  // if not enough ffa-spawns are
                                               // available, use all
        if (spawncycle < 0) continue;
        float dist = nearestenemy(
            vec(ents[spawncycle].x, ents[spawncycle].y, ents[spawncycle].z),
            d->team);
        if (!e || dist < 0 || (bestdist >= 0 && dist > bestdist)) {
          e = &ents[spawncycle];
          bestdist = dist;
        }
      }
...
void respawnself() {
  if (m_mp(gamemode))
    ...
    if (team_isspect(player1->team)) {
      addmsg(SV_SWITCHTEAM, "ri",
             m_teammode ? teamatoi(BotManager.GetBotTeam()) : rnd(2));
    }
    showscores(false);
    setscope(false);
    setburst(false);
    ...
    player1->weaponchanging -=
        player1->weapons[player1->gunselect]->weaponchangetime /
        2;  // 2011jan16:ft: for a little no-shoot after spawn
  }
}
...
inline const char *spawn_message() {
  if (spawnpermission == SP_WRONGMAP) {
    // Don't use "/n" within these messages. If you do, the words won't align to
    // the middle of the screen.
    if (securemapcheck(getclientmap()))
      return "3The server will NOT allow spawning or getmap!";  // Also see
                                                                // client.cpp
                                                                // which has a
                                                                // conoutf
                                                                // message
    else
      return "3You must be on the correct map to spawn. Type /getmap to "
             "download it.";
  } else if (m_coop)
  ...
  // Despite its many glaring (and occasionally fatal) inaccuracies, AssaultCube
  // itself has outsold the Encyclopedia Galactica because it is slightly
...
bool tryrespawn() {
  if (m_mp(gamemode) && multiplayer(nullptr) && bad_map()) {
    ...
    if (team_isspect(player1->team)) {
      ...
      int respawnmillis =
          player1->lastdeath + (m_arena ? 0 : (m_flags ? 5000 : 2000));
      if (lastmillis > respawnmillis) {
        player1->attacking = false;
        if (m_arena) {
          if (!arenaintermission)
            hudeditf(HUDMSG_TIMER, "waiting for new round to start...");
          else
...
void setkillmessage(const char *gun, bool gib, const char *message) {
  ...
  if (!valid_weapon(guni)) {
    ...
    defformatstring(aname)("%smessage_%s", gib ? "gib" : "frag",
                           gunnames[guni]);
...
COMMANDF(fragmessage, "ss", (const char *gun, const char *message) {
  setkillmessage(gun, false, message);
});
COMMANDF(gibmessage, "ss", (const char *gun, const char *message) {
  setkillmessage(gun, true, message);
});
...
COMMANDF(burstshots, "si", (char *gun, int *shots) {
  ...
  if (guni >= 0 && guns[guni].isauto) {
    if (*shots >= 0)
      burstshotssettings[guni] = min(*shots, (guns[guni].magsize - 1));
    else
      intret(burstshotssettings[guni]);
  } else
    conoutf("invalid gun specified");
});
...
void dodamage(int damage, playerent *pl, playerent *actor, int gun, bool gib,
              bool local) {
  if (pl->state != CS_ALIVE || intermission) return;
  pl->lastpain = lastmillis;
  // could the author of the FIXME below please elaborate what's to fix?!
  // (ft:2011mar28) I suppose someone wanted to play the hitsound for player1 or
  // spectated player (lucas:2011may22)
  playerent *h = player1->isspectating() && player1->followplayercn >= 0 &&
  ...
  if (actor == h && pl != actor) {
    if ((hitsound == 1 || (hitsound && h != player1)) && lasthit != lastmillis)
      audiomgr.playsound(S_HITSOUND, SP_HIGHEST);
    lasthit = lastmillis;
  }
  ...
  if (pl->health <= 0) {
    if (local) dokill(pl, actor, gib, gun >= 0 ? gun : actor->weaponsel->type);
  } else if (pl == player1)
...
void dokill(playerent *pl, playerent *act, bool gib, int gun) {
  ...
  exechook(HOOK_SP, "onKill", "%d %d %d %d", act->clientnum, pl->clientnum, gun,
           gib ? 1 : 0);
  ...
  void (*outf)(const char *s, ...) =
      (pl == player1 || act == player1) ? hudoutf : conoutf;

  if (pl == act) {
    outf("\f2%s suicided%s", pname, pl == player1 ? "!" : "");
  } else if (isteam(pl->team, act->team)) {
    if (pl == player1)
      outf("\f2you were %s by teammate %s", death, aname);
    else
      outf("%s%s %s teammate %s", act == player1 ? "\f3" : "\f2", aname, death,
           pname);
  } else {
    if (pl == player1)
      outf("\f2you were %s by %s", death, aname);
    else
  ...
  if (pl == act || isteam(pl->team, act->team)) {
    ...
    act->frags += (gib && gun != GUN_GRENADE && gun != GUN_SHOTGUN) ? 2 : 1;
...
void pstat_weap(int *cn) {
  ...
  if (pl)
    loopi(NUMGUNS)
        concatformatstring(weapstring, "%s%d %d", strlen(weapstring) ? " " : "",
                           pl->pstatshots[i], pl->pstatdamage[i]);
  result(weapstring);
}
...
void timeupdate(int milliscur, int millismax) {
  ...
  if (!minutesremaining) {
    ...
    extern int gametimedisplay;  // only output to console if no hud-clock with
                                 // game time is being shown
    int sec =
        60 -
        ((gametimecurrent + (lastmillis - lastgametimeupdate)) / 1000) % 60;
...
void zapplayerflags(playerent *p) {
  loopi(2) if (flaginfos[i].state == CTFF_STOLEN && flaginfos[i].actor == p)
      initflag(i);
}
  ...
  if (!cleanonly) {
    loopv(ents) {
      ...
      if (e.type == CTF_FLAG) {
        e.spawned = true;
        if (e.attr2 >= 2) {
          conoutf("\f3invalid ctf-flag entity (%d)", i);
          e.attr2 = 0;
        }
...
void showmapstats() {
  ...
  if (m_flags && F2F < 1000)
    conoutf("  Flag-to-flag distance is: %d", (int)sqrtf(F2F));
  if (item_fail)
    conoutf(
        "  There are one or more items too close to each other in this map");
}
...
void startmap(const char *name, bool reset,
              bool norespawn)  // called just after a map load
{
  ...
  if (m_valid(gamemode) && !m_mp(gamemode))
    respawnself();
  else
    findplayerstart(player1);
  if (good_map() == MAP_IS_BAD)
    conoutf(
        "You cannot play in this map due to quality requisites. Please, report "
        "this incident.");
  if (mapstats_hud) showmapstats();

  if (!reset) return;

  player1->frags = player1->flagscore = player1->deaths =
      player1->lifesequence = player1->points = player1->tks = 0;
  loopv(players) if (players[i]) players[i]->frags = players[i]->flagscore =
      players[i]->deaths = players[i]->lifesequence = players[i]->points =
          players[i]->tks = 0;
  if (editmode) toggleedit(true);
  intermission = false;
  showscores(false);
  closemenu(
      "Download demo");  // close and reset demo list, because it may be no
                         // longer accurate, since a new game has started
  ...
  if (mapstartonce && mapstartonce[0]) {
    addsleep(0, mapstartonce);  // do this as a sleep to make sure map changes
                                // don't recurse inside a welcome packet
    // BTW: in v1.0.4 sleep 1 was required to make it work on initial mapload
    // [flowtron:2010jun25]
    alias("mapstartonce", "");
  }
...
void flagmsg(int flag, int message, int actor, int flagtime) {
  ...
  const char *teamstr =
      m_ktf ? "the" : neutral ? ownerstr : own ? "your" : "the enemy";
  const char *flagteam =
      (m_ktf && !neutral) ? (teammate ? "your teammate " : "your enemy ") : "";
  ...
  switch (message) {
    case FM_PICKUP:
      ...
      if (firstperson) {
        ...
        hudoutf("\f2%s%s has %s flag", flagteam, colorname(act), teamstr);
      break;
    case FM_LOST:
    case FM_DROP: {
      ...
      if (firstperson) {
        ...
        hudoutf("\f2%s %s %s flag", colorname(act), droplost, teamstr);
      break;
    }
    case FM_RETURN:
      audiomgr.playsound(S_FLAGRETURN, SP_HIGHEST);
      if (firstperson)
        hudoutf("\f2you returned your flag");
      else
        hudoutf("\f2%s returned %s flag", colorname(act), teamstr);
      break;
    case FM_SCORE:
      ...
      if (firstperson) {
        ...
        hudoutf("\f2%s scored for %s", colorname(act),
                neutral ? teamnames[act->team]
                        : teammate ? "your team" : "the enemy team");
      break;
    case FM_KTFSCORE: {
      ...
      int m = flagtime / 60;
      if (m)
        hudoutf("\f2%s%s%s kept the flag for %d min %d s now", tc, ta, tb, m,
                flagtime % 60);
      else
    ...
    case FM_SCOREFAIL:  // sound?
      hudoutf("\f2%s failed to score (own team flag not taken)",
              firstperson ? "you" : colorname(act));
      break;
    case FM_RESET:
      audiomgr.playsound(S_FLAGRETURN, SP_HIGHEST);
...
char *votestring(int type, const char *arg1, const char *arg2,
                 const char *arg3) {
  const char *msgs[] = {"kick player %s, reason: %s",
                        "ban player %s, reason: %s",
                        "remove all bans",
                        "set mastermode to %s",
                        "%s autoteam",
                        "force player %s to team %s",
                        "give admin to player %s",
                        "load map %s in mode %s%s%s",
                        "%s demo recording for the next match",
                        "stop demo recording",
                        "clear all demos",
                        "set server description to '%s'",
                        "shuffle teams"};
  ...
  switch (type) {
    ...
    case SA_GIVEADMIN: {
      ...
      if (type == SA_KICK || type == SA_BAN) {
        string reason = "";
        if (m_teammode)
          formatstring(reason)("%s (%d tk%s, ping %d)", arg2, p->tks,
                               p->tks == 1 ? "" : "s", p->ping);
        else
        ...
        formatstring(out)(msg, colorname(p),
                          team_isvalid(team) ? teamnames[team] : "");
      } else
        formatstring(out)(msg, colorname(p));
      break;
    }
    ...
    case SA_MAP: {
      ...
      if (arg3 && arg3[0]) {
        int time = atoi(arg3);
        if (time > 0 && time != defaultgamelimit(n))
          formatstring(timestr)(" for %d minute%s", time, time == 1 ? "" : "s");
      }

      if (n >= GMODE_NUM) {
        formatstring(out)(msg, arg1, modestr(n - GMODE_NUM, modeacronyms > 0),
                          " (in the next game)", timestr);
      } else {
...
void callvote(int type, const char *arg1, const char *arg2, const char *arg3) {
  ...
  if (v) {
    ...
    exechook(HOOK_SP_MP, "onCallVote", "%d %d [%s] [%s]", type,
             player1->clientnum, arg1, arg2);
  } else
    conoutf("\f3invalid vote");
}
...
void displayvote(votedisplayinfo *v) {
  ...
  DELETEP(curvote);
  curvote = v;
  conoutf("%s called a vote: %s", v->owner ? colorname(v->owner) : "",
          curvote->desc);
...
void votecount(int v) {
  if (curvote && v >= 0 && v < VOTE_NUM) curvote->stats[v]++;
}
...
const char *modestrings[] = {
    "tdm", "coop", "dm",   "lms",   "ts",     "ctf",   "pf",   "btdm",
    "bdm", "lss",  "osok", "tosok", "bosok",  "htf",   "tktf", "ktf",
...
void whois(int *cn) {
  loopv(players) if (players[i] && players[i]->type == ENT_PLAYER &&
                     (*cn == -1 || players[i]->clientnum == *cn)) {
    playerent *p = players[i];
    uint2ip(p->address, ip);
    if (ip[3] != 0 || player1->clientrole == CR_ADMIN)
      conoutf("WHOIS client %d:\tname %s , IP %d.%d.%d.%d , %d teamkill%s",
              p->clientnum, colorname(p), ip[0], ip[1], ip[2], ip[3], p->tks,
              p->tks == 1 ? "" : "s");  // full IP
    else
      conoutf("WHOIS client %d:\tname %s , IP %d.%d.%d.x , %d teamkill%s",
              p->clientnum, colorname(p), ip[0], ip[1], ip[2], p->tks,
              p->tks == 1 ? "" : "s");  // censored IP
  }
}
COMMAND(whois, "i");
...
void setadmin(int *claim, char *password) {
  if (!*claim && (player1->clientrole)) {
    ...
    addmsg(SV_SETADMIN, "ris", *claim,
           genpwdhash(player1->name, password, sessionid));
}
  ...
  loopv(players) if (players[i]) {
    ...
    formatstring(m.cmd)(
        ...
        menu == kickmenu
            ...
            : (menu == banmenu
                   ? "ban"
                   : (menu == forceteammenu ? "forceteam" : "giveadmin")),
        i,
        (menu == kickmenu || menu == banmenu)
            ? (strlen(kbr) > 8 ? kbr : " NONE")
            : "");  // 8==3 + "format-extra-chars"
...
playerent *updatefollowplayer(int shiftdirection) {
  playerent *f = players.inrange(player1->followplayercn)
                     ? players[player1->followplayercn]
                     : NULL;  // last spectated player

  // collect spec-able players
  int omit_team = m_teammode && !watchingdemo && player1->team != TEAM_SPECT
                      ? team_opposite(team_base(player1->team))
                      : TEAM_NUM;  // don't spect enemy team in team mode
  bool stayondeadplayers = spectatepersistent || !m_arena;
  vector<playerent *> available;
  loopv(players) if (players[i]) {
    if (team_group(players[i]->team) == omit_team)
      continue;  // if not team SPECT and not watchingdemo and teammode: don't
                 // spectate enemy team
    if ((players[i]->state == CS_DEAD || players[i]->isspectating()) &&
        (!stayondeadplayers || players[i] != f))
  ...
  int idx =
      ((oldidx + shiftdirection) % available.length() + available.length()) %
      available.length();
...
void spectate() {
  ...
  if (!team_isspect(player1->team))
    addmsg(SV_SWITCHTEAM, "ri", TEAM_SPECT);
  else
    tryrespawn();
}
...
void setfollowplayer(int cn) {
  ...
  if (players.inrange(cn) && players[cn] && !m_botmode) {
    if (!(m_teammode && player1->team != TEAM_SPECT && !watchingdemo &&
          team_base(players[cn]->team) != team_base(player1->team))) {
...
void spectatemode(int mode) {
  if ((player1->state != CS_DEAD && player1->state != CS_SPECTATE &&
       !team_isspect(player1->team)) ||
      (!m_teammode && !team_isspect(player1->team) &&
       servstate.mastermode == MM_MATCH))
    return;  // during ffa matches only SPECTATORS can spectate
  ...
  switch (mode) {
    ...
    case SM_FOLLOW3RD_TRANSPARENT: {
      if (players.length() && updatefollowplayer())
        break;
      else
        mode = SM_FLY;
    }
    case SM_FLY: {
      if (player1->spectatemode != SM_FLY) {
        ...
        if (f) {
          ...
          entinmap(player1);  // or drop 'em at a random place
      }
      break;
    }
    case SM_OVERVIEW:
...
void togglespect()  // cycle through all spectating modes
{
  if (m_botmode)
    ...
    int mode;
    if (player1->spectatemode == SM_NONE)
      mode = SM_FOLLOW1ST;  // start with 1st person spect
    else
      mode = SM_FOLLOW1ST +
             ((player1->spectatemode - SM_FOLLOW1ST + 1) %
              (SM_OVERVIEW - SM_FOLLOW1ST));  // replace SM_OVERVIEW by SM_NUM
                                              // to enable overview mode
    spectatemode(mode);
  }
}
...
void spectatecn() {
  int spectcn = -1;
  if (player1->isspectating() && player1->spectatemode >= SM_FOLLOW1ST &&
      player1->spectatemode <= SM_FOLLOW3RD_TRANSPARENT)
...
void serverextension(char *ext, char *args) {
  ...
  if (n > 0)
    addmsg(SV_EXTENSION, "rsis", ext, n, args);
  else
    addmsg(SV_EXTENSION, "rsi", ext, n);
}
