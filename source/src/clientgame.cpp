...
int teamatoi (const char *name) { return getlistindex (name, teamnames, true, -1); }
...
void playerinfo (int *cn, const char *attr)
{
    ...
    if (!p)
    {
        if (!m_botmode && multiplayer (nullptr)) // bot clientnums are still glitchy, causing this message to
    ...
    if (p == player1 || (team_base (p->team) == team_base (player1->team) && m_teammode)
...
playerent *newplayerent () // create a new blank player
{
    ...
    setskin (d, rnd (6));
    weapon::equipplayer (d); // flowtron : avoid overwriting d->spawnstate(gamemode) stuff from
...
void showrespawntimer ()
{
    ...
    if (m_arena)
    {
        ...
        showhudtimer (5, arenaintermission, "FIGHT!",
                      lastspawnattempt >= arenaintermission && lastmillis < lastspawnattempt + 100);
    }
    else if (player1->state == CS_DEAD && m_flags
             && (!player1->isspectating () || player1->spectatemode == SM_DEATHCAM))
    {
        int secs = 5;
...
sl_semaphore *sleepssemaphore = new sl_semaphore (1, nullptr); // guard the sleeps-vector with a semaphore, so
...
void addsleep_ (int *msec, char *cmd, int *persist) { addsleep (*msec, cmd, *persist != 0); }
...
void findplayerstart (playerent *d, bool mapcenter, int arenaspawn)
{
    ...
    if (!mapcenter)
    {
        ...
        else
        {
            ...
            loopi (r)
            {
                spawncycle = clentstats.hasffaspawns ?
                ...
                float dist
                = nearestenemy (vec (ents[spawncycle].x, ents[spawncycle].y, ents[spawncycle].z), d->team);
...
void respawnself ()
{
    ...
    else
    {
        ...
        player1->weaponchanging -= player1->weapons[player1->gunselect]->weaponchangetime
...
bool tryrespawn ()
{
    ...
    {
        hudoutf ("This map is not supported in multiplayer. Read the docs about map "
                 "quality/dimensions.");
    }
...
void dodamage (int damage, playerent *pl, playerent *actor, int gun, bool gib, bool local)
{
    ...
    playerent *h = player1->isspectating () && player1->followplayercn >= 0
                   && (player1->spectatemode == SM_FOLLOW1ST || player1->spectatemode == SM_FOLLOW3RD
...
{ entity (-1, -1, -1, CTF_FLAG, 0, 0, 0, 0), entity (-1, -1, -1, CTF_FLAG, 0, 1, 0, 0) };
               ...
               bool norespawn) // called just after a map load
{
    ...
    if (good_map () == MAP_IS_BAD)
        conoutf ("You cannot play in this map due to quality requisites. Please, report "
                 "this incident.");
    if (mapstats_hud) showmapstats ();

    if (!reset) return;

    player1->frags = player1->flagscore = player1->deaths = player1->lifesequence = player1->points
    = player1->tks = 0;
    loopv (players) if (players[i]) players[i]->frags = players[i]->flagscore = players[i]->deaths
    = players[i]->lifesequence = players[i]->points = players[i]->tks = 0;
    if (editmode) toggleedit (true);
    intermission = false;
    showscores (false);
    closemenu ("Download demo"); // close and reset demo list, because it may be no
    ...
    case FM_SCORE:
        ...
        if (firstperson)
        {
            ...
            hudoutf ("\f2%s scored for %s", colorname (act),
                     neutral ? teamnames[act->team] : teammate ? "your team" : "the enemy team");
        break;
    case FM_KTFSCORE:
    {
...
votedisplayinfo *
newvotedisplayinfo (playerent *owner, int type, const char *arg1, const char *arg2, const char *arg3)
{
...
void scallvote (int *type, const char *arg1, const char *arg2)
{
    if (type && inmainloop)
    {
        ...
        case SA_BAN:
        {
            if (!arg1 || !isdigit (arg1[0]) || !arg2 || strlen (arg2) <= 3 || !multiplayer (nullptr))
            {
...
= { "tdm",   "coop",  "dm",  "lms",  "ts",  "ctf", "pf",   "btdm", "bdm",  "lss",    "osok",
    "tosok", "bosok", "htf", "tktf", "ktf", "tpf", "tlss", "bpf",  "blss", "btsurv", "btosok" };

void setnext (char *mode, char *map)
{
    if (!multiplayer (nullptr))
    { // RR 10/12/12 - Is this the action we want?
...
void whois (int *cn)
{
                        ...
                        && (*cn == -1 || players[i]->clientnum == *cn))
    {
        ...
        if (ip[3] != 0 || player1->clientrole == CR_ADMIN)
            conoutf ("WHOIS client %d:\tname %s , IP %d.%d.%d.%d , %d teamkill%s", p->clientnum,
                     colorname (p), ip[0], ip[1], ip[2], ip[3], p->tks,
                     p->tks == 1 ? "" : "s"); // full IP
        else
            conoutf ("WHOIS client %d:\tname %s , IP %d.%d.%d.x , %d teamkill%s", p->clientnum,
                     colorname (p), ip[0], ip[1], ip[2], p->tks,
...
void refreshsopmenu (void *menu, bool init)
{
    ...
    loopv (players) if (players[i])
    {
        ...
        m.cmd) ("%s %d%s",
                menu == kickmenu ?
                "kick" :
                (menu == banmenu ? "ban" : (menu == forceteammenu ? "forceteam" : "giveadmin")),
                i,
                (menu == kickmenu || menu == banmenu) ? (strlen (kbr) > 8 ? kbr : " NONE") : ""); // 8==3 + "format-extra-chars"
...
{
    if ((player1->state != CS_DEAD && player1->state != CS_SPECTATE && !team_isspect (player1->team))
        || (!m_teammode && !team_isspect (player1->team) && servstate.mastermode == MM_MATCH))
...
void togglespect () // cycle through all spectating modes
{
    ...
    else
    {
        ...
        if (player1->spectatemode == SM_NONE)
            ...
            mode = SM_FOLLOW1ST
                   + ((player1->spectatemode - SM_FOLLOW1ST + 1) % (SM_OVERVIEW - SM_FOLLOW1ST)); // replace SM_OVERVIEW by SM_NUM
