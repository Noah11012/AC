...
void lightray(float bx, float by, const persistent_entity &light,
              float fade = 1,
              bool flicker = false)  // done in realtime, needs to be fast
{
  ...
  if (maxtmus) {
    ...
    if (light.attr3 || light.attr4)  // coloured light version, special case
                                     // because most lights are white
    {
      if (flicker) {
        int dimness =
            rnd((((255 << PRECBITS) -
                  (int(light.attr2) + int(light.attr3) + int(light.attr4)) *
                      fadescale / 3) >>
                 (PRECBITS + 4)) +
                1);
        x += stepx * dimness;
    ...
    {
      if (flicker) {
        int dimness = rnd((((255 << PRECBITS) - (light.attr2 * fadescale)) >>
                           (PRECBITS + 4)) +
                          1);
        x += stepx * dimness;
        y += stepy * dimness;
  ...
  } else  // the old (white) light code, here for the few people with old video
...
void calclightsource(const persistent_entity &l, float fade = 1,
                     bool flicker = true) {
  int reach = l.attr1;
  int sx = l.x - reach;
  int ex = l.x + reach;
...
void postlightarea(const block &a)  // median filter, smooths out random noise
                                    // in light and makes it more mipable
{
...
VARF(ambient, 0, 0, 0xFFFFFF, if (!noteditmode("ambient")) {
  hdr.ambient = ambient;
  calclight();
  unsavededits++;
});
...
void calclight() {
  ...
  uchar r = (hdr.ambient >> 16) & 0xFF, g = (hdr.ambient >> 8) & 0xFF,
        b = hdr.ambient & 0xFF;
  if (!r && !g) {
    if (!b) b = 10;
    r = g = b;
    ...
    r = g = b = max(max(r, g),
                    b);  // the old (white) light code, here for the few people
                         // with old video cards that don't support overbright
  sqr *s = S(0, 0);
  loopirev(cubicsize) {
    s->r = r;
    s->g = g;
  ...
  seedMT(ents.length() +
         hdr.maprevision);  // static seed -> nothing random here
...
static inline bool insidearea(const block &a, const block &b) {
  return b.x >= a.x && b.y >= a.y && b.x + b.xs <= a.x + a.xs &&
         b.y + b.ys <= a.y + a.ys;
}

void preparedynlight(dlight &d) {
  block area = {(int)d.o.x - d.reach, (int)d.o.y - d.reach, d.reach * 2 + 1,
                d.reach * 2 + 1};
...
void adddynlight(physent *owner, const vec &o, int reach, int expire, int fade,
...
void dodynlights() {
  ...
  loopv(dlights) {
    dlight &d = dlights[i];
    persistent_entity l((int)d.o.x, (int)d.o.y, (int)d.o.z, LIGHT, d.reach, d.r,
                        d.g, d.b);
  ...
  for (int y = by; y < b.ys + by; y++)
    for (int x = bx; x < b.xs + bx; x++) {
      ...
      *dest = *q;
      ...
      if (texmap) {  // translate texture slot numbers
...
void freeblockp(block *b) { delete[](uchar *) b; }
