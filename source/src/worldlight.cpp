...
void lightray (float bx,
               float by,
               const persistent_entity &light,
               float fade = 1,
               bool flicker = false) // done in realtime, needs to be fast
{
    ...
    if (maxtmus)
    {
                                        ...
                                        // because most lights are white
        {
            ...
            {
                int dimness
                = rnd ((((255 << PRECBITS) - (int(light.attr2) + int(light.attr3) + int(light.attr4)) * fadescale / 3)
                        >> (PRECBITS + 4))
                       + 1);
                x += stepx * dimness;
                y += stepy * dimness;
...
void postlightarealine (sqr *s, int len)
{
    ...
    {
#define median(m)                                                                          \
    s->m = (s->m * 2 + SW (s, 1, 0)->m * 2 + SW (s, 0, 1)->m * 2 + SW (s, -1, 0)->m * 2    \
            + SW (s, 0, -1)->m * 2 + SW (s, 1, 1)->m + SW (s, 1, -1)->m + SW (s, -1, 1)->m \
            + SW (s, -1, -1)->m)                                                           \
