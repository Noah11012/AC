...
#define RENDERSTRIPS(strips, type)                                                                             \
    ...
    {                                                                                                          \
        if (hasMDA && !ati_mda_bug)                                                                            \
            glMultiDrawArrays_ (type, strips.first.getbuf (), strips.count.getbuf (), strips.first.length ()); \
        else                                                                                                   \
            loopv (strips.first) glDrawArrays (type, strips.first[i], strips.count[i]);                        \
...
{
    if (skystrips.tris.first.empty () && skystrips.tristrips.first.empty () && skystrips.quads.first.empty ())
        return;
...
{
    if (showm && !showef)
        hudeditf (HUDMSG_MIPSTATS, "1x1/2x2/4x4/8x8: %d / %d / %d / %d", a[0], a[1], a[2], a[3]);
}
...
bool editfocusdetails (sqr *s)
{
    if (showef)
    {
        ...
        hudeditf (HUDMSG_EDITFOCUS,
                  ...
                  "(%02X%02X%02X)\fr",
                  cubetypename (s->type), g, s->floor, s->ceil, s->wtex, g, s->utex, s->ftex,
                  s->ctex, s->vdelta, s->tag, s->r, s->g, s->b);
        return true;
    }
    return false;
}
                    ...
                    int dir) // wall quads
{
    ...
    bool first = striptype != STRIP_WALL || striptex != wtex || ox != x1 || oy != y1
                 || ofloor != floor1 || oceil != ceil1 || odir != dir,
    ...
    else // continue strip
    {
        int lighterr = lighterror * 2;
        if ((!hf && !ohf)
            && ((iabs (ol1r - l2->r) < lighterr && iabs (ol1g - l2->g) < lighterr && iabs (ol1b - l2->b) < lighterr)
...
static void rendershadow_flatdelta (int x,
                                    int y,
                                    float h1,
                                    float h4,
                                    float h3,
