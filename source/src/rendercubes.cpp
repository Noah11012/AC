...
#define RENDERSTRIPS(strips, type)                                           \
  if (strips.first.length()) {                                               \
    if (hasMDA && !ati_mda_bug)                                              \
      glMultiDrawArrays_(type, strips.first.getbuf(), strips.count.getbuf(), \
                         strips.first.length());                             \
    else                                                                     \
      loopv(strips.first)                                                    \
          glDrawArrays(type, strips.first[i], strips.count[i]);              \
    strips.first.setsize(0);                                                 \
    strips.count.setsize(0);                                                 \
  }

void renderstripssky() {
  if (skystrips.tris.first.empty() && skystrips.tristrips.first.empty() &&
      skystrips.quads.first.empty())
    return;
...
void addstrip(int type, int tex, int start, int n) {
  ...
  strips &s =
      (type == GL_QUADS ? sb->quads
                        : (type == GL_TRIANGLES ? sb->tris : sb->tristrips));
  if (type != GL_TRIANGLE_STRIP && s.first.length() &&
      s.first.last() + s.count.last() == start) {
    s.count.last() += n;
    return;
  }
  s.first.add(start);
...
enum { STRIP_FLOOR = 1, STRIP_DELTA, STRIP_WALL };
...
#define TEXTURESCALE \
  (float(texturescale) * ((uniformtexres && t->scale > 1.0f) ? 1.0f : t->scale))

int striptype = 0, striptex, oh, oy, ox,
    odir;  // the o* vars are used by the stripification
...
const char *cubetypename(int t) {
  return t >= 0 && t < SEMISOLID ? cubetypenames[t] : "unknown";
}

void mipstats(const int a[]) {
  if (showm && !showef)
    hudeditf(HUDMSG_MIPSTATS, "1x1/2x2/4x4/8x8: %d / %d / %d / %d", a[0], a[1],
             a[2], a[3]);
}
...
bool editfocusdetails(sqr *s) {
  if (showef) {
    ...
    hudeditf(HUDMSG_EDITFOCUS,
             "%s: \fs%s%d..%d\fr W:%d \fs%sU:%d F:%d C:%d v:%d t:0x%X "
             "(%02X%02X%02X)\fr",
             cubetypename(s->type), g, s->floor, s->ceil, s->wtex, g, s->utex,
             s->ftex, s->ctex, s->vdelta, s->tag, s->r, s->g, s->b);
    return true;
  }
  return false;
}
...
#define stripend(verts)                                              \
  if (striptype) {                                                   \
    ...
    if (mergestrips) switch (len) {                                  \
        ...
        case 4:                                                      \
          type = GL_QUADS;                                           \
          swap(verts.last(), verts[verts.length() - 2]);             \
          break;                                                     \
      }                                                              \
...
void render_flat(int wtex, int x, int y, int size, int h, sqr *l1, sqr *l4,
                 sqr *l3, sqr *l2, bool isceil)  // floor/ceil quads
{
  if (showm) {
    l3 = l1 = &sbright;
    l4 = l2 = &sdark;
  ...
  bool first = striptype != STRIP_FLOOR || x != ox + size || striptex != wtex ||
               h != oh || y != oy;
...
void render_flatdelta(int wtex, int x, int y, int size, float h1, float h4,
                      float h3, float h2, sqr *l1, sqr *l4, sqr *l3, sqr *l2,
                      bool isceil)  // floor/ceil quads on a slope
{
  ...
  bool first =
      striptype != STRIP_DELTA || x != ox + size || striptex != wtex || y != oy;
                  ...
                  sqr *l3)  // floor/ceil tris on a corner cube
{
  ...
  addstrip(mergestrips ? GL_TRIANGLES : GL_TRIANGLE_STRIP, h->ftex,
           verts.length() - 3, 3);
  ...
  addstrip(mergestrips ? GL_TRIANGLES : GL_TRIANGLE_STRIP, h->ctex,
           verts.length() - 3, 3);
  nquads++;
}
...
void render_square(int wtex, float floor1, float floor2, float ceil1,
                   ...
                   sqr *l1, sqr *l2, bool flip, int dir)  // wall quads
{
  if (showm) {
    l1 = &sbright;
    l2 = &sdark;
  ...
  bool first = striptype != STRIP_WALL || striptex != wtex || ox != x1 ||
               oy != y1 || ofloor != floor1 || oceil != ceil1 || odir != dir,
  ...
  } else  // continue strip
  {
    int lighterr = lighterror * 2;
    if ((!hf && !ohf) &&
        ((iabs(ol1r - l2->r) < lighterr && iabs(ol1g - l2->g) < lighterr &&
          iabs(ol1b - l2->b) < lighterr) ||
         !wtex))  // skip vertices if light values are close enough
    {
      verts.setsize(verts.length() - 2);
      nquads--;
    } else {
...
void rendershadow_tri(sqr *h, int x1, int y1, int x2, int y2, int x3,
                      int y3)  // floor tris on a corner cube
{
  ...
  addstrip(mergestrips ? GL_TRIANGLES : GL_TRIANGLE_STRIP, DEFAULT_FLOOR,
           shadowverts.length() - 3, 3);
}
...
static void rendershadow_flatdelta(int x, int y, float h1, float h4, float h3,
                                   float h2)  // floor quads on a slope
{
...
void rendershadow(int x, int y, int xs, int ys, const vec &texgenS,
                  const vec &texgenT) {
  x = max(x, 1);
  y = max(y, 1);
  xs = min(xs, ssize - 1);
...
#define df(x) s->floor - (x->vdelta / 4.0f)
  ...
  for (shadowvertex *v = shadowverts.getbuf(), *end = &v[shadowverts.length()];
       v < end; v++) {
    float vx = v->x, vy = v->y;
