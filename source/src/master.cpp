...
struct messagebuf {
  ...
  bool equals(const messagebuf &m) const {
    return buf.length() == m.buf.length() &&
           !memcmp(buf.getbuf(), m.buf.getbuf(), buf.length());
  }

  bool endswith(const messagebuf &m) const {
    return buf.length() >= m.buf.length() &&
           !memcmp(&buf[buf.length() - m.buf.length()], m.buf.getbuf(),
                   m.buf.length());
  }
...
void setupserver(int port, const char *ip = nullptr) {
  ...
  if (!setuppingsocket()) fatal("failed to create ping socket");
  ...
  for (;;) {
    ...
    loopv(gameservers) {
      ...
      if (s.address.host == addr.host && s.address.port == addr.port) {
        if (s.lastping &&
            (!s.lastpong || ENET_TIME_GREATER(s.lastping, s.lastpong))) {
          client *c = findclient(s);
          if (c) {
...
void checkgameservers() {
  ...
  loopv(gameservers) {
    gameserver &s = *gameservers[i];
    if (s.lastping && s.lastpong &&
        ENET_TIME_LESS_EQUAL(s.lastping, s.lastpong)) {
      if (ENET_TIME_DIFFERENCE(servtime, s.lastpong) > KEEPALIVE_TIME) {
        delete gameservers.remove(i--);
        updateserverlist = true;
      }
    } else if (!s.lastping ||
               ENET_TIME_DIFFERENCE(servtime, s.lastping) > PING_TIME) {
...
void reqauth(client &c, uint id, char *name) {
  if (ENET_TIME_DIFFERENCE(servtime, c.lastauth) < AUTH_THROTTLE) return;
...
void checkclients() {
  ...
  if (ENET_SOCKETSET_CHECK(readset, serversocket)) {
    ...
    if (clients.length() >= CLIENT_LIMIT || checkban(bans, address.host))
      ...
      loopv(clients) if (clients[i]->address.host == address.host) {
        dups++;
        if (oldest < 0 ||
            clients[i]->connecttime < clients[oldest]->connecttime)
          oldest = i;
      }
  ...
  loopv(clients) {
    client &c = *clients[i];
    if ((c.message || c.output.length()) &&
        ENET_SOCKETSET_CHECK(writeset, c.socket)) {
      const char *data =
          c.output.length() ? c.output.getbuf() : c.message->getbuf();
    ...
    if (ENET_TIME_DIFFERENCE(servtime, c.lastinput) >=
        (c.registeredserver ? KEEPALIVE_TIME : CLIENT_TIME)) {
      purgeclient(i--);
      continue;
    }
  }
...
void banclients() {
  loopvrev(clients) if (checkban(bans, clients[i]->address.host))
      purgeclient(i);
}

volatile bool reloadcfg = true;

void reloadsignal(int signum) { reloadcfg = true; }
